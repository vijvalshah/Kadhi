# Contributing to Kadhi

Thank you for your interest in contributing to Kadhi! We welcome bug reports, feature requests, and pull requests from the community.
...
*installing* Kadhi and the wrong tool here, since an editable checkout needs to be
...
- `cryptography` — `[dev]` also pulls it in (it's the `[sign]` extra) so the
  ed25519 signing tests (`kadhi adapters sign` / `kadhi attest`) run in CI
- `reportlab` — `[dev]` also pulls it in (it's the `[pdf]` extra) so the
  Annex XI/XII PDF tests (`kadhi train --annex-xi *.pdf`) run in CI
...
```bash
ruff check src/kadhi_cli/ scripts/ tests/ benchmarks/
```
...
# Auto-fix issues
ruff check --fix src/kadhi_cli/ scripts/ tests/
```
...
src/kadhi_cli/
  ...
  recipes/            - Ready-made configs for popular models (171 recipes)
...
```bash
pytest tests/ --cov=kadhi_cli --cov-report=html
```
...
| test_runs.py | `kadhi runs` CLI commands |
...
| test_callback.py | `KadhiTrainerCallback` (mock-based) |
...
| test_doctor.py | `kadhi doctor` command, version checking, system resources, dependency table |
| test_quickstart.py | `kadhi quickstart` demo, data/config creation, --dry-run |
...
| test_quality_filter.py | Perplexity + coherence scoring, `kadhi data filter` |
...
| test_trace_to_pref.py | Trace-to-Preference: LangChain/OpenAI/Kadhi-serve parsers, pair builder, CLI (v0.26.0) |
| test_quant_check.py | Quant-Lobotomy: classify_delta, resolve_model_ref, render formats, CLI (v0.26.0) |
| test_cans.py | Kadhi Cans: manifest schema, pack/unpack, tar traversal, fork security, CLI (v0.26.0) |
...
| test_part_a_wave2.py | Live Wire Part A: kadhi can run / publish + DeployTarget schema (v0.33.0) |
...
| test_why.py | `kadhi why` heuristic explainer: NaN / plateau / divergence / grad-norm / LR bounds (v0.34.0 Part C) |
| test_crash_reporter.py | `.crash` bundle: classify, redact secrets, write under cwd, oversize truncation (v0.34.0 Part D) |
| test_replay.py | `kadhi runs replay`: summarise + downsample + CLI rendering (v0.34.0 Part E) |
| test_profiling.py | Auto-profiling: ProfilerSchedule + path containment + torch-less degradation (v0.34.0 Part F) |
| test_tui.py | `kadhi tui`: CLI bounds + missing-textual error + row builders (v0.34.0 Part G) |
...
| test_v0500_part_b.py | v0.50.0 Part B — Long-context GRPO + vLLM sleep; `validate_long_context_grpo_compat` (task / backend / ring-attention exclusivity + null-byte + bool guards); `validate_vllm_sleep_mode_compat` (transformers / unsloth allowlist + null-byte); `apply_vllm_sleep_mode` deferred stub with v0.50.1 marker; KadhiConfig integration (vllm_sleep_mode requires task='grpo' — code-review HIGH fix) (v0.50.0 Part B) |
| test_v0500_part_c.py | v0.50.0 Part C — Multi-turn agent rollout backend allowlist (art / ruler / nemo_gym / openenv); frozen `RolloutBackendSpec` + `MappingProxyType` immutability; `validate_rollout_backend` (bool rejected); `required_rollout_package` per-entry mapping; `launch_rollout` deferred stub; KadhiConfig task-gate + mlx rejection (v0.50.0 Part C) |
| test_v0500_part_d.py | v0.50.0 Part D — 7 stability/efficiency knobs (`ref_model_ema_alpha` / `replay_buffer_size` / `async_grpo_prefetch` / `tis_threshold` / `mask_truncated_completions` / `defer_rerolling` / `skip_zero_advantage` / `off_policy_mask_threshold`); explicit bool-rejection field_validator across all numeric fields (tdd-guide HIGH fix); `mask_truncated_completions` requires `tis_threshold` cross-validator; KadhiConfig task-gate naming every offending field; `grpo_fp16` task-gate (code-review HIGH fix) (v0.50.0 Part D) |
| test_v0500_part_e.py | v0.50.0 Part E — `task='prm'` (Process Reward Model) + `vision_grpo` flag; `validate_prm_compat` (data.format / modality / mlx gates); `validate_vision_grpo_compat` (task ∈ {grpo, ppo} / modality='vision' / non-mlx); `build_prm_trainer` deferred stub; KadhiConfig integration with all rejection paths exercised (v0.50.0 Part E) |
...
- **If a maintainer ends up doing the work instead, you get told, in the thread,
  before or when it lands.** This has been broken once (#313):
...
- **A claim that goes quiet is released with a comment, not silently.** We check
  in first, and "life got in the way" needs no explanation — see #280 for the
  shape.
...
- [ ] `ruff check src/kadhi_cli/ scripts/ tests/ benchmarks/` passes
...
Heavy ML imports (torch, transformers, trl) are imported inside command handlers so the CLI stays fast. Users can run `kadhi version` or `kadhi --help` instantly without waiting for PyTorch to load.
...
No external dependencies required. All runs, metrics, and eval results go to `~/.kadhi/experiments.db`.
...
3. Add a template YAML under `src/kadhi_cli/templates/` + a `manifest.json` entry (see the existing 21 templates)
...
2. Register in `src/kadhi_cli/cli.py` with `@app.command()`
...
Look for issues labeled `good first issue` in the issue tracker. These are beginner-friendly tasks that help you get familiar with the codebase.
...
> CI previously ran via GitHub Actions workflows under `.github/workflows/`, which have been
> removed as part of detaching this repo from GitHub. The checks below describe what CI used to
> enforce and should be reinstated under whatever CI system this project adopts next:
...
- **transformers-floor** on Ubuntu / Python 3.11 (#478): installs
  `.[dev]` against a pinned `transformers` version, and runs the static `dtype=` / `torch_dtype=`
...
0. **Refresh the dev stack, or read the last `dependency drift` run** (#323).
   ...
   `pip install -U -e ".[dev]"` before testing, or check the latest `dependency drift` run — it
...
2. Update version in `pyproject.toml` and `src/kadhi_cli/__init__.py`
...
7. CI verifies that no fragment was left behind, then publishes to PyPI on the `v*` tag
...
- **Security:** Report security issues via [SECURITY.md](SECURITY.md) — privately
