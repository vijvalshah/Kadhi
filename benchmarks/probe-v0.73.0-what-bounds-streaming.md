...
peft 0.18.1 · Python 3.10.8. Kadhi v0.73.0 (working tree at 57c18e5).
-->
...
triples the usable microbatch (1 -> 3) for +9.6% throughput, but Kadhi's own
...
1. Kadhi says publicly that layer streaming is **bound by host-to-device
...
## 7. Kadhi's own `use_cut_ce` flag cannot engage. Three blockers, all measured
...
**llama** model trips over a **gemma2** incompatibility. Kadhi pins
...
below measure **the kernel**, not Kadhi's flag.
...
**Finding 8b — this is the part with a cost.** Kadhi's central claim, and the
...
- The difference, **0.744 GB per 512 tokens**, is the cross-entropy pipeline:
  **11.32 bytes per logit element**. Kadhi's own peak-VRAM pre-flight charges
...
- **Kadhi's `training.use_cut_ce` end to end through `kadhi train`.** It cannot
