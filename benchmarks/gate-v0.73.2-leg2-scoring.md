# Gate record — v0.73.2, `kadhi ship` leg 2
...
Fed straight back through `kadhi ship --evidence`, with no model loaded, it
...
in review, and `kadhi ci init` wires `ship --evidence` as a PR merge gate.
...
layer streaming; `kadhi ship` appears only as the instrument. Nothing in this
