...
`kadhi train`.** SmolLM2-135M streamed in bf16, batch 1, `quantization: none`,
...
## Live end-to-end verification (through the real `kadhi train`)
...
- **security** — HIGH: with the probe on, a predicted miss no longer refused
  before the GPU was touched, so a shared `kadhi.yaml` could drive a real
...
- **verification-loop** — `kadhi --help` 0.85 s, exit 0; every new public symbol
