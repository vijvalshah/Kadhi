...
> stayed green while `kadhi train --task orpo` was broken for anyone installing
...
>   namespace, which is what Kadhi imports. The 0.29 break is an `ImportError` at
...
Kadhi's wrappers apply PEFT manually (`get_peft_model`) before constructing the
