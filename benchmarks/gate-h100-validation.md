<!--
Measurement record for Kadhi layer streaming on 8x H100, published verbatim.
...
> state of the shipped code. A second defect, #328, was diagnosed to a cause in Kadhi
...
5. **A model trained by streaming is as good as one trained resident.** Paired
   over five disjoint training subsets and judged by Kadhi's own `kadhi ship`:
...
11. **#328 has a cause in Kadhi, not in torch** (STEP 15). `should_enable_hf_gradient_checkpointing`
...
15. **`kadhi train --gpus N` had never launched** (STEP 20). `accelerate launch`
    takes a script path and Kadhi handed it `sys.executable`, so accelerate parsed
...
20. **A leg-2 suite investigation, one real finding and one of my own errors**
    ...
    inverted too and was not**: that was measured at 64 new tokens where `kadhi ship`
...
21. **Reported measurement budget matters, and mine did not match the tool's.**
    `kadhi ship` generates at `BEHAVIOURAL_MAX_NEW_TOKENS = 256`; two of my suite
...
Not investigated further and **no Kadhi code changed** — the brief forbids editing
...
genuine `kadhi train` with `stream_layers: true` on the same model works:
...
`~/.kadhi/spectrum/weights/<slug>/`:

```
resolved: /root/.kadhi/spectrum/weights/Qwen__Qwen2.5-0.5B-Instruct
   config.json        symlink= False
   model.safetensors  symlink= False
```

So the defect was in my harness, not in Kadhi, and the symlink guard is doing its
...
once under `~/.kadhi`. For this session's four models that is ~250 GB duplicated,
which is a live constraint against 886 GB.

That run is also the milestone the brief asked for on its own terms — the first
real `kadhi train` layer-streaming run on Linux, and it worked unmodified.
...
```json
{
  "weights": "/root/.kadhi/spectrum/weights/NousResearch__Meta-Llama-3.1-8B-Instruct",
...
- **Not established:** the exact threshold, whether it is per-layer bytes,
  ...
  against.
- **Not done:** no Kadhi code was changed. The brief forbids editing the code to
...
`kadhi train` is affected. Two attempts, neither of which supports a claim.

**Attempt 1 — streamed vs resident through `kadhi train`.** Same data, same
...
bit-exactness harness controls for this by copying adapters across; `kadhi train`
has no reason to.
...
`kadhi train` does not pin the adapter-init seed, so process-to-process scatter
...
comparison mean anything. That gap is itself worth reporting: two `kadhi train`
...
alpha=16, and the same `kadhi train` entry point — DeepSpeed is reached through
Kadhi's own `--deepspeed` flag, so this is one tool against itself.

### Getting DeepSpeed to run at all took three fixes, and they are findings

1. **Kadhi ships no ZeRO-3 CPU-offload preset.** `utils/deepspeed.CONFIGS` has
...
3. **torch 2.13 + DeepSpeed 0.19.4 + transformers 4.57.6 crash on the LR
   ...
   does not expose one param group per scheduler value. Nothing in Kadhi is in
...
Against the configuration Kadhi actually recommends (NF4), it is 5.61x the
...
repeats per configuration, each a full `kadhi train` (4 epochs, 256 steps),
...
**No fix attempted and no Kadhi code changed**, per the brief. At the time this
...
hitting this could not work around it from `kadhi.yaml`. That escape hatch now
...
1. **The tok/s are not comparable to the `kadhi train` figures elsewhere in this
   record.** This harness replays one fixed padded batch with no data pipeline
   and counts all 256 positions; `kadhi train` counts real tokens on short rows.
...
1. **Upstream**: `MatMul4Bit` uses `save_for_backward` for `B` and the
   ...
   the cost returns to O(window). This is the clean fix and it is not in Kadhi.
2. **In Kadhi**: do not route streamed NF4 layers through `MatMul4Bit` at all —
...
Everything above was measured against **unmodified Kadhi at `2c9a078`** (plus the
...
```
$ python -m kadhi_cli.cli train --config stream05b.yaml --yes      # 8 visible
...
$ CUDA_VISIBLE_DEVICES=6 python -m kadhi_cli.cli train --config stream05b.yaml --yes
Layer streaming ready: 24 layers, 0.18 GB pinned RAM store, 2 x 8 MB VRAM buffers
...
cards is skipped by hardware behind `KADHI_TEST_MULTI_GPU`, not left failing.

```
CUDA_VISIBLE_DEVICES=6 pytest tests/test_stream_multi_gpu_guard.py   13 passed, 1 skipped
KADHI_TEST_MULTI_GPU=1  pytest tests/test_stream_multi_gpu_guard.py   14 passed
```
...
`kadhi ship --task-mode metric` scores with plain exact match, no judge and no
...
The judge is **Kadhi's own `kadhi ship`**, not a hand-rolled script — that was the
...
on 3000 narrow examples costs general ability. `kadhi ship` caught it, in both
...
### FINDING 4 — Kadhi's advertised multi-GPU path does not run at all

The first attempt died on every rank before training started:

```
$ torchrun --nproc_per_node 8 -m kadhi_cli.cli train --config b_ds.yaml --deepspeed ...
Error: ValueError: You can't train a model that has been loaded with
`device_map='auto'` in any distributed mode.
```

This is not about DeepSpeed. `kadhi train --gpus 8` prints

```
To train on 8 GPUs, re-run under accelerate:
    accelerate launch --num_processes 8 kadhi train -c /root/run/b_ds.yaml
```
...
Variant 2 is the remaining candidate implementable inside Kadhi: do not send the
...
Shipped as `install_dequant_forward` in `src/kadhi_cli/utils/layer_stream_runtime.py`,
...
it is done through the real `kadhi train`, not a harness.
...
`kadhi train`, both confounded because the CLI pinned no adapter-init seed, and it
recorded the gap: *"two `kadhi train` runs of one unchanged config do not reproduce
...
only". It has a cause, and the cause is in Kadhi.
...
Three independent lines say the checkpoint doing that recompute is **not Kadhi's**:
...
Windows box. Real `kadhi train`, Llama-3.1-8B + LoRA r=16, bf16, batch 4, seq 1024,
...
**Against what Kadhi documented:** FlashAttention claimed "2-4x speedup and
...
1. **`training.use_liger: true` crashed at step 0.** Kadhi patched the model but
   ...
   use the HF-native flag, i.e. what a fixed Kadhi does. **Fixed.**
2. **`data.max_length` above 1024 was silently ignored on every SFT run** —
   `max_length=512` gave 512 tokens/sample, `max_length=4096` gave **1024**. Kadhi
   ...
   the longest sequence Kadhi could produce, so the benchmark ran at the operating
   point users actually got.
3. **The FlashAttention 3 branch is unreachable.** Kadhi selects FA3 when
   ...
   3.0.0: Kadhi requests `flash_attention_3` while
...
**vLLM works.** `kadhi serve --backend vllm` came up in 38–118 s and returned 200 on
...
transformers **4.57.6 -> 5.14.1**, past Kadhi's own `<5.0.0` cap. Separate venvs were
...
Also recorded because the record is the working one: a suspicion that Kadhi leaked
`VLLM::EngineCore` processes holding 51 GB was **disproved** by a dedicated SIGINT
test — the orphans were the harness `SIGKILL`ing after 5 s, and Kadhi's shutdown is
clean in ~6 s.
...
Kadhi's own `kadhi ship --task-mode metric`.
...
### The blocker: `kadhi train --gpus N` had never launched

```
accelerate launch --num_processes 4 /root/venv/bin/python -m kadhi_cli.cli train ...
  File "/root/venv/bin/python", line 1
    ELF
SyntaxError: source code cannot contain null bytes
```

`accelerate launch` takes a **script path**, or a module behind `--module`. Kadhi
...
- `_install_convert_deps()` ran only after an auto-clone, so `--llama-cpp` or an
  existing `~/.kadhi/llama.cpp` died on `ModuleNotFoundError: sentencepiece` every
  time. Now on every branch, skipped when the packages are importable.

**AWQ: Kadhi's code is correct, the dependency set is impossible** (#338). At
...
'attention_type'`; at >= 4.57 it fails at import. Kadhi's own trl 0.26.2 needs
...
### `kadhi ship` leg 2 on a real model (#316): two of three suites measure nothing
...
row ordering plus GPU nondeterminism, **not in the trainer RNG**, because Kadhi
...
0.26.2, which is what `pip install "kadhi-cli[train]"` resolves today and what this box
...
### `kadhi ship`'s tool-call and JSON suites, repaired (#316, `c87fd00`)
...
STEP 23 ended by recording that Kadhi exposed no training-seed knob at all, and that
...
`kadhi ship` suites scored the envelope the harness produced — a bare prompt, a
...
torch 2.13.0+cu130, transformers 4.57.6, trl 0.26.2, peft 0.20.0, kadhi 0.72.4.
...
whose honest yardstick is within-arm spread**, and, as STEP 23 recorded, Kadhi
...
### The repaired `kadhi ship` suites, on the model STEP 24 could not re-run
...
Verified through the real `kadhi ship` CLI end to end, not only through the component
...
rather than raising. In a *scoring* function feeding `kadhi ship`'s leg 2, `0.0`
...
**`kadhi ship` generates with `BEHAVIOURAL_MAX_NEW_TOKENS = 256`, and this was
...
Five identical `kadhi train` + `kadhi ship` runs, Llama-3.1-8B, streamed, in parallel
on five cards:
...
`kadhi ship`'s default `forgetting_threshold` is **0.05**. `mini_common_sense` moves
...
```bash
until ! pgrep -f "kadhi_cli.cli train"; do sleep 10; done
```
...
`kadhi draft distill` exists to make a small draft agree with a specific target more
...
`kadhi draft distill` refuses up front, which is correct, while `kadhi draft measure`
...
imports Kadhi's own CLI after patching two things:

- `set_seed(S)` at process start, because Kadhi applies `get_peft_model` **before**
...
`llama3-70b-fsdp2` is one of three multi-GPU recipes Kadhi ships. It has never been
...
`bnb_4bit_quant_storage` is a knob Kadhi already has, and `quant_menu.py` already
...
- **The defect is still not demonstrated end-to-end through `kadhi train`**, but
...
- **`kadhi ship`'s leg-2 suites are coarse.** `mini_common_sense` has 24 items, so
...
- **FlashAttention 3 is unmeasured**, on a Hopper box, which was one of the stated
  ...
  and Kadhi's own FA3 detection cannot fire at all (#334). The second means no
  amount of build effort would have produced a measurement through Kadhi.
- **The FA/Liger numbers are one operating point** — 8B + LoRA + seq 1024 + batch 4,
  ...
  FA's advantage grows with sequence length; 1024 was the longest sequence Kadhi
...
| `runbench.sh` / `variance.sh` / `runbench8.sh` | one `kadhi train` with VRAM and SM-clock sampling; n repeats; 8-GPU variant under torchrun |
...
All of them import `kadhi_cli` unmodified: every measurement above the
measurement/fix boundary was taken against untouched Kadhi. The two `src/` changes
