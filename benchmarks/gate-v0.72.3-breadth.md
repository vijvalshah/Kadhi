<!--
Measurement record for Kadhi layer streaming, published verbatim.
...
Real `kadhi train --stream-layers` on SmolLM2-135M, **batch 2** (the first batch > 1 ever
...
Real `kadhi train --stream-layers` on SmolLM2-135M at **batch 2 × accum 4** (effective
...
2. **End-to-end `kadhi train --resume` cannot be demonstrated on this box, for a reason
...
## GATE 5 — disk-kind detection (the `kadhi doctor` rider) — **PASS** (2026-07-28)
...
`kadhi doctor --disk` reports it:
`Disk type │ NVMe — layer streaming can use the disk overflow tier`.

**Opt-in, following the command's own `--nccl` convention.** Measured: `kadhi doctor`
