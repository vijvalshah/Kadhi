...
Consequence: **every adapter produced by `kadhi train --stream-layers` in
v0.72.0 is inert outside the streaming path** — `kadhi merge`, `kadhi serve`,
`kadhi chat` and `PeftModel.from_pretrained` all load it as a no-op. The
