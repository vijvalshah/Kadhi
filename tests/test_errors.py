...
def test_missing_fastapi_error():
    ...
    with patch("kadhi_cli.utils.errors.console", test_console):
        ...
        format_friendly_error(exc, verbose=False)
    output = buf.getvalue()
    assert "kadhi-cli[serve]" in output
...
def test_missing_datasketch_error():
    ...
    with patch("kadhi_cli.utils.errors.console", test_console):
        ...
        format_friendly_error(exc, verbose=False)
    output = buf.getvalue()
    assert "kadhi-cli[data]" in output
...
def test_missing_deepspeed_error():
    ...
    with patch("kadhi_cli.utils.errors.console", test_console):
        ...
        format_friendly_error(exc, verbose=False)
    output = buf.getvalue()
    assert "kadhi-cli[deepspeed]" in output
...
def test_missing_train_dep_points_at_train_extra(module):
    ...
    assert "kadhi-cli[train]" in output
...
def test_validation_error():
    ...
    test_console = Console(file=buf, stderr=False)
    with patch("kadhi_cli.utils.errors.console", test_console):
        exc = Exception("2 validation error for KadhiConfig")
        format_friendly_error(exc, verbose=False)
    output = buf.getvalue()
