...
class TestResolveNumGpus:
    ...
    def test_resolve_integer(self):
        from kadhi_cli.utils.topology import resolve_num_gpus
    ...
    def test_resolve_auto_uses_detected(self):
        ...
        with patch(
            "kadhi_cli.utils.topology._detected_gpu_count", return_value=8
        ):
    ...
    def test_resolve_auto_cpu_returns_zero(self):
        ...
        with patch(
            "kadhi_cli.utils.topology._detected_gpu_count", return_value=0
        ):
...
class TestDetectTopology:
    ...
    def test_single_gpu_has_no_interconnect(self):
        ...
        with patch(
            "kadhi_cli.utils.topology._detected_gpu_count", return_value=1
        ):
            topo = detect_topology()
    ...
    def test_multi_gpu_nvlink_detected(self):
        ...
        with (
            patch(
                "kadhi_cli.utils.topology._detected_gpu_count", return_value=4
            ),
            patch(
                "kadhi_cli.utils.topology._count_nvlink_pairs", return_value=6
            ),
        ):
            topo = detect_topology()
    ...
    def test_multi_gpu_pcie_fallback(self):
        ...
        with (
            patch(
                "kadhi_cli.utils.topology._detected_gpu_count", return_value=2
            ),
            patch(
                "kadhi_cli.utils.topology._count_nvlink_pairs", return_value=0
            ),
        ):
            topo = detect_topology()
...
class TestSuggestStrategy:
    ...
    def test_single_gpu_no_strategy(self):
        from kadhi_cli.utils.topology import suggest_strategy
...
class TestLauncherArgv:
    ...
    def test_single_process_no_launcher(self):
        from kadhi_cli.utils.launcher import build_accelerate_argv

        # num_processes=1 → no wrapper needed
        argv = build_accelerate_argv(num_processes=1, script_args=["kadhi", "train"])
        assert argv == ["kadhi", "train"]

    def test_multi_process_wraps(self):
        from kadhi_cli.utils.launcher import build_accelerate_argv

        argv = build_accelerate_argv(num_processes=4, script_args=["kadhi", "train"])
    ...
    def test_multi_process_mixed_precision(self):
        ...
        argv = build_accelerate_argv(
            num_processes=4, script_args=["kadhi", "train"], mixed_precision="bf16"
        )
    ...
    def test_invalid_num_processes(self):
        ...
        with pytest.raises(ValueError, match="num_processes"):
            build_accelerate_argv(num_processes=0, script_args=["kadhi", "train"])
    ...
    def test_invalid_mixed_precision(self):
        ...
        with pytest.raises(ValueError, match="mixed_precision"):
            build_accelerate_argv(
                num_processes=2, script_args=["kadhi", "train"], mixed_precision="int8"
            )
    ...
    def test_invalid_num_machines_low(self):
        ...
        with pytest.raises(ValueError, match="num_machines"):
            build_accelerate_argv(
                num_processes=2, script_args=["kadhi", "train"], num_machines=0
            )
    ...
    def test_invalid_num_machines_high(self):
        ...
        with pytest.raises(ValueError, match="num_machines"):
            build_accelerate_argv(
                num_processes=2, script_args=["kadhi", "train"], num_machines=10_000
            )
...
class TestLauncherDetectActive:
    ...
    def test_not_in_distributed(self):
        from kadhi_cli.utils.launcher import is_in_distributed
...
class TestLauncherAdvice:
    ...
    def test_format_advice_command(self):
        from kadhi_cli.utils.launcher import format_advice

        text = format_advice(num_processes=4, script_args=["kadhi", "train", "-c", "kadhi.yaml"])
        assert "accelerate launch" in text
        assert "--num_processes 4" in text
        assert "kadhi train" in text
...
class TestZeroPlusPlus:
    ...
    def test_zero_pp_config_exists(self):
        from kadhi_cli.utils.deepspeed import CONFIGS

        assert "zero++" in CONFIGS or "zero_pp" in CONFIGS

    def test_zero_pp_has_stage_3_base(self):
        from kadhi_cli.utils.deepspeed import get_deepspeed_config
    ...
    def test_zero_pp_write_file(self):
        ...
        disable.
        """
        from kadhi_cli.utils.deepspeed import write_deepspeed_config
...
class TestFsdp2Compile:
    ...
    def test_validate_compile_ok(self):
        ...
        with patch("kadhi_cli.utils.fsdp.is_fsdp_available", return_value=True):
            errors = validate_fsdp2_compile_config(
                use_compile=True,
...
class TestMiiBackend:
    ...
    def test_is_mii_available_false_when_stubbed_to_none(self):
        from kadhi_cli.utils.mii import is_mii_available
    ...
    def test_create_mii_pipeline_mocked(self):
        from kadhi_cli.utils.mii import create_mii_pipeline
...
class TestFsdp2CompileHelper:
    ...
    def test_no_fsdp_leaves_kwargs_untouched(self):
        from kadhi_cli.utils.fsdp import apply_fsdp_training_kwargs
...
class TestTrainValidatorGating:
    """CLI-level tests proving the new validators actually block `kadhi train`
