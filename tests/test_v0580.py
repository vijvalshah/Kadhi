"""Tests for v0.58.0 — kadhi loop CLI-first data flywheel.
...
from kadhi_cli.utils.canary_router import (
    BucketStats,
    CanaryPolicy,
    rollback,
    route,
)
from kadhi_cli.utils.loop_budget import (
    check_budget,
    parse_budget_string,
    reset_daily_counter_if_new_day,
)
from kadhi_cli.utils.loop_daemon import (
    WatchConfig,
...
from kadhi_cli.utils.loop_iteration import (
    IterationRecord,
    list_iterations,
    new_iteration_id,
    read_iteration,
...
from kadhi_cli.utils.loop_state import (
    LOOP_STATUSES,
    LoopState,
    default_state_path,
    init_state,
...
class TestStateIO:
    def test_default_state_path(self, tmp_path, monkeypatch):
        ...
        assert default_state_path().endswith(os.path.join(".kadhi", "loop.yaml"))
    ...
    def test_read_state_invalid_json_rejected(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        target = tmp_path / ".kadhi" / "loop.yaml"
        target.parent.mkdir()
...
class TestIterationIO:
    ...
    def test_read_invalid_manifest_rejected(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        d = tmp_path / ".kadhi-loops" / "iter-bad"
    ...
    def test_list_skips_invalid_id_directories(self, tmp_path, monkeypatch):
        ...
        bad = tmp_path / ".kadhi-loops" / "weird\x00"
...
class TestCLI:
    ...
    def test_init_command(self, tmp_path, monkeypatch):
        ...
        assert result.exit_code == 0, result.output
        assert (tmp_path / ".kadhi" / "loop.yaml").exists()
...
class TestSourceWiring:
    def test_cli_registers_loop_typer(self):
        cli_src = (_REPO_ROOT / "src" / "kadhi_cli" / "cli.py").read_text(encoding="utf-8")
        assert "from kadhi_cli.commands import loop as _loop_cmd" in cli_src
    ...
    def test_version_bumped_to_0_58_0(self):
        ...
        init = (_REPO_ROOT / "src" / "kadhi_cli" / "__init__.py").read_text(encoding="utf-8")
    ...
    def test_no_top_level_torch_import_in_loop_modules(self):
        for name in [
            ...
            src = (_REPO_ROOT / "src" / "kadhi_cli" / "utils" / name).read_text(encoding="utf-8")
    ...
    def test_command_module_uses_typer_app(self):
        src = (_REPO_ROOT / "src" / "kadhi_cli" / "commands" / "loop.py").read_text(encoding="utf-8")
