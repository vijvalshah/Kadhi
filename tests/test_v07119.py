...
* #81 — Quant Menu (gptq / awq / hqq:Nbit / aqlm / eetq / mxfp4 / fp8) was
  rejected by the KadhiConfig modality gate for ``modality in {vision, audio}``.
...
_SFT_SRC = (_REPO / "src/kadhi_cli/trainer/sft.py").read_text(encoding="utf-8")
_MP_SRC = (_REPO / "src/kadhi_cli/utils/multipack_trainer.py").read_text(encoding="utf-8")
_SCHEMA_SRC = (_REPO / "src/kadhi_cli/config/schema.py").read_text(encoding="utf-8")
...
def _attach(instance):
    from kadhi_cli.utils.multipack_trainer import attach_multipack_state
...
class TestMultipackDistributedDataLoader:
    def test_routes_through_accelerator_prepare_when_distributed(self):
        ...
        from kadhi_cli.utils.multipack_sampler import MultipackBatchSampler
        from kadhi_cli.utils.multipack_trainer import make_multipack_trainer_class
...
class TestPatchInvariants:
    def test_version_bumped(self):
        ...
        assert parts >= (0, 71, 19), kadhi_cli.__version__
