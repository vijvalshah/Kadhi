...
def test_kadhiconfig_long_context_grpo_happy():
    cfg = load_config_from_string(_grpo_yaml("  long_context_grpo: true\n"))
    assert cfg.training.long_context_grpo is True


def test_kadhiconfig_long_context_grpo_with_ring_attention_rejected():
...
def test_kadhiconfig_long_context_grpo_on_sft_rejected():
    yaml = """
base: test-llama
task: sft
data:
...
def test_kadhiconfig_vllm_sleep_mode_on_mlx_rejected():
    yaml = """
base: test-llama
task: grpo
backend: mlx
