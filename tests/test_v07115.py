...
KadhiConfig rejects — mirrors the v0.71.13 #229 ``local_rl`` fix), #246
(CMA-ES default scorer reloaded the base per candidate — now load once and
reuse across the population loop), #245 (``kadhi loop`` ``estimate_cost`` was a
hard ``0.0`` placeholder — now wires v0.34 ``run_cost.estimate_run_cost_usd``
off the last completed run), #244 (``kadhi train --track-energy --energy-out``
persists the measurement so ``kadhi bom emit --energy`` can consume it), and
...
# ---------------------------------------------------------------------------
# #261 — iterative_dpo._default_train_fn renders a KadhiConfig-valid YAML
# ---------------------------------------------------------------------------
...
class TestIterativeDpoDefaultTrainFn:
    def test_renders_schema_valid_config(self, tmp_path, monkeypatch):
        ...
        The bug: ``output: {dir: ...}`` (a mapping) — KadhiConfig.output is a
        ...
        from kadhi_cli.utils import iterative_dpo as idpo
...
def _patch_cmaes_libs(monkeypatch, base_loads):
    ...
    import kadhi_cli.eval.custom as ev
...
class TestEstimateCost:
    ...
    def _patch_tracker(self, monkeypatch, runs):
        import kadhi_cli.experiment.tracker as trk
    ...
    def test_uses_last_completed_priced_run(self, monkeypatch):
        from kadhi_cli.utils import loop_stages
    ...
    def test_estimate_raises_is_swallowed(self, monkeypatch):
        from kadhi_cli.utils import loop_stages, run_cost
...
class TestEnergyOutHandoff:
    ...
    def test_write_energy_json_roundtrips_into_bom_consumer(self, tmp_path, monkeypatch):
        ...
        from kadhi_cli.commands.train import _write_energy_json
        from kadhi_cli.utils.energy import EnergyMeasurement
    ...
    def test_full_handoff_into_bom_emit(self, tmp_path, monkeypatch):
        ...
        to ``kadhi bom emit --energy`` (the #256 consumer) and confirm the BOM
...
class TestPatchInvariants:
    ...
    def test_loop_stages_no_heavy_top_level_imports(self):
        ...
        src = (
            ...
            / "src" / "kadhi_cli" / "utils" / "loop_stages.py"
