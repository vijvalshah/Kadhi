...
- HIGH H6 — symlink rejection on dataset_path through `kadhi plan`.
...
def test_plan_yaml_symlink_rejected(tmp_path, monkeypatch):
    from kadhi_cli.commands.plan import _load_yaml_config
...
def test_compute_dataset_sha_empty_returns_zero():
    from kadhi_cli.utils.terraform_plan import compute_dataset_sha
...
def test_flag_downstream_risk_unknown_llama_variant_does_not_trip_mau_gate():
    ...
    """
    from kadhi_cli.utils.license_advisor import flag_downstream_risk
...
# ---------------------------------------------------------------------------
# Drift-on-modify regression — `kadhi plan` -> mutate -> `kadhi apply`
# ---------------------------------------------------------------------------
