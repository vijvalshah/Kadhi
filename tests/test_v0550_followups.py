...
from kadhi_cli.utils.eval_lock_coverage import (
    compute_coverage,
    lock_suite,
)
...
class TestImmutableRegistries:
    def test_recommended_scorers_is_mappingproxy(self):
        from kadhi_cli.utils.eval_lock_coverage import _RECOMMENDED_SCORERS
    ...
    def test_metric_direction_is_mappingproxy(self):
        from kadhi_cli.utils.eval_gate_hook import _METRIC_DIRECTION
...
class TestShellEscape:
    ...
    def test_render_no_handrolled_escape_symbol_remains(self):
        from kadhi_cli.utils import eval_gate_hook as mod
...
class TestNoHeavyImports:
    ...
    @pytest.mark.parametrize(
        "module",
        [
            "src/kadhi_cli/utils/eval_design.py",
            "src/kadhi_cli/utils/canary_discovery.py",
            "src/kadhi_cli/utils/eval_lock_coverage.py",
            "src/kadhi_cli/utils/eval_gate_hook.py",
            "src/kadhi_cli/utils/_eval_text.py",
            "src/kadhi_cli/commands/_eval_v0550.py",
        ],
    )
...
class TestSharedTextUtils:
    def test_shared_module_exposes_row_text_and_tokenize(self):
        from kadhi_cli.utils import _eval_text
    ...
    def test_canary_no_longer_imports_from_eval_design(self):
        ...
        assert "from kadhi_cli.utils.eval_design import _row_text" not in src
        assert "from kadhi_cli.utils._eval_text import" in src
...
# ---------------------------------------------------------------------------
# `kadhi eval against` — run-vs-run paired-bootstrap CI
# ---------------------------------------------------------------------------
