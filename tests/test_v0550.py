"""Tests for v0.55.0 — kadhi eval design / discover / lock / coverage / gate.
...
class TestPrePushHookRendering:
    def test_basic_render(self):
        ...
        assert "kadhi eval against" in body
...
class TestSourceGrep:
    ...
    def test_eval_py_registers_v0550_module(self):
        text = (self.REPO_ROOT / "src" / "kadhi_cli" / "commands" / "eval.py").read_text(
            encoding="utf-8",
        )
    ...
    def test_v0550_module_uses_lazy_imports(self):
        ...
        text = (self.REPO_ROOT / "src" / "kadhi_cli" / "commands" / "_eval_v0550.py").read_text(
            encoding="utf-8",
        )
    ...
    def test_version_bumped_to_0_55_0(self):
        ...
        init_text = (
            self.REPO_ROOT / "src" / "kadhi_cli" / "__init__.py"
...
# ---------------------------------------------------------------------------
# `kadhi eval against` — run-vs-run regression check (Part D)
# ---------------------------------------------------------------------------
...
class TestHookTemplate:
    """v0.55.0 hook template references KADHI_CANDIDATE_RUN_ID env var."""

    def test_template_calls_kadhi_eval_against(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        (tmp_path / "evals").mkdir()
        (tmp_path / "evals" / "locked.json").write_text("{}")
        ...
        assert "KADHI_CANDIDATE_RUN_ID" in body
        # Hook is bash-strict.
        assert "set -euo pipefail" in body
