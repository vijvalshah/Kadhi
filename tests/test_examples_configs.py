...
through several schema changes, so ``kadhi train --config`` on any of them
...
def test_example_config_parses(config_path: Path):
    """Each example must load as a valid KadhiConfig.

    This is what ``kadhi train --config <path>`` does before it touches a
