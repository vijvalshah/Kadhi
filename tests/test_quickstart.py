"""Tests for kadhi quickstart command."""
...
from kadhi_cli.commands.quickstart import DEMO_CONFIG, DEMO_DATA

runner = CliRunner()


def test_quickstart_help():
    """kadhi quickstart --help works."""
...
def test_quickstart_dry_run(tmp_path, monkeypatch):
    """kadhi quickstart --dry-run creates files but does not train."""
    ...
    config_path = tmp_path / "quickstart_kadhi.yaml"
...
def test_quickstart_dry_run_existing_files(tmp_path, monkeypatch):
    """kadhi quickstart --dry-run does not overwrite existing files."""
    monkeypatch.chdir(tmp_path)
...
def test_quickstart_cancel(tmp_path, monkeypatch):
    """kadhi quickstart can be cancelled."""
    monkeypatch.chdir(tmp_path)
...
def test_quickstart_yes_dry_run(tmp_path, monkeypatch):
    """kadhi quickstart --yes --dry-run skips confirmation."""
    monkeypatch.chdir(tmp_path)
    result = runner.invoke(app, ["quickstart", "--yes", "--dry-run"])
    assert result.exit_code == 0
    assert "Dry run" in result.output
    assert (tmp_path / "quickstart_data.jsonl").exists()
    assert (tmp_path / "quickstart_kadhi.yaml").exists()
