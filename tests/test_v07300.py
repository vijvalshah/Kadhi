...
class TestDataMaxLengthReachesTrl:
    ...
    Kadhi builds a ``transformers.TrainingArguments``, not a ``trl.SFTConfig``.
...
class TestLigerTellsTrlItIsOn:
    ...
    Kadhi patches the model with ``apply_liger_kernel()`` but never set
...
class TestSglangDecodesItsRuntimeResponse:
    ...
    def test_a_json_string_is_decoded(self):
        ...
        from kadhi_cli.utils.sglang import decode_sglang_response
...
# ==========================================================================
# #77 — `kadhi train --gpus N` never launched at all
# ==========================================================================
class TestMultiGpuLaunchArgvIsRunnable:
    """The whole multi-GPU entry point was dead.

    ``accelerate launch`` takes a **script path** (or ``--module NAME``), and Kadhi
    ...
    def test_the_python_dash_m_form_becomes_accelerates_module_flag(self):
        ...
        argv = build_accelerate_argv(
            num_processes=4,
            script_args=[_sys.executable, "-m", "kadhi_cli.cli", "train", "-c", "s.yaml"],
        )
        ...
        assert argv[argv.index("--module") + 1] == "kadhi_cli.cli"
    ...
    def test_single_process_is_still_directly_executable(self):
        ...
        argv = build_accelerate_argv(
            num_processes=1, script_args=[_sys.executable, "-m", "kadhi_cli.cli", "train"]
        )
        assert argv == [_sys.executable, "-m", "kadhi_cli.cli", "train"]
...
def _fire_on_save(callback, output_dir, step, *, should_save=True, world_zero=None):
    ...
    run Kadhi can produce today does. Pass it explicitly to drive them apart, the
...
class TestCheckpointAdaptersAreCanonicalToo:
    ...
    def test_a_checkpoint_written_under_compile_is_normalised(self, tmp_path):
        ...
        from kadhi_cli.utils.peft_wiring import build_compile_prefix_callback
    ...
    def test_a_writing_rank_that_is_not_world_zero_still_repairs(self, tmp_path):
        ...
        would leave exactly those nodes holding an unrepaired adapter. Kadhi sets
...
class TestNoReexecHintKeepsTheUsersFlags:
    ...
    The hint was hand-built as ``["kadhi", "train", "-c", config]`` a few lines
