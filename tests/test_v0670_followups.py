...
- kadhi.lock version-drift advisory check
"""
...
class TestCmaesPlateauConvergence:
    def test_converges_on_flat_landscape(self, tmp_path, monkeypatch) -> None:
        ...
        from kadhi_cli.utils.cmaes_merge import build_cmaes_plan, run_cmaes_merge
...
class TestCmaesValidatorBoundaries:
    def test_population_min_accepted(self) -> None:
        from kadhi_cli.utils.cmaes_merge import MIN_POPULATION, validate_population_size
    ...
    def test_population_max_accepted(self) -> None:
        from kadhi_cli.utils.cmaes_merge import MAX_POPULATION, validate_population_size

        assert validate_population_size(MAX_POPULATION) == MAX_POPULATION

    def test_generations_min_accepted(self) -> None:
        from kadhi_cli.utils.cmaes_merge import MIN_GENERATIONS, validate_generations

        assert validate_generations(MIN_GENERATIONS) == MIN_GENERATIONS

    def test_generations_max_accepted(self) -> None:
        from kadhi_cli.utils.cmaes_merge import MAX_GENERATIONS, validate_generations
...
class TestVectorBankForwardCompat:
    ...
    def test_write_bank_symlink_rejected(self, tmp_path, monkeypatch) -> None:
        ...
        from kadhi_cli.utils.vector_bank import VectorBank, write_bank
...
class TestValidateUserIdBool:
    def test_bool_rejected(self) -> None:
        from kadhi_cli.utils.vector_bank import validate_user_id
...
class TestVectorBankBoundaries:
    def test_vector_dim_at_max_accepted(self) -> None:
        from kadhi_cli.utils.vector_bank import MAX_VECTOR_DIM, VectorBank
...
class TestPRRenderEmptySections:
    def test_all_empty_sections(self) -> None:
        ...
        from kadhi_cli.utils.adapter_pr import AdapterPR, render_pr_markdown
    ...
    def test_json_handles_empty_sections(self) -> None:
        from kadhi_cli.utils.adapter_pr import AdapterPR, render_pr_json
...
# -----------------------------------------------------------------------------
# Part E — kadhi_version drift is advisory-only (not a drift signal)
# -----------------------------------------------------------------------------


class TestKadhiLockVersionDriftAdvisory:
    def test_version_change_not_drift(self) -> None:
        """``kadhi_version`` differing between expected and actual locks
        should NOT count as drift — operators upgrade Kadhi legitimately."""
        from kadhi_cli.utils.kadhi_lock import KadhiLock, check_lock_drift
        ...
        expected = KadhiLock(kadhi_version="0.67.0", **base)
        actual = KadhiLock(kadhi_version="0.68.0", **base)
    ...
    def test_created_at_change_not_drift(self) -> None:
        ...
        expected = KadhiLock(created_at="2026-05-24", **base)
        actual = KadhiLock(created_at="2026-06-01", **base)
...
class TestBisectNonMonotonic:
    def test_non_monotonic_returns_valid_result(self) -> None:
        ...
        from kadhi_cli.utils.adapter_bisect import (
            BisectPlan,
            BisectResult,
            run_bisect,
        )
...
class TestSourceWiringRegressions:
    ...
    def test_all_modules_have_future_annotations(self) -> None:
        ...
        for module in (
            "src/kadhi_cli/utils/cmaes_merge.py",
            "src/kadhi_cli/utils/vector_bank.py",
            "src/kadhi_cli/utils/mole_routing.py",
            "src/kadhi_cli/utils/adapter_pr.py",
            "src/kadhi_cli/utils/kadhi_lock.py",
            "src/kadhi_cli/utils/adapter_bisect.py",
            "src/kadhi_cli/commands/lock.py",
        ):
    ...
    def test_supported_strategies_has_cmaes(self) -> None:
        from kadhi_cli.utils.adapter_merge import SUPPORTED_STRATEGIES

        assert "cmaes" in SUPPORTED_STRATEGIES

    def test_subprocess_call_uses_argv_list(self) -> None:
        """`kadhi adapters bisect` must use argv list mode (no shell=True)."""
        from pathlib import Path

        root = Path(__file__).resolve().parent.parent
        src = (root / "src" / "kadhi_cli" / "commands" / "adapters.py").read_text(
            encoding="utf-8"
        )
