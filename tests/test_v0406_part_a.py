...
def _trainer_source(name: str) -> str:
    path = Path(__file__).resolve().parent.parent / "src" / "kadhi_cli" / "trainer" / f"{name}.py"
...
def test_attach_relora_callback_returns_false_when_disabled() -> None:
    ...
    from kadhi_cli.utils.peft_wiring import attach_relora_callback
...
def test_attach_relora_callback_attaches_when_enabled() -> None:
    ...
    from kadhi_cli.utils.peft_wiring import attach_relora_callback
    from kadhi_cli.utils.relora import ReLoRACallback
...
def test_apply_pre_lora_patches_runs_on_gemma4(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "kadhi_cli.utils.peft_patches.apply_gemma4_clippable_patch", fake_patcher
    )
    model = MagicMock()
...
def test_apply_pre_lora_patches_swallows_exceptions(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "kadhi_cli.utils.peft_patches.apply_gemma4_clippable_patch", _boom
    )
    # Must not raise.
...
def test_apply_post_lora_patches_invokes_strip(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "kadhi_cli.utils.peft_patches.strip_lora_dropout_for_3d_experts",
        fake_strip,
    )
    model = MagicMock()
...
def test_apply_post_lora_patches_swallows_exceptions(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "kadhi_cli.utils.peft_patches.strip_lora_dropout_for_3d_experts", _boom
    )
