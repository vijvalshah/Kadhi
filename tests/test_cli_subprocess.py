...
These tests run `kadhi` (or `python -m kadhi_cli`) as a real subprocess,
...
# Use `python -m kadhi_cli` for reliability across platforms (no need for
# the entry-point script to be installed or on PATH).
KADHI_CMD = [sys.executable, "-m", "kadhi_cli"]
...
def run_kadhi(
    *args: str, timeout: int = TIMEOUT, env: dict | None = None,
    ...
    """Run kadhi CLI as a subprocess and return CompletedProcess."""
...
class TestEntryPoint:
    ...
    def test_version(self):
        ...
        result = run_kadhi("version")
        assert result.returncode == 0
        assert __version__ in result.stdout

    def test_version_full(self):
        result = run_kadhi("version", "--full")
        assert result.returncode == 0
        assert "Python" in result.stdout

    def test_unknown_command_fails(self):
        result = run_kadhi("nonexistent_command_xyz")
        assert result.returncode != 0
...
class TestCommandHelp:
    ...
    def test_command_help(self, cmd):
        result = run_kadhi(cmd, "--help")
        assert result.returncode == 0, (
            f"`kadhi {cmd} --help` failed (rc={result.returncode}):\n{result.stderr}"
        )
        # Should contain some usage info
        assert len(result.stdout) > 20, f"`kadhi {cmd} --help` output too short"

    @pytest.mark.parametrize("subcmd", ALL_DATA_SUBCOMMANDS)
    def test_data_subcommand_help(self, subcmd):
        result = run_kadhi("data", subcmd, "--help")
        assert result.returncode == 0, (
            f"`kadhi data {subcmd} --help` failed (rc={result.returncode}):\n{result.stderr}"
        )

    @pytest.mark.parametrize("subcmd", ALL_RUNS_SUBCOMMANDS)
    def test_runs_subcommand_help(self, subcmd):
        result = run_kadhi("runs", subcmd, "--help")
        assert result.returncode == 0, (
            f"`kadhi runs {subcmd} --help` failed (rc={result.returncode}):\n{result.stderr}"
        )

    def test_data_no_args_shows_help(self):
        result = run_kadhi("data")
...
class TestErrorHandling:
    ...
    def test_train_missing_config(self):
        result = run_kadhi("train", "--config", "nonexistent_file.yaml")
        assert result.returncode == 1

    def test_chat_missing_model(self):
        result = run_kadhi("chat", "--model", "nonexistent_model_path")
        assert result.returncode == 1
    ...
    def test_init_unknown_template(self):
        result = run_kadhi("init", "--template", "nonexistent_template")
        assert result.returncode == 1

    def test_export_missing_model(self):
        result = run_kadhi("export", "--model", "nonexistent_model_path")
        assert result.returncode == 1

    def test_merge_missing_adapter(self):
        result = run_kadhi("merge", "--adapter", "nonexistent_adapter_path")
        assert result.returncode == 1
...
class TestEncoding:
    ...
    def test_error_output_is_ascii_safe(self):
        ...
        result = run_kadhi("train", "--config", "nonexistent.yaml")
...
class TestDataCommands:
    ...
    def test_data_inspect_jsonl(self, tmp_path):
        ...
        result = run_kadhi("data", "inspect", str(data_file))
        assert result.returncode == 0
        assert "5" in result.stdout  # row count

    def test_data_inspect_nonexistent_file(self):
        result = run_kadhi("data", "inspect", "nonexistent_file.jsonl")
        assert result.returncode == 1
    ...
    def test_data_validate_jsonl(self, tmp_path):
        ...
        result = run_kadhi("data", "validate", str(data_file))
        assert result.returncode == 0
...
class TestRunCommands:
    def test_runs_replay_no_color_has_no_escape_bytes(self, tmp_path, monkeypatch):
        ...
        result = run_kadhi(
            ...
            "replay",
            run_id,
            env={"NO_COLOR": "1", "KADHI_DB_PATH": str(db_path)},
        )
...
class TestInitCommand:
    ...
    def test_init_creates_config_file(self, tmp_path, template):
        """kadhi init --template X should create a valid YAML config."""
        result = run_kadhi(
            "init",
            "--template", template,
    ...
    def test_init_default_output(self, tmp_path, monkeypatch):
        """kadhi init --template chat writes kadhi.yaml in cwd."""
        monkeypatch.chdir(tmp_path)
        result = run_kadhi("init", "--template", "chat")
        assert result.returncode == 0
...
class TestDoctorCommand:
    ...
    def test_doctor_runs(self):
        ...
        result = run_kadhi("doctor", timeout=120)
        assert result.returncode == 0
...
class TestVerboseFlag:
    ...
    def test_verbose_with_version(self):
        result = run_kadhi("--verbose", "version")
        assert result.returncode == 0

    def test_verbose_short_flag(self):
        result = run_kadhi("-V", "version")
        assert result.returncode == 0

    def test_verbose_error_shows_traceback(self):
        result = run_kadhi("--verbose", "train", "--config", "nonexistent.yaml")
        assert result.returncode == 1
...
class TestQuickstart:
    ...
    def test_quickstart_help(self):
        result = run_kadhi("quickstart", "--help")
        assert result.returncode == 0
...
class TestEvalCommand:
    ...
    def test_eval_help(self):
        result = run_kadhi("eval", "--help")
        assert result.returncode == 0

    def test_eval_missing_model(self):
        result = run_kadhi("eval", "benchmark", "--model", "nonexistent_model")
        assert result.returncode == 1
