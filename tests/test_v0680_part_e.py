"""v0.68.0 Part E — ``kadhi local-rl`` personal-LLM flywheel daemon.
...
train scheduler is the stub. Smaller-scope cousin of v0.58 ``kadhi loop``.
"""
...
class TestAllowlists:
    def test_backend_frozenset(self) -> None:
        from kadhi_cli.utils.local_rl import SUPPORTED_LOCAL_RL_BACKENDS
...
class TestCli:
    ...
    def test_train_no_once_renders_scheduler(
        ...
        assert result.exit_code == 0, (result.output, repr(result.exception))
        assert (tmp_path / "sched" / "kadhi-local-rl.timer").is_file()
