...
Closes #225 (kadhi compile), #226 (kadhi distill-prompt), #227 (kadhi
compile-tools), #229 (kadhi local-rl train). Lifts the v0.68.1 deferred
...
# ===========================================================================
# #229 — kadhi local-rl train (live DPO/KTO/ORPO runner + scheduler)
# ===========================================================================


def _seed_db(path: str, n_pairs: int) -> None:
    from kadhi_cli.utils.local_rl import init_local_rl_db, record_thumb
...
class TestStateTable:
    def test_init_creates_state_table(self, tmp_path, monkeypatch):
        ...
        from kadhi_cli.utils.local_rl import init_local_rl_db
    ...
    def test_get_set_state_roundtrip(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.utils.local_rl import (
            get_state,
            init_local_rl_db,
            set_state,
        )
    ...
    def test_get_state_empty_key_rejected(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.utils.local_rl import init_local_rl_db, set_state
    ...
    def test_state_table_auto_migrates_old_db(self, tmp_path, monkeypatch):
        ...
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.utils.local_rl import get_state
...
class TestCountNewThumbs:
    def test_counts_all_when_since_none(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.utils.local_rl import count_new_thumbs_since
...
class TestPairsToRows:
    def _pairs(self):
        from kadhi_cli.utils.local_rl import DpoPair

        return (DpoPair(prompt="p", chosen="c", rejected="r"),)

    def test_dpo_shape(self):
        from kadhi_cli.utils.local_rl import pairs_to_rows
...
class TestRunNightlyTrain:
    def _cfg(self, db, method="dpo"):
        from kadhi_cli.utils.local_rl import LocalRLConfig
    ...
    def test_kto_rows_written(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.utils.local_rl import run_nightly_train
    ...
    def test_skip_no_new_thumbs(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.utils.local_rl import run_nightly_train, set_state
    ...
    def test_default_train_fn_renders_schema_valid_config(self, tmp_path, monkeypatch):
        """The real ``_default_train_fn`` must render a YAML that the KadhiConfig
        ...
        from kadhi_cli.utils import local_rl as lr
...
class TestScheduler:
    def test_build_train_argv(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.utils.local_rl import init_local_rl_db
        from kadhi_cli.utils.local_rl_scheduler import build_train_argv
    ...
    def test_systemd_service_has_execstart(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.utils.local_rl import init_local_rl_db
        from kadhi_cli.utils.local_rl_scheduler import render_systemd_service
    ...
    def test_systemd_timer_oncalendar(self):
        from kadhi_cli.utils.local_rl_scheduler import render_systemd_timer
    ...
    def test_launchd_plist(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.utils.local_rl import init_local_rl_db
        from kadhi_cli.utils.local_rl_scheduler import render_launchd_plist
        ...
        assert "com.kadhi.local-rl" in out

    def test_write_scheduler_files(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.utils.local_rl import init_local_rl_db
        from kadhi_cli.utils.local_rl_scheduler import (
            LAUNCHD_PLIST_NAME,
    ...
    def test_systemd_quote_escapes(self):
        from kadhi_cli.utils.local_rl_scheduler import _systemd_quote
...
class TestTrainCli:
    def _import_app(self):
        from kadhi_cli.commands.local_rl import app
    ...
    def test_once_runs_live(self, tmp_path, monkeypatch):
        ...
        import kadhi_cli.utils.local_rl as lr
    ...
    def test_no_once_renders_scheduler(self, tmp_path, monkeypatch):
        ...
        assert res.exit_code == 0, (res.output, repr(res.exception))
        assert os.path.isfile(str(tmp_path / "sched" / "kadhi-local-rl.timer"))
...
# ===========================================================================
# #226 — kadhi distill-prompt (live teacher/student dataset prep)
# ===========================================================================
...
class TestExtractPrompt:
    def test_prompt_field(self):
        from kadhi_cli.utils.prompt_distill import extract_prompt
...
class TestPrepareDistillDataset:
    def _plan(self, tmp_path, strategy):
        from kadhi_cli.utils.prompt_distill import build_distill_prompt_plan
    ...
    def test_sft_writes_messages(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.utils.prompt_distill import prepare_distill_dataset
...
class TestDistillPromptCli:
    def test_cli_live(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.commands.distill_prompt import distill_prompt_cmd

        _write_traces(str(tmp_path / "traces.jsonl"), ["a", "b"])
        import kadhi_cli.utils.prompt_distill as pd
...
# ===========================================================================
# #225 — kadhi compile (DSPy / GEPA / TextGrad dispatcher)
# ===========================================================================
...
class TestLoadEvalExamples:
    def test_json_list(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.utils.prompt_compile import load_eval_examples
...
class TestRunCompileSeam:
    def _plan(self, tmp_path):
        from kadhi_cli.utils.prompt_compile import build_compile_plan
    ...
    def test_override_seam(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        import kadhi_cli.utils.prompt_compile as pc
    ...
    def test_non_plan_rejected(self):
        from kadhi_cli.utils.prompt_compile import run_compile
    ...
    def test_missing_dspy_friendly_importerror(self, tmp_path, monkeypatch):
        ...
        with pytest.raises(ImportError) as exc:
            run_compile(plan)
        assert "kadhi-cli[compile]" in str(exc.value)

    def test_missing_textgrad_friendly(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.utils.prompt_compile import build_compile_plan, run_compile
...
class TestCompileCli:
    def test_plan_only(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.commands.compile_cmd import compile_cmd
...
# ===========================================================================
# #227 — kadhi compile-tools (TextGrad / GEPA tool-schema optimiser)
# ===========================================================================
...
class TestOptimiseDescription:
    def test_override_returns_str(self, monkeypatch):
        import kadhi_cli.utils.compile_tools as ct
    ...
    def test_missing_textgrad_friendly(self):
        from kadhi_cli.utils.compile_tools import _optimise_description
...
class TestRunToolCompile:
    def _plan(self, tmp_path, output="out.json"):
        from kadhi_cli.utils.compile_tools import build_tool_compile_plan
    ...
    def test_non_plan_rejected(self):
        from kadhi_cli.utils.compile_tools import run_tool_compile
...
class TestCompileToolsCli:
    def test_plan_only(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.commands.compile_tools import compile_tools_cmd
...
class TestReviewFixes:
    def test_db_path_rejects_newline(self, tmp_path, monkeypatch):
        ...
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.utils.local_rl import validate_db_path
    ...
    def test_model_rejects_newline(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.utils.local_rl import LocalRLConfig, init_local_rl_db
    ...
    def test_cli_once_subprocess_failure_exit_1(self, tmp_path, monkeypatch):
        # CORRECTNESS MEDIUM: a failed kadhi-train subprocess -> friendly exit 1.
        monkeypatch.chdir(tmp_path)
        import subprocess
        ...
        def boom(**kw):
            raise subprocess.CalledProcessError(1, ["kadhi", "train"])
...
class TestCoverageGaps:
    ...
    def test_compile_result_validation(self):
        from kadhi_cli.utils.prompt_compile import CompileResult
    ...
    def test_extract_prompt_truncation(self):
        from kadhi_cli.utils.prompt_distill import _MAX_PROMPT_CHARS, extract_prompt
    ...
    def test_preference_empty_student_skipped(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.utils.prompt_distill import (
    ...
    def test_spec_path_extension_rejected(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.utils.compile_tools import validate_spec_path
...
class TestReachableInternals:
    ...
    def test_build_provider_fn_delegates_to_make_judge(self, monkeypatch):
        import kadhi_cli.utils.data_forge as df
        import kadhi_cli.utils.prompt_distill as pd
...
class TestPatchInvariants:
    def test_version_bumped(self):
        ...
        parts = tuple(int(x) for x in kadhi_cli.__version__.split(".")[:3])
