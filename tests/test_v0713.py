...
Closes:
- #180 Live CodeCarbon hook (EnergyTracker context manager) + `kadhi train --track-energy`
- #181 PDF rendering for Annex XI/XII (`write_annex_doc(..., fmt="pdf")`)
- #182 Kadhi Can manifest v3 `attestations` field + embed in-toto Statement
...
- #188 Auto-attach the repro-receipt into `kadhi airgap-bundle`
...
class TestEnergyTracker:
    def test_imports(self):
        from kadhi_cli.utils.energy import EnergyMeasurement, EnergyTracker  # noqa: F401
...
class TestTopDomains:
    def test_imports(self):
        from kadhi_cli.utils.annex_xi import (  # noqa: F401
            extract_top_domains,
            load_top_domains_from_jsonl,
        )

    def test_extract_top_domains_counts_and_shares(self):
        from kadhi_cli.utils.annex_xi import extract_top_domains
...
class TestAnnexPdf:
    def test_atomic_write_bytes_imports(self):
        from kadhi_cli.utils.paths import atomic_write_bytes  # noqa: F401

    def test_atomic_write_bytes_roundtrip(self, tmp_path, monkeypatch):
        from kadhi_cli.utils.paths import atomic_write_bytes
    ...
    def test_render_annex_pdf_starts_with_pdf_magic(self):
        from kadhi_cli.utils.annex_xi import render_annex_pdf
    ...
    def test_write_annex_doc_markdown_default(self, tmp_path, monkeypatch):
        from kadhi_cli.utils.annex_xi import write_annex_doc
...
# ---------------------------------------------------------------------------
# #182 — Kadhi Can manifest v3 `attestations` field
# ---------------------------------------------------------------------------
...
class TestCanManifestV3:
    def test_version_bumped_to_3(self):
        from kadhi_cli.cans.schema import (
            CAN_FORMAT_VERSION,
    ...
    def test_v1_and_v2_still_load(self):
        from kadhi_cli.cans.schema import Manifest
    ...
    def test_validate_attestation_statement_helper(self):
        from kadhi_cli.cans.schema import validate_attestation_statement
    ...
    def test_pack_entry_embeds_attestations(self, tmp_path, monkeypatch):
        ...
        from kadhi_cli.cans import pack as pack_mod
...
class TestAuditInstrumentation:
    def test_imports(self):
        from kadhi_cli.cli import _emit_audit_event, _split_command_args  # noqa: F401

    def test_split_command_args_basic(self):
        from kadhi_cli.cli import _split_command_args

        cmd, args = _split_command_args(["kadhi", "train", "--config", "x"])
        assert cmd == "train"
    ...
    def test_split_command_args_skips_log_level_value(self):
        ...
        cmd, args = _split_command_args(
            ["kadhi", "--log-level", "debug", "train", "--yes"]
        )
        assert cmd == "train"
        assert args == ["--yes"]
    ...
    def test_split_command_args_root_only(self):
        ...
        cmd, args = _split_command_args(["kadhi"])
        assert cmd == "(root)"
        assert args == []
    ...
    def test_emit_audit_event_env_opt_out(self, tmp_path, monkeypatch):
        ...
        log = tmp_path / "audit.jsonl"
        monkeypatch.setenv("KADHI_AUDIT_LOG_PATH", str(log))
        monkeypatch.setenv("KADHI_NO_AUDIT_LOG", "1")
        monkeypatch.setattr(cli_mod, "_audit_disabled", False)
        cli_mod._emit_audit_event(["kadhi", "version"], 0)
    ...
    def test_emit_audit_event_never_raises(self, tmp_path, monkeypatch):
        import kadhi_cli.cli as cli_mod
        import kadhi_cli.utils.audit_log as audit_mod
        ...
        cli_mod._emit_audit_event(["kadhi", "x"], 1)  # must NOT raise
    ...
    def test_subprocess_emits_audit_line(self, tmp_path):
        """End-to-end: a real `kadhi` invocation via run() writes an audit line."""
        log = tmp_path / "audit.jsonl"
        env = dict(os.environ)
        env["KADHI_AUDIT_LOG_PATH"] = str(log)
        env.pop("KADHI_NO_AUDIT_LOG", None)
        env["PYTHONUTF8"] = "1"
        r = subprocess.run(
            [sys.executable, "-m", "kadhi_cli", "version"],
...
class TestAirgapReproReceipt:
    def test_plan_accepts_repro_receipt_field(self, tmp_path, monkeypatch):
        from kadhi_cli.utils.airgap_bundle import AirgapBundlePlan
    ...
    def test_build_embeds_receipt_member(self, tmp_path, monkeypatch):
        ...
        from kadhi_cli.utils.airgap_bundle import (
            AirgapBundlePlan,
            build_airgap_bundle,
    ...
    def test_cli_explicit_repro_receipt(self, tmp_path, monkeypatch):
        from kadhi_cli.cli import app
        from kadhi_cli.utils.airgap_bundle import inspect_airgap_bundle
...
class TestReviewFollowups:
    ...
    def test_extract_top_domains_urls_per_row_cap(self, monkeypatch):
        import kadhi_cli.utils.annex_xi as ax
    ...
    def test_render_annex_pdf_escapes_operator_markup(self):
        from kadhi_cli.utils.annex_xi import AnnexXIData, render_annex_pdf
    ...
    def test_load_attestation_file_oversize_rejected(self, tmp_path, monkeypatch):
        from kadhi_cli.commands.can import _load_attestation_files
    ...
    def test_split_command_args_value_opt_followed_by_flag(self):
        ...
        cmd, args = _split_command_args(
            ["kadhi", "--log-level", "--yes", "train", "--config", "x"]
        )
    ...
    def test_split_command_args_trailing_value_opt(self):
        ...
        cmd, args = _split_command_args(["kadhi", "--log-level"])
        assert cmd == "(root)"
    ...
    def test_emit_audit_event_nonzero_exit_code(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("KADHI_NO_AUDIT_LOG", raising=False)
        monkeypatch.setattr(cli_mod, "_audit_disabled", False)
        cli_mod._emit_audit_event(["kadhi", "train", "--config", "x"], 3)
        rows = [json.loads(x) for x in log.read_text().splitlines() if x.strip()]
    ...
    def test_subprocess_no_audit_log_flag_suppresses(self, tmp_path):
        ...
        env["PYTHONUTF8"] = "1"
        r = subprocess.run(
            [sys.executable, "-m", "kadhi_cli", "--no-audit-log", "version"],
            env=env, capture_output=True, text=True, timeout=120,
        )
        assert r.returncode == 0, (r.stdout, r.stderr)
        assert not log.exists()
    ...
    def test_manifest_from_payload_coerces_bad_receipt_to_none(self):
        from kadhi_cli.utils.airgap_bundle import _manifest_from_payload

        m = _manifest_from_payload({
            "kadhi_version": "0.71.3", "created_at": "x", "model_dir": "model",
