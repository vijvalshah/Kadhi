"""Tests for kadhi bench CLI command."""
...
def test_bench_model_not_found():
    """kadhi bench with nonexistent model should fail gracefully."""
...
def test_bench_custom_prompts(tmp_path, monkeypatch):
    ...
    with patch("kadhi_cli.commands.infer._load_model") as mock_load, \
         patch("kadhi_cli.commands.infer._generate") as mock_generate:
...
def test_bench_happy_path(tmp_path, monkeypatch):
    ...
    with patch("kadhi_cli.commands.infer._load_model") as mock_load, \
         ...
         patch("kadhi_cli.utils.gpu.detect_device") as mock_detect_device:
