...
class TestKTOSweepParams:
    ...
    def test_sweep_run_single_routes_to_kto_trainer(self):
        ...
        with mock_patch("kadhi_cli.data.loader.load_dataset", return_value=fake_dataset), \
             ...
             mock_patch("kadhi_cli.trainer.kto.KTOTrainerWrapper.setup"), \
             mock_patch(
                 "kadhi_cli.trainer.kto.KTOTrainerWrapper.train", return_value=fake_result
             ) as mock_train:
...
class TestKTOTrainResults:
    ...
    def test_train_result_duration_minutes_format(self):
        ...
        with mock_patch("kadhi_cli.trainer.kto.time.time", side_effect=fake_time):
            result = wrapper.train()
...
class TestKTOInitTemplate:
    """Test that kadhi init produces correct output for KTO."""

    def test_init_kto_template_creates_file(self, tmp_path):
        """kadhi init --template kto should write a file with kto task."""
    ...
    def test_init_kto_template_produces_valid_config(self, tmp_path):
        """The file written by kadhi init --template kto should parse to a valid KadhiConfig."""
