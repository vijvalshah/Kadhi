...
def test_version_bump_to_0_53_9():
    ...
    parts = tuple(int(p) for p in kadhi_cli.__version__.split(".") if p.isdigit())
    assert parts >= (0, 53, 9)
...
def test_train_event_buffer_push_and_drain():
    ...
    from kadhi_cli.utils.train_event_buffer import TrainEventBuffer
...
def test_push_train_event_silent_on_bad_input():
    from kadhi_cli.utils.train_event_buffer import (
        get_global_buffer,
        push_train_event,
        reset_global_buffer,
    )
...
def test_detect_backend_fallback_transformers():
    from kadhi_cli.utils.backend_detect import detect_backend
...
def test_detect_backend_env_hint(monkeypatch):
    ...
    monkeypatch.setenv("KADHI_BENCH_BACKEND", "VLLM")
    assert detect_backend("anything") == "vllm"
    monkeypatch.setenv("KADHI_BENCH_BACKEND", "bogus-name")
...
def test_set_auth_token_concurrent_rotation_safe():
    ...
    from kadhi_cli.ui.app import get_auth_token, set_auth_token
...
def test_synthetic_workflow_doc_exists():
    ...
    assert "kadhi data generate" in body
    assert "kadhi data filter" in body
    assert "kadhi data score" in body
    assert "kadhi train" in body
