...
- #116: `proxy_run_for_weights` live mix proxy + `kadhi data mix --live`.
...
from kadhi_cli.recipes.catalog import RECIPES, get_recipe
...
def test_trainer_source_grep_curriculum_wired_everywhere():
    ...
    root = Path(__file__).resolve().parent.parent / "src" / "kadhi_cli" / "trainer"
    trainers = [
...
def test_proxy_rejects_outside_cwd(tmp_path, monkeypatch):
    ...
    outside = Path(_tf.gettempdir()) / "definitely_outside_kadhi.yaml"
...
def test_overlay_yaml_loads_through_config_schema(tmp_path):
    ...
    from kadhi_cli.utils.mix_proxy import _render_overlay_yaml
...
def test_proxy_happy_path_reads_tracker(tmp_path, monkeypatch):
    ...
    with patch.object(subprocess, "run", return_value=fake_result), \
            patch(
                "kadhi_cli.utils.mix_proxy.ExperimentTracker",
                return_value=fake_tracker,
                create=True,
            ):
        # We need to patch the lazy import inside `_read_final_eval_loss`:
        import kadhi_cli.experiment.tracker as tracker_mod
