...
Closes:
  ...
  * #252 — EditGovernor edit-count increment is now atomic (baseline-delta
           merge under the cross-process lock) so concurrent ``kadhi edit set``
...
class TestProjHelpers:
    def test_is_transposed_proj_conv1d(self):
        from kadhi_cli.utils.edit_kernels import _is_transposed_proj
    ...
    def test_proj_out_dim_conv1d_uses_nf(self):
        from kadhi_cli.utils.edit_kernels import _proj_out_dim
...
class TestRank1UpdateTransposed:
    def test_conv1d_post_condition(self):
        ...
        from kadhi_cli.utils.edit_kernels import _rank1_update
...
class TestAlphaEditProjectTransposed:
    def test_conv1d_shape_and_determinism(self):
        from kadhi_cli.utils.edit_kernels import _alphaedit_project
...
class TestApplyKernelsGpt2EndToEnd:
    def _run(self, method, model):
        from kadhi_cli.utils.edit_kernels import measure_target_prob, run_edit_kernel
    ...
    def test_memit_gpt2_edits_full_band(self):
        ...
        from kadhi_cli.utils.edit_kernels import apply_memit_edit
    ...
    def test_alphaedit_conv1d_weight_orientation(self):
        ...
        (review H3)."""
        from kadhi_cli.utils.edit_kernels import (
            _down_proj,
...
class TestLoadCovCorpus:
    def test_parses_jsonl_text_field(self, tmp_path, monkeypatch):
        from kadhi_cli.commands.edit import _load_cov_corpus
    ...
    def test_oversize_rejected(self, tmp_path, monkeypatch):
        ...
        import kadhi_cli.commands.edit as edit_mod
        from kadhi_cli.commands.edit import _load_cov_corpus
...
class TestIsMixtralModel:
    def test_basic(self):
        from kadhi_cli.utils.longlora import is_mixtral_model
...
class TestMixtralStillNotMistral:
    def test_is_mistral_model_excludes_mixtral(self):
        from kadhi_cli.utils.longlora import is_mistral_model
...
class TestMixtralInAllowlist:
    def test_supported(self):
        from kadhi_cli.utils.longlora import is_supported_longlora_arch
    ...
    def test_separate_qkv_families_includes_mixtral(self):
        from kadhi_cli.utils.longlora import _SEPARATE_QKV_FAMILIES
...
class TestValidateLongloraCompatMixtral:
    def test_accepts_mixtral(self, monkeypatch):
        from kadhi_cli.utils import longlora

        monkeypatch.setattr(
            "kadhi_cli.utils.flash_attn.is_flash_attn_v3_available", lambda: False
        )
        # Should not raise.
...
class TestMixtralForwardOverride:
    ...
    def test_patches_mixtral_attention(self):
        ...
        with LongLoRAForwardOverride(model, group_size=4):
            assert getattr(
                model.attn.q_proj.forward, "_kadhi_longlora_patched", False
            )
            assert getattr(
                model.attn.k_proj.forward, "_kadhi_longlora_patched", False
            )
        # Restored on exit.
        assert not getattr(
            model.attn.q_proj.forward, "_kadhi_longlora_patched", False
...
class TestPatchInvariants:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "kadhi_cli.utils.longlora",
            "kadhi_cli.commands.edit",
        ],
    )
    ...
    def test_edit_kernels_no_top_level_safetensors(self):
        ...
        mod = importlib.import_module("kadhi_cli.utils.edit_kernels")
        with open(mod.__file__, encoding="utf-8") as fh:
            src = fh.read()
