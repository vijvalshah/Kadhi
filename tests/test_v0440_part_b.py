...
from kadhi_cli.utils.checkpoint_trigger import (
    TRIGGER_FILENAME,
    consume_trigger,
    should_save_now,
    trigger_path,
...
from kadhi_cli.utils.onboarding import (
    VALID_QUANT,
    VALID_TASKS,
...
def test_build_linux_desktop_entry():
    spec = build_linux_desktop_entry(name="Kadhi CLI", command="kadhi ui")
    assert spec.platform == "linux"
    assert spec.filename == "kadhi-cli.desktop"
    assert "Exec=kadhi ui" in spec.content
    assert "Categories=Development;" in spec.content


def test_build_macos_command_file():
    spec = build_macos_command_file(name="Kadhi", command="kadhi ui")
...
def test_build_windows_cmd():
    spec = build_windows_cmd(name="Kadhi", command="kadhi ui")
...
def test_build_for_current_platform_returns_a_spec():
    ...
    if plat == "unknown":
        with pytest.raises(NotImplementedError):
            build_for_current_platform(name="Kadhi", command="kadhi ui")
    else:
        spec = build_for_current_platform(name="Kadhi", command="kadhi ui")
