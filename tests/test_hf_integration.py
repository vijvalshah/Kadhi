...
class TestResolveToken:
    def test_env_hf_token_wins(self, monkeypatch, tmp_path):
        ...
        from kadhi_cli.utils.hf import resolve_token
...
class TestResolveEndpoint:
    def test_default_endpoint(self, monkeypatch):
        ...
        from kadhi_cli.utils.hf import resolve_endpoint
...
class TestGetHfApi:
    def test_requires_huggingface_hub(self, monkeypatch):
        from kadhi_cli.utils.hf import get_hf_api
    ...
    def test_passes_token_and_endpoint(self, monkeypatch):
        from kadhi_cli.utils import hf
...
class TestHFPushCallback:
    def test_callback_import(self):
        from kadhi_cli.monitoring.hf_push import HFPushCallback  # noqa: F401

    def test_callback_uploads_checkpoint_on_save(self, tmp_path, monkeypatch):
        from kadhi_cli.monitoring.hf_push import HFPushCallback
        ...
        fake_api = MagicMock()
        monkeypatch.setattr("kadhi_cli.monitoring.hf_push.get_hf_api", lambda **_: fake_api)
...
class TestResolveLatestRevision:
    def test_returns_latest_step(self, tmp_path, monkeypatch):
        from kadhi_cli.monitoring.hf_push import resolve_latest_checkpoint_revision
        ...
        monkeypatch.setattr(
            "kadhi_cli.monitoring.hf_push.get_hf_api", lambda **_: fake_api
        )
...
class TestModelCardV2:
    def test_generate_enhanced_card_basic(self, tmp_path):
        from kadhi_cli.commands.push import generate_model_card_v2
...
class TestCollections:
    ...
    def test_add_to_collection_calls_api(self, monkeypatch):
        from kadhi_cli.utils.hf import add_to_collection
...
class TestDeployHfSpace:
    ...
    def test_render_gradio_chat_template(self):
        from kadhi_cli.commands.deploy import render_space_template
...
class TestAutoResume:
    ...
    def test_prepare_hf_resume_downloads_latest(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "kadhi_cli.monitoring.hf_push.resolve_latest_checkpoint_revision",
            lambda repo_id, token=None, endpoint=None: "checkpoint-300",
        )
        monkeypatch.setattr(
            "kadhi_cli.monitoring.hf_push._download_checkpoint", fake_download
        )
...
class TestValidateCollectionSlug:
    def test_accepts_valid(self):
        from kadhi_cli.utils.hf import validate_collection_slug
...
class TestBuildPushCallback:
    ...
    def test_happy_path_returns_callback(self, monkeypatch, tmp_path):
        from kadhi_cli.monitoring.hf_push import HFPushCallback, build_push_callback
...
class TestEvalScorecardEdgeCases:
    def test_non_numeric_score(self):
        from kadhi_cli.commands.push import _render_eval_scorecard
...
class TestDataPushHappyPathExtras:
    def test_create_repo_called_with_dataset_type(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("kadhi_cli.utils.hf.get_hf_api", lambda **_: fake_api)
        monkeypatch.setattr(
            "kadhi_cli.commands.data.get_hf_api", lambda **_: fake_api, raising=False
        )
