...
class TestResolvePreferenceLoss:
    def test_resolve_preference_task_returns_loss(self):
        from kadhi_cli.trainer.preference import resolve_preference_loss
...
class TestPreferenceTrainerWrapper:
    ...
    @pytest.mark.parametrize(
        "loss,wrapper_path",
        [
            ("dpo", "kadhi_cli.trainer.dpo.DPOTrainerWrapper"),
            ("simpo", "kadhi_cli.trainer.simpo.SimPOTrainerWrapper"),
            ("orpo", "kadhi_cli.trainer.orpo.ORPOTrainerWrapper"),
            ("ipo", "kadhi_cli.trainer.ipo.IPOTrainerWrapper"),
            ("bco", "kadhi_cli.trainer.bco.BCOTrainerWrapper"),
        ],
    )
    ...
    def test_setup_does_not_mutate_caller_cfg(self):
        ...
        with mock_patch("kadhi_cli.trainer.dpo.DPOTrainerWrapper") as mock_inner:
...
class TestPreferenceTrainRouting:
    def test_sweep_routes_to_preference_wrapper(self):
        ...
        with mock_patch(
            "kadhi_cli.data.loader.load_dataset", return_value=fake_dataset,
        ), mock_patch(
            "kadhi_cli.utils.gpu.detect_device", return_value=("cpu", "CPU"),
        ), mock_patch(
            "kadhi_cli.utils.gpu.get_gpu_info", return_value=fake_gpu_info,
        ), mock_patch(
            "kadhi_cli.experiment.tracker.ExperimentTracker",
        ) as mock_tracker_cls, mock_patch(
            "kadhi_cli.monitoring.display.TrainingDisplay",
        ), mock_patch(
            "kadhi_cli.trainer.preference.PreferenceTrainerWrapper.setup",
        ), mock_patch(
            "kadhi_cli.trainer.preference.PreferenceTrainerWrapper.train",
