...
- #86  kadhi data preprocess live tokenize
- #111 kadhi data forge --judge-provider
...
class TestPromptStrategyRuntime:
    def test_resolve_finds_callable(self):
        from kadhi_cli.utils.data_pipeline import resolve_prompt_strategy
    ...
    def test_apply_with_none_returns_row(self):
        from kadhi_cli.utils.data_pipeline import apply_prompt_strategy
        row = {"a": 1}
...
class TestPreprocessTokenize:
    ...
    def test_load_pretokenized_dataset_rejects_outside_cwd(self, tmp_path, monkeypatch):
        from kadhi_cli.utils.data_pipeline import load_pretokenized_dataset
...
class TestRunRecipeLive:
    def test_stub_gone(self, tmp_path, monkeypatch):
        from kadhi_cli.utils.recipe_dag import parse_recipe
...
class TestInstantiateTrainerPlugins:
    def test_cce_plugin_returns_advisory_callback(self):
        from kadhi_cli.utils.trainer_plugins import instantiate_trainer_plugins
...
def _create_test_app(host: str = "127.0.0.1", auth_token: str | None = None, **kwargs):
    ...
    from kadhi_cli.commands.serve import _create_app
...
class TestToolEndpointsLive:
    ...
    def test_bash_tool_executes_simple_command(self, monkeypatch):
        ...
        from kadhi_cli.trainer.rewards import SandboxProcessResult

        app = _create_test_app()
        client = TestClient(app)
        monkeypatch.setattr(
            "kadhi_cli.trainer.rewards._get_isolation_strategy", lambda: "namespaces"
        )
        monkeypatch.setattr(
            "kadhi_cli.trainer.rewards._run_bash_sandbox",
    ...
    def test_bash_tool_blocks_network_namespace(self):
        ...
        from kadhi_cli.trainer.rewards import _get_isolation_strategy, _run_bash_sandbox
    ...
    def test_bash_tool_windows_returns_501(self, monkeypatch):
        ...
        client = TestClient(app)
        monkeypatch.setattr(
            "kadhi_cli.trainer.rewards._get_isolation_strategy", lambda: "best-effort"
        )
        resp = client.post(
            "/v1/tools/bash",
            json={"command": "echo hello"},
    ...
    def test_bash_tool_bounded_streaming_kills_massive_output(self):
        if sys.platform == "win32":
            pytest.skip("bash sandbox not supported on Windows")
        from kadhi_cli.trainer.rewards import _run_sandboxed_subprocess
    ...
    def test_bash_sandbox_raises_on_strict_namespace_failure(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "kadhi_cli.trainer.rewards._get_isolation_strategy",
            lambda: "namespaces",
        )
        ...
        from kadhi_cli.trainer.rewards import _run_bash_sandbox
    ...
    def test_bash_sandbox_requests_strict_namespaces_by_name(self, monkeypatch):
        ...
        from kadhi_cli.trainer import rewards
    ...
    def test_web_search_with_allowlist_calls_backend(self):
        ...
        from kadhi_cli.commands.serve import _create_app
        from kadhi_cli.utils.server_tools import WebSearchConfig
...
class TestAnthropicMessagesStreaming:
    ...
    def test_stream_true_returns_sse(self):
        ...
        with patch(
            "kadhi_cli.commands.serve._generate_response",
...
class TestVllmAnthropicMessages:
    ...
    def test_vllm_stream_helper_emits_anthropic_events(self):
        from kadhi_cli.utils.vllm import _stream_anthropic_messages_vllm
...
class TestServeStreamHelper:
    def test_transformers_stream_helper_emits_anthropic_events(self):
        from kadhi_cli.commands.serve import _stream_anthropic_messages

        frames = list(
            _stream_anthropic_messages(
                msg_id="m1",
                model="test",
                text="hello world from kadhi",
                input_tokens=4,
                output_tokens=4,
            )
        )
...
class TestPublicSurface:
    def test_data_pipeline_exports(self):
        from kadhi_cli.utils import data_pipeline
        for name in (
    ...
    def test_recipe_run_exports(self):
        from kadhi_cli.utils import recipe_run
        assert hasattr(recipe_run, "run_recipe")

    def test_trainer_plugins_exports(self):
        from kadhi_cli.utils import trainer_plugins
...
class TestReviewFixesRecipeRun:
    ...
    def test_seed_rejects_symlink_via_lstat_on_raw_path(self, tmp_path, monkeypatch):
        ...
        from kadhi_cli.utils.recipe_dag import RecipeNode
        from kadhi_cli.utils.recipe_run import _node_seed
        node = RecipeNode(name="s", kind="seed", config={"path": "link.jsonl"})
        with pytest.raises(ValueError, match="symlink"):
            _node_seed(node, ())

    def test_redact_exc_message_strips_posix_paths(self):
        from kadhi_cli.utils.recipe_run import _redact_exc_message
    ...
    def test_node_code_row_injection_blocked_by_double_encode(self, tmp_path, monkeypatch):
        ...
        from kadhi_cli.utils.recipe_dag import RecipeNode
        from kadhi_cli.utils.recipe_run import _node_code
        monkeypatch.chdir(tmp_path)
    ...
    def test_run_recipe_resume_rehydrates_predecessor_outputs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.utils.recipe_dag import RecipeDAG, RecipeNode
        from kadhi_cli.utils.recipe_run import run_recipe
    ...
    def test_save_checkpoint_atomic_via_mkstemp(self, tmp_path, monkeypatch):
        ...
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.utils.recipe_run import _save_checkpoint
        outdir = tmp_path / "out"
        outdir.mkdir()
...
class TestReviewFixesDataScore:
    ...
    def test_load_jsonl_rows_rejects_symlink(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            from kadhi_cli.utils.data_score import load_jsonl_rows
    ...
    def test_extract_row_text_publicly_exposed(self):
        ...
        from kadhi_cli.utils.data_score import extract_row_text
...
class TestReviewFixesSSEHeaders:
    ...
    def test_sanitise_sse_field_strips_crlf_nul(self):
        from kadhi_cli.commands.serve import _sanitise_sse_field
...
class TestReviewFixesPreprocessAtomic:
    ...
    def test_preprocess_save_uses_atomic_pattern(self):
        src = Path(__file__).parent.parent / "src" / "kadhi_cli" / "commands" / "data.py"
...
class TestSftPretrainPreTokenizedShortCircuit:
    ...
    def test_pretrain_short_circuits_to_arrow_dataset(self, tmp_path, monkeypatch):
        ...
        from kadhi_cli.trainer.sft import _maybe_load_pretokenized as helper
    ...
    def test_cache_hash_mismatch_raises(self, tmp_path, monkeypatch):
        ...
        ``max_length`` without re-running ``kadhi data preprocess``) must
    ...
    def test_missing_metadata_proceeds_with_yellow_advisory(
        ...
        assert "kadhi data preprocess" in printed
