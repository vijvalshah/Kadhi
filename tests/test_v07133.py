...
* ``commands/draft.py`` — ``kadhi draft distill / measure / list``.
"""
...
class TestAdapterFuse:
    def test_module_exports_fuse_and_release(self):
        from kadhi_cli.utils.adapter_fuse import fuse_adapter_into, release_cuda
    ...
    def test_shrink_reuses_the_shared_implementation(self):
        ...
        from kadhi_cli.commands import shrink
        from kadhi_cli.utils.adapter_fuse import fuse_adapter_into, release_cuda
    ...
    def test_no_top_level_torch(self):
        ...
        path = Path(kadhi_cli.__file__).parent / "utils" / "adapter_fuse.py"
    ...
    def test_failed_save_does_not_orphan_the_staging_dir(self, tmp_path, monkeypatch):
        ...
        from kadhi_cli.utils import adapter_fuse
    ...
    def test_trainer_checkpoint_pickles_are_not_mistaken_for_an_attack(
        ...
        (a pickle IT wrote). A recursive scan refused Kadhi's own distill output
        and made `kadhi draft distill` impossible. Only TOP-LEVEL weights — the
        ones PEFT actually loads — are the threat surface."""
        from kadhi_cli.utils.strict_safetensors import (
        ...
        # made `kadhi draft distill` refuse its own trainer's output...
    ...
    def test_shallow_scan_still_catches_a_pickle_model_file(self, tmp_path):
        ...
        from kadhi_cli.utils.strict_safetensors import assert_safe_top_level_weights
...
class TestComputeAcceptance:
    def test_all_match(self):
        from kadhi_cli.utils.draft import compute_acceptance
...
class TestCountAcceptedAndRate:
    def test_count_accepted(self):
        from kadhi_cli.utils.draft import count_accepted
    ...
    def test_acceptance_rate(self):
        from kadhi_cli.utils.draft import acceptance_rate
    ...
    def test_count_accepted_spans_uses_max_align_chars_cap(self, monkeypatch):
        ...
        from kadhi_cli.utils import uld
        from kadhi_cli.utils.draft import count_accepted_spans
...
class TestClassify:
    def test_boundary_exact(self):
        from kadhi_cli.utils.draft import classify_acceptance
...
class TestSameTokenizer:
    ...
    def test_probe_corpus_is_non_trivial(self):
        ...
        from kadhi_cli.utils.draft import PROBE_CORPUS
...
class TestReport:
    def _report(self, **kw):
        from kadhi_cli.utils.draft import AcceptanceReport
    ...
    def test_to_dict_round_trip(self):
        from kadhi_cli.utils.draft import draft_report_to_dict
    ...
    def test_panel_names_the_verdict_and_rate(self):
        ...
        from kadhi_cli.utils.draft import render_draft_panel
...
def draft_registry(tmp_path, monkeypatch):
    """Point the draft registry at a temp file (never touch the real ~/.kadhi)."""
    path = tmp_path / "drafts.json"
    monkeypatch.setenv("KADHI_DRAFT_REGISTRY_PATH", str(path))
    return path


class TestDraftRegistry:
    def test_round_trip(self, draft_registry, tmp_path):
        from kadhi_cli.utils.draft import list_drafts, lookup_draft, register_draft
    ...
    def test_lookup_is_case_insensitive(self, draft_registry, tmp_path):
        ...
        from kadhi_cli.utils.draft import lookup_draft, register_draft
    ...
    def test_malformed_json_reads_as_empty(self, draft_registry):
        from kadhi_cli.utils.draft import list_drafts, lookup_draft
    ...
    def test_non_dict_payload_reads_as_empty(self, draft_registry):
        from kadhi_cli.utils.draft import list_drafts
    ...
    def test_atomic_write_leaves_no_temp_file(self, draft_registry, tmp_path):
        from kadhi_cli.utils.draft import register_draft

        draft = tmp_path / "d"
        draft.mkdir()
        register_draft("hf/t", str(draft))
        leftovers = [p.name for p in draft_registry.parent.glob(".kadhi.*")]
        assert leftovers == []
    ...
    def test_concurrent_registration_does_not_lose_an_entry(
        ...
        from kadhi_cli.utils.draft import list_drafts, register_draft
...
class TestSpecPairingLocal:
    def test_local_registry_beats_static_map(self, draft_registry, tmp_path):
        ...
        from kadhi_cli.utils.draft import register_draft
        from kadhi_cli.utils.spec_pairing import pick_draft_model
    ...
    def test_registry_blowup_does_not_crash_serve(self, draft_registry, monkeypatch):
        ...
        from kadhi_cli.utils import spec_pairing
...
class TestDraftNoTopLevelTorch:
    ...
    def test_is_torch_free(self, relpath):
        ...
        path = Path(kadhi_cli.__file__).parent.joinpath(*relpath)
        tree = ast.parse(path.read_text(encoding="utf-8"))
        for node in tree.body:
            if isinstance(node, ast.Import):
                for alias in node.names:
...
class TestDraftCliPlumbing:
    ...
    def test_trust_flag_is_threaded_into_resolution(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "kadhi_cli.utils.trust_remote.resolve_trust_remote_code", _fake_resolve
        )
        monkeypatch.setattr(
...
class TestDraftDistillCli:
    ...
    def test_run_distill_reports_rendered_config_validation_error(
        ...
        messages: list[str] = []
        monkeypatch.setattr(
            "kadhi_cli.config.loader.load_config_from_string",
    ...
    def test_generated_config_is_schema_valid(self, in_tmp_cwd):
        """The rendered YAML must actually load as a KadhiConfig."""
    ...
    def test_trainer_output_goes_to_a_nested_adapter_dir(self, in_tmp_cwd):
        ...
        from kadhi_cli.commands.draft import _ADAPTER_SUBDIR, _build_distill_config_yaml
    ...
    def test_happy_path_trains_fuses_and_registers(
        ...
        from kadhi_cli.commands import draft as draft_cmd
        from kadhi_cli.commands.draft import app
        from kadhi_cli.utils.draft import lookup_draft
...
class TestVocabSizeOfCompositeConfig:
    ...
    def _vocab_size_for(self, config):
        ...
        from kadhi_cli.commands.draft import _vocab_size_of
...
class TestMillionParamSizeGate:
    """Live-smoke regression: `kadhi draft distill` on SmolLM2-135M was REFUSED
...
class TestPromptTexts:
    def test_extracts_messages_and_drops_unusable_rows(self):
        from kadhi_cli.commands.draft import _prompt_texts
...
class TestDistillStepBudget:
    """#364 — ``kadhi draft distill --steps N`` must deliver ~N optimiser steps.
    ...
    def test_steps_are_delivered_not_quartered(self):
        ...
        from kadhi_cli.commands import draft
