...
Each suite has exactly 40 items, so ONE item is 2.5% against ``kadhi ship``'s
...
class TestJsonContainerExtraction:
    def test_fenced_object_is_scored_on_validity_not_packaging(self):
        ...
        from kadhi_cli.eval.gate_suites import _is_json_container
    ...
    def test_null_byte_and_oversize_still_rejected(self):
        from kadhi_cli.eval.gate_suites import _MAX_OUTPUT_LEN, _is_json_container
...
class TestSuitesStayOfflineAndDeterministic:
    ...
    def test_module_imports_no_network_or_torch(self):
        """The offline promise: ``kadhi ci init``'s core-only install must work."""
        import subprocess
        import sys

        code = (
            "import sys; import kadhi_cli.eval.gate_suites as g;"
