...
- #223  kadhi adapters pr --push  (GitHub PR publisher via `gh api`)
- #173  branch pointers into the v0.26 Registry lineage DAG
- #172  live canary verdict for kadhi adapters merge (replace UNKNOWN stub)
- #220  live eval-suite auto-wiring for kadhi adapters merge --strategy cmaes
- #176  pre-wired stage callbacks for kadhi loop watch
- #177  pack each kadhi loop iteration as a Kadhi Can + Registry lineage
"""
...
def temp_registry(tmp_path, monkeypatch):
    ...
    branches.mkdir()
    monkeypatch.setenv("KADHI_BRANCHES_DIR", str(branches))
...
def _write_cfg(tmp_path, name="kadhi.yaml", body="base: meta-llama/x\ntask: dpo\n"):
    p = tmp_path / name
...
class TestBranchRefKind:
    def test_branch_ref_in_valid_kinds(self):
        from kadhi_cli.registry.store import _VALID_KINDS
...
class TestBranchSchema:
    def test_registry_entry_id_field_default_none(self, temp_registry, tmp_path):
        from kadhi_cli.utils.adapter_branch import create_branch, load_branch
    ...
    def test_dataset_path_and_sha_mutually_exclusive(self, temp_registry, tmp_path):
        from kadhi_cli.utils.adapter_branch import create_branch
    ...
    def test_back_compat_v057_pointer(self, temp_registry, tmp_path):
        ...
        from kadhi_cli.utils.adapter_branch import _branches_dir, load_branch

        legacy = {
            "name": "legacy",
            "config_path": str(tmp_path / "kadhi.yaml"),
            "config_sha256": "a" * 64,
            "dataset_sha256": None,
            "base_model": "meta-llama/x",
            "created_at": 1.0,
            "kadhi_version": "0.57.0",
        }
...
class TestAttachBranchToRegistry:
    def test_happy(self, temp_registry, tmp_path):
        from kadhi_cli.registry.store import RegistryStore
        from kadhi_cli.utils.adapter_branch import (
...
class TestBranchFromRegistry:
    def test_derives_all_fields(self, temp_registry, tmp_path):
        from kadhi_cli.utils.adapter_branch import branch_from_registry
...
class TestBranchCli:
    ...
    def test_attach_to_registry_cli(self, temp_registry, tmp_path):
        ...
        from kadhi_cli.utils.adapter_branch import load_branch
...
class TestHistoryBranchEdges:
    def test_history_renders_branch_edge(self, temp_registry, tmp_path):
        ...
        from kadhi_cli.commands.history import history as history_cmd
        from kadhi_cli.utils.adapter_branch import (
            attach_branch_to_registry,
            create_branch,
        )
...
def _make_report():
    from kadhi_cli.utils.adapter_merge import MergeReport
...
class TestPredictMergedVerdict:
    def test_none_returns_unknown(self):
        from kadhi_cli.utils.adapter_merge import predict_merged_verdict
...
def _cmaes_plan(tmp_path, adapters, *, pop=4, gens=6):
    from kadhi_cli.utils.cmaes_merge import build_cmaes_plan
...
class TestBuildCmaesEvalFn:
    def test_eval_fn_merges_and_scores(self, tmp_path, monkeypatch):
        from kadhi_cli.utils.cmaes_merge import build_cmaes_eval_fn
    ...
    def test_run_cmaes_with_eval_fn_converges(self, tmp_path, monkeypatch):
        from kadhi_cli.utils.cmaes_merge import build_cmaes_eval_fn, run_cmaes_merge
    ...
    def test_eval_fn_isolates_scorer_failure(self, tmp_path, monkeypatch):
        from kadhi_cli.utils.cmaes_merge import (
...
class TestCmaesCliLive:
    ...
    def test_cmaes_runs_loop_and_writes_output(self, tmp_path, monkeypatch):
        import kadhi_cli.utils.cmaes_merge as cm
...
def loop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("KADHI_REGISTRY_DB_PATH", str(tmp_path / "registry.db"))
    monkeypatch.delenv("KADHI_LOOP_TRACE_DIR", raising=False)
    monkeypatch.delenv("KADHI_LOOP_SERVE_ENDPOINT", raising=False)

    from kadhi_cli.utils.loop_state import LoopState
...
class TestLoopStatePreWired:
    ...
    def test_init_state_persists(self, loop_env, tmp_path):
        from kadhi_cli.utils.loop_state import init_state, read_state
...
class TestHarvestStage:
    def test_produces_pairs(self, loop_env, tmp_path, monkeypatch):
        from kadhi_cli.utils.loop_stages import harvest_from_traces

        td = _write_trace_dir(tmp_path)
        monkeypatch.setenv("KADHI_LOOP_TRACE_DIR", td)
    ...
    def test_traces_present_but_no_pairs(self, loop_env, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KADHI_LOOP_TRACE_DIR", str(trace_dir))
        out = harvest_from_traces(loop_env())
        assert out["pairs_harvested"] == 0
        assert out["pairs_path"] is None
...
class TestTrainStage:
    def test_skips_without_pairs(self, loop_env):
        from kadhi_cli.utils.loop_stages import train_dpo_from_pairs
    ...
    def test_invokes_runner(self, loop_env, tmp_path, monkeypatch):
        import kadhi_cli.utils.loop_stages as ls
...
class TestGateStage:
    def test_skipped_when_train_skipped(self, loop_env):
        from kadhi_cli.utils.loop_stages import gate_against_baseline
...
class TestDeployStage:
    def test_noop_when_not_ok(self, loop_env):
        from kadhi_cli.utils.loop_stages import deploy_to_canary
    ...
    def test_posts_when_ok(self, loop_env, monkeypatch):
        ...
        monkeypatch.setenv("KADHI_LOOP_SERVE_ENDPOINT", "http://localhost:8000")
...
class TestPrewiredConfig:
    def test_build_returns_wired_config(self):
        from kadhi_cli.utils.loop_stages import (
...
class TestPrewiredE2E:
    def test_watch_harvests_and_trains(self, loop_env, tmp_path, monkeypatch):
        import kadhi_cli.utils.loop_stages as ls
        from kadhi_cli.utils.loop_iteration import list_iterations, read_iteration
        from kadhi_cli.utils.loop_state import write_state
        ...
        from kadhi_cli.utils.loop_stages import build_prewired_watch_config

        cfg = build_prewired_watch_config(
            max_iterations=1, poll_interval_sec=1.0,
        )
        from kadhi_cli.utils.loop_daemon import watch
...
class TestLoopStagesSourceWiring:
    def test_no_top_level_heavy_imports(self):
        ...
        src = (
            root / "src" / "kadhi_cli" / "utils" / "loop_stages.py"
...
class TestLoopCliPrewired:
    def _app(self):
        from kadhi_cli.commands.loop import app
    ...
    def test_init_pre_wired_flag(self, loop_env, tmp_path):
        ...
        assert result.exit_code == 0, (result.output, repr(result.exception))
        from kadhi_cli.utils.loop_state import read_state
...
# ===========================================================================
# #177 — pack each loop iteration as a v0.26 Kadhi Can + Registry lineage
# ===========================================================================


def _write_iteration(iteration_id, *, served="mymodel"):
    from kadhi_cli.utils.loop_iteration import IterationRecord, write_iteration
...
class TestRegistryNameFrom:
    ...
    def test_sanitises(self, raw, expected_start):
        from kadhi_cli.utils.loop_iteration import registry_name_from
...
class TestPackIterationAsCan:
    def test_writes_can_and_entry(self, loop_env, tmp_path):
        from kadhi_cli.registry.store import RegistryStore
        from kadhi_cli.utils.loop_iteration import pack_iteration_as_can
    ...
    def test_can_verifies(self, loop_env, tmp_path):
        from kadhi_cli.cans.unpack import inspect_can
        from kadhi_cli.utils.loop_iteration import pack_iteration_as_can
...
class TestWatchPackCans:
    def test_three_iterations_chain(self, loop_env, tmp_path, monkeypatch):
        ...
        from kadhi_cli.cans.unpack import inspect_can
        cans = glob.glob(os.path.join(".kadhi-loops", "*", "iteration.can"))
        assert len(cans) == 3
        for c in cans:
...
# ===========================================================================
# #223 — kadhi adapters pr --push (GitHub PR publisher)
# ===========================================================================
...
class TestResolveGithubToken:
    def test_github_token(self):
        from kadhi_cli.utils.adapter_pr import resolve_github_token
...
class TestPostPrComment:
    def test_happy_path_invokes_gh(self):
        from kadhi_cli.utils.adapter_pr import post_pr_comment
...
class TestPrCliPush:
    ...
    def test_push_happy(self, monkeypatch):
        ...
        import kadhi_cli.utils.adapter_pr as ap
...
class TestPackIterationSafely:
    def test_pack_failure_never_crashes_watch(self, loop_env, tmp_path, monkeypatch):
        ...
        import kadhi_cli.utils.loop_iteration as li
        from kadhi_cli.utils.loop_daemon import WatchConfig, watch
        from kadhi_cli.utils.loop_state import init_state
    ...
    def test_pack_iterations_false_skips_packing(self, loop_env, tmp_path):
        ...
        assert glob.glob(os.path.join(".kadhi-loops", "*", "iteration.can")) == []
        with RegistryStore() as store:
...
class TestDeploySsrfGuard:
    def test_public_http_endpoint_rejected(self, loop_env, monkeypatch):
        ...
        monkeypatch.setenv("KADHI_LOOP_SERVE_ENDPOINT", "http://8.8.8.8:8000")
        out = ls.deploy_to_canary(
            loop_env(),
            {"gate_verdict": "OK", "adapter_path": ".kadhi-loops/adapters/x"},
        )
        assert out["deployed"] is False
    ...
    def test_public_https_endpoint_rejected(self, loop_env, monkeypatch):
        ...
        monkeypatch.setenv("KADHI_LOOP_SERVE_ENDPOINT", "https://8.8.8.8:8000")
        out = ls.deploy_to_canary(
            loop_env(),
            {"gate_verdict": "OK", "adapter_path": ".kadhi-loops/adapters/x"},
        )
    ...
    def test_public_hostname_rejected(self, loop_env, monkeypatch):
        ...
        monkeypatch.setenv("KADHI_LOOP_SERVE_ENDPOINT", "https://evil.example.com")
        out = ls.deploy_to_canary(
            loop_env(),
            {"gate_verdict": "OK", "adapter_path": ".kadhi-loops/adapters/x"},
        )
    ...
    def test_loopback_endpoint_allowed_to_post(self, loop_env, monkeypatch):
        ...
        monkeypatch.setenv("KADHI_LOOP_SERVE_ENDPOINT", "http://127.0.0.1:8000")
        # Inject the poster so no real network call happens.
        monkeypatch.setattr(ls, "_DEPLOY_POSTER", lambda ep, name: True)
        out = ls.deploy_to_canary(
            loop_env(),
            {"gate_verdict": "OK", "adapter_path": ".kadhi-loops/adapters/win"},
        )
        assert out["deployed"] is True
        assert out["canary_verdict"] == "OK"
    ...
    def test_private_lan_endpoint_allowed(self, loop_env, monkeypatch):
        ...
        monkeypatch.setenv("KADHI_LOOP_SERVE_ENDPOINT", "https://192.168.1.5:8000")
        monkeypatch.setattr(ls, "_DEPLOY_POSTER", lambda ep, name: True)
        out = ls.deploy_to_canary(
            loop_env(),
            {"gate_verdict": "OK", "adapter_path": ".kadhi-loops/adapters/win"},
    ...
    def test_endpoint_is_local_accepts_abbreviated_loopback(self, host):
        ...
        from kadhi_cli.utils.loop_stages import _endpoint_is_local
    ...
    def test_deploy_to_canary_allows_abbreviated_loopback_endpoint(self, loop_env, monkeypatch):
        ...
        monkeypatch.setenv("KADHI_LOOP_SERVE_ENDPOINT", "https://127.1:8000")
        monkeypatch.setattr(ls, "_DEPLOY_POSTER", lambda ep, name: True)
        out = ls.deploy_to_canary(
            loop_env(),
            {"gate_verdict": "OK", "adapter_path": ".kadhi-loops/adapters/win"},
...
class TestNoHeavyTopLevelImports:
    ...
    @pytest.mark.parametrize("module", [
        "kadhi_cli.utils.cmaes_merge",
        "kadhi_cli.utils.loop_stages",
        "kadhi_cli.utils.loop_iteration",
        "kadhi_cli.utils.adapter_pr",
    ])
...
class TestMergeAdaptersCmaesGuard:
    def test_cmaes_strategy_rejected_with_path_hint(self, tmp_path, monkeypatch):
        from kadhi_cli.utils.adapter_merge import merge_adapters
...
class TestWriteMergedAdapterPublic:
    def test_public_symbol_and_alias(self):
        from kadhi_cli.utils import adapter_merge
...
class TestPackEntryRollback:
    def test_pack_failure_rolls_back_registry_entry(
        ...
        import kadhi_cli.cans.pack as pack_mod
        from kadhi_cli.registry.store import RegistryStore
        from kadhi_cli.utils.loop_iteration import pack_iteration_as_can
...
class TestWatchConfigValidation:
    def test_pack_iterations_must_be_bool(self):
        from kadhi_cli.utils.loop_daemon import WatchConfig
...
class TestCmaesScanGate:
    ...
    def test_cmaes_respects_backdoor_scan_gate(self, tmp_path, monkeypatch):
        ...
        import kadhi_cli.utils.adapter_scan as scan_mod
