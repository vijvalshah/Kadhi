"""Tests for v0.71.40 — `kadhi reward synth` + comma-split reward loader (#311).

Covers:
- utils/reward_synth.py: extract_golds / detect_kind / inducers / render / calibrate / synthesize.
- commands/reward.py: `kadhi reward synth` CLI (happy / refuse / plan-only / guards).
...
from kadhi_cli.utils import reward_synth as rs
...
class TestRewardSynthCli:
    ...
    def test_help(self):
        from kadhi_cli.commands.reward import app
...
class TestSelectRewardFn:
    def test_comma_split_returns_list(self):
        ...
        from kadhi_cli.trainer.grpo import _select_reward_fn
...
class TestCliRobustness:
    ...
    def test_registered_on_top_level_cli(self):
        from kadhi_cli.cli import app as root
...
class TestEnvsDocstringFix:
    def test_calculator_docstring(self):
        from kadhi_cli.envs import calculator
    ...
    def test_guess_number_docstring(self):
        from kadhi_cli.envs import guess_number
