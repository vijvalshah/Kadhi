...
* #154 ``KADHI_POSTHOG_KEY`` / ``KADHI_POSTHOG_ENDPOINT`` env override.
* #152 Multi-command ``--hub`` dispatch (chat / serve / infer / merge /
  export / push).
* #153 ``kadhi data download --hub <non-hf>`` live SDK lift.
* #155 Web UI Tool Outputs panel JS / HTML wiring.
* #156 SFT trainer-side ``record_call`` wire-up in
  ``monitoring/callback.KadhiTrainerCallback``.
"""
...
class TestMixExtra:
    ...
    def test_describe_default_optimizer_returns_label(self):
        from kadhi_cli.utils.data_mix import describe_default_optimizer
...
class TestPostHogEnvOverride:
    def test_resolve_uses_default_key_when_env_unset(self):
        from kadhi_cli.utils.trackers import _POSTHOG_DEFAULT_KEY, _resolve_posthog_target
    ...
    def test_resolve_env_key_overrides_default(self):
        ...
        env = {"KADHI_POSTHOG_KEY": "phc_user_project"}
    ...
    def test_resolve_env_endpoint_overrides_default(self):
        ...
        env = {"KADHI_POSTHOG_ENDPOINT": "https://eu.i.posthog.com/i/v0/e/"}
        resolved = _resolve_posthog_target(None, env=env)
        assert resolved is not None
        _, endpoint = resolved
    ...
    def test_resolve_rejects_http_endpoint(self):
        ...
        env = {"KADHI_POSTHOG_ENDPOINT": "http://attacker.example.com/"}
    ...
    def test_resolve_explicit_endpoint_locks_against_env(self):
        ...
        from kadhi_cli.utils.trackers import _POSTHOG_ENDPOINT, _resolve_posthog_target
    ...
    def test_resolve_rejects_control_char_key(self):
        ...
        env = {"KADHI_POSTHOG_KEY": "phc\nAuthorization: bypass"}
        assert _resolve_posthog_target(None, env=env) is None

    def test_resolve_rejects_null_byte_key(self):
        from kadhi_cli.utils.trackers import _resolve_posthog_target

        env = {"KADHI_POSTHOG_KEY": "phc\x00bypass"}
        assert _resolve_posthog_target(None, env=env) is None

    def test_resolve_rejects_oversize_key(self):
        # tdd-review MEDIUM #3: 257-char KADHI_POSTHOG_KEY must reject.
        from kadhi_cli.utils.trackers import _resolve_posthog_target

        env = {"KADHI_POSTHOG_KEY": "x" * 257}
        assert _resolve_posthog_target(None, env=env) is None
...
class TestHubPrefetchHelper:
    ...
    def test_prefetch_outside_cwd_cache_root_raises(self, tmp_path, monkeypatch):
        ...
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.utils.hubs import prefetch_model_from_hub
...
class TestCliHubFlags:
    @pytest.mark.parametrize(
        "cmd_module,cmd_name",
        [
            ("kadhi_cli.commands.chat", "chat"),
            ("kadhi_cli.commands.serve", "serve"),
            ("kadhi_cli.commands.infer", "infer"),
            ("kadhi_cli.commands.merge", "merge"),
            ("kadhi_cli.commands.export", "export"),
            ("kadhi_cli.commands.push", "push"),
        ],
    )
    ...
    def test_apply_hub_helper_imported_in_non_push_commands(self):
        ...
        for mod in ("chat", "serve", "infer", "merge", "export"):
            src = (REPO_ROOT / f"src/kadhi_cli/commands/{mod}.py").read_text(
                encoding="utf-8"
            )
    ...
    def test_push_uses_upload_repo_path(self):
        ...
        src = (REPO_ROOT / "src/kadhi_cli/commands/push.py").read_text(
            encoding="utf-8"
        )
...
# ----------------------------------------------------------------------
# #153 — `kadhi data download --hub <non-hf>` live SDK
# ----------------------------------------------------------------------
...
class TestDataDownloadNonHfLive:
    ...
    def test_data_download_no_longer_advises_v0_53_9(self):
        ...
        src = (REPO_ROOT / "src/kadhi_cli/commands/data.py").read_text(
            encoding="utf-8"
        )
...
class TestWebUiToolOutputsPanel:
    def test_index_html_has_tools_nav_entry(self):
        html = (REPO_ROOT / "src/kadhi_cli/ui/static/index.html").read_text(
            encoding="utf-8"
        )
    ...
    def test_app_js_has_load_tool_outputs(self):
        js = (REPO_ROOT / "src/kadhi_cli/ui/static/app.js").read_text(encoding="utf-8")
...
class TestCallbackToolBuffer:
    def test_on_step_end_method_exists(self):
        ...
        assert hasattr(KadhiTrainerCallback, "on_step_end")

    def test_on_step_end_no_op_when_inputs_absent(self):
        from kadhi_cli.monitoring.callback import KadhiTrainerCallback
        from kadhi_cli.utils.tool_outputs import (
            get_global_tool_buffer,
            reset_global_tool_buffer,
        )

        reset_global_tool_buffer()
        cb = KadhiTrainerCallback.__new__(KadhiTrainerCallback)
