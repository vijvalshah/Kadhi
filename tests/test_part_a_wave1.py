...
Covers:
  - #32 Live model scoring: judge / benchmark task dispatch in run_gate,
    ...
    factory shape.
  - #35 Registry attach: --attach-to-registry on `kadhi eval custom`,
    --registry-id auto-attach in `kadhi export`, registry artifact kind
    extensions.
"""
...
class TestRunGateErrorPropagation:
    def test_judge_task_failure_surfaces_score_none(self, tmp_path, monkeypatch):
        ...
        with patch("kadhi_cli.eval.judge.JudgeEvaluator") as mock_judge:
    ...
    def test_benchmark_task_runs_builtin_benchmark(self):
        ...
        from kadhi_cli.eval.forgetting import MINI_MMLU
...
class TestGateTaskResultSchema:
    def test_error_field_default_none(self):
        from kadhi_cli.eval.gate import GateTaskResult
...
class TestMakeModelGenerator:
    ...
    def test_returns_callable_with_mocked_transformers(self):
        from kadhi_cli.eval import quant_check
...
class TestRegistryAttachHelpers:
    def test_write_eval_json_containment(self, tmp_path, monkeypatch):
        from kadhi_cli.registry.attach import write_eval_json
...
class TestLookupEntryByOutputDir:
    def test_lookup_returns_none_when_no_match(self, tmp_path, monkeypatch):
        from kadhi_cli.registry.attach import lookup_entry_by_output_dir
...
# ---------------------------------------------------------------------------
# #35 — `kadhi eval custom --attach-to-registry` CLI integration
# ---------------------------------------------------------------------------
...
class TestEvalCustomAttachCLI:
    def test_attach_to_unknown_entry_errors(self, tmp_path, monkeypatch):
        ...
        monkeypatch.chdir(tmp_path)
        # Isolated registry DB so we don't pollute the user's ~/.kadhi
        monkeypatch.setenv(
            "KADHI_REGISTRY_DB_PATH", str(tmp_path / "reg.db"),
        )
        ...
        with patch(
            "kadhi_cli.eval.custom._create_default_generator",
