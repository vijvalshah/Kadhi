...
Two independent trl changes broke ``kadhi train``, and they moved on different
schedules:
...
class TestPromptLengthIsDecidedByCapabilityNotVersion:
    def test_kwarg_is_passed_when_the_config_accepts_it(self):
        from kadhi_cli.trainer._trl_compat import prompt_length_kwargs
...
class TestResolveTrlSymbol:
    ...
    def test_public_namespace_wins_over_experimental(self, monkeypatch):
        ...
        sentinel = object()
        monkeypatch.setitem(
            sys.modules, "kadhi_fake_exp", _fake_module("kadhi_fake_exp", DPOConfig=sentinel)
        )
        resolved = resolve_trl_symbol("DPOConfig", "kadhi_fake_exp")
    ...
    def test_experimental_fallback_is_used_when_the_symbol_left_trl(self, monkeypatch):
        ...
        monkeypatch.setitem(
            sys.modules,
            "kadhi_fake_exp2",
            _fake_module("kadhi_fake_exp2", KadhiAbsentConfig=sentinel),
        )
        assert resolve_trl_symbol("KadhiAbsentConfig", "kadhi_fake_exp2") is sentinel
    ...
    def test_missing_everywhere_raises_an_import_error_naming_the_symbol(self):
        ...
        with pytest.raises(ImportError) as excinfo:
            resolve_trl_symbol("KadhiDefinitelyAbsent", "kadhi_module_that_does_not_exist")
        message = str(excinfo.value)
        assert "KadhiDefinitelyAbsent" in message
        assert trl.__version__ in message
        assert "kadhi_module_that_does_not_exist" in message

    def test_the_original_failure_is_chained_not_swallowed(self):
        from kadhi_cli.trainer._trl_compat import resolve_trl_symbol

        with pytest.raises(ImportError) as excinfo:
            resolve_trl_symbol("KadhiDefinitelyAbsent")
    ...
    def test_no_experimental_module_given_still_raises_cleanly(self):
        ...
        with pytest.raises(ImportError, match="KadhiDefinitelyAbsent"):
            resolve_trl_symbol("KadhiDefinitelyAbsent")
