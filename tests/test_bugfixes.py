...
class TestNoUnicodeInOutput:
    ...
    def test_loss_format_uses_ascii(self):
        ...
        from kadhi_cli.commands.runs import _fmt_loss
    ...
    def test_fmt_float_missing_returns_ascii(self):
        ...
        from kadhi_cli.commands.runs import _fmt_float
    ...
    def test_fmt_duration_missing_returns_ascii(self):
        ...
        from kadhi_cli.commands.runs import _fmt_duration
...
class TestDiffModelLoading:
    ...
    def test_load_model_uses_torch_dtype(self):
        ...
        from kadhi_cli.commands.diff import _load_model
...
class TestPlotextFallback:
    ...
    def test_stats_catches_unicode_error(self):
        ...
        from kadhi_cli.commands import data
...
class TestCPUErrorMessages:
    ...
    def test_tensor_size_error_mapped(self):
        ...
        from kadhi_cli.utils.errors import ERROR_MAP
...
class TestPPODatasetCompat:
    ...
    def test_ppo_dataset_in_constructor_flag(self):
        ...
        with mock_patch("kadhi_cli.trainer.ppo.PPOTrainerWrapper._setup_reward"), \
             mock_patch(
                 "kadhi_cli.trainer.ppo.PPOTrainerWrapper._setup_transformers"
             ), mock_patch(
...
class TestPPOExperimentalImport:
    ...
    def test_import_ppo_classes_returns_tuple(self):
        ...
        from kadhi_cli.trainer.ppo import _import_ppo_classes
    ...
    def test_import_experimental_fallback(self):
        ...
        import kadhi_cli.trainer.ppo as ppo_mod
...
class TestPPOExperimentalSetup:
    ...
    def test_experimental_api_passes_required_args(self):
        ...
        with mock_patch("kadhi_cli.trainer.ppo.PPOTrainerWrapper._setup_reward"), \
             mock_patch("kadhi_cli.trainer.ppo.PPOTrainerWrapper._setup_transformers"), \
             mock_patch(
                 "kadhi_cli.trainer.ppo._import_ppo_classes",
                 return_value=(FakePPOTrainer, FakePPOConfig, True),
             ), \
             mock_patch(
                 "kadhi_cli.trainer.ppo.PPOTrainerWrapper._get_or_create_reward_model",
                 return_value=fake_reward_model,
             ), \
             mock_patch(
                 "kadhi_cli.trainer.ppo.PPOTrainerWrapper._create_value_model",
...
class TestGRPOCPUMinNewTokens:
    ...
    def test_cpu_adds_generation_kwargs(self):
        ...
        with mock_patch("kadhi_cli.trainer.grpo.GRPOTrainerWrapper._setup_transformers"), \
...
class TestCPUDeviceMap:
    ...
    LOADERS = [
        ("kadhi_cli.trainer.grpo", "GRPOTrainerWrapper._setup_transformers"),
        ("kadhi_cli.trainer.ppo", "PPOTrainerWrapper._setup_transformers"),
        ("kadhi_cli.trainer.ppo", "PPOTrainerWrapper._get_or_create_reward_model"),
        ("kadhi_cli.trainer.ppo", "PPOTrainerWrapper._create_value_model"),
        ("kadhi_cli.trainer.ppo", "_load_reward_model"),
        ("kadhi_cli.trainer.sft", "SFTTrainerWrapper._setup_transformers"),
        ("kadhi_cli.trainer.dpo", "DPOTrainerWrapper._setup_transformers"),
        ("kadhi_cli.trainer.reward_model", "RewardModelTrainerWrapper._setup_transformers"),
    ]
    ...
    def test_the_helper_maps_cpu_to_cpu(self):
        ...
        from kadhi_cli.utils.gpu import resolve_device_map
...
class TestPPOTokenization:
    ...
    def test_ppo_tokenization_adds_input_ids(self):
        ...
        from kadhi_cli.trainer.ppo import _prepare_ppo_dataset
...
# --- BUG-013: kadhi data validate defaults to alpaca, no auto-detect (v0.14.2) ---
...
# --- BUG-014: kadhi data stats histogram broken on Windows (v0.14.2) ---
...
# --- BUG-015: kadhi ui --help missing auth token docs (v0.14.2) ---
...
class TestUIAuthDocs:
    ...
    def test_ui_help_mentions_auth(self):
        """kadhi ui --help should mention auth token."""
        from typer.testing import CliRunner
    ...
    def test_ui_help_shows_show_token_flag(self):
        """kadhi ui --help should show --show-token option."""
        import re
