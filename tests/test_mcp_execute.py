...
import kadhi_cli.mcp_server.registry as reg
from kadhi_cli.mcp_server.execution import ExecutionError, ExecutionManager, digest_file
...
class TestMcpExecutionGates:
    ...
    def test_allow_execute_enables_execution(self):
        ...
        manager.issue(kind="train", argv=["kadhi"], display_command="kadhi")
...
class TestConfigSnapshottingAndPlanning:
    def test_plan_creates_expected_snapshot(self, tmp_path, monkeypatch):
        ...
        out = spec.handler({"config": "kadhi.yaml"})

        token = out["confirmation_token"]
        plan = manager._plans[token]

        snapshot_file = Path(tmp_path) / ".kadhi" / "mcp-runs" / plan.run_id / "config.yaml"
    ...
    def test_snapshot_contains_exact_validated_content(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "kadhi.yaml").write_text(custom_yaml, encoding="utf-8")
    ...
    def test_original_config_modification_after_plan_does_not_affect_execution(
        ...
        # Mutate the original kadhi.yaml completely
        cfg_path.write_text(
    ...
    def test_execution_argv_references_snapshot(self, tmp_path, monkeypatch):
        ...
        expected_snapshot = str(
            Path(tmp_path) / ".kadhi" / "mcp-runs" / plan.run_id / "config.yaml"
        )
    ...
    def test_snapshot_run_id_matches_tracker_and_env(self, tmp_path, monkeypatch):
        ...
        with patch("subprocess.Popen") as mock_popen:
            ...
            assert call_env["KADHI_MCP_RUN_ID"] == planned_run_id
...
class TestSubprocessIsolationAndConcurrency:
    def test_subprocess_args_and_environment_isolation(self, tmp_path, monkeypatch):
        ...
        token = manager.issue(
            kind="train",
            argv=[sys.executable, "-m", "kadhi_cli.cli", "train"],
            display_command="kadhi train",
        )
        ...
        with patch("subprocess.Popen") as mock_popen:
            ...
            assert call_args == [sys.executable, "-m", "kadhi_cli.cli", "train"]
            assert call_kwargs["shell"] is False
            assert call_kwargs["stdin"] == subprocess.DEVNULL
            assert call_kwargs["cwd"] == str(tmp_path)
            assert "KADHI_MCP_RUN_ID" in call_kwargs["env"]
    ...
    def test_capacity_released_after_tracker_launch_failure(self, tmp_path, monkeypatch):
        ...
        with patch(
            "kadhi_cli.experiment.tracker.ExperimentTracker.launch_run",
