...
_TRAINER_DIR = Path(__file__).resolve().parent.parent / "src" / "kadhi_cli" / "trainer"
...
class TestPPORewardModelQuantMenu:
    ...
    def test_load_reward_model_routes_through_quant_menu_live(self, monkeypatch):
        ...
        from kadhi_cli.config.schema import TrainingConfig
        from kadhi_cli.trainer import ppo as ppo_mod
        ...
        from kadhi_cli.utils import quant_menu
        monkeypatch.setattr(
...
class TestRewardModelTaskQuantMenu:
    ...
    ``resolve_frozen_base_load_dtype`` in ``kadhi_cli.utils.gpu``.
    """
