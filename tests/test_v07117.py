...
* #254 — ``citation_style`` (+ ``shuffle_seed``) thread through the live
  diagnose citation probe + the ``kadhi diagnose`` CLI.
* #259 — serve-time MoLE: a training run writes ``mole_manifest.json`` next to
  ``mole_gate.pt``; ``kadhi serve --mole <dir>`` loads the base + frozen task
  LoRAs + the gate and blends per-token at decode time.
* #260 — ``kadhi serve --bank`` resolves the active VeRA/VB-LoRA user per
...
class TestRaftEpochCollator:
    def test_state_holds_epoch(self):
        from kadhi_cli.trainer.raft import RaftEpochState
    ...
    def test_collator_repermutes_per_epoch(self):
        ...
        from kadhi_cli.trainer.raft import RaftEpochShuffleCollator, RaftEpochState
...
class TestRaftEpochCallback:
    def test_callback_advances_epoch(self):
        from kadhi_cli.trainer.raft import RaftEpochState, make_raft_epoch_callback
...
class TestDiagnoseCitationStyle:
    def test_run_live_diagnose_threads_style(self):
        from kadhi_cli.utils.diagnose import live as live_mod
    ...
    def test_score_citation_called_with_style(self, monkeypatch):
        ...
        def _fake_score_citation(rows, generator, **kwargs):
            ...
            from kadhi_cli.utils.diagnose.report import FailureScore
        ...
        import kadhi_cli.utils.diagnose.citation as cit_mod
...
class TestBankActiveUserContextVar:
    def test_uses_contextvar(self):
        from kadhi_cli.utils import vector_bank
...
class TestMoleServeManifest:
    def _manifest(self):
        from kadhi_cli.utils.mole_routing import MoleServeManifest
    ...
    def test_write_load_roundtrip(self, tmp_path, monkeypatch):
        from kadhi_cli.utils.mole_routing import (
            load_mole_manifest,
            write_mole_manifest,
        )
    ...
    def test_load_missing_dir(self, tmp_path, monkeypatch):
        from kadhi_cli.utils.mole_routing import load_mole_manifest
...
class TestMoleTrainWritesManifest:
    def test_train_writes_manifest_source(self):
        from kadhi_cli.trainer import mole_routing as trainer_mole
...
class TestPatchInvariants:
    def test_version_bumped(self):
        ...
        assert parts >= (0, 71, 17), kadhi_cli.__version__

    @pytest.mark.parametrize(
        "module",
        ["kadhi_cli.utils.raft", "kadhi_cli.utils.mole_routing"],
    )
