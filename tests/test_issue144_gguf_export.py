"""#144 — two defects found by the first CUDA-box run of `kadhi export --format gguf`.
...
passing ``--llama-cpp`` or already holding ``~/.kadhi/llama.cpp`` hit
...
class TestConvertDepsAreInstalledOnEveryPath:
    def test_a_user_supplied_llama_cpp_still_gets_the_convert_deps(
        ...
        """`--llama-cpp /path` and an already-cloned ~/.kadhi/llama.cpp both skipped
