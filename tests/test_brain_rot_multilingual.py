...
per-language registry under ``kadhi_cli/utils/brain_rot_lang.py`` plus an
...
from kadhi_cli.utils import brain_rot, brain_rot_lang
...
class TestScoreRowLang:
    ...
    def test_auto_uses_detected_lang(
        ...
        import kadhi_cli.utils.data_score as ds
...
class TestSourceWiring:
    def test_no_heavy_imports_in_brain_rot_lang(self) -> None:
        ...
        src = (
            root / "src" / "kadhi_cli" / "utils" / "brain_rot_lang.py"
    ...
    def test_brain_rot_does_not_eager_import_langdetect(self) -> None:
        ...
        root = Path(__file__).resolve().parent.parent
        src = (
            root / "src" / "kadhi_cli" / "utils" / "brain_rot.py"
        ).read_text(encoding="utf-8")
