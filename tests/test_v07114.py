...
* **#96** — live torch runtime for ``kadhi merge-sharded-fsdp-weights``
...
* **#140** — live ``kv_cache_type`` wiring on the **transformers** backend
  ...
  ``--kv-cache-type`` CLI flag on ``kadhi serve``).
...
class TestConsolidateShards:
    def test_merges_two_disjoint_shards(self, tmp_path, monkeypatch):
        ...
        from kadhi_cli.utils.fsdp_consolidate import (
            consolidate_shards,
            plan_consolidation,
        )
    ...
    def test_duplicate_key_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(
            logging.WARNING, logger="kadhi_cli.utils.fsdp_consolidate"
        ):
            consolidate_shards(plan)
        assert any(
    ...
    def test_non_plan_rejected(self):
        from kadhi_cli.utils.fsdp_consolidate import consolidate_shards
...
class TestApplyKvCacheType:
    def test_no_longer_raises_notimplemented(self):
        from kadhi_cli.utils.kv_cache import apply_kv_cache_type
    ...
    def test_quantized_cache_backend_available(self):
        from kadhi_cli.utils.kv_cache import quantized_cache_backend_available
...
class TestPlainKvKwargs:
    def test_deep_converts_nested_mappingproxy(self):
        from kadhi_cli.commands.serve import _plain_kv_kwargs
