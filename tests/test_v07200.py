...
class TestDecidePinning:
    ...
    def test_store_within_limit_is_pinned(self):
        from kadhi_cli.utils.layer_stream import decide_pinning
...
class TestBufferValidation:
    def test_default_is_two(self):
        from kadhi_cli.utils.layer_stream import DEFAULT_STREAM_BUFFERS

        assert DEFAULT_STREAM_BUFFERS == 2

    def test_valid_range_accepted(self):
        from kadhi_cli.utils.layer_stream import validate_stream_buffers
...
class TestThroughputEstimate:
    def test_uses_c_equals_six_for_gradient_checkpointing(self):
        ...
        from kadhi_cli.utils.layer_stream import FLOPS_PER_PARAM_PER_TOKEN

        assert FLOPS_PER_PARAM_PER_TOKEN == 6

    def test_hand_computed_rate(self):
        from kadhi_cli.utils.layer_stream import estimate_stream_tokens_per_sec
    ...
    def test_epoch_seconds(self):
        from kadhi_cli.utils.layer_stream import estimate_epoch_seconds
...
class TestVramEstimate:
    def test_sums_the_plan_41_terms(self):
        from kadhi_cli.utils.layer_stream import estimate_stream_vram
...
class TestLayerSpec:
    def test_nbytes_from_shape_and_dtype(self):
        from kadhi_cli.utils.layer_stream import LayerSpec
...
class TestStreamPlan:
    ...
    def test_render_panel_names_the_beta_status_and_tier(self):
        ...
        from kadhi_cli.utils.layer_stream import build_stream_plan, render_stream_panel
...
class TestNoTopLevelTorch:
    ...
    def test_layer_stream_has_no_top_level_torch(self):
        import kadhi_cli.utils.layer_stream as mod
    ...
    def test_importable_without_torch_installed(self):
        ...
        try:
            importlib.reload(importlib.import_module("kadhi_cli.utils.layer_stream"))
        finally:
...
class TestShardRoundTrip:
    ...
    def test_tied_embedding_stays_resident_and_unaffected(self, tmp_path):
        ...
        from kadhi_cli.utils.layer_shard import extras_shard_path, shard_checkpoint
...
class TestShardGuards:
    ...
    def test_too_many_layers_refused(self, tmp_path, monkeypatch):
        import kadhi_cli.utils.layer_shard as mod
...
class TestShardCacheDir:
    def test_slug_is_traversal_safe(self):
        from kadhi_cli.utils.layer_shard import model_slug
    ...
    def test_env_override_inside_tmp_is_honoured(self, tmp_path, monkeypatch):
        from kadhi_cli.utils.layer_shard import resolve_shard_dir

        monkeypatch.setenv("KADHI_LAYER_STREAM_CACHE_DIR", str(tmp_path))
    ...
    def test_env_override_outside_bounds_falls_back(self, monkeypatch):
        ...
        bad = "C:\\Windows\\System32" if os.name == "nt" else "/etc/kadhi-evil"
        monkeypatch.setenv("KADHI_LAYER_STREAM_CACHE_DIR", bad)
    ...
    def test_control_chars_in_override_rejected(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KADHI_LAYER_STREAM_CACHE_DIR", str(tmp_path / "cache") + "\x1b")
        got = os.path.realpath(resolve_shard_dir("m"))
        assert got.startswith(os.path.realpath(default_layer_stream_cache_dir()))
...
class TestDecoderOwner:
    def test_finds_the_module_that_owns_layers(self, tmp_path):
        ...
        from kadhi_cli.utils.layer_stream_runtime import decoder_owner
...
class TestRegressionAttributeTransparency:
    ...
    def test_attention_type_is_visible_when_the_inner_layer_has_it(self, tmp_path):
        ...
        from kadhi_cli.utils.layer_stream_runtime import StreamedDecoderLayer
...
class TestBufferPool:
    def test_slot_round_robin(self):
        from kadhi_cli.utils.layer_stream_runtime import LayerBufferPool
    ...
    def test_wait_after_load_returns_the_buffer(self, tmp_path):
        from kadhi_cli.utils.layer_shard import shard_checkpoint
        from kadhi_cli.utils.layer_stream_runtime import LayerBufferPool, RamSource
...
class TestPrefetcherDirection:
    ...
    def test_forward_prefetches_ascending(self):
        from kadhi_cli.utils.layer_stream_runtime import StreamPrefetcher
...
class TestExpandableSegments:
    ...
    def test_returns_a_bool(self):
        from kadhi_cli.utils.layer_stream_runtime import probe_expandable_segments
...
class TestStreamedForwardParityCpu:
    ...
    def test_all_aliases_of_a_tied_boundary_module_are_replaced(self):
        ...
        from kadhi_cli.utils.layer_stream_runtime import _replace_module_references
...
class TestRuntimeNoTopLevelTorch:
    def test_runtime_is_torch_lazy(self):
        import kadhi_cli.utils.layer_stream_runtime as mod
...
class TestHfGradientCheckpointingInteraction:
    ...
    def test_streaming_disables_hf_gradient_checkpointing(self):
        from kadhi_cli.utils.layer_stream import should_enable_hf_gradient_checkpointing
...
class TestStreamingResumeAccepted:
    ...
    def test_train_accepts_resume_with_streaming(self, tmp_path, monkeypatch):
        ...
        with open("kadhi.yaml", "w", encoding="utf-8") as fh:
            yaml.safe_dump(cfg, fh)
        result = CliRunner().invoke(
            app, ["train", "--config", "kadhi.yaml", "--resume", "latest", "--yes"]
        )
...
class TestShardWriteContainment:
    def test_out_dir_outside_the_allowed_roots_is_refused(self, tmp_path):
        ...
        src, _, _ = _fake_weights_dir(tmp_path)
        outside = (
            os.path.join("C:" + os.sep, "Windows", "System32", "kadhi-evil")
            if os.name == "nt"
            else "/etc/kadhi-evil"
        )
...
class TestSourceSizeProbe:
    ...
    def test_reports_the_source_byte_size(self, tmp_path):
        from kadhi_cli.utils.layer_shard import source_weight_bytes
...
class TestDiskTierPinsItsStagingOrRefuses:
    ...
    def _patch_async_source(self, monkeypatch, *, fails_when_pinned: bool):
        ...
        Patched on `kadhi_cli.utils.async_disk_source` because `_build_source`
        ...
        """
        import kadhi_cli.utils.async_disk_source as ads
...
class TestCachedIndexInvalidation:
    ...
    def test_missing_large_layer_shard_reshards(self, tmp_path):
        from kadhi_cli.utils.layer_shard import large_shard_path, shard_checkpoint
...
class TestMaterializeExtrasGuard:
    def test_incomplete_extras_shard_is_refused(self, tmp_path):
        ...
        from kadhi_cli.utils.layer_shard import extras_shard_path, shard_checkpoint
        from kadhi_cli.utils.layer_stream_runtime import build_meta_skeleton, materialize_extras
