"""v0.64.0 Part A — `kadhi tunability` probe across candidate bases.
...
def test_module_imports():
    from kadhi_cli.utils import tunability
...
def test_default_candidates_nonempty():
    from kadhi_cli.utils.tunability import DEFAULT_CANDIDATES

    assert len(DEFAULT_CANDIDATES) >= 6
    # Each entry must be CandidateBase
    from kadhi_cli.utils.tunability import CandidateBase
...
def test_validate_probe_steps_happy(value):
    from kadhi_cli.utils.tunability import validate_probe_steps
...
def test_validate_holdout_size_happy():
    from kadhi_cli.utils.tunability import validate_holdout_size
...
def test_tunability_result_happy():
    from kadhi_cli.utils.tunability import CandidateBase, TunabilityResult
...
def test_score_candidate_delta_math():
    ...
    from kadhi_cli.utils.tunability import score_candidate
...
def test_pareto_frontier_simple():
    ...
    from kadhi_cli.utils.tunability import CandidateBase, TunabilityResult, pareto_frontier
...
def test_pareto_frontier_empty():
    from kadhi_cli.utils.tunability import pareto_frontier
...
def test_tunability_report_frozen():
    from kadhi_cli.utils.tunability import CandidateBase, TunabilityReport, TunabilityResult
...
def test_run_tunability_with_mocked_probe(tmp_path):
    ...
    from kadhi_cli.utils.tunability import (
        CandidateBase,
        TunabilityResult,
        run_tunability,
    )
...
def test_run_tunability_rejects_empty_candidates(tmp_path):
    from kadhi_cli.utils.tunability import run_tunability
...
def test_run_tunability_rejects_invalid_probe_steps(tmp_path):
    from kadhi_cli.utils.tunability import CandidateBase, run_tunability
...
def test_write_report_non_report_rejected(tmp_path):
    from kadhi_cli.utils.tunability import write_report
...
def test_load_report_missing_file(tmp_path, monkeypatch):
    from kadhi_cli.utils.tunability import load_report
...
def test_version_bumped_to_0640():
    ...
    assert parts >= (0, 64, 0), kadhi_cli.__version__
...
def test_no_top_level_heavy_imports():
    ...
    src = Path(__file__).resolve().parent.parent / "src" / "kadhi_cli" / "utils" / "tunability.py"
    text = src.read_text(encoding="utf-8")
