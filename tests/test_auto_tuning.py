...
class TestLRFinderSchedule:
    ...
    def test_log_schedule_endpoints(self):
        from kadhi_cli.utils.lr_finder import compute_lr_schedule
...
class TestLRFinderRecommendation:
    ...
    def test_picks_steepest_descent(self):
        from kadhi_cli.utils.lr_finder import find_optimal_lr
...
class TestLRFinderCLI:
    """`kadhi train --find-lr` flag is registered and surfaces in help."""
...
class TestGradAccumMonitor:
    def test_should_adjust_when_high_pressure(self):
        from kadhi_cli.utils.grad_accum import GradAccumMonitor
    ...
    def test_recommend_caps_accum_at_max(self):
        from kadhi_cli.utils.grad_accum import MAX_ACCUM, GradAccumMonitor
...
class TestMixedPrecisionPicker:
    ...
    def test_known_quirk_mapping_includes_qwen_and_phi(self):
        from kadhi_cli.utils.mixed_precision import KNOWN_PRECISION_QUIRKS
...
class TestWarmupAutoSchedule:
    def test_basic_formula(self):
        from kadhi_cli.utils.warmup import compute_warmup_steps
    ...
    def test_ratio_at_max_accepted(self):
        from kadhi_cli.utils.warmup import MAX_WARMUP, compute_warmup_steps
...
class TestSpikeRecoveryStrategy:
    def test_should_recover_within_budget(self):
        from kadhi_cli.utils.spike_recovery import SpikeRecoveryStrategy
...
class TestConvergenceDetector:
    def test_detects_plateau(self):
        from kadhi_cli.utils.convergence import detect_plateau
...
class TestRecommendAction:
    def test_recommends_continue_for_decreasing(self):
        from kadhi_cli.utils.convergence import recommend_action
...
class TestAutopilotIntegration:
    def test_decide_warmup_returns_int(self):
        from kadhi_cli.autopilot.decisions import decide_warmup
    ...
    def test_decide_mixed_precision_routes_to_picker(self):
        from kadhi_cli.autopilot.decisions import decide_mixed_precision
...
class TestAutopilotConfigEmission:
    def test_generated_config_includes_warmup_auto_when_set(
        ...
        from kadhi_cli.autopilot.generate_config import generate_config
...
# --------------------------------------------------------------------------- #
# Cross-cutting: KadhiConfig top-level still serializes                         #
# --------------------------------------------------------------------------- #
