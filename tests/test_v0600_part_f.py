"""Tests for v0.60.0 Part F — ``kadhi airgap-bundle``.
...
class TestSecurityReviewFixes:
    ...
    def test_inspect_uses_data_filter_when_available(self, tmp_path, monkeypatch):
        ...
        src = (
            ...
            / "src" / "kadhi_cli" / "utils" / "airgap_bundle.py"
        )
...
class TestSourceWiring:
    def test_module_imports(self):
        from kadhi_cli.utils import airgap_bundle as m

        assert hasattr(m, "build_airgap_bundle")
        assert hasattr(m, "inspect_airgap_bundle")
        assert hasattr(m, "AirgapBundlePlan")

    def test_manifest_has_kadhi_version(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        model = _make_model_dir(tmp_path)
        from kadhi_cli.utils.airgap_bundle import (
            AirgapBundlePlan,
        ...
        manifest = build_airgap_bundle(plan)
        # Manifest carries kadhi_version + created_at for audit trail.
        with tarfile.open(plan.output) as tar:
            member = tar.getmember("manifest.json")
            extracted = tar.extractfile(member)
            assert extracted is not None
            payload = json.loads(extracted.read().decode("utf-8"))
        assert "kadhi_version" in payload
        assert "created_at" in payload
        assert payload["kadhi_version"] == manifest.kadhi_version
