...
- #216 Live HF Hub auto-download for SAE weights (`download_sae` +
  `hubs.snapshot_download`); `kadhi probe sae-diff --auto-download`.
- #217 Live truth + harm probe utilities (`apply_truth_probe`/`apply_harm_probe`
  + bundled packs + `kadhi probe truth/harm`).
- #218 Auto-measure `kadhi probe interference --measure <eval_suite>`.
- #219 `kadhi train --capture-activations <layer> --capture-prompts <jsonl>`.
...
class TestProbeKernel:
    def test_module_imports(self) -> None:
        from kadhi_cli.utils import probe_kernel
    ...
    def test_contrast_probe_separates_classes(self) -> None:
        from kadhi_cli.utils.probe_kernel import (
            apply_linear_probe,
            compute_contrast_probe,
        )
    ...
    def test_contrast_probe_unit_norm(self) -> None:
        from kadhi_cli.utils.probe_kernel import compute_contrast_probe
    ...
    def test_apply_linear_probe_shape_mismatch(self) -> None:
        from kadhi_cli.utils.probe_kernel import apply_linear_probe
    ...
    def test_flagged_rate(self) -> None:
        from kadhi_cli.utils.probe_kernel import flagged_rate
    ...
    def test_classify_probe_rate_bands(self) -> None:
        from kadhi_cli.utils.probe_kernel import classify_probe_rate
...
class TestExtractLayerActivations:
    def test_resolve_layer_module(self) -> None:
        from kadhi_cli.utils.live_eval import resolve_layer_module
    ...
    def test_extract_mean_pooled(self) -> None:
        from kadhi_cli.utils.live_eval import extract_layer_activations
...
class TestSleeperRealWeights:
    ...
    def test_load_npz(self, tmp_path, monkeypatch) -> None:
        ...
        from kadhi_cli.utils.sleeper_probe import load_probe_weights
...
class TestSleeperCli215:
    ...
    def test_sleeper_with_weights(self, tmp_path, monkeypatch) -> None:
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.commands.probe import app
...
class TestHubsSnapshotDownload:
    def test_validate_cache_dir_under_tmp(self, tmp_path) -> None:
        from kadhi_cli.utils.hubs import _validate_cache_dir
...
class TestDownloadSae:
    def test_rejects_non_allowlisted(self, tmp_path, monkeypatch) -> None:
        from kadhi_cli.utils import hubs
        from kadhi_cli.utils.sae_diff import download_sae
    ...
    def test_happy_path(self, tmp_path, monkeypatch) -> None:
        ...
        # Cache lands under ~/.kadhi/sae-cache/
        import os

        assert os.path.join(".kadhi", "sae-cache") in captured["cache_dir"]
    ...
    def test_default_cache_dir_under_home(self) -> None:
        ...
        from kadhi_cli.utils.sae_diff import default_sae_cache_dir

        d = default_sae_cache_dir()
        assert d.startswith(os.path.expanduser("~"))
        assert d.endswith(os.path.join(".kadhi", "sae-cache"))
...
class TestSaeDiffCli216:
    ...
    def test_auto_download_happy(self, tmp_path, monkeypatch) -> None:
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.commands import probe as probe_cmd
        from kadhi_cli.utils import sae_diff
...
class TestTruthHarmProbes:
    def _mod(self, mod_name):
        ...
        return importlib.import_module(f"kadhi_cli.utils.{mod_name}")
...
class TestProbePackTruthHarm:
    def test_packs_contain_three_kinds(self) -> None:
        from kadhi_cli.utils.probe_pack import BUNDLED_PACKS
    ...
    def test_pack_json_lists_truth_harm(self) -> None:
        from kadhi_cli.utils.probe_pack import get_probe_pack, render_pack_json
...
class TestMeasureInterferenceValidation:
    def test_bad_base(self) -> None:
        from kadhi_cli.utils.interference_live import measure_interference_losses
...
class TestParseAdapterSpecs:
    def test_happy(self, tmp_path, monkeypatch) -> None:
        ...
        (tmp_path / "a").mkdir()
        (tmp_path / "b").mkdir()
        from kadhi_cli.commands.probe import _parse_adapter_specs
...
class TestInterferenceMeasureCli:
    ...
    def test_measure_happy(self, tmp_path, monkeypatch) -> None:
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.commands.probe import app
        from kadhi_cli.utils import interference_live
...
# ===========================================================================
# #219 — kadhi train --capture-activations
# ===========================================================================
...
class TestLoadCapturePrompts:
    def test_jsonl_objects(self, tmp_path, monkeypatch) -> None:
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.commands.train import _load_capture_prompts
...
class TestCaptureActivations:
    ...
    def test_writes_activations_json(self, tmp_path, monkeypatch) -> None:
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.commands.train import _capture_activations
...
class TestReviewFollowups:
    ...
    def test_validate_bundled_base_canonicalises(self) -> None:
        from kadhi_cli.utils import probe_kernel as pk
    ...
    def test_synthetic_weights_deterministic(self) -> None:
        from kadhi_cli.utils.probe_kernel import synthetic_probe_weights
    ...
    def test_run_linear_probe_major(self) -> None:
        from kadhi_cli.utils.probe_kernel import run_linear_probe
    ...
    def test_run_bundled_weights_skips_allowlist(self) -> None:
        from kadhi_cli.utils.probe_kernel import run_bundled_probe
    ...
    def test_probespec_rejects_bad_hidden_dim(self) -> None:
        from kadhi_cli.utils.probe_kernel import ProbeSpec
    ...
    def test_proberesult_rejects_bad_verdict(self) -> None:
        from kadhi_cli.utils.probe_kernel import ProbeResult
...
class TestPatchInvariants:
    def test_version_bumped(self) -> None:
        ...
        major_minor = tuple(int(x) for x in kadhi_cli.__version__.split(".")[:3])
        assert major_minor >= (0, 71, 8)

    def test_new_modules_no_top_level_heavy(self) -> None:
        import pathlib

        src = pathlib.Path(__file__).resolve().parent.parent / "src" / "kadhi_cli"
        for rel in (
