...
class TestMiniLLMConfigOnPolicy:
    def test_on_policy_field_default(self):
        from kadhi_cli.utils.minillm import MiniLLMConfig
...
class TestMiniLLMRolloutLengthSchema:
    ...
    def test_distill_trainer_threads_explicit_length(self):
        ...
        src = (REPO_ROOT / "src" / "kadhi_cli" / "trainer" / "distill.py").read_text(
            encoding="utf-8"
        )
...
class TestMiniLLMOnPolicyRollout:
    ...
    def test_rollout_rejects_bad_config(self):
        ...
        from kadhi_cli.utils.minillm import minillm_on_policy_rollout
...
class TestDistillOnPolicyWiring:
    ...
    def test_minillm_no_top_level_torch(self):
        src = (REPO_ROOT / "src/kadhi_cli/utils/minillm.py").read_text(encoding="utf-8")
...
class TestULDAlignedStrategy:
    def test_strategy_registered(self):
        from kadhi_cli.utils.uld import SUPPORTED_ULD_STRATEGIES, validate_uld_strategy
    ...
    def test_aligned_does_not_require_top_k(self):
        from kadhi_cli.utils.uld import ULDConfig
...
class TestAlignTokenSequences:
    def test_identical_tokenization(self):
        from kadhi_cli.utils.uld import align_token_sequences
    ...
    def test_char_cap_truncates_difflib_branch(self):
        from kadhi_cli.utils.uld import _MAX_ALIGN_TOKENS, align_token_sequences
...
class TestAggregateAlignedLogits:
    def test_mean_pool(self):
        ...
        from kadhi_cli.utils.uld import aggregate_aligned_logits
...
class TestDistillUldAlignedWiring:
    ...
    def test_uld_no_top_level_torch(self):
        src = (REPO_ROOT / "src/kadhi_cli/utils/uld.py").read_text(encoding="utf-8")
        assert "\nimport torch" not in src
        assert "\nfrom torch" not in src


# ===========================================================================
# #110 — kadhi agent eval --sandbox
# ===========================================================================
...
class TestBuildEvalStub:
    def test_stub_is_python_and_b64_only(self):
        from kadhi_cli.utils.agent_sandbox import build_eval_stub
    ...
    def test_required_params_extracted(self):
        from kadhi_cli.utils.agent_sandbox import _required_path_params
...
class TestClassifySandboxOutcome:
    def test_timeout(self):
        from kadhi_cli.utils.agent_sandbox import classify_sandbox_outcome
...
class TestRunEvalInSandbox:
    ...
    def test_run_ok_stub(self):
        from kadhi_cli.utils.agent_sandbox import build_eval_stub, run_eval_in_sandbox
...
class TestScoreSandbox:
    def test_score_ok(self):
        from kadhi_cli.utils.agent_sandbox import score_sandbox
    ...
    def test_score_uses_override(self, monkeypatch):
        import kadhi_cli.utils.agent_sandbox as m
...
class TestAgentSandboxNoTopLevelHeavyImport:
    def test_no_top_level_torch(self):
        src = (REPO_ROOT / "src/kadhi_cli/utils/agent_sandbox.py").read_text(
            encoding="utf-8"
        )
        assert "\nimport torch" not in src
        # rewards.py is imported lazily inside run_eval_in_sandbox (not at top).
        assert "\nfrom kadhi_cli.trainer.rewards import" not in src


# ===========================================================================
# #16 — kadhi train --cloud modal
# ===========================================================================
_KADHI_YAML = (
    "base: hf-internal-testing/tiny-random-gpt2\n"
    "task: sft\n"
...
class TestValidateCloud:
    def test_modal(self):
        from kadhi_cli.cloud.modal import validate_cloud
...
class TestValidateGpu:
    ...
    def test_known(self, gpu):
        from kadhi_cli.cloud.modal import validate_gpu
...
class TestRenderModalStub:
    def test_structure(self):
        ...
        stub = render_modal_stub(
            _KADHI_YAML, gpu="a100", output_dir="./out", kadhi_version="0.71.18"
        )
        assert "import modal" in stub
        assert "modal.App" in stub
        assert 'gpu="A100"' in stub
        assert "kadhi-cli[train]==0.71.18" in stub
        assert "base64.b64decode" in stub

    def test_no_raw_config_injection(self):
        from kadhi_cli.cloud.modal import render_modal_stub

        secret_yaml = _KADHI_YAML + "INJECT_SENTINEL: pwned\n"
        stub = render_modal_stub(
            secret_yaml, gpu="h100", output_dir="./out", kadhi_version="0.71.18"
        )
    ...
    def test_oversize_config_rejected(self):
        from kadhi_cli.cloud.modal import _MAX_CONFIG_BYTES, render_modal_stub

        with pytest.raises(ValueError, match="exceeds"):
            render_modal_stub(
                "x" * (_MAX_CONFIG_BYTES + 1),
                gpu="a100", output_dir="./out", kadhi_version="0.71.18",
            )
    ...
    def test_bad_gpu_rejected(self):
        ...
        with pytest.raises(ValueError):
            render_modal_stub(
                _KADHI_YAML, gpu="v100", output_dir="./out", kadhi_version="0.71.18"
            )
    ...
    def test_bad_output_dir_rejected(self):
        ...
        with pytest.raises(ValueError):
            render_modal_stub(
                _KADHI_YAML, gpu="a100", output_dir="out\nINJECT",
                kadhi_version="0.71.18",
            )
    ...
    def test_kadhi_version_injection_rejected(self, bad_version):
        from kadhi_cli.cloud.modal import render_modal_stub

        with pytest.raises(ValueError, match="kadhi_version"):
            render_modal_stub(
                _KADHI_YAML, gpu="a100", output_dir="./out",
                kadhi_version=bad_version,
            )

    def test_kadhi_version_length_capped(self):
        from kadhi_cli.cloud.modal import _MAX_VERSION_LEN, render_modal_stub

        with pytest.raises(ValueError, match="kadhi_version"):
            render_modal_stub(
                _KADHI_YAML, gpu="a100", output_dir="./out",
                kadhi_version="0." * _MAX_VERSION_LEN,
            )

    def test_kadhi_version_pep440ish_accepted(self):
        from kadhi_cli.cloud.modal import render_modal_stub

        # Real-world version shapes must pass (dev / rc / local-version).
        stub = render_modal_stub(
            _KADHI_YAML, gpu="a100", output_dir="./out",
            kadhi_version="0.71.18.dev0+gabc123",
        )
        assert "kadhi-cli[train]==0.71.18.dev0+gabc123" in stub
...
class TestPlanModalRun:
    def test_plan(self, tmp_path, monkeypatch):
        ...
        from kadhi_cli.cloud.modal import CloudPlan, plan_modal_run

        plan = plan_modal_run(
            "kadhi.yaml", gpu="a100", output_dir="./out", kadhi_version="0.71.18"
        )
        assert isinstance(plan, CloudPlan)
        assert plan.cloud == "modal"
        assert plan.gpu == "a100"
        assert plan.run_command == "modal run kadhi_modal_app.py"
        assert "modal.App" in plan.stub_text

    def test_outside_cwd_config_rejected(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.cloud.modal import plan_modal_run
...
class TestWriteStub:
    def test_writes_under_cwd(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        (tmp_path / "kadhi.yaml").write_text(_KADHI_YAML, encoding="utf-8")
        from kadhi_cli.cloud.modal import plan_modal_run, write_stub

        plan = plan_modal_run(
            "kadhi.yaml", gpu="t4", output_dir="./out", kadhi_version="0.71.18"
        )
        path = write_stub(plan)
        assert (tmp_path / "kadhi_modal_app.py").exists()
        assert "modal.App" in Path(path).read_text(encoding="utf-8")


class TestSubmitModalRun:
    def test_override_seam(self, monkeypatch):
        import kadhi_cli.cloud.modal as m
    ...
    def test_non_plan_rejected(self):
        from kadhi_cli.cloud.modal import submit_modal_run
...
class TestTrainCloudCli:
    ...
    def test_cloud_plan_only(self, tmp_path, monkeypatch):
        ...
        result = CliRunner().invoke(
            app, ["train", "--config", "kadhi.yaml", "--cloud", "modal", "--gpu", "a100"]
        )
        assert result.exit_code == 0, (result.output, repr(result.exception))
        assert (tmp_path / "kadhi_modal_app.py").exists()
        txt = _strip_ansi(result.output)
    ...
    def test_cloud_unknown_rejected(self, tmp_path, monkeypatch):
        ...
        result = CliRunner().invoke(
            app, ["train", "--config", "kadhi.yaml", "--cloud", "aws"]
        )
    ...
    def test_cloud_bad_gpu_rejected(self, tmp_path, monkeypatch):
        ...
        result = CliRunner().invoke(
            app, ["train", "--config", "kadhi.yaml", "--cloud", "modal", "--gpu", "v100"]
        )
        assert result.exit_code == 2
    ...
    def test_cloud_submit_via_override(self, tmp_path, monkeypatch):
        ...
        result = CliRunner().invoke(
            app,
            ["train", "--config", "kadhi.yaml", "--cloud", "modal",
...
class TestCloudNoTopLevelModal:
    def test_no_top_level_modal_import(self):
        src = (REPO_ROOT / "src/kadhi_cli/cloud/modal.py").read_text(encoding="utf-8")
