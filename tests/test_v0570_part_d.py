...
def isolated_branches_dir(tmp_path, monkeypatch):
    """Redirect KADHI_BRANCHES_DIR to tmp so tests don't pollute ~/.kadhi."""
...
def _make_config(tmp_path: Path, name: str = "kadhi.yaml", content: str = "base: test\n") -> Path:
    p = tmp_path / name
...
def test_create_branch_happy(tmp_path):
    ...
    snap = create_branch("v1", config_path="kadhi.yaml", base_model="meta/llama")
...
def test_create_branch_with_dataset(tmp_path):
    ...
    snap = create_branch("v1", config_path="kadhi.yaml", base_model="b",
...
def test_create_branch_invalid_name(tmp_path):
    ...
    with pytest.raises(ValueError, match="must match"):
        create_branch("../etc", config_path="kadhi.yaml", base_model="b")
    with pytest.raises(ValueError, match="non-empty"):
        create_branch("", config_path="kadhi.yaml", base_model="b")


def test_create_branch_null_byte_name(tmp_path):
    _make_config(tmp_path)
    with pytest.raises(ValueError, match="null"):
        create_branch("v\x001", config_path="kadhi.yaml", base_model="b")


def test_create_branch_bool_name(tmp_path):
    _make_config(tmp_path)
    with pytest.raises(TypeError):
        create_branch(True, config_path="kadhi.yaml", base_model="b")  # type: ignore[arg-type]
...
def test_create_branch_empty_base_model(tmp_path):
    _make_config(tmp_path)
    with pytest.raises(ValueError, match="non-empty"):
        create_branch("v1", config_path="kadhi.yaml", base_model="")


def test_create_branch_null_byte_base_model(tmp_path):
    _make_config(tmp_path)
    with pytest.raises(ValueError, match="null"):
        create_branch("v1", config_path="kadhi.yaml", base_model="x\x00")
...
def test_create_branch_bool_base_model_rejected(tmp_path):
    ...
    _make_config(tmp_path)
    with pytest.raises(TypeError):
        create_branch("v1", config_path="kadhi.yaml",
...
def test_branches_dir_crlf_env_rejected(monkeypatch, tmp_path):
    """Control char (CRLF) in KADHI_BRANCHES_DIR falls back to default.
    ...
    from kadhi_cli.utils.adapter_branch import _branches_dir

    original = _os.environ.get

    def fake_get(key, default=None):
        if key == "KADHI_BRANCHES_DIR":
            return "/some\npath"
...
def test_create_branch_oversize_config(tmp_path):
    # 1 MiB + 1 byte
    (tmp_path / "kadhi.yaml").write_bytes(b"x" * (1_048_577))
    with pytest.raises(ValueError, match="cap"):
        create_branch("v1", config_path="kadhi.yaml", base_model="b")
...
def test_list_branches_sorted(tmp_path):
    _make_config(tmp_path)
    create_branch("b", config_path="kadhi.yaml", base_model="x")
    create_branch("a", config_path="kadhi.yaml", base_model="x")
    create_branch("c", config_path="kadhi.yaml", base_model="x")
...
def test_load_branch_roundtrip(tmp_path):
    _make_config(tmp_path)
    created = create_branch("v1", config_path="kadhi.yaml", base_model="m")
...
def test_delete_branch_true_when_present(tmp_path):
    _make_config(tmp_path)
    create_branch("v1", config_path="kadhi.yaml", base_model="m")
...
def test_write_checkout_writes_target(tmp_path):
    ...
    snap = create_branch("v1", config_path="kadhi.yaml", base_model="m")
...
def test_write_checkout_detects_drift(tmp_path):
    ...
    (tmp_path / "kadhi.yaml").write_text("base: drifted\n", encoding="utf-8")
...
def test_branches_dir_env_override_outside_falls_back(monkeypatch, tmp_path):
    ...
    We point KADHI_BRANCHES_DIR at a tmp-rooted path that IS in-bounds (under
    ...
    monkeypatch.setenv("KADHI_BRANCHES_DIR", str(in_bounds))
    monkeypatch.chdir(tmp_path)
    resolved = _branches_dir()
...
def test_branches_dir_null_byte_env_ignored(monkeypatch, tmp_path):
    """Null-byte env override is silently ignored; default ~/.kadhi/branches used.
    ...
    monkeypatch.chdir(tmp_path)
    resolved = _branches_dir()
    # Falls back to ~/.kadhi/branches — must NOT contain a null byte
    assert "\x00" not in str(resolved)
    assert ".kadhi" in str(resolved) and "branches" in str(resolved)
...
def test_branch_cli_help():
    result = runner.invoke(kadhi_app, ["adapters", "branch", "--help"])
    assert result.exit_code == 0, (result.output, repr(result.exception))


def test_checkout_cli_help():
    result = runner.invoke(kadhi_app, ["adapters", "checkout", "--help"])
    assert result.exit_code == 0, (result.output, repr(result.exception))


def test_branches_cli_help():
    result = runner.invoke(kadhi_app, ["adapters", "branches", "--help"])
    assert result.exit_code == 0, (result.output, repr(result.exception))


def test_branch_cli_create_and_list(tmp_path, monkeypatch):
    monkeypatch.setenv("KADHI_BRANCHES_DIR", str(tmp_path / "br"))
    monkeypatch.chdir(tmp_path)
    _make_config(tmp_path)
    r1 = runner.invoke(kadhi_app, [
        "adapters", "branch", "v1",
        "-c", "kadhi.yaml", "--base", "meta/llama",
    ])
    assert r1.exit_code == 0, (r1.output, repr(r1.exception))
    assert "v1" in r1.output

    r2 = runner.invoke(kadhi_app, ["adapters", "branches"])
...
def test_branch_cli_invalid_name(tmp_path, monkeypatch):
    ...
    monkeypatch.chdir(tmp_path)
    _make_config(tmp_path)
    result = runner.invoke(kadhi_app, [
        "adapters", "branch", "../etc",
        "-c", "kadhi.yaml", "--base", "m",
    ])
...
def test_branches_cli_empty(tmp_path, monkeypatch):
    monkeypatch.setenv("KADHI_BRANCHES_DIR", str(tmp_path / "br"))
    monkeypatch.chdir(tmp_path)
    result = runner.invoke(kadhi_app, ["adapters", "branches"])
    assert result.exit_code == 0, (result.output, repr(result.exception))
