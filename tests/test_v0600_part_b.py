"""Tests for v0.60.0 Part B — `kadhi adapters sign / verify`.
...
class TestManifest:
    def test_imports(self):
        from kadhi_cli.utils.adapter_sign import (
            AdapterManifest,
            SignatureRecord,
    ...
    def test_manifest_deterministic(self, tmp_path, monkeypatch):
        ...
        from kadhi_cli.utils.adapter_sign import compute_adapter_manifest
...
class TestSign:
    def test_sign_unsigned_backend(self, tmp_path, monkeypatch):
        ...
        sig_file = adapter / ".kadhi-signature.json"
...
class TestSecurityReviewFixes:
    ...
    def test_oversized_signature_rejected(self, tmp_path, monkeypatch):
        ...
        monkeypatch.chdir(tmp_path)
        adapter = _make_adapter(tmp_path)
        sig = adapter / ".kadhi-signature.json"
    ...
    def test_symlinked_signature_file_rejected(self, tmp_path, monkeypatch):
        ...
        os.symlink(str(target), str(adapter / ".kadhi-signature.json"))
        from kadhi_cli.utils.adapter_sign import verify_adapter
...
class TestSourceWiring:
    def test_module_imports_clean(self):
        import kadhi_cli.utils.adapter_sign as m
    ...
    def test_signature_filename_constant(self):
        from kadhi_cli.utils.adapter_sign import _SIGNATURE_FILENAME

        assert _SIGNATURE_FILENAME == ".kadhi-signature.json"
