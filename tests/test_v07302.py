"""v0.73.2 — ``kadhi ship`` leg 2 stops lying in both directions.
...
- **Noise floor** — new capability. Greedy decoding is not deterministic on
  ...
  unchanged model), and ``kadhi ship`` compared against a 0.05 threshold without
...
from kadhi_cli.eval.forgetting import MINI_BENCHMARKS, extract_mcq_letter, score_answer
...
def _over_refusal():
    from kadhi_cli.eval.gate_suites import MINI_OVER_REFUSAL
...
class TestIssue346PermissiveControls:
    ...
    def test_an_echoed_menu_entry_is_not_a_tool_call(self):
        ...
        from kadhi_cli.eval.gate_suites import tool_names_in_prompt
...
class TestNoiseFloorIsPure:
    def test_identical_runs_have_a_zero_floor(self):
        from kadhi_cli.utils.ship_verdict import compute_noise_floor
...
class TestNoiseFloorChangesTheVerdict:
    def test_a_regression_under_the_floor_is_not_a_regression(self):
        from kadhi_cli.utils.ship_verdict import compute_benchmark_deltas, compute_noise_floor
    ...
    def test_a_task_win_under_the_floor_is_not_a_win(self):
        from kadhi_cli.utils.ship_verdict import TASK_AXIS, build_task_win, compute_noise_floor
    ...
    def test_without_a_floor_nothing_changes(self):
        ...
        from kadhi_cli.utils.ship_verdict import build_task_win, compute_benchmark_deltas
...
class TestNoiseFloorSurvivesTheEvidenceRoundTrip:
    """#312's property is that ``kadhi ship`` output replays as input. A verdict
    ...
    def test_evidence_carries_the_floor(self):
        from kadhi_cli.utils.ship_verdict import verdict_to_evidence
    ...
    def test_a_malformed_floor_block_is_refused_not_dropped(self, block):
        ...
        from kadhi_cli.utils.ship_verdict import noise_floor_from_evidence
...
class TestAHostileEvidenceFloorIsBoundedAndLoud:
    ...
    but it is a far quieter edit to miss in review, and `kadhi ci init` wires
...
class TestUntrustedNamesCannotDriveTheTerminal:
    ...
    ``kadhi_cli.utils.terminal``, shared with every other command module)."""
...
class TestTheMcpEvidenceReaderHonoursTheFloor:
    ...
    to a DIFFERENT verdict through `kadhi mcp serve`'s ``ship_evidence`` tool —
    ...
    def test_the_mcp_tool_applies_a_stored_floor(self, tmp_path, monkeypatch):
        from kadhi_cli.mcp_server.registry import tool_ship_evidence
...
class TestBaselineProvenanceStamp:
    ...
    def test_leg2_no_longer_name_warns_on_stale_suite(self, capsys):
        ...
        from kadhi_cli.commands.ship import _leg2_scores
...
def _const_task_scorer(monkeypatch, value=0.5):
    ...
    """
    monkeypatch.setattr(
        "kadhi_cli.commands.ship._build_task_floor_scorer",
...
class TestTheFloorWideningTheThresholdIsAnnounced:
    def test_a_floor_above_the_threshold_warns(self, capsys, monkeypatch):
        from kadhi_cli.commands.ship import _measure_noise_floor
...
def _panel_text(verdict):
    ...
    from kadhi_cli.utils.ship_verdict import render_ship_panel
...
class TestThePanelPrintsItsLegOneMarker:
    ...
    def test_the_rubric_still_prints_it_too(self):
        ...
        from kadhi_cli.utils.ship_verdict import format_ship_rubric
...
def _run_ship_cli(monkeypatch, base_gen, tuned_gen, extra_args=None):
    """Drive the REAL `kadhi ship` live path with injected generators.
...
class TestMeasureNoiseFloorBranches:
    ...
    def test_metric_mode_measures_the_leg_one_axis(self, tmp_path, monkeypatch):
        from kadhi_cli.commands.ship import _measure_noise_floor
        from kadhi_cli.utils.ship_verdict import TASK_AXIS
...
class TestBareFunctionShapeGuard:
    ...
    def test_non_call_shapes_are_rejected(self, obj):
        from kadhi_cli.eval.gate_suites import _looks_like_a_bare_function
