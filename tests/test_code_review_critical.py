...
def test_load_reward_fn_requires_domain_and_config_carries_it():
    ...
    from kadhi_cli.trainer.rewards import load_reward_fn, math_verify_reward
...
def test_ppo_setup_reward_loads_verifiable_end_to_end():
    ...
    from kadhi_cli.trainer.rewards import math_verify_reward
...
def test_grpo_setup_passes_verifiable_domain(monkeypatch):
    ...
    # grpo.setup() does `from kadhi_cli.trainer.rewards import load_reward_fn`
    # at call time, so patching the source module attribute is effective.
    monkeypatch.setattr("kadhi_cli.trainer.rewards.load_reward_fn", _spy)
...
def test_modal_stub_cannot_inject_via_output_dir():
    ...
    stub = render_modal_stub(
        "task: sft\nbase: x\n", gpu="a100", output_dir=payload, kadhi_version="0.0.1"
    )
...
def test_eval_gate_halts_training_end_to_end(tmp_path, monkeypatch):
    ...
    cb = KadhiTrainerCallback(_StubDisplay(), eval_gate_config=cfg)
...
def test_no_eval_gate_config_never_halts(tmp_path, monkeypatch):
    ...
    cb = KadhiTrainerCallback(_StubDisplay())  # eval_gate_config=None
...
def test_trainer_wires_eval_gate_config(name):
    """Every trainer that builds KadhiTrainerCallback must pass eval_gate_config."""
    src = (Path(kadhi_cli.__file__).parent / "trainer" / f"{name}.py").read_text(
        encoding="utf-8"
    )
