"""v0.64.0 Part B — `kadhi plan` / `apply` (Terraform UX) tests."""
...
def test_module_imports():
    from kadhi_cli.utils import terraform_plan
...
def test_compute_config_sha_deterministic():
    from kadhi_cli.utils.terraform_plan import compute_config_sha
...
def test_training_plan_frozen():
    from kadhi_cli.utils.terraform_plan import TrainingPlan
...
def test_build_plan_happy(tmp_path, monkeypatch):
    from kadhi_cli.utils.terraform_plan import build_plan
...
def test_write_state_roundtrip(tmp_path, monkeypatch):
    from kadhi_cli.utils.terraform_plan import (
        TrainingPlan,
        TrainingState,
        read_state,
        write_state,
...
def test_write_state_outside_cwd_rejected(tmp_path, monkeypatch):
    from kadhi_cli.utils.terraform_plan import TrainingPlan, TrainingState, write_state
...
def test_read_state_missing(tmp_path, monkeypatch):
    from kadhi_cli.utils.terraform_plan import read_state
...
def test_detect_drift_rejects_non_state():
    from kadhi_cli.utils.terraform_plan import detect_drift
...
def test_cli_plan_happy(tmp_path, monkeypatch):
    ...
    assert result.exit_code == 0, (result.output, repr(result.exception))
    assert (tmp_path / "kadhi.tfstate").exists()
...
def test_drift_report_frozen():
    from kadhi_cli.utils.terraform_plan import DriftReport
...
def test_no_heavy_top_level_imports():
    ...
    src = (
        ...
        / "src" / "kadhi_cli" / "utils" / "terraform_plan.py"
    )
    text = src.read_text(encoding="utf-8")
    import re
