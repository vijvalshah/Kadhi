"""v0.69.0 Part A — `kadhi build` dbt-for-SFT DAG parser + topo sort + plan."""
...
class TestSourceWiring:
    def test_cli_registers_build(self) -> None:
        ...
        cli = (root / "src" / "kadhi_cli" / "cli.py").read_text(encoding="utf-8")
        assert (
            'name="build"' in cli
            or "build_cmd" in cli
            or "from kadhi_cli.commands import build" in cli
        )

    def test_no_heavy_top_level_imports(self) -> None:
        root = Path(__file__).resolve().parent.parent
        src = (root / "src" / "kadhi_cli" / "utils" / "build_dag.py").read_text(encoding="utf-8")
