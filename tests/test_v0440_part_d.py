...
from kadhi_cli.utils.fetch_examples import (
    CATALOG,
    fetch_examples_dir,
    get_entry,
    list_entries,
...
from kadhi_cli.utils.llama_proxy import (
    build_argv,
    known_subcommands,
    resolve,
)
from kadhi_cli.utils.reasoning_parser import (
    known_parsers,
    parser_description,
    validate_parser_name,
)
from kadhi_cli.utils.sweep_config import (
    SweepSpec,
    load_sweep_yaml,
    parse_sweep_yaml,
)
...
def test_cli_quantize_prints_command(tmp_path, monkeypatch):
    ...
    assert result.exit_code == 0, (result.output, repr(result.exception))
    assert "kadhi export" in result.output
