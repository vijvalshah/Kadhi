...
`kadhi train --task orpo` failed at import for anyone who pip-installed fresh.
...
_WRAPPERS = {
    "bco": ("kadhi_cli.trainer.bco", "BCOTrainerWrapper", "BCOConfig"),
    "dpo": ("kadhi_cli.trainer.dpo", "DPOTrainerWrapper", "DPOConfig"),
    "ipo": ("kadhi_cli.trainer.ipo", "IPOTrainerWrapper", "DPOConfig"),
    "kto": ("kadhi_cli.trainer.kto", "KTOTrainerWrapper", "KTOConfig"),
    "orpo": ("kadhi_cli.trainer.orpo", "ORPOTrainerWrapper", "ORPOConfig"),
    "simpo": ("kadhi_cli.trainer.simpo", "SimPOTrainerWrapper", "CPOConfig"),
}
...
class TestTheCanaryCoversWhatItClaims:
    ...
    def test_every_wrapper_that_passes_max_prompt_length_is_listed(self):
        ...
        trainer_dir = pathlib.Path(__file__).resolve().parents[1] / "src" / "kadhi_cli" / "trainer"
...
class TestTheTrlBoundsAreConsistentWithTheCode:
    """The installed TRL must provide every symbol through Kadhi's real resolver."""
    ...
    def test_the_installed_trl_provides_every_symbol_the_trainers_import(self):
        ...
        assert not broken, (
            f"installed trl cannot supply {sorted(broken)} through Kadhi's "
