"""Part A wave 2 — #34 kadhi can run / publish (v0.33.0).
...
Covers:
  ...
  - CLI: `kadhi can run` confirmation panel, `kadhi can publish` smoke.
"""
...
class TestCanFormatVersionBump:
    ...
    def test_pack_writes_v3(self, tmp_path, monkeypatch):
        from kadhi_cli.cans.schema import CAN_FORMAT_VERSION
...
class TestDeployTargetValidation:
    def test_known_kinds_accepted(self):
        from kadhi_cli.cans.schema import DeployTarget
    ...
    def test_manifest_with_deploy_targets(self):
        from kadhi_cli.cans.schema import DeployTarget, Manifest
...
class TestCaptureEnv:
    def test_capture_env_writes_python_version(self, tmp_path):
        from kadhi_cli.cans.run import capture_env
    ...
    def test_capture_env_handles_pip_failure(self, tmp_path, monkeypatch):
        ...
        from kadhi_cli.cans import run as run_mod
...
class TestRunCan:
    def test_outside_cwd_rejected(self, tmp_path, monkeypatch):
        from kadhi_cli.cans.run import run_can
...
class TestPublishCan:
    def test_outside_cwd_rejected(self, tmp_path, monkeypatch):
        from kadhi_cli.cans.publish import publish_can
    ...
    def test_no_token_raises(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "kadhi_cli.cans.publish.resolve_token", lambda: "",
        )
        can = _build_can(tmp_path / "r.can")
    ...
    def test_commit_message_sanitized(self, tmp_path, monkeypatch):
        from kadhi_cli.cans.publish import _sanitize_commit_message
    ...
    def test_happy_path_uploads_with_mocked_hf(self, tmp_path, monkeypatch):
        from kadhi_cli.cans import publish as publish_mod
