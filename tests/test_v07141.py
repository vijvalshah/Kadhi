"""v0.71.41 — `kadhi reward stress` adversarial verifier probe + telemetry doc-fix.
...
from kadhi_cli.utils import reward_stress as rst
...
class TestStressCli:
    def test_help(self):
        r = runner.invoke(kadhi_app, ["reward", "stress", "--help"])
    ...
    def test_bad_attacks_exit_1(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        v = _write(tmp_path, "v.py", _ROBUST_VERIFIER)
        r = runner.invoke(kadhi_app, ["reward", "stress", v.name, "--attacks", "bogus"])
    ...
    def test_bad_threshold_exit_1(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        v = _write(tmp_path, "v.py", _ROBUST_VERIFIER)
        r = runner.invoke(kadhi_app, ["reward", "stress", v.name, "--threshold", "9"])
        assert r.exit_code == 1, (r.output, repr(r.exception))
    ...
    def test_bad_max_gameable_exit_1(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        v = _write(tmp_path, "v.py", _ROBUST_VERIFIER)
        r = runner.invoke(kadhi_app, ["reward", "stress", v.name, "--max-gameable", "2"])
        assert r.exit_code == 1, (r.output, repr(r.exception))
    ...
    def test_empty_attacks_list_exit_1(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        v = _write(tmp_path, "v.py", _ROBUST_VERIFIER)
        r = runner.invoke(kadhi_app, ["reward", "stress", v.name, "--attacks", ","])
        assert r.exit_code == 1, (r.output, repr(r.exception))
    ...
    def test_target_raises_at_import_exit_1(self, tmp_path, monkeypatch):
        ...
        r = runner.invoke(kadhi_app, ["reward", "stress", v.name])
        assert r.exit_code == 1, (r.output, repr(r.exception))
    ...
    def test_builtin_no_references_exit_1(self, tmp_path, monkeypatch):
        # `kadhi reward stress accuracy` with no --references cannot probe a
        # gold-requiring builtin — must exit 1 with a helpful message, NOT a
        # false "robust" exit 0.
        monkeypatch.chdir(tmp_path)
        r = runner.invoke(kadhi_app, ["reward", "stress", "accuracy"])
        assert r.exit_code == 1, (r.output, repr(r.exception))
...
class TestHardening:
    ...
    def test_stress_registered(self):
        r = runner.invoke(kadhi_app, ["reward", "--help"])
        assert r.exit_code == 0, (r.output, repr(r.exception))
    ...
    def test_target_outside_cwd_rejected(self, tmp_path, monkeypatch):
        ...
        r = runner.invoke(kadhi_app, ["reward", "stress", "../evil.py"])
        assert r.exit_code == 1, (r.output, repr(r.exception))
    ...
    def test_symlinked_target_rejected(self, tmp_path, monkeypatch):
        ...
        r = runner.invoke(kadhi_app, ["reward", "stress", link.name])
        assert r.exit_code == 1, (r.output, repr(r.exception))
