...
Covers:
  ...
  * #122  — ``is_flash_attn_v3_available`` + KadhiConfig LongLoRA+FA3 reject
...
class TestOomFriendlyMessage:
    def test_cuda_oom_names_batch_size_and_grad_accum(self):
        ...
        with patch("kadhi_cli.utils.errors.console", test_console):
...
class TestFlashAttnV3Available:
    ...
    def test_returns_false_when_transformers_says_no(self, monkeypatch):
        from kadhi_cli.utils import flash_attn as mod
...
class TestLongLoraRejectsFlashAttnV3:
    def test_schema_rejects_longlora_when_fa3_present(self, monkeypatch):
        # Force FA3 detected -> KadhiConfig schema gate must reject.
        monkeypatch.setattr(
            "kadhi_cli.utils.flash_attn.is_flash_attn_v3_available",
            lambda: True,
        )
        yaml_in = """
...
class TestIsQwenModel:
    def test_basic(self):
        from kadhi_cli.utils.longlora import is_qwen_model
...
class TestIsPhiModel:
    def test_basic(self):
        from kadhi_cli.utils.longlora import is_phi_model
