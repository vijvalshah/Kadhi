...
Tests for ``kadhi_cli/utils/cmaes_merge.py``:
...
- ``kadhi adapters merge --strategy cmaes --eval <suite> --budget 1h`` CLI plumbing
...
class TestPublicSurface:
    def test_module_importable(self) -> None:
        from kadhi_cli.utils import cmaes_merge
...
class TestValidatePopulationSize:
    def test_happy(self) -> None:
        from kadhi_cli.utils.cmaes_merge import validate_population_size
...
class TestValidateGenerations:
    def test_happy(self) -> None:
        from kadhi_cli.utils.cmaes_merge import validate_generations
...
class TestCmaesPlan:
    def test_construct(self) -> None:
        from kadhi_cli.utils.cmaes_merge import CmaesPlan
...
class TestCmaesResult:
    def test_construct(self) -> None:
        from kadhi_cli.utils.cmaes_merge import CmaesResult
...
class TestRunCmaesMerge:
    ...
    def test_non_plan_rejected(self) -> None:
        from kadhi_cli.utils.cmaes_merge import run_cmaes_merge
...
class TestSourceWiring:
    def test_no_top_level_heavy_imports(self) -> None:
        ...
        src = (root / "src" / "kadhi_cli" / "utils" / "cmaes_merge.py").read_text(
            encoding="utf-8"
        )
    ...
    def test_adapter_merge_lists_cmaes(self) -> None:
        ...
        root = Path(__file__).resolve().parent.parent
        src = (root / "src" / "kadhi_cli" / "utils" / "adapter_merge.py").read_text(
            encoding="utf-8"
        )
        assert "cmaes" in src
