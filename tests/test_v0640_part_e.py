...
def test_module_imports():
    from kadhi_cli.utils import completions
...
def test_supported_shells():
    from kadhi_cli.utils.completions import SUPPORTED_SHELLS
...
def test_validate_shell_happy(v):
    from kadhi_cli.utils.completions import validate_shell
...
def test_render_bash_script_basic():
    from kadhi_cli.utils.completions import render_bash_script

    text = render_bash_script()
    assert "_kadhi_complete" in text or "complete -F" in text
    assert "kadhi" in text


def test_render_zsh_script_basic():
    from kadhi_cli.utils.completions import render_zsh_script

    text = render_zsh_script()
    assert "#compdef kadhi" in text or "_kadhi" in text


def test_render_fish_script_basic():
    from kadhi_cli.utils.completions import render_fish_script

    text = render_fish_script()
    assert "complete -c kadhi" in text or "complete --command kadhi" in text


def test_render_completion_script_dispatch():
    from kadhi_cli.utils.completions import render_completion_script

    bash = render_completion_script("bash")
    assert "kadhi" in bash
    zsh = render_completion_script("zsh")
    assert "kadhi" in zsh
    fish = render_completion_script("fish")
    assert "kadhi" in fish
...
def test_complete_recipe_name_returns_list():
    from kadhi_cli.utils.completions import complete_recipe_name
...
def test_no_heavy_top_level_imports():
    ...
    src = Path(__file__).resolve().parent.parent / "src" / "kadhi_cli" / "utils" / "completions.py"
