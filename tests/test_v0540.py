...
- Part C: Cross-project learning (advise_history + `kadhi advise compare`).
"""
...
from kadhi_cli.commands import advise as advise_cmd
from kadhi_cli.utils.advise import (
    CHOICES,
    TASK_CATEGORIES,
    DatasetProfile,
    ROIEstimate,
...
from kadhi_cli.utils.advise_history import (
    HistoryEntry,
    history_path,
    load_history,
    record_verdict,
...
class TestHistoryPath:
    def test_default_under_home(self, monkeypatch):
        monkeypatch.delenv("KADHI_ADVISE_HISTORY_PATH", raising=False)
        p = history_path()
        assert p.endswith(os.path.join(".kadhi", "advise_history.jsonl"))
    ...
    def test_env_override_in_tempdir(self, monkeypatch, tmp_path):
        ...
        override = os.path.join(sysdir, "kadhi_advise_test.jsonl")
        monkeypatch.setenv("KADHI_ADVISE_HISTORY_PATH", override)
        p = history_path()
    ...
    def test_env_override_out_of_bounds_falls_back(self, monkeypatch):
        ...
        monkeypatch.setenv("KADHI_ADVISE_HISTORY_PATH", "/etc/advise.jsonl")
        p = history_path()
    ...
    def test_env_null_byte_falls_back(self, monkeypatch):
        ...
        from kadhi_cli.utils import advise_history

        original = advise_history.os.environ
        try:
            advise_history.os.environ = {"KADHI_ADVISE_HISTORY_PATH": "x\x00y"}  # type: ignore[assignment]
            p = history_path()
        finally:
...
class TestCLI:
    ...
    def test_compare_after_record(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KADHI_ADVISE_HISTORY_PATH", str(history))
...
class TestSourceWiring:
    def test_cli_registers_advise(self):
        cli_path = Path(__file__).resolve().parent.parent / "src" / "kadhi_cli" / "cli.py"
    ...
    def test_advise_module_imports(self):
        ...
        from kadhi_cli.commands import advise as _cmd  # noqa: F401
        from kadhi_cli.utils import advise as _advise  # noqa: F401
        from kadhi_cli.utils import advise_history as _hist  # noqa: F401


class TestArgvRewriter:
    def test_no_advise_in_argv(self):
        from kadhi_cli.cli import _rewrite_advise_argv
        argv = ["kadhi", "train", "--config", "x.yaml"]
        assert _rewrite_advise_argv(argv) == argv

    def test_advise_with_subcommand_unchanged(self):
        from kadhi_cli.cli import _rewrite_advise_argv
        for sub in ("run", "explain", "compare", "--help"):
            argv = ["kadhi", "advise", sub]
            assert _rewrite_advise_argv(argv) == argv

    def test_advise_with_data_injects_run(self):
        from kadhi_cli.cli import _rewrite_advise_argv
        argv = ["kadhi", "advise", "data.jsonl"]
        out = _rewrite_advise_argv(argv)
        assert out == ["kadhi", "advise", "run", "data.jsonl"]

    def test_advise_with_data_and_flags(self):
        from kadhi_cli.cli import _rewrite_advise_argv
        argv = ["kadhi", "advise", "data.jsonl", "--goal", "summarize"]
        out = _rewrite_advise_argv(argv)
        assert out[2] == "run"
...
class TestAdviseSchemaField:
    """Plan's cross-cutting bullet: schema-only `advise: AdviseConfig` on KadhiConfig."""
...
"""
    ...
    def test_advise_null_byte_goal_rejected(self):
        ...
        from kadhi_cli.config.schema import AdviseConfig
...
class TestNextCommandFor:
    @pytest.mark.parametrize("choice,expected_token", [
        ("PROMPT_ENG", "kadhi chat"),
        ("RAG", "RAG is outside"),
        ("SFT", "kadhi autopilot"),
        ("DPO", "kadhi autopilot"),
        ("GRPO", "kadhi autopilot"),
    ])
