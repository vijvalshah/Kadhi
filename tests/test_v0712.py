...
class TestSigningPrimitives:
    def test_is_signing_available(self):
        from kadhi_cli.utils.signing import is_signing_available
    ...
    def test_generate_and_roundtrip(self):
        from kadhi_cli.utils.signing import (
    ...
    def test_private_key_from_pem_rejects_non_ed25519(self):
        ...
        from kadhi_cli.utils.signing import private_key_from_pem
    ...
    def test_load_private_key_file_missing(self, tmp_path):
        from kadhi_cli.utils.signing import load_private_key_file
    ...
    def test_resolve_signing_key_env(self, tmp_path):
        ...
        key_path = tmp_path / "k.pem"
        key_path.write_text(generate_ed25519_private_pem(), encoding="utf-8")
        key = resolve_signing_key(None, env={"KADHI_SIGNING_KEY": str(key_path)})
        assert key is not None

    def test_resolve_signing_key_missing_raises(self):
        from kadhi_cli.utils.signing import resolve_signing_key
...
class TestAdapterSignEd25519:
    def _key(self, tmp_path):
        from kadhi_cli.utils.signing import generate_ed25519_private_pem
    ...
    def test_sign_ed25519_writes_real_signature(self, tmp_path, monkeypatch):
        ...
        from kadhi_cli.utils.adapter_sign import sign_adapter
        ...
        sig_file = json.loads((adir / ".kadhi-signature.json").read_text())
    ...
    def test_verify_ed25519_passes_on_clean_adapter(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.utils.adapter_sign import sign_adapter, verify_adapter
    ...
    def test_verify_ed25519_fails_when_signature_corrupted(self, tmp_path, monkeypatch):
        ...
        sig_path = adir / ".kadhi-signature.json"
    ...
    def test_verify_with_matching_trusted_key(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.utils.adapter_sign import sign_adapter, verify_adapter
        from kadhi_cli.utils.signing import load_private_key_file, public_key_pem
    ...
    def test_sign_ed25519_without_key_raises(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        monkeypatch.delenv("KADHI_SIGNING_KEY", raising=False)
        from kadhi_cli.utils.adapter_sign import sign_adapter
...
# ---------------------------------------------------------------------------
# #179 — ed25519 backend for kadhi attest
# ---------------------------------------------------------------------------
class TestAttestEd25519:
    ...
    def test_sign_attestation_ed25519(self, tmp_path):
        from kadhi_cli.utils.attest import sign_attestation, verify_attestation
...
class TestNamespacePinConcurrency:
    def test_wal_mode_enabled(self, tmp_path):
        from kadhi_cli.utils.namespace_pin import NamespacePinStore
    ...
    def test_concurrent_puts_preserve_first_seen(self, tmp_path):
        ...
        from kadhi_cli.utils.namespace_pin import (
            NamespacePin,
            NamespacePinStore,
...
class TestDownloadRepoNamespacePin:
    def _setup(self, tmp_path, monkeypatch):
        ...
        monkeypatch.chdir(tmp_path)
        monkeypatch.setenv(
            "KADHI_NAMESPACE_PIN_DB", str(tmp_path / "pins.db")
        )
...
class TestLicenseExtraction:
    def test_from_adapter_config(self, tmp_path):
        from kadhi_cli.utils.license_matrix import extract_license_from_adapter
...
class TestLicenseOverrideAudit:
    def test_record_license_override_writes_audit(self, tmp_path, monkeypatch):
        ...
        from kadhi_cli.utils.license_matrix import record_license_override
...
class TestMergeScanGate:
    def _run(self, monkeypatch, tmp_path, args, *, scan_overall="OK", scan_raises=None):
        ...
        if scan_raises is not None:
            ...
            monkeypatch.setattr("kadhi_cli.utils.adapter_scan.scan_adapter", _scan)
        else:
            monkeypatch.setattr(
                "kadhi_cli.utils.adapter_scan.scan_adapter", _fake_scan(scan_overall)
            )
        monkeypatch.setattr(
            "kadhi_cli.utils.adapter_merge.merge_adapters",
...
class TestMergeLicenseAutoExtract:
    def _run(self, monkeypatch, tmp_path, args, audit_log=None):
        ...
        monkeypatch.setattr(
            "kadhi_cli.utils.adapter_scan.scan_adapter", _fake_scan("OK")
        )
        monkeypatch.setattr(
            "kadhi_cli.utils.adapter_merge.merge_adapters",
            lambda *a, **k: _fake_merge_report(),
        ...
        if audit_log is not None:
            monkeypatch.setenv("KADHI_AUDIT_LOG_PATH", str(audit_log))
        monkeypatch.chdir(tmp_path)
        return CliRunner().invoke(app, args)
...
class TestReviewFollowups:
    ...
    def test_verify_attestation_non_bytes_typeerror(self):
        from kadhi_cli.utils.attest import verify_attestation
    ...
    def test_attest_verify_non_ed25519_sidecar_exit3(self, tmp_path, monkeypatch):
        ...
        from kadhi_cli.commands.attest import app
    ...
    def test_verify_adapter_strict_raises_on_unsigned(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from kadhi_cli.utils.adapter_sign import verify_adapter
    ...
    def test_pin_store_rejects_outside_cwd(self):
        ...
        with pytest.raises(ValueError, match="must stay under"):
            NamespacePinStore("/etc/kadhi_evil_pin.db")
    ...
    def test_require_cryptography_raises_when_unavailable(self, monkeypatch):
        from kadhi_cli.utils import signing

        monkeypatch.setattr(signing, "is_signing_available", lambda: False)
        with pytest.raises(ValueError, match="kadhi-cli\\[sign\\]"):
    ...
    def test_check_license_compat_empty_and_nonlist(self):
        from kadhi_cli.utils.license_matrix import check_license_compat
...
class TestSecurityFixes:
    ...
    def test_download_repo_fails_open_on_store_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.chdir(tmp_path)
        monkeypatch.setenv("KADHI_NAMESPACE_PIN_DB", str(tmp_path / "pins.db"))
        from kadhi_cli.utils import hubs
        ...
        with patch(
            "kadhi_cli.utils.namespace_pin.NamespacePinStore", boom
