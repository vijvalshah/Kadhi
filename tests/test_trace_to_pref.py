...
class TestLangChainParser:
    def test_parse_event_with_thumbs_up(self):
        from kadhi_cli.data.traces.parsers import parse_langchain
...
class TestOpenAIParser:
    def test_parse_completion_with_feedback(self):
        from kadhi_cli.data.traces.parsers import parse_openai
...
# ---------------------------------------------------------------------------
# Kadhi serve trace parser
# ---------------------------------------------------------------------------


class TestKadhiServeParser:
    def test_parse_kadhi_serve_dir(self, tmp_path):
...
class TestPairBuilder:
    def test_build_pairs_from_thumbs(self):
        ...
        from kadhi_cli.data.traces.parsers import Trace
...
class TestEmptyAndMalformed:
    ...
    def test_parse_kadhi_serve_non_directory_returns_empty(self, tmp_path):
        from kadhi_cli.data.traces.parsers import parse_kadhi_serve

        file_not_dir = tmp_path / "file.txt"
        file_not_dir.write_text("hi", encoding="utf-8")
        assert list(parse_kadhi_serve(str(file_not_dir))) == []


class TestLiterals:
    def test_format_literal_accepts_known(self):
        from kadhi_cli.data.traces.pair_builder import SUPPORTED_SIGNALS
    ...
    def test_parser_registry(self):
        from kadhi_cli.data.traces.parsers import SUPPORTED_FORMATS

        assert "langchain" in SUPPORTED_FORMATS
        assert "openai" in SUPPORTED_FORMATS
        assert "kadhi-serve" in SUPPORTED_FORMATS
