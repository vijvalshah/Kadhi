...
def test_orpo_length_normalization_restores_odds_ratio():
    ...
    torch = pytest.importorskip("torch")
    from kadhi_cli.utils.preference_combine import compute_orpo_term
...
def test_ipo_beta_schedule_uses_ipo_tau_not_dpo_beta():
    src = (Path(kadhi_cli.__file__).parent / "trainer" / "ipo.py").read_text(
        encoding="utf-8"
    )
...
def test_beta_schedule_callback_sets_beta_from_start():
    ...
    from kadhi_cli.utils.dpo_variants import BetaScheduleCallback
...
def test_kto_negative_one_label_is_undesirable():
    ...
    from kadhi_cli.data.formats import _convert_kto
...
def test_apply_llama_pro_freeze_freezes_all_but_new_blocks():
    ...
    from kadhi_cli.utils.block_expansion import apply_llama_pro_freeze
...
def test_block_expansion_freeze_uses_actual_added_not_requested(monkeypatch):
    ...
    import kadhi_cli.utils.block_expansion as be
...
def _src(rel: str) -> str:
    return (Path(kadhi_cli.__file__).parent / rel).read_text(encoding="utf-8")
...
def test_deploy_measure_cache_key_includes_candidates():
    from kadhi_cli.utils.deploy_measure import compute_cache_key
...
def test_namespace_created_at_differs_both_directions():
    from kadhi_cli.utils.namespace_pin import _created_at_differs
...
def test_namespace_pin_flags_forward_created_at_drift(tmp_path):
    from kadhi_cli.utils.namespace_pin import NamespacePinStore, verify_namespace
