...
- ``kadhi data doctor`` — template-compat report (EOS-missing / BOS
  duplication / no-system-role / unknown-role / truncation risk) +
  ``--show-mask N`` loss-mask X-ray through the REAL collator path.
- ``kadhi data lint`` — preference-data linter (dpo/kto): length bias
...
- ``commands/diagnose.py::_load_evidence`` — O_NOFOLLOW + fstat hardening
  (backport of the v0.71.25 ``kadhi ship`` pattern).
"""
...
class _TypeErrorOnKwargTokenizer(_FakeTokenizer):
    ...
    `kadhi data lint --model` length_fn's inner TypeError fallback."""
...
class TestDoctorCheck:
    def test_valid_check(self):
        from kadhi_cli.utils.data_doctor import DoctorCheck
...
class TestOverallVerdict:
    def test_empty_is_ok(self):
        from kadhi_cli.utils.data_doctor import overall_verdict

        assert overall_verdict([]) == "OK"

    def test_worst_wins(self):
        from kadhi_cli.utils.data_doctor import DoctorCheck, overall_verdict
...
class TestDoctorReport:
    def test_to_dict_roundtrip_shape(self):
        from kadhi_cli.utils.data_doctor import DoctorCheck, compose_doctor_report
    ...
    def test_rows_scanned_cannot_exceed_total(self):
        from kadhi_cli.utils.data_doctor import DoctorReport
...
class TestSampleIndices:
    def test_empty(self):
        from kadhi_cli.utils.data_doctor import sample_indices
...
class TestResolveTokenizer:
    def test_passthrough_object(self):
        from kadhi_cli.utils.data_doctor import resolve_tokenizer
...
class TestCheckChatTemplate:
    def test_present(self):
        from kadhi_cli.utils.data_doctor import check_chat_template
...
class TestCheckTemplateRender:
    def test_all_render_ok(self):
        from kadhi_cli.utils.data_doctor import check_template_render
...
class TestEosTokenIds:
    ...
    def test_list_eos_token_id_normalised(self):
        from kadhi_cli.utils.data_doctor import _eos_token_ids
    ...
    def test_multi_eos_list_actually_used_by_the_check(self):
        ...
        from kadhi_cli.utils.data_doctor import check_eos_in_labels
...
class TestCheckBosDuplication:
    def test_single_bos_ok(self):
        from kadhi_cli.utils.data_doctor import check_bos_duplication
...
class TestCheckSystemRole:
    def test_no_system_rows_is_ok(self):
        from kadhi_cli.utils.data_doctor import check_system_role
...
class TestCheckUnknownRoles:
    def test_known_roles_ok(self):
        from kadhi_cli.utils.data_doctor import check_unknown_roles
...
class TestRunDoctor:
    def test_happy_path(self):
        from kadhi_cli.utils.data_doctor import run_doctor
    ...
    def test_zero_convertible_rows_raises(self):
        ...
        with pytest.raises(ValueError, match="kadhi data lint"):
...
class TestMaskedToken:
    def test_valid(self):
        from kadhi_cli.utils.data_doctor import MaskedToken
...
class TestRenderMaskPreview:
    def test_assistant_only_strategy(self):
        from kadhi_cli.utils.data_doctor import render_mask_preview
...
class TestNoTopLevelHeavyImport:
    @pytest.mark.parametrize(
        "relpath",
        [
            "src/kadhi_cli/utils/data_doctor.py",
            "src/kadhi_cli/utils/data_lint.py",
            "src/kadhi_cli/commands/data_doctor.py",
        ],
    )
...
class TestLintCheck:
    ...
    def test_valid_check(self):
        from kadhi_cli.utils.data_lint import LintCheck
...
class TestLintReport:
    def test_to_dict_roundtrip_shape(self):
        from kadhi_cli.utils.data_lint import LintCheck, compose_lint_report
    ...
    def test_rows_scanned_cannot_exceed_total(self):
        from kadhi_cli.utils.data_lint import LintReport
...
class TestExtractPrefText:
    def test_string_passthrough(self):
        from kadhi_cli.utils.data_lint import extract_pref_text
...
class TestCohensD:
    def test_identical_distributions_zero(self):
        from kadhi_cli.utils.data_lint import cohens_d
...
class TestCheckLengthBias:
    def test_balanced_lengths_ok(self):
        from kadhi_cli.utils.data_lint import check_length_bias
...
class TestCheckLabelImbalance:
    def test_balanced_ok(self):
        from kadhi_cli.utils.data_lint import check_label_imbalance
...
class TestCheckIdenticalPairs:
    def test_no_dupes_ok(self):
        from kadhi_cli.utils.data_lint import check_identical_pairs
...
class TestCheckPromptLeak:
    def test_no_leak_ok(self):
        from kadhi_cli.utils.data_lint import check_prompt_leak
...
class TestCheckNearDuplicates:
    def test_no_dupes_ok(self):
        from kadhi_cli.utils.data_lint import check_near_duplicates
...
class TestRunLint:
    def test_dpo_happy_path(self):
        from kadhi_cli.utils.data_lint import run_lint
...
# ===========================================================================
# CLI layer — kadhi data doctor / kadhi data lint
# ===========================================================================
...
def _patch_tokenizer(monkeypatch, tok=None):
    ...
    import kadhi_cli.utils.data_doctor as engine
...
class TestLoadTokenizerUsesResolvedTrust:
    def test_resolved_trust_value_is_what_gets_passed_through(self, monkeypatch):
        ...
        import kadhi_cli.commands.data_doctor as cli_mod
