...
``kadhi train`` on an RTX 3050 Laptop (4 GB, Windows, torch 2.5.1), SmolLM2-135M
...
from kadhi_cli.utils.layer_stream import decide_measured_fit, decide_stream_fit
...
class TestTheProbeIsActuallyWiredIntoSetup:
    ...
    def _plan(predicted, available):
        from kadhi_cli.trainer.stream_setup import _ProbePlan
    ...
    def test_an_unexpected_exception_still_releases_the_pinned_ram_store(
        ...
        wrapper = self._wrapper(monkeypatch, None)
        monkeypatch.setattr(
            "kadhi_cli.utils.layer_stream_runtime.measure_step_peak_bytes", _boom
        )
...
class TestTheDeferralHasACeiling:
    ...
    def _budget(probe, predicted_gb, free_gb, *, on_cuda=False, monkeypatch=None):
        ...
        if on_cuda:
            ...
            monkeypatch.setattr(
                "kadhi_cli.utils.layer_stream_runtime.measure_gemm_tflops",
                lambda *_a, **_k: None,
            )
    ...
    def test_far_over_budget_is_refused_outright_even_with_the_probe_on(self, monkeypatch):
        ...
        sharding plus a real allocation at that shape, driven by a kadhi.yaml
