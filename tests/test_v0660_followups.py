...
def test_interference_cell_verdict_bool_raises_type_error():
    from kadhi_cli.utils.interference import InterferenceCell
...
def test_sae_feature_change_raises_frozen_instance_error():
    from kadhi_cli.utils.sae_diff import SaeFeatureChange
...
def test_sae_diff_report_raises_frozen_instance_error():
    from kadhi_cli.utils.sae_diff import SaeFeatureDiffReport
...
def test_row_influence_raises_frozen_instance_error():
    from kadhi_cli.utils.blame import RowInfluence
...
def test_blame_result_raises_frozen_instance_error():
    from kadhi_cli.utils.blame import BlameResult
...
def test_interference_matrix_raises_frozen_instance_error():
    from kadhi_cli.utils.interference import InterferenceMatrix
...
def test_probe_entry_raises_frozen_instance_error():
    from kadhi_cli.utils.probe_pack import ProbeEntry
...
def test_probe_pack_raises_frozen_instance_error():
    from kadhi_cli.utils.probe_pack import ProbeEntry, ProbePack
...
def test_interference_render_markdown_escapes_adapter_name():
    ...
    from kadhi_cli.utils.interference import (
        InterferenceCell,
        InterferenceMatrix,
...
def test_blame_render_markdown_escapes_layer():
    ...
    from kadhi_cli.utils.blame import BlameResult, render_blame_markdown
...
def test_probe_pack_render_markdown_escapes_description():
    from kadhi_cli.utils.probe_pack import (
        ProbeEntry,
        ProbePack,
...
def test_count_dataset_rows_rejects_symlink_via_o_nofollow(tmp_path, monkeypatch):
    ...
    from kadhi_cli.utils.blame import plan_blame
...
def test_load_sae_weights_rejects_symlink_via_o_nofollow(tmp_path, monkeypatch):
    from kadhi_cli.utils.sae_diff import load_sae_weights
...
def test_count_dataset_rows_raises_on_oversize(tmp_path, monkeypatch):
    ...
    from kadhi_cli.utils import blame
...
def test_sleeper_lower_index_immutable():
    from kadhi_cli.utils.sleeper_probe import _LOWER_INDEX
...
def test_probe_interference_rejects_string_loss(tmp_path, monkeypatch):
    ...
    result = runner_.invoke(kadhi_app, ["probe", "interference", p.name])
...
def test_classify_interference_boundary_minus_epsilon_ok():
    from kadhi_cli.utils.interference import classify_interference
...
def test_no_path_resolve_used_in_v0_66_modules():
    ...
    from kadhi_cli.utils import blame, interference, probe_pack, sae_diff, sleeper_probe
...
def test_compute_feature_diff_rejects_negative_top_k():
    from kadhi_cli.utils.sae_diff import compute_feature_diff
