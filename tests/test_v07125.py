"""v0.71.25 "kadhi ship — SHIP / DON'T-SHIP verdict engine" tests.

`kadhi ship` fuses two questions into one binary decision after fine-tuning:
...
2 = DON'T SHIP / 1 = runtime error, mirroring ``kadhi diagnose``.
"""
...
class TestDecideShip:
    def test_both_legs_pass_ships(self):
        ...
        assert verdict.kadhi_version == __version__
...
class TestRenderAndSerialize:
    ...
    def test_to_dict_round_trips(self):
        ...
        assert loaded["kadhi_version"] == __version__
...
class TestShipCliEvidence:
    ...
    def test_evidence_writes_output_json(self):
        ...
        with runner.isolated_filesystem():
            ...
            assert data["kadhi_version"] == __version__
    ...
    def test_evidence_outside_cwd_rejected(self):
        ...
        # Evidence read/parse problems are coded exit 1 (mirrors `kadhi diagnose`).
        assert res.exit_code == 1, (res.output, repr(res.exception))
...
from kadhi_cli.eval.forgetting import MINI_BENCHMARKS, build_mcq_prompt  # noqa: E402
...
class TestShipCliSecurity:
    ...
    def test_judge_url_ollama_allowed(self, monkeypatch):
        ...
        from kadhi_cli.eval import judge as judge_mod
