"""v0.56.0 — `kadhi diagnose` post-training model report card.
...
from kadhi_cli.utils.diagnose import (
    FAILURE_MODES,
    FailureReport,
    FailureScore,
    classify_score,
...
from kadhi_cli.utils.diagnose.badge import render_badge_svg
from kadhi_cli.utils.diagnose.contamination import score_contamination
from kadhi_cli.utils.diagnose.forgetting import score_forgetting
from kadhi_cli.utils.diagnose.format import (
    is_valid_json,
    is_valid_tool_call,
    matches_regex,
    score_format,
)
from kadhi_cli.utils.diagnose.memorization import score_memorization, split_prefix
from kadhi_cli.utils.diagnose.mode_collapse import score_mode_collapse
from kadhi_cli.utils.diagnose.refusal import looks_like_refusal, score_refusal
from kadhi_cli.utils.diagnose.report import THRESHOLDS
from kadhi_cli.utils.diagnose.report import classify_score as classify_v2
from kadhi_cli.utils.diagnose.runner import (
    build_report,
    write_report,
)
from kadhi_cli.utils.diagnose.runner import (
...
class TestTrainDiagnoseGate:
    ...
    def test_run_diagnose_gate_helper_major_exits(self, tmp_path: Path) -> None:
        ...
        from kadhi_cli.commands.train import _run_diagnose_gate
    ...
    def test_diagnose_gate_skips_nonzero_local_rank(self, monkeypatch: pytest.MonkeyPatch) -> None:
        from kadhi_cli.commands.train import _should_run_diagnose_gate_on_rank
...
class TestSourceWiring:
    def test_cli_registers_diagnose(self) -> None:
        source = (_PROJECT_ROOT / "src" / "kadhi_cli" / "cli.py").read_text(encoding="utf-8")
    ...
    def test_no_top_level_heavy_imports(self) -> None:
        ...
        probe_modules = [
            "src/kadhi_cli/utils/diagnose/__init__.py",
            "src/kadhi_cli/utils/diagnose/report.py",
            "src/kadhi_cli/utils/diagnose/_common.py",
            "src/kadhi_cli/utils/diagnose/forgetting.py",
            "src/kadhi_cli/utils/diagnose/refusal.py",
            "src/kadhi_cli/utils/diagnose/format.py",
            "src/kadhi_cli/utils/diagnose/mode_collapse.py",
            "src/kadhi_cli/utils/diagnose/memorization.py",
            "src/kadhi_cli/utils/diagnose/contamination.py",
            "src/kadhi_cli/utils/diagnose/citation.py",
            "src/kadhi_cli/utils/diagnose/badge.py",
            "src/kadhi_cli/utils/diagnose/runner.py",
        ]
...
class TestReviewFixCoverage:
    ...
    def test_write_report_uses_realpath(self) -> None:
        source = (
            _PROJECT_ROOT / "src" / "kadhi_cli" / "utils" / "diagnose" / "runner.py"
        ).read_text(
            encoding="utf-8"
        )
    ...
    def test_neutral_score_centralised(self) -> None:
        from kadhi_cli.utils.diagnose.runner import neutral_score
    ...
    def test_diagnose_uses_typer_exit_not_sys_exit(self) -> None:
        source = (_PROJECT_ROOT / "src" / "kadhi_cli" / "commands" / "diagnose.py").read_text(
            encoding="utf-8"
        )
    ...
    def test_badge_symlink_rejected(self, tmp_path: Path) -> None:
        ...
        os.symlink(target, link)
        from kadhi_cli.commands.diagnose import _write_badge
    ...
    # `kadhi ship`'s loader) so the size check now reads via os.fstat on the
    ...
    def test_evidence_size_cap(self, tmp_path: Path) -> None:
        ...
        from kadhi_cli.commands.diagnose import _load_evidence
    ...
    def test_matches_regex_invalid_pattern_rejected(self) -> None:
        ...
        source = (
            _PROJECT_ROOT / "src" / "kadhi_cli" / "utils" / "diagnose" / "format.py"
        ).read_text(
            encoding="utf-8"
        )
    ...
    def test_refusal_input_capped(self) -> None:
        from kadhi_cli.utils.diagnose.refusal import _MAX_REFUSAL_SCAN
    ...
    def test_contamination_combined_cap(self) -> None:
        ...
        source = (
            _PROJECT_ROOT / "src" / "kadhi_cli" / "utils" / "diagnose" / "contamination.py"
        ).read_text(
            encoding="utf-8"
        )
    ...
    def test_format_valid_kinds_frozenset(self) -> None:
        from kadhi_cli.utils.diagnose.format import _VALID_KINDS
    ...
    def test_tokenize_delegates(self) -> None:
        from kadhi_cli.utils._eval_text import tokenize as shared
        from kadhi_cli.utils.diagnose._common import tokenize as local
    ...
    def test_extract_row_text(self) -> None:
        from kadhi_cli.utils.diagnose._common import extract_row_text
    ...
    # tdd-review LOW — build_report forwards kadhi_version + extras.
    def test_build_report_forwards_kadhi_version(self) -> None:
        report = build_report(
            run_id="r1", base="b", adapter="a",
            scores={}, kadhi_version="0.56.0",
            extras={"k": "v"},
        )
        assert report.kadhi_version == "0.56.0"
