...
_LIVE = {
    "sft": ("kadhi_cli.trainer.sft", "SFTTrainerWrapper", _sft_rows, {}),
    "bco": ("kadhi_cli.trainer.bco", "BCOTrainerWrapper", _pref_rows, {}),
    "dpo": ("kadhi_cli.trainer.dpo", "DPOTrainerWrapper", _pref_rows, {}),
    "ipo": ("kadhi_cli.trainer.ipo", "IPOTrainerWrapper", _pref_rows, {}),
    "kto": ("kadhi_cli.trainer.kto", "KTOTrainerWrapper", _kto_rows, {}),
    "orpo": ("kadhi_cli.trainer.orpo", "ORPOTrainerWrapper", _pref_rows, {}),
    "simpo": ("kadhi_cli.trainer.simpo", "SimPOTrainerWrapper", _pref_rows, {}),
    "reward_model": (
...
class TestEveryConfigClassAcceptsTheSeed:
    ...
    def test_the_installed_configs_accept_the_field(self, field):
        _requires_train_extra()
        from kadhi_cli.trainer._trl_compat import config_accepts
...
def _schema_tasks():
    ...
    annotation = KadhiConfig.model_fields["task"].annotation
...
def _modules_defining_setup():
    """`kadhi_cli.trainer` module name -> its `setup()` AST node.
    ...
    spec = importlib.util.find_spec("kadhi_cli.trainer")
    found = {}
...
class TestEveryWrapperAppliesTheSeed:
    ...
    def _setup_body(name):
        ...
        assert name in setups, f"kadhi_cli.trainer.{name} defines no setup()"
        return setups[name]
    ...
    def test_the_derivation_found_the_wrappers(self):
        ...
        assert {"sft", "grpo", "orpo", "unlearn"} <= set(_MODULES), (
            f"derived only {sorted(_MODULES)} from kadhi_cli.trainer, which "
    ...
    def test_the_wrapper_seeds_inside_setup(self, name):
        ...
        assert seeded, (
            f"kadhi_cli.trainer.{name}.setup() never calls apply_training_seed(), "
    ...
    def test_the_seed_is_applied_before_the_model_is_built(self, name):
        ...
        assert built, (
            f"kadhi_cli.trainer.{name}.setup() has no recognisable model build; "
        ...
        assert min(seeded) < min(built), (
            f"kadhi_cli.trainer.{name}: apply_training_seed() is called at line "
    ...
    def test_the_wrapper_threads_the_seed_into_its_config(self, name):
        ...
        spec = importlib.util.find_spec(f"kadhi_cli.trainer.{name}")
        source = open(spec.origin, encoding="utf-8").read()
        assert "training_seed_kwargs(" in source, (
            f"kadhi_cli.trainer.{name} builds a TrainingArguments subclass "
...
class TestSeedingHelper:
    ...
    def test_kwargs_carry_both_fields(self):
        from kadhi_cli.utils.seeding import training_seed_kwargs
    ...
    def test_applying_the_seed_makes_torch_reproducible(self):
        ...
        from kadhi_cli.utils.seeding import apply_training_seed
    ...
    def test_the_module_imports_without_torch(self):
        ...
        code = (
            ...
            "import kadhi_cli.utils.seeding as s;"
