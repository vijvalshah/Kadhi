...
* H5 — `kadhi steer train --plan-only` deferred-marker stripping regression guard.
...
* M3/M4 — kadhi steer train `--base` over-cap + null-byte rejection.
...
class TestRaftFieldOversizeBoundary:
    def test_query_at_cap_accepted(self):
        from kadhi_cli.data.formats import _MAX_RAFT_FIELD_LEN, _convert_raft
...
class TestRaftNullByteRejection:
    def test_null_byte_golden_doc_rejected(self):
        from kadhi_cli.data.formats import _convert_raft
...
# ---------- `kadhi steer train --plan-only` (live as of v0.71.10 #201) ----------
...
class TestEditMarkerRegressionGuard:
    ...
    def test_grace_dispatches_to_grace_edit(self, monkeypatch):
        import kadhi_cli.utils.grace_codebook as gc
        from kadhi_cli.utils.knowledge_edit import apply_edit, build_edit_plan
...
class TestSteeringNameBoundary:
    def test_at_max_length_accepted(self):
        from kadhi_cli.utils.steering import validate_steering_name
...
"""
    ...
    def test_grace_in_default_edit_layer(self):
        from kadhi_cli.utils.knowledge_edit import _DEFAULT_EDIT_LAYER  # type: ignore
    ...
    def test_grace_in_edit_metadata_mapping(self):
        from kadhi_cli.utils.knowledge_edit import _EDIT_METHOD_METADATA  # type: ignore
...
class TestSourceWiring:
    def test_list_steers_uses_context_manager(self):
        src = Path(__file__).resolve().parent.parent / "src" / "kadhi_cli" / "commands" / "steer.py"
    ...
    def test_version_string_is_at_least_v0620(self):
        ...
        assert (major, minor) >= (0, 62), kadhi_cli.__version__
