"""Tests for kadhi migrate — config import from LLaMA-Factory, Axolotl, Unsloth."""
...
class TestLlamaFactoryMigration:
    """LLaMA-Factory → Kadhi config migration."""
    ...
    def test_sft_lora_target_all(self, tmp_path):
        """lora_target: all → auto (Kadhi auto-detects)."""
        from kadhi_cli.migrate.llamafactory import migrate_llamafactory
    ...
    def test_round_trip_valid_config(self, tmp_path):
        """Migrated config loads as valid KadhiConfig."""
        from kadhi_cli.config.loader import load_config_from_string
        from kadhi_cli.migrate.common import config_to_yaml
        from kadhi_cli.migrate.llamafactory import migrate_llamafactory
        ...
        kadhi_config = load_config_from_string(yaml_str)
        assert kadhi_config.base == "meta-llama/Llama-3.1-8B-Instruct"
        assert kadhi_config.task == "sft"
...
class TestAxolotlMigration:
    """Axolotl → Kadhi config migration."""
    ...
    def test_multi_dataset_warning(self, tmp_path):
        """Multiple datasets triggers a warning (Kadhi uses single dataset)."""
        from kadhi_cli.migrate.axolotl import migrate_axolotl
...
class TestUnslothMigration:
    """Unsloth notebook → Kadhi config migration."""
...
class TestCommon:
    ...
    def test_validate_input_path(self, tmp_path, monkeypatch):
        ...
        from kadhi_cli.migrate.common import validate_input_path
    ...
    def test_validate_output_path(self, tmp_path, monkeypatch):
        ...
        from kadhi_cli.migrate.common import validate_output_path

        monkeypatch.chdir(tmp_path)
        validated = validate_output_path(Path("kadhi.yaml"))
        assert str(validated).endswith("kadhi.yaml")
...
class TestMigrateCLI:
    """CLI tests for kadhi migrate command."""

    def test_help(self):
        """kadhi migrate --help shows usage."""
        result = runner.invoke(app, ["migrate", "--help"])
        assert result.exit_code == 0
        assert "migrate" in result.output.lower()

    def test_llamafactory_sft(self, tmp_path, monkeypatch):
        """Full CLI: kadhi migrate --from llamafactory config.yaml."""
        monkeypatch.chdir(tmp_path)
        cfg_file = tmp_path / "config.yaml"
        cfg_file.write_text(LLAMA_FACTORY_SFT, encoding="utf-8")
    ...
    def test_axolotl_sft(self, tmp_path, monkeypatch):
        """Full CLI: kadhi migrate --from axolotl config.yaml."""
        monkeypatch.chdir(tmp_path)
        cfg_file = tmp_path / "config.yaml"
        cfg_file.write_text(AXOLOTL_SFT, encoding="utf-8")
        result = runner.invoke(app, [
    ...
    def test_unsloth_notebook(self, tmp_path, monkeypatch):
        """Full CLI: kadhi migrate --from unsloth finetune.ipynb."""
        monkeypatch.chdir(tmp_path)
        nb_file = tmp_path / "finetune.ipynb"
        nb_file.write_text(json.dumps(UNSLOTH_SFT_NOTEBOOK), encoding="utf-8")
        result = runner.invoke(app, [
    ...
    def test_overwrite_confirmation(self, tmp_path, monkeypatch):
        """Existing kadhi.yaml asks for confirmation."""
        monkeypatch.chdir(tmp_path)
        cfg_file = tmp_path / "config.yaml"
        cfg_file.write_text(LLAMA_FACTORY_SFT, encoding="utf-8")
...
class TestMigrateSecurity:
    """Security tests for kadhi migrate."""
...
class TestMigrateTDDGaps:
    ...
    def test_oversized_input_file(self, tmp_path, monkeypatch):
        ...
        import kadhi_cli.migrate.common as common_mod
