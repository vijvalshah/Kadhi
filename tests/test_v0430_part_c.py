...
from kadhi_cli.utils.vscode_setup import build_launch_json, write_vscode_launch
...
class TestBuildLaunchJson:
    def test_default(self):
        ...
        train = next(c for c in configs if c["name"] == "kadhi train")
        assert "kadhi.yaml" in train["args"]
        assert train["module"] == "kadhi_cli.cli"

    def test_custom_config_path(self):
        payload = build_launch_json(config_path="my-cfg.yaml")
        train = next(c for c in payload["configurations"] if c["name"] == "kadhi train")
