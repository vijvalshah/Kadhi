...
- ``utils/spectrum_scan.py`` — pure-numpy singular-value SNR kernel +
  ...
  (NO model load) + ``~/.kadhi/spectrum/<slug>.json`` scan cache.
- ``kadhi spectrum scan`` — SNR table + ready-to-paste
...
class TestEstimateSigma:
    def test_iqr_over_1349(self):
        from kadhi_cli.utils.spectrum_scan import estimate_sigma
...
class TestMarchenkoPasturThreshold:
    def test_symmetric_in_n_m(self):
        from kadhi_cli.utils.spectrum_scan import marchenko_pastur_threshold
...
class TestComputeSnr:
    def test_signal_beats_pure_noise(self):
        from kadhi_cli.utils.spectrum_scan import compute_snr
...
class TestClassifyModule:
    ...
    def test_classification(self, name, expected):
        from kadhi_cli.utils.spectrum_scan import classify_module
...
class TestIterWeightMatrices:
    def test_yields_only_2d_weights(self, tmp_path):
        from kadhi_cli.utils.spectrum_scan import iter_weight_matrices
...
class TestSelectUnfrozenParameters:
    ...
    def test_top_50_percent_per_group(self):
        from kadhi_cli.utils.spectrum_scan import select_unfrozen_parameters
    ...
    def test_modules_filter(self):
        from kadhi_cli.utils.spectrum_scan import LayerSNR, select_unfrozen_parameters
...
class TestScanCache:
    def test_roundtrip(self, tmp_path):
        from kadhi_cli.utils.spectrum_scan import (
            ScanResult,
            read_cached_scan,
            scan_weights_dir,
            write_cached_scan,
    ...
    def test_missing_returns_none(self, tmp_path):
        from kadhi_cli.utils.spectrum_scan import read_cached_scan
...
class TestCacheDirOverride:
    def test_default_under_dot_kadhi(self):
        from kadhi_cli.utils.spectrum_scan import default_spectrum_cache_dir

        d = default_spectrum_cache_dir()
        assert d.endswith(str(Path(".kadhi") / "spectrum"))
    ...
    def test_env_override_under_cwd(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KADHI_SPECTRUM_CACHE_DIR", str(target))
        try:
    ...
    def test_env_override_control_char_rejected(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KADHI_SPECTRUM_CACHE_DIR", "bad\nvalue")
...
# ---------------------------------------------------------------------------
# Part B — `kadhi spectrum scan` CLI
# ---------------------------------------------------------------------------
class TestSpectrumScanCli:
    def test_help_lists_scan(self):
        from kadhi_cli.commands import spectrum as spectrum_cmd
...
class TestSpectrumScanRobustness:
    ...
    def test_symlinked_shard_skipped(self, tmp_path):
        from kadhi_cli.utils.spectrum_scan import _discover_safetensors
...
class TestParamPrefix:
    def test_strips_weight_suffix(self):
        from kadhi_cli.utils.spectrum_scan import param_prefix
