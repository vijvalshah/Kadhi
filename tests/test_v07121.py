...
class TestIsAttentionProjection:
    def test_canonical_projections(self):
        from kadhi_cli.utils.advanced_precision import is_attention_projection
...
class TestIsBlackwellGpu:
    def test_false_without_cuda(self, monkeypatch):
        ...
        from kadhi_cli.utils.advanced_precision import is_blackwell_gpu
...
def _enable_fp8_gates(monkeypatch):
    ...
    monkeypatch.setattr("kadhi_cli.utils.fp8.is_fp8_gpu_supported", lambda: True)
...
class TestApplyFp8Attention:
    ...
    def test_non_hopper_friendly_gate(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "kadhi_cli.utils.fp8.is_fp8_gpu_supported", lambda: False
        )
...
class TestApplyNvfp4:
    def test_none_model_rejected(self):
        from kadhi_cli.utils.advanced_precision import apply_nvfp4
    ...
    def test_non_blackwell_friendly_gate(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "kadhi_cli.utils.advanced_precision.is_blackwell_gpu", lambda: False
        )
    ...
    def test_missing_torchao_friendly(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "kadhi_cli.utils.advanced_precision.is_blackwell_gpu", lambda: True
        )
        monkeypatch.setitem(sys.modules, "torchao", None)
        with pytest.raises(RuntimeError, match="torchao"):
            apply_nvfp4(_tiny_attn_model())
...
class TestV028PrecisionWiring:
    ...
    def test_fp8_attention_gate_failure_degrades(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "kadhi_cli.utils.advanced_precision.apply_fp8_attention", _gate
        )
        result = apply_v028_speed_memory(
            model=object(),
    ...
    def test_fp8_attention_applied(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "kadhi_cli.utils.advanced_precision.apply_fp8_attention",
    ...
    def test_nvfp4_gate_failure_degrades(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "kadhi_cli.utils.advanced_precision.apply_nvfp4", _gate
        )
        result = apply_v028_speed_memory(
    ...
    def test_nvfp4_applied(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "kadhi_cli.utils.advanced_precision.apply_nvfp4", lambda model: 2,
        )
        result = apply_v028_speed_memory(
            model=object(), tcfg=self._tcfg(nvfp4=True), base_model="x/y",
        )
...
class TestParseVersionTuple:
    ...
    def test_matrix(self, raw, expected):
        from kadhi_cli.utils.grpo_long_context import _parse_version_tuple
...
class TestVllmSupportsSleepMode:
    def test_false_when_vllm_missing(self, monkeypatch):
        from kadhi_cli.utils.grpo_long_context import vllm_supports_sleep_mode
...
class TestApplyVllmSleepMode:
    def test_none_rejected(self):
        from kadhi_cli.utils.grpo_long_context import apply_vllm_sleep_mode
...
class TestVllmSleepCycle:
    def test_sleep_then_wake(self):
        from kadhi_cli.utils.grpo_long_context import vllm_sleep_cycle
    ...
    def test_engine_without_sleep_warns_not_crashes(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="kadhi_cli.utils.grpo_long_context"):
...
class TestMaybeEnableTrlSleepMode:
    def test_sets_kwarg_when_trl_exposes_it(self):
        from kadhi_cli.utils.grpo_long_context import maybe_enable_trl_sleep_mode
...
def _write_rollout_module(tmp_path, monkeypatch, body: str) -> str:
    ...
    name = f"kadhi_test_rollout_{next(_MODULE_COUNTER)}"
...
class TestResolveRolloutFunc:
    def test_resolves_callable(self, tmp_path, monkeypatch):
        from kadhi_cli.utils.agent_rollout import resolve_rollout_func
...
class TestRolloutResult:
    def test_frozen(self):
        ...
        from kadhi_cli.utils.agent_rollout import RolloutResult
    ...
    def test_row_cap(self):
        from kadhi_cli.utils.agent_rollout import (
            _MAX_ROLLOUT_ROWS,
            RolloutResult,
        )
...
class TestLaunchRolloutExternal:
    ...
    def test_missing_package_friendly(self, monkeypatch, name, pkg):
        from kadhi_cli.utils import agent_rollout
...
class TestRolloutLiveWiredFlags:
    def test_openenv_live_wired(self):
        from kadhi_cli.utils.agent_rollout import get_rollout_backend_spec
...
class TestLoraKeyMapping:
    def test_hf_to_mlx_strips_prefix_and_lowers(self):
        from kadhi_cli.utils.apple_adapter import hf_key_to_mlx
    ...
    def test_mlx_to_hf_round_trip(self):
        from kadhi_cli.utils.apple_adapter import hf_key_to_mlx, mlx_key_to_hf
    ...
    def test_mlx_non_lora_returns_none(self):
        from kadhi_cli.utils.apple_adapter import mlx_key_to_hf
...
class TestConvertArrays:
    ...
    def test_hf_to_mlx_transposes(self):
        from kadhi_cli.utils.apple_adapter import convert_hf_to_mlx_arrays
    ...
    def test_mlx_to_hf_round_trip_values(self):
        from kadhi_cli.utils.apple_adapter import (
            convert_hf_to_mlx_arrays,
...
class TestConvertAppleAdapterLive:
    ...
    def test_sign_writes_signature(self, tmp_path, monkeypatch):
        ...
        assert (tmp_path / "out" / ".kadhi-signature.json").is_file()
    ...
    def test_non_plan_rejected(self):
        from kadhi_cli.utils.apple_adapter import convert_apple_adapter
    ...
    def test_report_frozen(self):
        ...
        from kadhi_cli.utils.apple_adapter import ConversionReport
...
class TestIsExpertWeightKey:
    ...
    def test_fused_expert_keys(self, key):
        from kadhi_cli.utils.delinearize_llama4 import is_expert_weight_key
...
class TestDelinearizeTensor:
    def test_reshapes_2d_to_3d(self):
        torch = _torch_or_skip()
        from kadhi_cli.utils.delinearize_llama4 import delinearize_tensor
...
class TestReadNumExperts:
    def test_text_config_nested(self, tmp_path):
        from kadhi_cli.utils.delinearize_llama4 import read_num_experts
...
class TestRunDelinearize:
    def test_happy_path(self, tmp_path, monkeypatch):
        ...
        from kadhi_cli.utils.delinearize_llama4 import (
            plan_delinearize,
            run_delinearize,
        )
    ...
    def test_non_plan_rejected(self):
        from kadhi_cli.utils.delinearize_llama4 import run_delinearize
    ...
    def test_result_frozen(self):
        ...
        from kadhi_cli.utils.delinearize_llama4 import DelinearizeResult
...
class TestPatchInvariants:
    def test_version_bumped(self):
        ...
        parts = tuple(int(p) for p in kadhi_cli.__version__.split(".")[:3])
...
class TestReviewFollowupsPrecision:
    ...
    def test_v028_degrade_is_hermetic(self, monkeypatch):
        ...
        from kadhi_cli.config.loader import load_config_from_string
        from kadhi_cli.utils import advanced_precision, v028_features
...
class TestReviewFollowupsSleepMode:
    ...
    def test_sleep_without_wake_up_warns_and_runs(self, caplog):
        ...
        with caplog.at_level(
            logging.WARNING, logger="kadhi_cli.utils.grpo_long_context"
        ):
...
class TestReviewFollowupsRollout:
    ...
    def test_message_list_prompts_alias_broken(self):
        ...
        from kadhi_cli.utils.agent_rollout import _normalise_rollout_rows
...
class TestReviewFollowupsAppleAdapter:
    ...
    def test_infer_num_layers_gpt2_style(self):
        ...
        from kadhi_cli.utils.apple_adapter import _infer_num_layers
    ...
    def test_npz_decompression_cap(self, tmp_path, monkeypatch):
        ...
        from kadhi_cli.utils import apple_adapter
...
class TestReviewFollowupsDelinearize:
    ...
    def test_num_experts_exact_boundary(self, value, ok):
        from kadhi_cli.utils.delinearize_llama4 import _validate_num_experts
    ...
    def test_plan_direct_construction_outside_cwd_rejected(
        ...
        from kadhi_cli.utils.delinearize_llama4 import DelinearizePlan
    ...
    def test_weight_file_size_cap_branch(self, tmp_path, monkeypatch):
        pytest.importorskip("torch")
        from kadhi_cli.utils import delinearize_llama4
