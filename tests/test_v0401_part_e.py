...
Closes:
  - M3: `kadhi recipes show <typo>` suggests close matches via difflib
  - JSONL BOM auto-strip (Windows PowerShell users)
  - `kadhi data sample` filename includes strategy to prevent overwrite
"""
...
def test_suggest_recipes_returns_close_matches():
    from kadhi_cli.commands.recipes import _suggest_recipes
...
def test_jsonl_loader_strips_utf8_bom(tmp_path: Path):
    from kadhi_cli.data.loader import _load_jsonl
...
def test_data_sample_default_filename_includes_strategy():
    ...
    from kadhi_cli.commands.data import sample_data
