...
def test_package_docstring_has_no_mojibake():
    assert kadhi_cli.__doc__ is not None
    assert "вЂ" not in kadhi_cli.__doc__
    assert "—" in kadhi_cli.__doc__
...
def test_docs_commands_lists_every_registered_command():
    """`docs/commands.md` calls itself "the full kadhi command list". It was
    ...
    documented = set(re.findall(r"^kadhi ([a-z0-9-]+)", doc, re.M))
