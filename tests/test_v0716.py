...
Closes #231 (live `kadhi build` runner), #232 (live Magpie generator),
...
class TestMagpiePrefix:
    def test_default_is_chatml(self) -> None:
        from kadhi_cli.utils import magpie
...
class TestResolveTokenizer:
    def test_duck_typed_object_returned_as_is(self) -> None:
        from kadhi_cli.utils.diagnose import _common
...
class TestSplitPrefixTokenizer:
    def test_whitespace_default_unchanged(self) -> None:
        from kadhi_cli.utils.diagnose.memorization import split_prefix
...
class TestScoreMemorizationTokenizer:
    def test_exact_echo_subword_flags_memorization(self) -> None:
        from kadhi_cli.utils.diagnose.memorization import score_memorization
    ...
    def test_tokenizer_resolved_once(self, monkeypatch: pytest.MonkeyPatch) -> None:
        from kadhi_cli.utils.diagnose import _common, memorization
...
class TestItemParameters:
    def test_frozen(self) -> None:
        ...
        from kadhi_cli.utils import irt
...
class TestAugmentProviderFix:
    def test_ollama_provider_constructs(self) -> None:
        ...
        from kadhi_cli.commands.data import _load_augment_provider
    ...
    def test_generate_non_string_safe(self) -> None:
        from kadhi_cli.commands.data import _AugmentProvider
...
class TestMagpieReviewFixes:
    ...
    def test_quality_fn_crash_warns_and_keeps(
        ...
        # The kadhi.magpie logger has propagate=False under the project logging
        ...
        logger = logging.getLogger("kadhi.magpie")
        handler = _Capture()
...
class TestPatchInvariants:
    ...
    def test_no_heavy_top_level_imports(self) -> None:
        ...
        root = pathlib.Path(__file__).resolve().parent.parent / "src" / "kadhi_cli"
        for rel in (
            "utils/build_dag.py",
            "utils/magpie.py",
            "utils/irt.py",
