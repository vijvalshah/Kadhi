"""Tests for kadhi runs CLI commands."""
...
def _use_temp_db(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setenv("KADHI_DB_PATH", db_path)
...
def tracker(tmp_path: Path) -> ExperimentTracker:
    ...
    db_path = Path(os.environ["KADHI_DB_PATH"])
    return ExperimentTracker(db_path=db_path)


def test_runs_empty():
    """kadhi runs with no runs should show a message, not crash."""
...
def test_runs_list_with_data(tracker):
    """kadhi runs should show a table when runs exist."""
...
def test_runs_show_not_found():
    """kadhi runs show nonexistent should fail."""
...
def test_runs_show_with_data(tracker):
    """kadhi runs show should display run details."""
    run_id = tracker.start_run(
        config_dict={"base": "test-model", "task": "sft"},
        device="cpu",
        device_name="CPU",
...
def test_runs_show_a_terminated_run_reads_terminated_not_running(tracker):
    ...
    rewritten to 'terminated' still displayed as 'running' in `kadhi runs
...
def test_runs_compare_not_found():
    """kadhi runs compare with bad IDs should fail."""
    result = runner.invoke(app, ["runs", "compare", "run_1", "run_2"])
    assert result.exit_code == 1


def test_runs_compare_with_data(tracker):
    """kadhi runs compare should show side-by-side table."""
