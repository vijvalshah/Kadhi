"""Tests for kadhi deploy ollama command and Ollama utilities."""
...
from kadhi_cli.utils.ollama import (
...
# Patch targets — lazy imports in deploy.py resolve to kadhi_cli.utils.ollama
_OLLAMA = "kadhi_cli.utils.ollama"
...
def test_validate_model_name_with_colon():
    valid, err = validate_model_name("kadhi-model:latest")
    assert valid is True
...
@patch(f"{_OLLAMA}.subprocess.run")
def test_list_kadhi_models_found(mock_run):
    mock_run.return_value = MagicMock(
        returncode=0,
        stdout=(
            "NAME                    ID              SIZE      MODIFIED\n"
            "kadhi-my-model:latest    abc123def456    4.1 GB    2 hours ago\n"
            "llama3.1:latest         xyz789abc123    8.0 GB    3 days ago\n"
            "kadhi-code:latest        def456ghi789    3.2 GB    1 day ago\n"
        ),
    )
    models = list_kadhi_models()
    assert len(models) == 2
    assert models[0]["name"] == "kadhi-my-model:latest"
    assert models[1]["name"] == "kadhi-code:latest"


@patch(f"{_OLLAMA}.subprocess.run")
def test_list_kadhi_models_empty(mock_run):
    mock_run.return_value = MagicMock(
        returncode=0,
        stdout="NAME    ID    SIZE    MODIFIED\n",
    )
    assert list_kadhi_models() == []


@patch(f"{_OLLAMA}.subprocess.run")
def test_list_kadhi_models_ollama_not_found(mock_run):
    mock_run.side_effect = FileNotFoundError
    assert list_kadhi_models() == []


@patch(f"{_OLLAMA}.subprocess.run")
def test_list_kadhi_models_timeout(mock_run):
    mock_run.side_effect = subprocess.TimeoutExpired(cmd="ollama", timeout=10)
    assert list_kadhi_models() == []


@patch(f"{_OLLAMA}.subprocess.run")
def test_list_kadhi_models_nonzero(mock_run):
...
def test_remove_model_success(mock_run):
    mock_run.return_value = MagicMock(returncode=0, stdout="deleted 'kadhi-test'")
    success, msg = remove_model("kadhi-test")
    assert success is True
...
def test_kadhi_model_prefix():
    assert KADHI_MODEL_PREFIX == "kadhi-"
...
# ─── CLI: kadhi deploy ollama --list ───


@patch(f"{_OLLAMA}.detect_ollama", return_value="0.6.2")
@patch(f"{_OLLAMA}.list_kadhi_models", return_value=[])
def test_deploy_list_empty(mock_list, mock_detect):
    result = runner.invoke(app, ["deploy", "ollama", "--list"])
    assert result.exit_code == 0
    assert "no kadhi-deployed" in result.output.lower()


@patch(f"{_OLLAMA}.detect_ollama", return_value="0.6.2")
@patch(
    f"{_OLLAMA}.list_kadhi_models",
    return_value=[{"name": "kadhi-test:latest", "size": "4.1 GB"}],
)
def test_deploy_list_with_models(mock_list, mock_detect):
    result = runner.invoke(app, ["deploy", "ollama", "--list"])
    assert result.exit_code == 0
    assert "kadhi-test" in result.output
...
def test_deploy_remove_failure(mock_rm, mock_detect):
    result = runner.invoke(app, ["deploy", "ollama", "--remove", "kadhi-bad", "--yes"])
    assert result.exit_code == 1
    assert "not found" in result.output.lower()
...
# ─── CLI: kadhi deploy ollama (deploy mode) ───
...
def test_deploy_gguf_not_found(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    result = runner.invoke(
        app, ["deploy", "ollama", "--model", "missing.gguf", "--name", "kadhi-test"]
    )
    assert result.exit_code == 1
    assert "not found" in result.output.lower()
...
def test_deploy_ollama_not_installed(mock_detect, tmp_path, monkeypatch):
    ...
    result = runner.invoke(
        app,
        ["deploy", "ollama", "--model", str(gguf), "--name", "kadhi-test"],
    )
    assert result.exit_code == 1
    assert "not found" in result.output.lower()
...
def test_auto_detect_from_kadhi_yaml(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    (tmp_path / "kadhi.yaml").write_text(
        "base: test\ndata:\n  train: data.jsonl\n  format: chatml\n",
        encoding="utf-8",
    )
    from kadhi_cli.commands.deploy import _auto_detect_template

    result = _auto_detect_template()
    assert result == "chatml"


def test_auto_detect_no_kadhi_yaml(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    from kadhi_cli.commands.deploy import _auto_detect_template
...
def test_auto_detect_invalid_yaml(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    (tmp_path / "kadhi.yaml").write_text(":::invalid", encoding="utf-8")
    from kadhi_cli.commands.deploy import _auto_detect_template
...
def test_export_deploy_unsupported_target():
    ...
    from kadhi_cli.commands.export import _auto_deploy_ollama
...
def test_export_deploy_ollama_success(mock_detect_fn, mock_deploy_fn):
    ...
    _auto_deploy_ollama(Path("model.gguf"), "mymodel", "ollama", "kadhi-mymodel")
...
def test_export_deploy_ollama_create_fails(mock_detect_fn, mock_deploy_fn):
    ...
    with pytest.raises((SystemExit, click.exceptions.Exit)):
        _auto_deploy_ollama(Path("m.gguf"), "model", "ollama", "kadhi-model")
