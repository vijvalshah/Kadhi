...
class TestBomSpec:
    def test_imports(self):
        from kadhi_cli.utils.bom import (
            BomEntry,
            build_cyclonedx_bom,
            build_spdx_bom,
            render_bom,
    ...
    def test_bom_entry_frozen(self):
        from kadhi_cli.utils.bom import BomEntry
    ...
    def test_cyclonedx_shape(self):
        from kadhi_cli.utils.bom import BomEntry, build_cyclonedx_bom
    ...
    def test_spdx_shape(self):
        from kadhi_cli.utils.bom import BomEntry, build_spdx_bom
    ...
    def test_render_bom_format_dispatch(self):
        from kadhi_cli.utils.bom import BomEntry, render_bom
    ...
    def test_write_bom_atomic_and_containment(self, tmp_path, monkeypatch):
        from kadhi_cli.utils.bom import BomEntry, write_bom
...
class TestAttestation:
    ...
    def test_attestation_statement_frozen(self):
        from kadhi_cli.utils.attest import AttestationStatement

        s = AttestationStatement(
            stage="train",
            subject_name="adapter-v1",
            subject_sha256="a" * 64,
            builder_id="kadhi-cli@0.59.0",
            invocation={"command": "kadhi train"},
    ...
    def test_slsa_provenance_shape(self):
        ...
        assert prov["runDetails"]["builder"]["id"] == "kadhi-cli@0.59.0"
    ...
    def test_sign_attestation_stub_signature(self):
        ...
        from kadhi_cli.utils.attest import SignatureBackend, sign_attestation
    ...
    def test_sign_attestation_rejects_unknown_backend(self):
        from kadhi_cli.utils.attest import sign_attestation
...
class TestAnnexXI:
    ...
    def test_annex_xi_data_frozen(self):
        from kadhi_cli.utils.annex_xi import AnnexXIData
    ...
    def test_top_domains_capped_at_10(self):
        ...
        from kadhi_cli.utils.annex_xi import render_annex_xi_markdown
        md = render_annex_xi_markdown(d)
        # We slice to top-10
        for i in range(10):
    ...
    def test_negative_flops_rejected(self):
        ...
        with pytest.raises(ValueError):
            AnnexXIData(
                ...
                train_co2_kg=0.0, top_domains=(), kadhi_version="0.59.0",
    ...
    def test_write_annex_doc_atomic(self, tmp_path, monkeypatch):
        from kadhi_cli.utils.annex_xi import AnnexXIData, write_annex_doc
...
class TestAuditLog:
    def test_imports(self):
        from kadhi_cli.utils.audit_log import (
            AuditEvent,
            append_audit_event,
            redact_event,
            rotate_if_needed,
    ...
    def test_audit_event_frozen_and_validated(self):
        from kadhi_cli.utils.audit_log import AuditEvent

        ev = AuditEvent(
            timestamp="2026-05-18T12:00:00+00:00",
            command="train",
            args=("--config", "kadhi.yaml"),
            exit_code=0,
            host_id="laptop-01",
            operator_id="alpamys",
        )
    ...
    def test_redact_event_strips_secrets(self):
        from kadhi_cli.utils.audit_log import AuditEvent, redact_event
    ...
    def test_rotate_if_needed_renames_at_cap(self, tmp_path):
        from kadhi_cli.utils.audit_log import rotate_if_needed
...
class TestReproReceipt:
    def test_imports(self):
        from kadhi_cli.utils.repro_receipt import (
            ReproReceipt,
            build_repro_receipt,
            write_repro_receipt,
        )
    ...
    def test_receipt_captures_basic_env(self):
        from kadhi_cli.utils.repro_receipt import build_repro_receipt
    ...
    def test_mps_receipt_captures_chip_and_unified_memory(self, monkeypatch):
        from kadhi_cli.utils import repro_receipt
    ...
    def test_new_accelerator_fields_preserve_old_constructor_signature(self):
        from kadhi_cli.utils.repro_receipt import ReproReceipt
...
class TestEnergy:
    def test_imports(self):
        from kadhi_cli.utils.energy import (
            EnergyMeasurement,
            adjust_for_pue,
            measure_run_energy,
    ...
    def test_measure_returns_zero_when_codecarbon_missing(self):
        from kadhi_cli.utils.energy import measure_run_energy
    ...
    def test_validate_electricity_map_endpoint_loopback_ok(self):
        from kadhi_cli.utils.energy import validate_electricity_map_endpoint
    ...
    def test_adjust_for_pue(self):
        from kadhi_cli.utils.energy import adjust_for_pue
...
class TestBomEnergyAttach:
    def test_bom_with_energy_metadata(self):
        from kadhi_cli.utils.bom import BomEntry, attach_energy, build_cyclonedx_bom
        from kadhi_cli.utils.energy import EnergyMeasurement
        ...
        assert "kadhi:energy_kwh" in names
        assert "kadhi:co2_kg" in names
...
class TestSourceWiring:
    def test_cli_registers_bom(self):
        cli_path = Path(kadhi_cli.__file__).parent / "cli.py"
    ...
    def test_version_is_at_least_0_59(self):
        ...
        """
        major, minor, _patch = kadhi_cli.__version__.split(".")
        assert int(major) == 0 and int(minor) >= 59, kadhi_cli.__version__

    def test_no_top_level_heavy_imports(self):
        """v0.59 modules must not import torch/transformers at module top."""
        utils = Path(kadhi_cli.__file__).parent / "utils"
...
class TestAuditCli:
    ...
    def test_audit_log_list_empty(self, tmp_path, monkeypatch):
        ...
        monkeypatch.chdir(tmp_path)
        monkeypatch.setenv("KADHI_AUDIT_LOG_PATH", str(tmp_path / "audit.jsonl"))
        runner = CliRunner()
...
class TestReviewFollowups:
    ...
    def test_default_log_path_rejects_unsafe_env_override(self, monkeypatch, tmp_path):
        from kadhi_cli.utils.audit_log import default_log_path

        # /etc/cron.d is outside $HOME / $CWD / $TMPDIR — fall back to default.
        monkeypatch.setenv("KADHI_AUDIT_LOG_PATH", "/etc/cron.d/x")
        monkeypatch.chdir(tmp_path)
        resolved = default_log_path()
        # The default is ~/.kadhi/audit.jsonl — confirm we didn't honour the override.
    ...
    def test_default_log_path_honours_in_bounds_env(self, monkeypatch, tmp_path):
        ...
        monkeypatch.chdir(tmp_path)
        monkeypatch.setenv("KADHI_AUDIT_LOG_PATH", str(target))
        resolved = default_log_path()
    ...
    def test_default_log_path_rejects_null_byte_override(self):
        ...
        from kadhi_cli.utils.audit_log import _validate_log_path_override
    ...
    def test_default_log_path_handles_env_read_value_error(self, monkeypatch, tmp_path):
        ...
        from kadhi_cli.utils import audit_log

        monkeypatch.chdir(tmp_path)

        def boom(key, default=None):
            if key == "KADHI_AUDIT_LOG_PATH":
    ...
    def test_annex_xi_escapes_markdown_active_chars(self):
        from kadhi_cli.utils.annex_xi import AnnexXIData, render_annex_xi_markdown
    ...
    def test_write_attestation_rejects_symlink_target(self, tmp_path, monkeypatch):
        from kadhi_cli.utils.attest import AttestationStatement, write_attestation
    ...
    def test_read_audit_tail_bool_limit_rejected(self):
        from kadhi_cli.utils.audit_log import read_audit_tail
...
class TestBomEnergyCli:
    def test_emit_energy_happy_path(self, tmp_path, monkeypatch):
        ...
        assert "kadhi:energy_kwh" in written
