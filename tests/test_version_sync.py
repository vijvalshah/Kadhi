...
`pyproject.toml` and `src/kadhi_cli/__init__.py` both carry the version, and the
...
disagrees with `kadhi version`, and CI stays green. Until now that drift was
...
INIT = ROOT / "src" / "kadhi_cli" / "__init__.py"
...
def _init_version(text: str) -> str:
    ...
    assert match, "no __version__ line in src/kadhi_cli/__init__.py"
    return match.group(1)
...
class TestVersionSync:
    def test_the_two_files_agree(self):
        ...
        assert declared == package, (
            f"pyproject.toml says {declared!r} but src/kadhi_cli/__init__.py says "
            f"{package!r} — the release checklist requires BOTH to be bumped. "
            f"The wheel would ship metadata that disagrees with `kadhi version`."
        )
