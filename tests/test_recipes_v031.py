...
from kadhi_cli.recipes.catalog import RECIPES, get_recipe, search_recipes
...
class TestPartAVision:
    ...
    @pytest.mark.parametrize("name,task,model", PART_A_VISION)
    def test_recipe_loads_as_kadhiconfig(self, name: str, task: str, model: str) -> None:
        recipe = get_recipe(name)
...
class TestRecipeCatalog80:
    ...
    def test_every_recipe_loads_as_kadhiconfig(self, name: str) -> None:
        recipe = RECIPES[name]
        cfg = load_config_from_string(recipe.yaml_str)
...
# ---------------------------------------------------------------------------
# CLI smoke (v0.31.0 recipes are reachable via `kadhi recipes`)
# ---------------------------------------------------------------------------

class TestRecipesCLIv031:
    """v0.31.0 recipes are reachable through `kadhi recipes show`."""
    ...
    def test_show_recipe(self, name: str, _task: str, _model: str) -> None:
        ...
        assert result.exit_code == 0, (
            f"`kadhi recipes show {name}` failed: {result.output!r}"
        )
        # The model id appears verbatim in the YAML output.
        assert _model in result.output

    def test_list_runs_clean(self) -> None:
        """`kadhi recipes list` exits 0 (Rich table rendering is terminal-width-dependent)."""
    ...
    def test_search_finds_v031_recipes(self, query: str, expected: str) -> None:
        """`kadhi recipes search <kw>` surfaces v0.31.0 recipes by keyword."""
