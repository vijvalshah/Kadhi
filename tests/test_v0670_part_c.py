...
Tests for ``kadhi_cli/utils/mole_routing.py``:
...
- New ``task='moe_lora_routing'`` Literal on KadhiConfig
- KadhiConfig cross-validator gates mole config to the correct task/backend
"""
...
class TestPublicSurface:
    def test_module_importable(self) -> None:
        from kadhi_cli.utils import mole_routing
...
class TestMoleGatingConfig:
    def test_construct(self) -> None:
        from kadhi_cli.utils.mole_routing import MoleGatingConfig
...
class TestValidateMoleCompat:
    def test_happy_path(self) -> None:
        from kadhi_cli.utils.mole_routing import validate_mole_compat
...
# -----------------------------------------------------------------------------
# KadhiConfig integration — task='moe_lora_routing' Literal
# -----------------------------------------------------------------------------
...
class TestSourceWiring:
    def test_no_top_level_heavy_imports(self) -> None:
        ...
        src = (root / "src" / "kadhi_cli" / "utils" / "mole_routing.py").read_text(
            encoding="utf-8"
        )
        head_lines = [
            line
