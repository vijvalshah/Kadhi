"""Tests for kadhi ui — Web UI command and API endpoints."""
...
class TestUICommand:
    """Test the kadhi ui CLI command."""

    def test_ui_command_registered(self):
        """kadhi ui should be a registered command."""
    ...
    def test_ui_command_fastapi_import_error(self):
        """kadhi ui should fail gracefully if FastAPI not installed."""
        from typer.testing import CliRunner
    ...
    def test_ui_command_options(self):
        """kadhi ui should accept --port, --host, --no-browser options."""
        from typer.testing import CliRunner
...
class TestDataInspect:
    ...
    def test_inspect_jsonl(self, tmp_path):
        ...
        client = TestClient(create_app())
        with patch("kadhi_cli.ui.app.Path.cwd", return_value=tmp_path):
            response = client.post(
                "/api/data/inspect",
...
class TestTrainEndpoints:
    ...
    def test_start_training(self, tmp_path):
        ...
        with patch("kadhi_cli.ui.app.subprocess.Popen", return_value=mock_popen):
            client = TestClient(create_app())
            response = client.post(
                "/api/train/start",
                json={
...
class TestStaticFiles:
    ...
    def test_static_dir_exists(self):
        ...
        from kadhi_cli.ui.app import STATIC_DIR
