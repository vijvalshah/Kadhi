...
from kadhi_cli.data.traces.pair_builder import PreferencePair
from kadhi_cli.data.traces.quality import (
    DEFAULT_MIN_CONFIDENCE,
    JudgeFilterReport,
    judge_filter_pairs,
)
from kadhi_cli.monitoring.trace_logger import (
    DEFAULT_CAP_MB,
    TraceLogWriter,
)
...
class TestFromTracesJudgeCli:
    ...
    def test_judge_flag_filters_pairs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("kadhi_cli.eval.judge.JudgeEvaluator", FakeJudgeCls)
    ...
    def test_judge_help_text(self):
        runner = CliRunner()
        result = runner.invoke(kadhi_app, ["data", "from-traces", "--help"])
...
class TestServeTraceLogWiring:
    def test_serve_help_lists_trace_log(self):
        runner = CliRunner()
        result = runner.invoke(kadhi_app, ["serve", "--help"])
        assert result.exit_code == 0
