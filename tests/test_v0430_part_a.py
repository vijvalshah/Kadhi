...
class TestIsTelemetryEnabled:
    ...
    def test_explicit_on(self, val):
        assert is_telemetry_enabled({"KADHI_TELEMETRY": val}) is True

    @pytest.mark.parametrize("val", ["0", "false", "no", "off", "", "garbage"])
    def test_explicit_off_or_garbage(self, val):
        assert is_telemetry_enabled({"KADHI_TELEMETRY": val}) is False


class TestBuildTelemetryPayload:
    def test_required_fields(self):
        payload = build_telemetry_payload(
            kadhi_version="0.43.0", command="train"
        )
        assert payload["kadhi_version"] == "0.43.0"
    ...
    def test_with_duration(self):
        payload = build_telemetry_payload(
            kadhi_version="0.43.0", command="train", duration_seconds=12.5
        )
        assert payload["duration_seconds"] == 12.5

    def test_int_duration_coerced_to_float(self):
        payload = build_telemetry_payload(
            kadhi_version="0.43.0", command="train", duration_seconds=10
        )
    ...
    def test_invalid_version(self):
        with pytest.raises(ValueError):
            build_telemetry_payload(kadhi_version="", command="train")
        with pytest.raises(ValueError):
            build_telemetry_payload(kadhi_version=None, command="train")  # type: ignore[arg-type]

    def test_null_byte_version(self):
        with pytest.raises(ValueError, match="null"):
            build_telemetry_payload(kadhi_version="1.0\x00", command="train")

    def test_invalid_command(self):
        with pytest.raises(ValueError):
            build_telemetry_payload(kadhi_version="0.43.0", command="")

    def test_null_byte_command(self):
        with pytest.raises(ValueError, match="null"):
            build_telemetry_payload(kadhi_version="0.43.0", command="train\x00")

    def test_bool_duration_rejected(self):
        with pytest.raises(ValueError):
            build_telemetry_payload(
                kadhi_version="0.43.0", command="train", duration_seconds=True  # type: ignore[arg-type]
            )

    def test_nonfinite_duration_rejected(self):
        with pytest.raises(ValueError, match="finite"):
            build_telemetry_payload(
                kadhi_version="0.43.0", command="train", duration_seconds=float("inf")
            )
        with pytest.raises(ValueError, match="finite"):
            build_telemetry_payload(
                kadhi_version="0.43.0",
                command="train",
    ...
    def test_negative_duration_rejected(self):
        with pytest.raises(ValueError, match=">= 0"):
            build_telemetry_payload(
                kadhi_version="0.43.0", command="train", duration_seconds=-1
            )
...
class TestRegistryImmutability:
    def test_report_to_backends_immutable(self):
        from kadhi_cli.utils.trackers import _REPORT_TO_BACKENDS
