...
- Cross-validator: KadhiConfig restricts ``multipack`` to sft / pretrain
...
from kadhi_cli.utils.multipack import (
...
# ---- KadhiConfig task gate ------------------------------------------------


def _base_kadhi_kwargs(task: str = "sft", **overrides):
    cfg = {
...
def test_multipack_allowed_for_sft():
    cfg = KadhiConfig(**_base_kadhi_kwargs(task="sft"))
    assert cfg.training.multipack is True


def test_multipack_allowed_for_pretrain():
    kwargs = _base_kadhi_kwargs(task="pretrain")
    kwargs["data"]["format"] = "plaintext"
    cfg = KadhiConfig(**kwargs)
    assert cfg.training.multipack is True
...
def test_multipack_rejected_for_non_sft_pretrain(task):
    kwargs = _base_kadhi_kwargs(task=task)
...
def test_multipack_off_does_not_trip_task_gate():
    ...
    kwargs = _base_kadhi_kwargs(task="dpo")
...
def test_multipack_rejected_on_mlx_backend():
    kwargs = _base_kadhi_kwargs(task="sft")
    kwargs["backend"] = "mlx"
