...
- Part A: ``kadhi ship --emit-evidence`` — a verdict re-serialised into the
  ``--evidence`` INPUT schema, so ``--emit-evidence -> --evidence`` round-trips
  to an identical verdict (#312). Output was not input before this.
- Part B: ``ShipConfig`` under ``EvalConfig`` + ``kadhi ship --config kadhi.yaml``
  reading leg-1/leg-2 defaults (CLI flag > config > hard default).
- Part C: ``kadhi ship --push owner/repo#N`` — the verdict as a PR comment
...
# ---------------------------------------------------------------------------
# Part B — ShipConfig schema + kadhi ship --config reader
# ---------------------------------------------------------------------------
...
class TestShipConfigSchema:
    def test_defaults(self):
        from kadhi_cli.config.schema import ShipConfig
...
class TestShipConfigReader:
    ...
    def test_config_threshold_flips_offline_verdict(self):
        ...
        with runner.isolated_filesystem():
            ...
            Path("kadhi.yaml").write_text(_CONFIG_LENIENT, encoding="utf-8")
    ...
    def test_config_fills_every_live_leg_flag(self, monkeypatch):
        ...
        with runner.isolated_filesystem():
            ...
            res = runner.invoke(
                ship_cmd.app, ["--base", "m", "--adapter", "a", "--config", "kadhi.yaml"]
            )
...
# ---------------------------------------------------------------------------
# Part C — render_ship_pr_markdown + kadhi ship --push
# ---------------------------------------------------------------------------
...
class TestRenderShipPrMarkdown:
    def test_heading_names_decision(self):
        ...
        assert "SHIP" in md
        assert md.lstrip().startswith("## kadhi ship")
...
class TestShipPushCli:
    def test_push_posts_comment_and_preserves_exit_code(self, monkeypatch):
        ...
        with runner.isolated_filesystem():
            ...
            assert "kadhi ship" in calls["body"]
...
class TestComputeProvenance:
    def test_provenance_shape(self):
        from kadhi_cli.commands.ship import _compute_provenance
        from kadhi_cli.config.loader import load_config_from_string
...
class TestStalenessGate:
    ...
    def test_producer_stamps_raw_evidence_without_gating(self):
        ...
        with runner.isolated_filesystem():
            ...
            res = runner.invoke(
                ship_cmd.app,
                ["--evidence", "raw.json", "--config", "kadhi.yaml",
            ...
            gate = runner.invoke(
                ship_cmd.app, ["--evidence", "out.json", "--config", "kadhi.yaml"]
            )
...
class TestCiInitV2:
    def test_render_without_config_has_no_config_flag(self):
        ...
        yaml_text = render_kadhi_gate_workflow(
            data_path="data/train.jsonl",
            suite_path="expectations.yaml",
            evidence_path="ship_evidence.json",
        )
        assert "kadhi ship --evidence" in yaml_text
    ...
    def test_render_with_config_binds_the_gate(self):
        ...
        yaml_text = render_kadhi_gate_workflow(
            ...
            suite_path="expectations.yaml",
            evidence_path="ship_evidence.json",
            config_path="kadhi.yaml",
        )
        run = yaml.safe_load(yaml_text)["jobs"]["kadhi-gate"]["steps"][-1]["run"]
        assert "--config" in run and "kadhi.yaml" in run

    def test_ci_init_with_config_writes_bound_workflow(self):
        import yaml

        from kadhi_cli.cli import app as main_app

        with runner.isolated_filesystem():
            Path("kadhi.yaml").write_text(_CONFIG_MIN, encoding="utf-8")
            res = runner.invoke(main_app, ["ci", "init", "--config", "kadhi.yaml"])
            assert res.exit_code == 0, (res.output, repr(res.exception))
            wf = Path(".github/workflows/kadhi-gate.yml").read_text(encoding="utf-8")
            run = yaml.safe_load(wf)["jobs"]["kadhi-gate"]["steps"][-1]["run"]
            assert "--config" in run and "kadhi.yaml" in run
...
class TestSecurityHardening:
    ...
    def test_safe_read_text_enforces_byte_cap(self):
        from kadhi_cli.commands.ship import _safe_read_text
    ...
    def test_safe_hash_file_skips_oversize(self):
        from kadhi_cli.commands.ship import _safe_hash_file
    ...
    def test_bound_workflow_yaml_parses(self):
        ...
        text = render_kadhi_gate_workflow(
            data_path="data/train.jsonl",
            suite_path="expectations.yaml",
            evidence_path="ship_evidence.json",
            config_path="kadhi.yaml",
        ...
        run = doc["jobs"]["kadhi-gate"]["steps"][-1]["run"]
        assert "--config" in run and "kadhi.yaml" in run
