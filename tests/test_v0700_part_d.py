...
punts this; Kadhi ships the schema + state-manifest builder here, with
...
class TestRLCheckpointPublicSurface:
    def test_module_imports(self):
        from kadhi_cli.utils import rl_checkpoint
...
class TestValidateSaveEverySteps:
    def test_happy(self):
        from kadhi_cli.utils.rl_checkpoint import validate_save_every_steps
...
class TestRLCheckpointConfig:
    def test_defaults(self):
        from kadhi_cli.utils.rl_checkpoint import RLCheckpointConfig
...
class TestRLCheckpointState:
    """State manifest written to .kadhi-rl-ckpt/step-NNN/manifest.json.

    Frozen + JSON-serialisable + per-field validation.
    """

    def test_basic(self):
        from kadhi_cli.utils.rl_checkpoint import RLCheckpointState

        st = RLCheckpointState(
            step=500,
            checkpoint_dir="./.kadhi-rl-ckpt/step-500",
            task="grpo",
            has_optimizer=True,
            has_ref_model=False,
            has_rollout_buffer=False,
            kadhi_version="0.70.0",
        )
        assert st.step == 500
        assert st.task == "grpo"
