...
class TestNF4Sharding:
    ...
    def test_rebuilt_state_matches_the_one_bitsandbytes_built(self, tmp_path, double_quant):
        ...
        from kadhi_cli.utils.layer_stream_runtime import rebuild_quant_state
    ...
    def test_code_table_divergence_is_refused(self, tmp_path):
        ...
        from kadhi_cli.utils.layer_shard import _CodeTables
...
class TestQuantCacheInvalidation:
    ...
    def test_nf4_cache_is_not_reused_for_a_bf16_request(self, tmp_path):
        from kadhi_cli.utils.layer_shard import QUANT_NF4, QUANT_NONE, shard_checkpoint
    ...
    def test_double_quant_change_invalidates(self, tmp_path):
        from kadhi_cli.utils.layer_shard import QUANT_NF4, shard_checkpoint
...
class TestHostileIndexIsRefused:
    ...
    def _spec(self, **over):
        from kadhi_cli.utils.layer_shard import NF4WeightSpec
    ...
    def test_shape_claiming_more_than_the_packed_bytes_is_refused(self, tmp_path):
        ...
        from kadhi_cli.utils.layer_shard import QUANT_NF4, read_shard_index, shard_checkpoint
    ...
    def test_an_honest_shard_passes_the_consistency_check(self, tmp_path):
        ...
        from kadhi_cli.utils.layer_shard import ABSMAX_SUFFIX, NF4WeightSpec
        from kadhi_cli.utils.layer_stream_runtime import validate_quant_shape
    ...
    def test_quant_specs_must_agree_with_the_quant_field(self, tmp_path):
        ...
        non-4bit skeleton."""
        from kadhi_cli.utils.layer_shard import QUANT_NONE, NF4WeightSpec, ShardIndex
...
class TestPeftDispatchesTheBnbLoraPath:
    ...
    def test_stream_setup_threads_double_quant_into_sharder_and_runtime(
        ...
        monkeypatch.setattr(
            "kadhi_cli.utils.layer_stream.free_ram_bytes", lambda: 10_000_000_000
        )
        monkeypatch.setattr(
            "kadhi_cli.utils.layer_stream.detect_disk_kind", lambda *_a, **_k: "nvme"
        )

        captured = {}
        import kadhi_cli.utils.layer_shard as shard_mod
        import kadhi_cli.utils.layer_stream_runtime as rt_mod
...
class TestNF4SpecFromShard:
    ...
    def test_zero_dim_offset_keeps_its_shape(self, tmp_path):
        from kadhi_cli.utils.layer_shard import NESTED_OFFSET_SUFFIX
        from kadhi_cli.utils.layer_stream_runtime import RamSource
...
class TestNF4StreamedModel:
    ...
    def test_pool_streams_the_quant_sidecars(self, tmp_path):
        from kadhi_cli.utils.layer_shard import ABSMAX_SUFFIX
    ...
    def test_rebuilt_view_shares_storage_with_the_pooled_buffer(self, tmp_path):
        ...
        from kadhi_cli.utils.layer_stream_runtime import decoder_owner, rebuild_params4bit
...
class TestNF4StoreEstimate:
    def test_plain_dtype_is_unscaled(self):
        from kadhi_cli.utils.layer_stream import estimate_stream_store_bytes
...
class TestPreflightUsesTheStreamedSize:
    ...
    def _run(
        ...
        monkeypatch.setattr(
            "kadhi_cli.utils.layer_shard.source_weight_bytes", lambda *_a, **_k: on_disk
        )
        ...
        monkeypatch.setattr(
            "kadhi_cli.utils.layer_stream.detect_disk_kind", lambda *_a, **_k: disk_kind
        )
        cfg = load_config_from_string(
            f"""
base: {weights}
...
class TestDeviceMapValue:
    ...
    def test_indexed_devices_become_ints(self, device, expected):
        from kadhi_cli.utils.layer_stream_runtime import _device_map_value
...
class TestTrainerWiring:
    def test_streaming_setup_threads_nf4_end_to_end(self, tmp_path, monkeypatch):
        ...
        the model the way ``kadhi train`` actually builds it."""
        import torch
