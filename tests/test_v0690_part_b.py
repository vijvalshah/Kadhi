...
from kadhi_cli.utils import expectations
...
class TestSourceWiring:
    def test_cli_registers_expect(self) -> None:
        ...
        assert 'name="expect"' in cli or "from kadhi_cli.commands import expect" in cli

    def test_no_heavy_top_level_imports(self) -> None:
        root = Path(__file__).resolve().parent.parent
        src = (root / "src" / "kadhi_cli" / "utils" / "expectations.py").read_text(encoding="utf-8")
        for forbidden in (
            "\nimport torch",
            "\nimport transformers",
            "\nimport peft",
