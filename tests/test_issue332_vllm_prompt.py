...
class TestBothBackendsShareOneBuilder:
    ...
    def test_transformers_backend_calls_the_shared_builder(self):
        ...
        src = Path("src/kadhi_cli/commands/serve.py").read_text(encoding="utf-8")
    ...
    def test_no_second_hand_rolled_prompt_remains_in_the_serve_backends(self):
        ...
        serve_src = Path("src/kadhi_cli/commands/serve.py").read_text(encoding="utf-8")
        vllm_src = Path("src/kadhi_cli/utils/vllm.py").read_text(encoding="utf-8")
        ...
        sglang_src = Path("src/kadhi_cli/utils/sglang.py").read_text(encoding="utf-8")
    ...
    def test_sglang_does_not_redefine_the_finish_reason_vocabulary(self):
        ...
        assert "from kadhi_cli.utils.vllm import _FINISH_REASONS" in sglang_src
...
class TestResolveFinishReason:
    def test_engine_reported_length_is_reported_as_length(self):
        from kadhi_cli.utils.vllm import resolve_finish_reason
...
class TestDashboardBackendWarning:
    ...
    def test_no_warning_for_backends_that_serve_metrics(self):
        from kadhi_cli.commands.serve import _dashboard_warning
...
class TestMaxModelLenFlag:
    ...
    def test_serve_vllm_forwards_max_model_len(self, tmp_path):
        ...
        with patch(
            ...
            "kadhi_cli.utils.vllm.create_vllm_app", return_value=MagicMock()
        ):
