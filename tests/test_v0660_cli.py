...
Verify the `kadhi probe` group is registered + all four sub-commands work
end-to-end, plus the `kadhi adapters blame --live` runner is wired.
"""
...
# ---------------------------------------------------------------------------
# kadhi probe --help / pack / sleeper / interference / sae-diff
# ---------------------------------------------------------------------------


def test_probe_group_registered():
    result = runner.invoke(kadhi_app, ["probe", "--help"])
...
def test_probe_pack_list():
    result = runner.invoke(kadhi_app, ["probe", "pack", "--list"])
    assert result.exit_code == 0, (result.output, repr(result.exception))
...
def test_probe_pack_unknown_base():
    result = runner.invoke(kadhi_app, ["probe", "pack", "unknown/model"])
...
def test_probe_pack_no_args_exits_2():
    result = runner.invoke(kadhi_app, ["probe", "pack"])
    assert result.exit_code == 2
...
def test_probe_sleeper_no_evidence_renders_metadata():
    result = runner.invoke(
        kadhi_app, ["probe", "sleeper", "meta-llama/Llama-3-8B"]
    )
    assert result.exit_code == 0, (result.output, repr(result.exception))
    out = _strip_ansi(result.output)
...
def test_probe_sleeper_unknown_base_exits_2():
    result = runner.invoke(kadhi_app, ["probe", "sleeper", "unknown/model"])
    assert result.exit_code == 2
...
def test_probe_interference_happy(tmp_path):
    ...
    result = runner.invoke(kadhi_app, ["probe", "interference", p.name])
    # All small (< 20%) -> exit 0
    assert result.exit_code == 0, (result.output, repr(result.exception))
...
def test_probe_interference_missing_evidence_file(tmp_path):
    result = runner.invoke(kadhi_app, ["probe", "interference", "nonexistent.json"])
    assert result.exit_code == 2


def test_probe_sae_diff_help():
    result = runner.invoke(kadhi_app, ["probe", "sae-diff", "--help"])
...
# ---------------------------------------------------------------------------
# kadhi adapters blame --live (Part B closes #171)
# ---------------------------------------------------------------------------
...
def test_adapters_blame_top_k_flag_present():
    result = runner.invoke(kadhi_app, ["adapters", "blame", "--help"])
    assert result.exit_code == 0
    assert "--top-k" in _strip_ansi(result.output)
