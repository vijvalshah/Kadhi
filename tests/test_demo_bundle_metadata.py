"""`kadhi data demo` metadata must describe the fixture it actually ships.
...
- ``grpo_demo`` declared ``format="reasoning"``, which is not a format Kadhi
  accepts anywhere.
...
def test_description_claims_no_row_count(name: str):
    ...
    ``kadhi data inspect`` reports the real number from the file, which cannot
    disagree with it.
    """
    ...
    assert not re.search(r"\d+[\s-]*row", description, flags=re.IGNORECASE), (
        ...
        "Counts belong in `kadhi data inspect`, not in metadata."
    )
