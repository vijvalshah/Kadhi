...
from kadhi_cli.utils.behavior_battery import (
    _agreement_rate,
    classify_behavior_score,
    compute_behavior_diff,
)
from kadhi_cli.utils.checklist_dsl import (
    CheckListSpec,
    CheckListTest,
    parse_checklist_spec,
    run_checklist_spec,
)
from kadhi_cli.utils.irt import (
    ItemDifficulty,
    pick_irt_subset,
)
...
class TestSourceWiring:
    def test_behavior_battery_uses_traversable_path(self):
        ...
        src = (
            ...
            / "src" / "kadhi_cli" / "utils" / "behavior_battery.py"
        )
    ...
    def test_checklist_uses_shared_helper(self):
        ...
        src = (
            Path(__file__).resolve().parent.parent
            / "src" / "kadhi_cli" / "utils" / "checklist_dsl.py"
        )
        text = src.read_text(encoding="utf-8")
    ...
    def test_cli_uses_dedup_helpers(self):
        ...
        src = (
            Path(__file__).resolve().parent.parent
            / "src" / "kadhi_cli" / "commands" / "_eval_v0650.py"
        )
        text = src.read_text(encoding="utf-8")
...
class TestIrtRowsCap:
    def test_malformed_rows_count_toward_cap(self, tmp_path, monkeypatch):
        ...
        from kadhi_cli.utils.irt import _MAX_ROWS, load_response_rows
...
class TestInvEmptyStringRejected:
    def test_inv_all_whitespace_responses_rejected(self):
        from kadhi_cli.utils.checklist_dsl import _inv_pass
...
class TestLoadResponseRowsWarnsOnSkip:
    def test_warning_emitted_on_malformed(self, tmp_path, monkeypatch, caplog):
        ...
        from kadhi_cli.utils.irt import load_response_rows
        ...
        with caplog.at_level(logging.WARNING, logger="kadhi_cli.utils.irt"):
