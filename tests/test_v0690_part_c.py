...
class TestSourceWiring:
    def test_no_heavy_imports(self) -> None:
        ...
        src = (root / "src" / "kadhi_cli" / "utils" / "magpie.py").read_text(encoding="utf-8")
        for forbidden in (
            "\nimport torch",
            "\nimport transformers",
            "\nimport anthropic",
    ...
    def test_cli_registered(self) -> None:
        root = Path(__file__).resolve().parent.parent
        cli = (root / "src" / "kadhi_cli" / "commands" / "data.py").read_text(encoding="utf-8")
