...
* #93 — package-data migration (kadhi_cli/data/_fixtures/)
...
class TestDetectSpaceSdk:
    def _import(self):
        from kadhi_cli.utils.hf_space import detect_space_sdk
...
class TestIsSupportedSpaceSdk:
    def test_known(self):
        from kadhi_cli.utils.hf_space import is_supported_space_sdk
...
class TestSendTelemetryPayload:
    ...
    def test_happy_path_2xx(self, monkeypatch):
        ...
        result = send_telemetry_payload(
            {"command": "train", "kadhi_version": "x"}, api_key="phc_live_test_key"
        )
...
class TestTrackerMissingDepMessage:
    ...
    def test_missing_mlflow_advisory(self, monkeypatch):
        ...
        assert "kadhi-cli[trackers]" in msg
...
class TestRemoteLoader:
    def test_looks_like_remote_uri(self):
        from kadhi_cli.data.loader import _looks_like_remote_uri
    ...
    def test_fsspec_missing_raises_friendly(self, monkeypatch, tmp_path):
        ...
        from kadhi_cli.data.loader import _load_remote_dataset
...
class TestPackageDataFixtures:
    def test_fixtures_dir_exists(self):
        ...
        pkg = Path(kadhi_cli.__file__).parent
    ...
    def test_bundle_source_prefers_package_data(self):
        from kadhi_cli.utils.demo_bundles import _bundle_source_path, get_bundle

        bundle = get_bundle("alpaca_demo")
        src = _bundle_source_path(bundle)
        # Should resolve under kadhi_cli/data/_fixtures (not examples/data/).
...
class TestVersionBump:
    ...
    def test_init_version(self):
        ...
        assert self._version_tuple(kadhi_cli.__version__) >= (0, 53, 8)
