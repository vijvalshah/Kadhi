"""v0.71.37 — every printed `pip install kadhi-cli[extra]` hint must be cmd.exe-safe.
...
pip rejects the requirement outright:
    ...
    ERROR: Invalid requirement: "'kadhi-cli[train]'": Expected package name at
    the start of dependency specifier

Measured on Windows before this fix:

    cmd.exe      'kadhi-cli[train]' -> ERROR   "kadhi-cli[train]" -> ok   bare -> ok
    PowerShell   'kadhi-cli[train]' -> ok      "kadhi-cli[train]" -> ok

Double quotes are the only spelling that works in *every* shell, which is why
the repo already says `pip install -e ".[dev]"`. Bare `kadhi-cli[train]` is not
an option: zsh globs the bracket and fails with `no matches found`.

Nothing in Kadhi can rescue the command once it is typed — pip and the shell own
it, and Kadhi is not installed yet when the README command runs. The only lever
...
# `pip install "kadhi-cli[x]"` and the Rich-escaped `pip install "kadhi-cli\[x]"`.
...
# including the `kadhi ui` one that started this — and the guard passes vacuously.
SINGLE_QUOTED = re.compile(r"pip install 'kadhi-cli\\*\[[a-z][a-z0-9-]*\]'")


def _package_root() -> pathlib.Path:
    import kadhi_cli

    return pathlib.Path(kadhi_cli.__file__).parent


def _repo_root() -> pathlib.Path | None:
    """src-layout: <repo>/src/kadhi_cli. None when installed as a plain wheel."""
...
class TestHintsAreCmdSafe:
    def test_no_single_quoted_hint_in_package(self):
        """Nothing under src/kadhi_cli may print the cmd-hostile spelling."""
        root = _package_root()
        offenders = []
        ...
        assert not offenders, (
            "single-quoted kadhi-cli[extra] hint — cmd.exe passes the quotes to "
            'pip and it errors out. Use \\"kadhi-cli[extra]\\" (works in every '
            "shell):\n" + "\n".join(offenders)
        )
        ...
        for path in targets:
            ...
            for lineno, line in enumerate(
                ...
                if in_fence and SINGLE_QUOTED.search(line):
                    offenders.append(f"{rel}:{lineno}: {line.strip()}")
        assert not offenders, (
            "single-quoted kadhi-cli[extra] in a docs code block — a Windows "
    ...
    def test_regex_actually_matches_the_broken_spelling(self):
        ...
        the plain `'kadhi-cli[fast]'` sites but not a single Rich-escaped one --
        i.e. it would have gone green while `kadhi ui` still printed a command
        that dies on cmd.exe. Both spellings are pinned here on purpose.
        """
        # Plain (YAML template comments, plain exception text).
        assert SINGLE_QUOTED.search("pip install 'kadhi-cli[train]'")
        # Rich-escaped, exactly as it sits in ui.py: TWO backslash chars.
        assert SINGLE_QUOTED.search(
            '"Install with: [bold]pip install \'kadhi-cli\\\\[ui]\'[/]"'
        )
        # The spellings we are migrating *to* must not be flagged.
        assert not SINGLE_QUOTED.search('pip install "kadhi-cli[train]"')
        assert not SINGLE_QUOTED.search('pip install "kadhi-cli\\\\[ui]"')
        assert not SINGLE_QUOTED.search("pip install kadhi-cli")
...
class TestDoubleQuotesSurviveRich:
    ...
    def test_rich_keeps_escaped_bracket_inside_double_quotes(self):
        ...
        assert render('[bold]pip install "kadhi-cli\\[ui]"[/]') == (
