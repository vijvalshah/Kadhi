...
class TestPlaintextDataFormat:
    ...
    def test_format_signature_exists(self):
        ...
        from kadhi_cli.data.formats import FORMAT_SIGNATURES
    ...
    def test_convert_plaintext(self):
        ...
        from kadhi_cli.data.formats import format_to_messages
...
class TestTxtFileLoading:
    ...
    def test_txt_extension_supported(self):
        ...
        from kadhi_cli.data.loader import SUPPORTED_EXTENSIONS
...
class TestPretrainSweepParams:
    ...
    def test_sweep_run_single_routes_to_pretrain_trainer(self):
        ...
        with mock_patch("kadhi_cli.data.loader.load_dataset", return_value=fake_dataset), \
             mock_patch("kadhi_cli.utils.gpu.detect_device", return_value=("cpu", "CPU")), \
             mock_patch("kadhi_cli.utils.gpu.get_gpu_info", return_value=fake_gpu_info), \
             mock_patch("kadhi_cli.experiment.tracker.ExperimentTracker") as mock_tracker_cls, \
             mock_patch("kadhi_cli.monitoring.display.TrainingDisplay"), \
             mock_patch("kadhi_cli.trainer.pretrain.PretrainTrainerWrapper.setup"), \
             mock_patch(
                 "kadhi_cli.trainer.pretrain.PretrainTrainerWrapper.train",
...
class TestPretrainTrainResults:
    ...
    def test_train_result_duration_minutes_format(self):
        ...
        with mock_patch("kadhi_cli.trainer.pretrain.time.time", side_effect=fake_time):
            result = wrapper.train()
...
class TestPretrainInitTemplate:
    """Test that kadhi init produces correct output for pretrain."""

    def test_init_pretrain_template_creates_file(self, tmp_path):
        """kadhi init --template pretrain should write a file with pretrain task."""
        from typer.testing import CliRunner

        from kadhi_cli.cli import app

        runner = CliRunner()
        output = tmp_path / "kadhi.yaml"
    ...
    def test_init_pretrain_template_produces_valid_config(self, tmp_path):
        """The file written by kadhi init --template pretrain should parse."""
    ...
    def test_init_moe_template_creates_file(self, tmp_path):
        """kadhi init --template moe should write a file with moe_lora."""
        from typer.testing import CliRunner
...
class TestPretrainWizardPath:
    ...
    def test_wizard_pretrain_task_sets_plaintext_format(self):
        ...
        from kadhi_cli.commands.init import _interactive_wizard

        with mock_patch("kadhi_cli.commands.init.Prompt.ask", side_effect=[
            "some-model",
            "pretrain",
            "./corpus.txt",
            "1",
    ...
    def test_wizard_pretrain_does_not_prompt_for_format(self):
        ...
        with mock_patch("kadhi_cli.commands.init.Prompt.ask", side_effect=record_prompt):
            config_text = _interactive_wizard()
...
class TestPretrainMoEIntegration:
    ...
    def test_moe_lora_calls_get_moe_target_modules(self):
        ...
        with mock_patch("transformers.AutoModelForCausalLM.from_pretrained",
             ...
             mock_patch(
                 "kadhi_cli.utils.moe.detect_moe_model", return_value=True
             ), \
             mock_patch(
    ...
    def test_moe_aux_loss_coeff_sets_router_config(self):
        ...
        with mock_patch("transformers.AutoModelForCausalLM.from_pretrained",
             ...
             mock_patch("kadhi_cli.utils.moe.get_moe_target_modules",
                        return_value=None):
            from kadhi_cli.trainer.pretrain import PretrainTrainerWrapper
    ...
    def test_moe_lora_non_moe_model_skips_targets(self):
        ...
        with mock_patch("transformers.AutoModelForCausalLM.from_pretrained",
             ...
             mock_patch("peft.prepare_model_for_kbit_training"), \
             mock_patch(
                 "kadhi_cli.utils.moe.detect_moe_model", return_value=False
             ), \
             mock_patch(
                 "kadhi_cli.utils.moe.get_moe_target_modules",
                 return_value=None,
