...
* CLI plumbing for ``kadhi merge --save-format`` (4bit / 4bit_forced / fp16)
* CLI plumbing for ``kadhi export --format torchao --quant-config <yaml>``
...
class TestMerge4bitWiring:
    ...
    def test_happy_path_with_mocks(self, tmp_path, monkeypatch):
        from kadhi_cli.utils import save_formats
...
# --- CLI plumbing for `kadhi merge --save-format` ----------------------------


class TestMergeSaveFormatCLI:
    def test_save_format_help_lists_flag(self):
        from kadhi_cli.commands.merge import merge
    ...
    def test_no_double_quant_flag_reaches_merge_4bit(self, tmp_path, monkeypatch):
        ...
        with patch(
            ...
            "kadhi_cli.utils.save_formats.merge_4bit",
...
# --- CLI plumbing for `kadhi export --format torchao` ------------------------
...
class TestValidateQuantConfigPath:
    ...
    def test_load_quant_config_yaml_happy(self, tmp_path, monkeypatch):
        from kadhi_cli.utils.save_formats import load_quant_config
...
class TestDetectPrequantizedSymlinkRejection:
    ...
    def test_config_json_symlink_returns_none(self, tmp_path, monkeypatch):
        ...
        from kadhi_cli.autopilot.decisions import detect_prequantized_format_from_path
