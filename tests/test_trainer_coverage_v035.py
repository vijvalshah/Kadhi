...
exercised on every CI matrix job. Each test builds a minimal KadhiConfig
...
def test_apply_v028_speed_memory_no_exception(task: str, feature: str) -> None:
    ...
    """
    from kadhi_cli.utils import v028_features as vf
...
def test_apply_v028_speed_memory_invokes_cut_ce_patcher(task: str) -> None:
    ...
    with patch("kadhi_cli.utils.cut_ce.apply_cut_ce", return_value=True):
        result = vf.apply_v028_speed_memory(
...
def test_apply_v028_speed_memory_invokes_fp8(task: str) -> None:
    ...
    tcfg = _make_tcfg("fp8")
    with patch("kadhi_cli.utils.fp8.apply_fp8_training", return_value=True):
        result = vf.apply_v028_speed_memory(
            model=MagicMock(), tcfg=tcfg,
            base_model="meta-llama/Llama-3.2-1B", console=None,
        )
...
@pytest.mark.parametrize(
    ...
    (
        ...
        "kadhi_cli.trainer.dpo",
        "kadhi_cli.trainer.pretrain",
        "kadhi_cli.trainer.grpo",
        "kadhi_cli.trainer.kto",
        "kadhi_cli.trainer.orpo",
        "kadhi_cli.trainer.simpo",
        "kadhi_cli.trainer.ipo",
        "kadhi_cli.trainer.ppo",
...
class TestQuantNameTranslators:
    ...
    def test_quant_name_to_vllm_kwargs_known(self, name, expected) -> None:
        from kadhi_cli.utils.auto_quant import quant_name_to_vllm_kwargs
    ...
    def test_quant_name_to_bnb_kwargs(self) -> None:
        from kadhi_cli.utils.auto_quant import quant_name_to_bnb_kwargs
...
class TestFreeEngine:
    def test_free_engine_no_torch(self) -> None:
        ...
        from kadhi_cli.utils.auto_quant import free_engine
...
class TestTryReloadWithFallback:
    ...
    def test_picked_loads_first_try(self) -> None:
        from kadhi_cli.utils.auto_quant import try_reload_with_fallback
...
class TestVllmCreateEngineQuantizationParam:
    def test_create_vllm_engine_accepts_quantization_kwarg(self) -> None:
        ...
        from kadhi_cli.utils import vllm as vllm_mod
...
def test_benchmark_kernel_combos_cpu_returns_none_times() -> None:
    ...
    from kadhi_cli.utils.kernel_picker import benchmark_kernel_combos
...
def test_bench_and_pick_kernel_returns_none_on_cpu() -> None:
    ...
    from kadhi_cli.utils.v028_features import _bench_and_pick_kernel
