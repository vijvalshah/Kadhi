...
Covers:
- #101: KadhiPluginCallback + attach_plugin_callback wired into 13 trainers.
...
- #106: utils/recipe_run.run_recipe — stub + `kadhi data recipe --execute`.
"""
...
# ----------------------------------------------------------------------
# #101 — KadhiPluginCallback + attach_plugin_callback
# ----------------------------------------------------------------------
...
def clear_plugins_fixture(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("KADHI_PLUGIN_STATE_PATH", str(tmp_path / "plugins.json"))
...
def test_build_plugin_callback_skips_disabled(clear_plugins_fixture):
    ...
    from kadhi_cli.monitoring.plugin_callback import build_plugin_callback
    from kadhi_cli.plugins import disable_plugin, register_plugin
...
@pytest.mark.parametrize(
    "trainer_file",
    [
        "src/kadhi_cli/trainer/sft.py",
        "src/kadhi_cli/trainer/dpo.py",
        "src/kadhi_cli/trainer/grpo.py",
        "src/kadhi_cli/trainer/kto.py",
        "src/kadhi_cli/trainer/orpo.py",
        "src/kadhi_cli/trainer/simpo.py",
        "src/kadhi_cli/trainer/ipo.py",
        "src/kadhi_cli/trainer/bco.py",
        "src/kadhi_cli/trainer/ppo.py",
        "src/kadhi_cli/trainer/pretrain.py",
        "src/kadhi_cli/trainer/reward_model.py",
        "src/kadhi_cli/trainer/embedding.py",
        "src/kadhi_cli/trainer/distill.py",
    ],
)
def test_every_trainer_wires_attach_plugin_callback(trainer_file: str):
    ...
    assert "attach_plugin_callback," in src or (
        "from kadhi_cli.utils.peft_wiring import" in src
...
def test_ngram_config_threaded_into_generate_response(monkeypatch):
    ...
    from kadhi_cli.utils.ngram_spec import NgramSpecConfig
    ...
    monkeypatch.setattr("kadhi_cli.commands.serve._generate_response", _capture)
...
# ----------------------------------------------------------------------
# #106 — recipe_run stub + `kadhi data recipe --execute`
# ----------------------------------------------------------------------
...
def test_data_recipe_execute_flag_present(tmp_path: Path):
    """`kadhi data recipe --help` lists `--execute`."""
...
def test_instantiate_trainer_plugins_in_dunder_all():
    ...
    import kadhi_cli.utils.trainer_plugins as mod
