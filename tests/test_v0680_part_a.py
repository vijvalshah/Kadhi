"""v0.68.0 Part A — ``kadhi compile`` (DSPy/GEPA prompt-program compiler).

Tests for ``kadhi_cli/utils/prompt_compile.py`` + ``kadhi_cli/commands/compile_cmd.py``.
...
class TestPublicSurface:
    def test_module_importable(self) -> None:
        from kadhi_cli.utils import prompt_compile
    ...
    def test_allowlist_is_frozenset(self) -> None:
        from kadhi_cli.utils.prompt_compile import SUPPORTED_PROMPT_OPTIMIZERS
...
class TestValidatePromptOptimizer:
    def test_happy(self) -> None:
        from kadhi_cli.utils.prompt_compile import validate_prompt_optimizer
...
class TestValidateMaxIters:
    def test_happy(self) -> None:
        from kadhi_cli.utils.prompt_compile import validate_max_iters
    ...
    def test_overcap_rejected(self) -> None:
        from kadhi_cli.utils.prompt_compile import (
            MAX_COMPILE_ITERS,
            validate_max_iters,
        )
...
class TestValidateProgramPath:
    def test_happy(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        from kadhi_cli.utils.prompt_compile import validate_program_path
...
class TestCompilePlan:
    def test_frozen(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        from kadhi_cli.utils.prompt_compile import CompilePlan
...
class TestRunCompileDeferred:
    def test_live_missing_dep_friendly_importerror(
        ...
        with pytest.raises(ImportError, match=r"kadhi-cli\[compile\]"):
            run_compile(plan)
...
class TestSourceWiring:
    def test_no_top_level_heavy_imports(self) -> None:
        path = (
            ...
            / "src" / "kadhi_cli" / "utils" / "prompt_compile.py"
        )
    ...
    def test_uses_atomic_write_helper(self) -> None:
        path = (
            Path(__file__).resolve().parent.parent
            / "src" / "kadhi_cli" / "commands" / "compile_cmd.py"
        )
