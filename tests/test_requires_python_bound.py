...
resolves a torch wheel nobody here has ever run, and the failure is not a Kadhi
error message — it is a loader crash inside `c10.dll` / `libc10.so` before any
Kadhi code executes, so the user has nothing to act on.
...
def _parse_bounds(spec: str) -> tuple[tuple[int, int], tuple[int, int]]:
    ...
    assert ceiling, (
        ...
        f"extension before Kadhi runs. Set an upper bound matching the CI matrix."
    )
