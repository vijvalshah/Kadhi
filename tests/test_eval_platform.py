...
from kadhi_cli.eval.custom import (
    MAX_EVAL_TASKS,
    MAX_REGEX_INPUT_LEN,
...
class TestRunEval:
    def test_run_eval_with_mock_generator(self):
        from kadhi_cli.eval.custom import run_eval
...
class TestEvalConfig:
    def test_eval_config_default(self):
        from kadhi_cli.config.schema import EvalConfig
        config = EvalConfig()
    ...
    def test_kadhi_config_with_eval(self):
    ...
    def test_kadhi_config_without_eval(self):
        from kadhi_cli.config.schema import KadhiConfig
        config = KadhiConfig(
            base="test-model",
            data={"train": "data.jsonl"},
...
class TestCallbackAutoEval:
    ...
    def test_auto_eval_called_when_enabled(self):
        ...
        with patch("kadhi_cli.commands.eval.benchmark") as mock_bench:
