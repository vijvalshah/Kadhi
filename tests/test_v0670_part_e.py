...
Tests for ``kadhi_cli/utils/kadhi_lock.py``:

- Frozen ``KadhiLock`` dataclass (model SHA + dataset hash + env hash)
...
- Composes with v0.64 Part C ``kadhi env`` (env_hash is operator-supplied)
- CLI smoke (`kadhi lock`)
"""
...
class TestPublicSurface:
    def test_module_importable(self) -> None:
        from kadhi_cli.utils import kadhi_lock

        assert hasattr(kadhi_lock, "KadhiLock")
        assert hasattr(kadhi_lock, "LockDrift")
        assert hasattr(kadhi_lock, "compute_lock_closure")
        assert hasattr(kadhi_lock, "write_lock")
        assert hasattr(kadhi_lock, "read_lock")
        assert hasattr(kadhi_lock, "check_lock_drift")
...
class TestKadhiLock:
    def test_construct(self) -> None:
        from kadhi_cli.utils.kadhi_lock import KadhiLock
        ...
        assert lock.kadhi_version == "0.67.0"
    ...
    def test_frozen(self) -> None:
        ...
        with pytest.raises(dataclasses.FrozenInstanceError):
            lock.kadhi_version = "9.99"  # type: ignore[misc]
...
class TestWriteReadLock:
    ...
    def test_read_symlink_rejected(self, tmp_path, monkeypatch) -> None:
        ...
        real.write_text('{"kadhi_version": "0.67.0"}', encoding="utf-8")
...
class TestCheckLockDrift:
    def _make_lock(self, **overrides):
        ...
        return KadhiLock(**base)

    def test_no_drift(self) -> None:
        from kadhi_cli.utils.kadhi_lock import check_lock_drift
...
class TestCliSmoke:
    def test_lock_help(self) -> None:
        ...
        from kadhi_cli.commands.lock import app
    ...
    def test_lock_write_command(self, tmp_path, monkeypatch) -> None:
        ...
        assert (tmp_path / "kadhi.lock").exists()

    # v0.71.1 #224 — auto-glue with `kadhi env lock`.
    def test_lock_write_auto_derives_env_hash(self, tmp_path, monkeypatch) -> None:
        ...
        from kadhi_cli.commands.lock import app
        from kadhi_cli.utils.env_lock import compute_env_hash, snapshot_env, write_lock

        monkeypatch.chdir(tmp_path)
        # Stand up a kadhi-env.lock the way `kadhi env lock` would.
        env = snapshot_env()
        write_lock(env, "kadhi-env.lock")
        ...
        result = runner.invoke(
            ...
            [
                ...
                "--base-sha", "a" * 64,
                "--dataset-sha", "b" * 64,
                # NOTE: no --env-hash; must auto-derive from kadhi-env.lock.
                "--output", "kadhi.lock",
            ],
        )
        assert result.exit_code == 0, (result.output, repr(result.exception))
        written = json.loads((tmp_path / "kadhi.lock").read_text(encoding="utf-8"))
    ...
    def test_lock_write_explicit_env_hash_wins(self, tmp_path, monkeypatch) -> None:
        # Explicit --env-hash takes precedence over any kadhi-env.lock.
        import json
        ...
        monkeypatch.chdir(tmp_path)
        write_lock(snapshot_env(), "kadhi-env.lock")
        runner = CliRunner()
        result = runner.invoke(
            app,
            [
    ...
    def test_lock_show_command(self, tmp_path, monkeypatch) -> None:
        ...
        result = runner.invoke(app, ["show", "kadhi.lock"])
        assert result.exit_code == 0, (result.output, repr(result.exception))
...
class TestSourceWiring:
    def test_no_top_level_heavy_imports(self) -> None:
        ...
        src = (root / "src" / "kadhi_cli" / "utils" / "kadhi_lock.py").read_text(
            encoding="utf-8"
        )
        head_lines = [
            line
