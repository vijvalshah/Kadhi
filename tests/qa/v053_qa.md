...
```bash
kadhi merge -a ./adapter -o ./merged
for q in q4_0 q4_k_m q5_k_m q8_0 f16 f32; do
  kadhi export --model ./merged --format gguf --quant "$q" \
...
```bash
kadhi export --model ./merged --format awq  --bits 4 --output ./out/awq
kadhi export --model ./merged --format gptq --bits 4 --output ./out/gptq
```
...
### #109 — `kadhi deploy autopilot --measure`
...
`kadhi_cli.utils.deploy_measure`) is the production hook for live model
...
EOF
kadhi export --model ./merged --format torchao --quant-config ./q.yaml \
  --output ./out/torchao
```

## v0.53.2

### #71 — ONNX export pipeline (`kadhi export --format onnx`)
...
is host-RAM-bound (not a bug in `kadhi_cli` or `optimum`). Re-run on a
...
- `kadhi push --repo <user/repo> --model ./output` — flag accepted, repo_id
  validator from v0.29.0 wired (`utils/hf.validate_repo_id`); pre-flight
  containment on `--model` confirmed via `is_under_cwd`.
- `kadhi train --push-as <user/repo>` — flag accepted; HFPushCallback path
  unchanged.
- `kadhi data push --hf-dataset <user/repo>` — flag accepted; JSONL
  containment + repo_id validator confirmed.
- `kadhi deploy hf-space --model <user/repo> --space <user/space>` — flag
...
`kadhi export --format onnx --model TinyLlama/TinyLlama-1.1B-Chat-v1.0` ran
...
**Mitigation:** Larger ONNX exports need a 16 GB+ RAM host. Kadhi wrapping is
correct; this is a host-RAM ceiling, not a Kadhi bug. Recorded for future QA
on a beefier box.
