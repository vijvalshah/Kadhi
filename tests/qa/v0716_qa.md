...
`pytest` green ≠ "works on a real `kadhi.yaml`".

---

## #75 — synth data QA (`data from-traces` / `data augment` / `data generate`)

### `kadhi data from-traces` (no provider — offline) — PASS
Constructed an OpenAI-trace JSONL with a thumbs-up and a thumbs-down output for
the same prompt:

```
kadhi data from-traces --logs traces.jsonl --format openai --signal thumbs_up -o pairs.jsonl
```
→ `Wrote 1 preference pair(s)`; the pair is a correct
`{prompt, chosen, rejected, source}` row. PII reminder panel printed. ✅

### `kadhi data generate` (live Ollama) — PASS
```
kadhi data generate --provider ollama --model qwen2.5:0.5b -p "Write a short factual Q&A about Python lists" -n 1 -o gen.jsonl
```
→ `Generated 2 examples`, alpaca format, sane content (list-vs-tuple Q&A). ✅

### `kadhi data augment` (live Ollama) — **BUG FOUND + FIXED**
First run raised `ImportError: cannot import name 'OllamaProvider' from
kadhi_cli.data.providers.ollama`. Root cause: `_load_augment_provider`
...
```
kadhi data augment -i in.jsonl -o out.jsonl --strategy rephrase --provider ollama --model qwen2.5:0.5b --count 1
```
...
## #231 — live `kadhi build` runner — PASS
...
```
kadhi data gen-magpie --base qwen2.5:0.5b --provider ollama --target 2 --output magpie.jsonl --no-quality-filter --max-tokens 96
```
...
```
kadhi eval irt-subset resp.jsonl --size small --model 2pl   → selects ["split"]
kadhi eval irt-subset resp.jsonl --size tiny  --model 3pl   → runs, guessing bounded
```
...
round-trip). The live `kadhi diagnose --tokenizer` wiring lands with the live
