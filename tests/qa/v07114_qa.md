...
Verified `kadhi export --format onnx` end-to-end on a locally-built tiny GPT-2
(2 layers / 2 heads / n_embd=32 / vocab 512, saved with safetensors so it loads
under torch 2.5.1).

```bash
kadhi export --model ./model --format onnx --output ./onnx_out
```
