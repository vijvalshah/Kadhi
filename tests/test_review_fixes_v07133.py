...
MLX optimizer construction, Rich-markup escaping in `kadhi data`, the ASR infer
metric-guard / control-strip / task-validation / exit-code fixes, the distill KD
off-by-one shift, the load_config_from_string ValueError contract, `kadhi runs`
...
class TestVllmTrustRemoteCode:
    ...
    def test_default_is_false(self, monkeypatch):
        from kadhi_cli.utils.vllm import create_vllm_engine
    ...
    def test_serve_vllm_passes_resolved_trust(self, monkeypatch):
        ...
        import kadhi_cli.commands.serve as serve
        ...
        monkeypatch.setattr(
            "kadhi_cli.utils.vllm.create_vllm_engine", _fake_engine
        )
        monkeypatch.setattr(
            "kadhi_cli.utils.vllm.create_vllm_app",
            lambda **kw: object(),
        )
        serve._serve_vllm(
            model_path=Path("m"),
...
class TestVisionImageContainment:
    def test_absolute_traversal_dropped(self, tmp_path):
        from kadhi_cli.data.loader import _validate_vision_images
...
class TestAdapterScope:
    def _run(self, requested, active, names):
        ...
        from kadhi_cli.commands.serve import _adapter_scope
...
class TestDryRunNoReexec:
    def test_dry_run_does_not_execvp(self, tmp_path, monkeypatch):
        ...
        CliRunner().invoke(
            app, ["train", "--config", "kadhi.yaml", "--gpus", "2", "--dry-run"]
        )
...
class TestReexecForwardsFlags:
    def test_capture_flags_forwarded(self, tmp_path, monkeypatch):
        ...
        CliRunner().invoke(
            app,
            [
                "train", "--config", "kadhi.yaml", "--gpus", "2", "--yes",
...
# --------------------------------------------------------------------------
# HIGH-6 — `kadhi data inspect` must escape dataset-derived Rich markup
# --------------------------------------------------------------------------
class TestDataInspectEscape:
    def test_bracket_slash_does_not_crash(self, tmp_path, monkeypatch):
        from kadhi_cli.commands.data import app as data_app
...
class TestAsrInferMetricGuard:
    def test_oversized_reference_does_not_crash(self, tmp_path, monkeypatch):
        import kadhi_cli.commands.infer as infer
...
# --------------------------------------------------------------------------
# MEDIUM — `kadhi runs` escapes config-derived strings
# --------------------------------------------------------------------------
class TestRunsEscape:
    def _install_fake_tracker(self, monkeypatch, run):
        ...
        monkeypatch.setattr(
            "kadhi_cli.experiment.tracker.ExperimentTracker", _FakeTracker
        )
    ...
    def test_list_does_not_crash(self, monkeypatch):
        from kadhi_cli.commands.runs import app as runs_app
...
class TestConstantTimeToken:
    def test_wrong_token_401_right_token_ok(self):
        ...
        from kadhi_cli.ui.app import create_app, set_auth_token
...
class TestAtomicWrites:
    def test_vscode_writes_and_honours_force(self, tmp_path, monkeypatch):
        ...
        monkeypatch.chdir(tmp_path)
        out = write_vscode_launch(config_path="kadhi.yaml", target_dir=".vscode")
        assert Path(out).exists()
        payload = json.loads(Path(out).read_text())
        assert "configurations" in payload
        # Second write without force must refuse.
        with pytest.raises(FileExistsError):
            write_vscode_launch(config_path="kadhi.yaml", target_dir=".vscode")
    ...
    def test_lr_finder_writes_under_cwd(self, tmp_path, monkeypatch):
        from kadhi_cli.utils.lr_finder import save_lr_finder_report
