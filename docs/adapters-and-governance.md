...
> Adapter lifecycle/management, the model registry, Kadhi Cans, the data flywheel (`kadhi loop`), knowledge editing, activation steering, and the supply-chain / governance surfaces (scan, sign, BOM, attest, audit, airgap).

**Contents:**

- [Adapter Lifecycle (`kadhi adapters {merge,pr,bisect}`, `kadhi lock`)](#adapter-lifecycle-kadhi-adapters-mergeprbisect-kadhi-lock)
- [Data Flywheel (`kadhi loop`)](#data-flywheel-kadhi-loop)
- [Knowledge Editing (`kadhi edit set`, ROME / MEMIT / AlphaEdit)](#knowledge-editing-kadhi-edit-set-rome--memit--alphaedit)
- [Activation Steering (`kadhi steer`)](#activation-steering-kadhi-steer)
...
- [Kadhi Cans (Shareable Recipes)](#kadhi-cans-shareable-recipes)
- [Bill of Materials (`kadhi bom emit`)](#bill-of-materials-kadhi-bom-emit)
- [Provenance Attestations (`kadhi attest emit`)](#provenance-attestations-kadhi-attest-emit)
- [EU AI Act Annex XI/XII Auto-Doc (`kadhi train --annex-xi`)](#eu-ai-act-annex-xixii-auto-doc-kadhi-train---annex-xi)
- [Audit Log (`kadhi audit-log`)](#audit-log-kadhi-audit-log)
- [Reproducibility Receipt (`kadhi train --repro-receipt`)](#reproducibility-receipt-kadhi-train---repro-receipt)
- [Adapter Backdoor Scanner (`kadhi adapters scan`)](#adapter-backdoor-scanner-kadhi-adapters-scan)
- [Adapter Sign + Verify (`kadhi adapters sign` / `verify`)](#adapter-sign--verify-kadhi-adapters-sign--verify)
- [Strict Safetensors Mode (`kadhi adapters check-safetensors`)](#strict-safetensors-mode-kadhi-adapters-check-safetensors)
...
- [Airgap Bundle (`kadhi airgap-bundle`)](#airgap-bundle-kadhi-airgap-bundle)

---

## Adapter Lifecycle (`kadhi adapters {merge,pr,bisect}`, `kadhi lock`)

v0.57 shipped `adapters diff / merge / blame / branch`. v0.67 finishes the lifecycle: evolutionary merge driven by your eval, GitHub-shaped PRs for adapter review, a shared `kadhi.lock` for team reproducibility, and binary-search bisect over training history.
...
kadhi env lock                                     # v0.64 — capture env hash
kadhi lock write \
  --base-model meta-llama/Llama-3.1-8B \
  --base-sha $BASE_SHA \
  --dataset-sha $DATA_SHA \
  --env-hash $(jq -r .closure kadhi-env.lock) \
  -o kadhi.lock
# Teammates re-check the lock; exit 3 on drift.
kadhi lock check kadhi.lock \
  --base-model meta-llama/Llama-3.1-8B \
  --base-sha $BASE_SHA --dataset-sha $DATA_SHA --env-hash $ENV_HASH

# 4. Bisect a training history to find the step that broke an eval.
kadhi adapters bisect \
  ckpt-step-100 ckpt-step-200 ckpt-step-400 ckpt-step-800 \
  --eval-command "kadhi eval custom --model {ckpt} --tasks eval.jsonl" \
  -o bisect.json
# Exits 3 on BROKEN_AT — pipe into `kadhi adapters blame` for attribution.
```

CMA-ES is pure-Python (no `cma` dependency); the eval is operator-supplied via a closure so any scoring code works. PR rendering escapes Markdown table cells, so crafted metric names cannot inject table rows or links. The lockfile composes with v0.64 `kadhi env lock` — drift in any of `{base_model, base_model_sha, dataset_sha, env_hash, closure_sha}` exits 3 (`kadhi_version` and `created_at` are advisory-only). Bisect uses `shlex.split` + `shlex.quote(ckpt)` in argv-list mode (no `shell=True`), so checkpoint ids cannot inject shell metacharacters.

VeRA / VB-LoRA bank storage (`kadhi_cli.utils.vector_bank`) supports live multi-tenant serving,
and `task='moe_lora_routing'` trains the live sparse per-token MoLE gating kernel.


## Data Flywheel (`kadhi loop`)

The full *production traces → preference pairs → Eval-Gated DPO → canary deploy → rollback* loop, driven from a single CLI. Connects v0.26 Trace-to-Preference + Eval-Gated Training + Registry lineage + Quant-Lobotomy verdicts + Kadhi Cans + v0.25 Autopilot + v0.54 Advise + v0.55 Eval Design + v0.56 Diagnose.

```bash
# One-time setup
kadhi loop init registry://abc12 --eval evals/lock.json --baseline registry://prod \
...
kadhi loop watch --poll-interval 300
...
# per-run via --pre-wired. Pack each iteration as a shareable Kadhi Can with
# Registry lineage via --pack-cans.
kadhi loop init registry://abc12 --eval evals/lock.json --baseline registry://prod --pre-wired
kadhi loop watch --pre-wired --pack-cans --poll-interval 300

# Unpack a recorded iteration's Kadhi Can for what-if analysis (v0.71.4)
kadhi loop replay iter-20260515T120000-abcdef01 --extract ./iter-dump

# Background subprocess (writes PID, no shell)
kadhi loop watch --detach

# Promote a canary at 5% traffic with auto-rollback on MAJOR verdict
kadhi loop canary registry://candidate --traffic 5% --autoroll-on-regress
...
kadhi loop replay iter-20260515T120000-abcdef01
```

State lives in `.kadhi/loop.yaml` (atomic write, cwd-contained, symlink-rejected). Per-iteration manifests under `.kadhi-loops/<iter-id>/iteration.json` are laid out so a v0.26 Kadhi Can can wrap them directly. The canary router is deterministic (SHA-256 hash of conversation id) and sticky-on-rollback — a flaky verdict can't ping-pong traffic between adapters.


## Knowledge Editing (`kadhi edit set`, ROME / MEMIT / AlphaEdit)
...
The sequential edit governor is persisted (SQLite, cross-process-locked, `KADHI_EDIT_GOVERNOR_DB` override) so the per-base-model edit count + norm-blowup verdict survive across separate `kadhi edit set` runs. The count increment is atomic — two concurrent `kadhi edit set` runs on the same base are merged under the cross-process lock (no lost increment). `kadhi edit set` consults the governor automatically: it refuses BEFORE the model load past the per-base cap or after a BLOWUP verdict, and records the measured `||ΔW||_F` after each edit. Pass `--no-governor` to opt out. `--registry-id <id>` attaches the edited model (or GRACE codebook) into the Registry lineage.


## Activation Steering (`kadhi steer`)

Sometimes you don't want to retrain — you want to *push* the model along a learned direction at decode time. Kadhi ships three control-vector backends:
...
kadhi steer train --base meta-llama/Llama-3.1-8B-Instruct \
                 --method caa --name safety-v1 \
                 --pairs ./data/pairs.jsonl

# Apply at decode time via kadhi serve
kadhi serve --model ./adapter --steer safety-v1 --steer-strength 1.5

# List locally-stored steering vectors
kadhi steer list
```

Steering names are validated against a strict regex (`^[A-Za-z0-9][A-Za-z0-9._\-]{0,127}$` — no path separators, no shell metacharacters); strength is bounded `|s| <= 10.0`. The trained vectors land in the Kadhi Registry under the `steering_vector` artifact kind so lineage is preserved.

As of v0.71.10 the fit and the decode hook are **live** (validated on SmolLM2-135M): `kadhi steer train` captures residual-stream activations (CAA / RepE) or per-head `o_proj`-input activations (ITI) on the contrastive pairs, computes the control vector, and persists `steering_vector.safetensors` + `steering_config.json`. `kadhi serve --steer <name>` installs a forward hook on the loaded model that adds `strength × vector` at decode time (transformers backend; `--steer` is rejected with a clear error on vLLM/SGLang). RepE / ITI need at least two contrastive pairs; CAA works from one.
...
```bash
kadhi edit set --base ./model --method grace \
              --subject "The CEO of Acme is" --target "Jane Doe"
```

```yaml
# Or via kadhi.yaml when training a model with GRACE-aware lookups
training:
  grace_codebook: true
...
`grace` joins the existing `rome` / `memit` / `alphaedit` allowlist on `kadhi edit set`; the sequential edit governor still gates the call when the per-base-model edit count or norm-blowup verdict trips. GRACE is live: `kadhi edit set --method grace --output ./ckpt` captures the residual key at the subject's last token, optimises a replacement value, and appends a `(key, value)` triple to a `grace_codebook.json` sidecar (atomic, cwd-contained). At inference the codebook is applied via a forward hook that substitutes the residual whenever it falls within an epsilon ball of a stored key — so the base weights are never modified and thousands of edits survive without norm blowup.


## Model Registry & Lineage

Every fine-tune you ship should be reproducible. Kadhi's local registry (`~/.kadhi/registry.db`) tracks each entry by a content hash of its config + data + base model, plus lineage pointers to parent entries.

```bash
# Register a completed run
kadhi registry push --run-id run_202611_abc123 --name llama31-chat --tag v1

# List entries (filter by name, tag, base model, task)
kadhi registry list
kadhi registry list --name llama31-chat --tag prod

# Show full details: config, eval baseline, artifacts, ancestors
kadhi registry show llama31-chat-v1

# Side-by-side config diff + eval delta between two entries
kadhi registry diff llama31-chat-v1 llama31-chat-v2

# Full-text search across name / base model / task / notes
kadhi registry search "medical reasoning"

# Promote an entry (add a tag, e.g. "prod")
kadhi registry promote llama31-chat-v1 --tag prod

# Delete (cascades to artifacts + lineage links)
kadhi registry delete llama31-chat-v1 --yes
```

**Lineage DAG** — every entry can point to a parent (its ancestor run). Walk the DAG for any name with:

```bash
kadhi history llama31-chat
```

**Refs resolve flexibly** — you can use a registry ID, a name (latest), or `name:tag`. Ambiguous prefixes raise an error rather than silently picking the wrong entry. Registry files are stored with `600` perms on POSIX; override the path with `KADHI_REGISTRY_DB_PATH`.


## Adapter Management (git for LoRA)

`kadhi adapters` is the git-for-LoRA surface: weight-aware diff, four merge strategies, leave-one-out blame, and SHA-256 branch snapshots. All commands operate on `adapter_model.safetensors` directories (peft-compatible).

```bash
# Per-layer ΔW Frobenius diff + effective-rank drift + top-K changed projections
kadhi adapters diff ./run-v17 ./run-v18

# Machine-readable JSON for CI
kadhi adapters diff ./run-v17 ./run-v18 --format json --output diff.json

# Weighted merge with linear / ties / dare / svd strategies
kadhi adapters merge ./run-v17 ./run-v18 ./run-v19 -o ./merged --strategy ties \
  --weights 0.5,0.3,0.2 --density 0.2

# DARE merge (deterministic via --seed)
kadhi adapters merge ./run-v17 ./run-v18 -o ./merged --strategy dare \
  --density 0.5 --seed 42
...
kadhi adapters arithmetic "coder + 0.5*math - toxic" \
...
- Branch pointers live under `~/.kadhi/branches/` (override via `KADHI_BRANCHES_DIR`, constrained to `$HOME` / `$CWD` / `$TMPDIR`).
- `kadhi adapters checkout` SHA-checks the source config — refuses to restore when the source has drifted from the snapshot, so reproducibility never silently lies.

**v0.66.0:** `kadhi adapters blame` is now LIVE — the v0.57 `NotImplementedError` stub (#171) is lifted via a DataInf-style influence-function approximation. Pass `--top-k 50` to control the reported top-influencer count; pass a real `probe_fn` (Python API) to feed real gradients, or use the default deterministic synthetic probe for offline planning.

**v0.71.4:** the merge verdict is now LIVE — `kadhi adapters merge … --canary suite.json` lifts the `MergeReport.verdict` `UNKNOWN` stub. A pre-scored `{"baseline_scores","candidate_scores"}` suite classifies the blend OK / MINOR / MAJOR with no model load; a `{"tasks":[...]}` suite uses an injectable scorer. `--strict-verdict` exits 2 on MAJOR. The backdoor-scan and license-conflict gates now run for **every** strategy, including `cmaes`. `kadhi adapters branch <name> --from-registry <id>` / `--attach-to-registry <id>` link training-env snapshots into the Registry lineage DAG (shown as a `branches` node in `kadhi history`).

**v0.71.34 — task-vector arithmetic.** `kadhi adapters arithmetic "<expr>"` applies task arithmetic (arXiv:2212.04089) to LoRA deltas: **add** (blend two skills), **scale** (`2*coder`), and — the differentiator — **negate** (`- toxic` removes a behaviour). Names in the expression map to adapter dirs via repeatable `--adapter name=path`. The math is done at the effective-delta level: a LoRA contributes `ΔW = B·A`, so the coefficient is applied so that `ΔW` scales **linearly** (subtracting an adapter actually negates its delta — a naïve element-wise sum would scale by `c²`, making negation a no-op). Adapters must share the base model (`--allow-cross-base` to override) and rank (mixed ranks are refused, not silently approximated). Each input passes the v0.71.2 backdoor-scan gate (`--allow-unscanned` to skip). Output is a single loadable adapter; exit 0 = ok, 1 = refusal.


## Kadhi Cans (Shareable Recipes)
...
kadhi can pack --entry-id llama31-chat-v1 --out ./llama31-chat.can

# Embed in-toto attestations into the manifest (v3 cans; repeatable)
kadhi can pack --entry-id llama31-chat-v1 --out ./signed.can --attest att.json

# Preview the manifest without extracting
kadhi can inspect ./llama31-chat.can

# Verify schema + config parseability
kadhi can verify ./llama31-chat.can

# Fork with modifications (dotted-path overrides) and re-pack
kadhi can fork ./llama31-chat.can --out ./llama31-chat-hot.can \
  --modify training.lr=5e-5 --modify training.epochs=5

# Run a .can end-to-end: extract → train (→ optional deploy)
kadhi can run ./llama31-chat.can --yes
kadhi can run ./llama31-chat.can --yes --deploy --env-capture ./env.txt

# Publish a .can to HF Hub as a dataset
kadhi can publish ./llama31-chat.can --hf-hub me/llama31-chat-recipe
```

**Security** — tar extraction uses `filter="data"` on Python 3.12+ with symlink/hardlink rejection fallback for older runtimes. Size cap: 100 MB. `DataRef.url` must be HTTPS. Fork overrides reject dunder keys (`__class__`, `__init__`) and null bytes. Manifest format version supports `1` and `2` (additive bump in v0.33.0 added `deploy_targets`). `kadhi can run` requires `--yes` (mandatory consent — auto-downloads data + auto-trains). `kadhi can publish` validates `repo_id` and resolves the HF token via env / cache files; commit messages are first-line + 200-char capped.



## Bill of Materials (`kadhi bom emit`)
...
```bash
# 1. Train and persist the measured energy (needs `pip install kadhi-cli[carbon]`):
kadhi train --config kadhi.yaml --track-energy --energy-out energy.json

# 2. Attach it to the BOM:
kadhi bom emit ... --format both --output bom --energy energy.json
```

`kadhi train --track-energy --energy-out <path>` writes the measured reading as JSON in
exactly the energy schema (`energy_kwh` / `co2_kg` / `pue` / `grid_intensity_g_per_kwh` /
`source`); `kadhi bom emit --energy <path>` then consumes it. (You can also hand-write the
JSON.) The path is validated (kept under cwd, symlinks rejected) on both the write and the
read. Energy lands in **both** outputs: under `metadata.properties` (`kadhi:energy_kwh`,
`kadhi:co2_kg`, …) in CycloneDX, and as `OTHER` annotations on the model package in SPDX.


## Provenance Attestations (`kadhi attest emit`)

Emit an **in-toto v1 Statement** wrapping a **SLSA-3 provenance v1 predicate** for each
Kadhi Can lifecycle stage:
...
the **`ed25519` backend is live** (`pip install kadhi-cli[sign]`):

```bash
kadhi attest emit --stage train --subject adapter-v1 --sha aaaa...64hex \
  --sign ed25519 --key signing.pem --output att.json   # writes att.json.sig
kadhi attest verify att.json --signature att.json.sig                 # exit 0 valid
kadhi attest verify att.json --signature att.json.sig --public-key trusted.pub
```
...
## EU AI Act Annex XI/XII Auto-Doc (`kadhi train --annex-xi`)
...
```bash
kadhi train --config kadhi.yaml --annex-xi annex.md           # markdown
kadhi train --config kadhi.yaml --annex-xi annex.pdf           # PDF (pip install kadhi-cli[pdf])
kadhi train --config kadhi.yaml --track-energy --annex-xi annex.md  # + measured kWh/CO2
```
...
`kadhi train --track-energy` wraps the training window in a codecarbon **offline** tracker
...
measurement as JSON for `kadhi bom emit --energy <path>` (the hand-off into an ML-BOM —
see [Attaching energy + CO₂](#attaching-energy--co2---energy)). Requires `pip install
kadhi-cli[carbon]`; without it, `--track-energy` is a graceful no-op.


## Audit Log (`kadhi audit-log`)

**Every `kadhi` command** now appends one HIPAA/SOC2-shaped record to the JSONL audit log at
`~/.kadhi/audit.jsonl` (override via `KADHI_AUDIT_LOG_PATH`, containment-checked to
`$HOME / $CWD / $TMPDIR`). Opt out per-invocation with `kadhi --no-audit-log <cmd>` or
globally with `KADHI_NO_AUDIT_LOG=1`. Tail or rotate:

```bash
kadhi audit-log tail --limit 50          # Rich table view
kadhi audit-log tail --json              # raw JSONL for SIEM ingestion
kadhi audit-log rotate --cap-mb 100      # force a rotation pass
kadhi --no-audit-log version             # skip the audit line for this command
```
...
## Reproducibility Receipt (`kadhi train --repro-receipt`)
...
```bash
kadhi train --config kadhi.yaml --repro-receipt repro.json
```
...
## Adapter Backdoor Scanner (`kadhi adapters scan`)
...
## Adapter Sign + Verify (`kadhi adapters sign` / `verify`)
...
(`pip install kadhi-cli[sign]`): it adds a real detached signature over the
Merkle root for *authentication* — proof the signer holds the private key.

```bash
# generate a keypair + sign in one shot
kadhi adapters sign ./adapter --backend ed25519 --generate-key signing.pem
# or sign with an existing key (or set KADHI_SIGNING_KEY)
kadhi adapters sign ./adapter --backend ed25519 --key signing.pem
# self-consistency verify (embedded public key)
kadhi adapters verify ./adapter
# genuine authentication: require the signer's key match a trusted one
kadhi adapters verify ./adapter --public-key trusted.pub
```
...
persists as `.kadhi-signature.json` (atomic write). `sigstore` keyless signing
...
## Strict Safetensors Mode (`kadhi adapters check-safetensors`)
...
the first time Kadhi sees a HuggingFace repo. Subsequent loads compare
...
malicious weights. Without this control, anyone with Kadhi pinned to that
...
Anthropic-AUP. `kadhi adapters merge --license apache-2.0 --license
...
`kadhi adapters scan` flags FAIL (or that can't be scanned) refuses the
merge with exit 3 unless you pass `--allow-unscanned`.


## Airgap Bundle (`kadhi airgap-bundle`)
...
```bash
kadhi airgap-bundle --model ./out --output bundle.tar --repro-receipt repro.json
```
...
extraction filter. `kadhi airgap-bundle` is intentionally a top-level
command (not `kadhi deploy airgap-bundle`) — it's an export operation,
not a deploy target.
