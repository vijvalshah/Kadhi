...
- [Production Trace Ecosystem (`kadhi ingest`)](#production-trace-ecosystem-kadhi-ingest)
- [Prompt Mining (`kadhi prune-prompt`)](#prompt-mining-kadhi-prune-prompt)
- [Active-Learning Sampler (`kadhi data active-sample`)](#active-learning-sampler-kadhi-data-active-sample)
...
- [Demo Datasets (`kadhi data demo`)](#demo-datasets-kadhi-data-demo)
...
- [Semantic dedup (`kadhi data dedup --semantic`)](#semantic-dedup-kadhi-data-dedup---semantic)
- [Topic map (`kadhi data topics`)](#topic-map-kadhi-data-topics)
- [Canaries (`kadhi data canary insertcheck`)](#canaries-kadhi-data-canary-insertcheck)
- [Data Recipe DAG](#data-recipe-dag)
- [Data Mixing Optimizer (BETA)](#data-mixing-optimizer-beta)
- [AOT Tokenization with `kadhi data preprocess`](#aot-tokenization-with-kadhi-data-preprocess)
- [Data Recipe DAG Runner (`kadhi data recipe --execute`)](#data-recipe-dag-runner-kadhi-data-recipe---execute)

---

## Semantic dedup (`kadhi data dedup --semantic`)

`kadhi data dedup` removes near-duplicates with MinHash by default — fast, no
...
```bash
kadhi data dedup train.jsonl --semantic -o clean.jsonl
kadhi data dedup train.jsonl --semantic --threshold 0.85 --field text -o clean.jsonl
kadhi data dedup train.jsonl --semantic --embed-model sentence-transformers/all-mpnet-base-v2
```
...
## Topic map (`kadhi data topics`)

See what you are actually training on:

```bash
kadhi data topics train.jsonl                       # 'auto' picks the cluster count
kadhi data topics train.jsonl --clusters 8 -o topics.json
```
...
## Canaries (`kadhi data canary insert|check`)
...
kadhi data canary insert train.jsonl -o canaried.jsonl --count 16 --manifest secrets.json

# 2. train on canaried.jsonl as usual, then:
kadhi data canary check --manifest secrets.json --base ./my-model --adapter ./lora
```
...
EOF
kadhi build build.yaml --dry-run                 # validate topology + plan
kadhi build build.yaml --output-dir built/       # live materialise (v0.71.6)
...
kadhi data best-of-n --provider ollama --model qwen2.5:7b \
...
kadhi data best-of-n --base Qwen/Qwen3.8-27B --revision <commit> \
...
kadhi data best-of-n --candidate-artifact candidates.jsonl \
...
prompts are not sampled or judged twice. Before final publication, Kadhi snapshots
...
Kadhi removes the previous manifest before replacing outputs and publishes the
...
`--resume`; Kadhi authenticates the checkpoint against the prompts and sampler
...
and return a ``dict`` or ``None``. Kadhi resolves the dotted path lazily (the module
...
**Trusted-input posture.** The dotted-path syntax causes Kadhi to import an
arbitrary Python module and call a function from it. Treat ``transform`` values
as *trusted input*: do not feed untrusted or operator-controlled YAML into ``kadhi
...
## Production Trace Ecosystem (`kadhi ingest`)

Closing the data flywheel without leaving your existing observability stack. `kadhi ingest` parses JSONL exports from every major SaaS dashboard and emits a normalised trace stream that `kadhi data from-traces` (v0.26) consumes.

```bash
# Six supported sources — adapters for the major SaaS vendors + raw OTel
kadhi ingest --source langfuse     --logs ./langfuse-export.jsonl --output traces.jsonl
kadhi ingest --source langsmith    --logs ./langsmith-runs.jsonl
kadhi ingest --source helicone     --logs ./helicone-requests.jsonl
kadhi ingest --source openpipe     --logs ./openpipe-export.jsonl
kadhi ingest --source otel         --logs ./otel-spans.jsonl
kadhi ingest --source openai-stored --logs ./oai-stored-completions.jsonl
```

With `--logs` the CLI never makes a network call — operators export from their SaaS dashboard or vendor API, then point `kadhi ingest` at the local file. Auth env vars (`LANGFUSE_PUBLIC_KEY` + `LANGFUSE_SECRET_KEY` / `LANGSMITH_API_KEY` / `HELICONE_API_KEY` / `OPENPIPE_API_KEY` / `OPENAI_API_KEY` / `OTEL_EXPORTER_OTLP_HEADERS`) are advisory on that path — Kadhi surfaces which ones authenticate the source. A PII reminder fires on every ingest run (matches v0.26.0 Trace-to-Preference policy).

### Live pull from Langfuse (`--pull`, #204)

Langfuse is the one source Kadhi can fetch directly, so there is no export step:
...
kadhi ingest --source langfuse --pull --since 7d --output traces.jsonl
```
...
## Prompt Mining (`kadhi prune-prompt`)

Production LLM apps often pin a multi-paragraph system prompt to every request. Fine-tuning with that prefix wastes tokens (the model learns to copy what's already in context). `kadhi prune-prompt` finds the longest character prefix shared by ≥ 95% of rows and strips it, so the FT model internalises the behaviour instead.

```bash
kadhi prune-prompt --input traces.jsonl --output pruned.jsonl --min-frequency 0.95
```

Binary-search over up-to-32 candidate templates finds the longest qualifying prefix (a longer threshold-meeting prefix may exist beyond the universal one — Kadhi does not early-exit on the 100% match). Two-pass file read with a 100 000-row DoS cap.
...
```bash
kadhi prune-prompt --input traces.jsonl --output pruned.jsonl --tokenizer Qwen/Qwen2.5-0.5B
```
...
## Active-Learning Sampler (`kadhi data active-sample`)
...
```bash
kadhi data active-sample --input traces.jsonl --output for-review.jsonl --budget 100
```

The output JSONL is a drop-in prompt set for `kadhi eval human` (v0.19). Budget is bounded `[1, 100 000]`.

**Webhooks (v0.71.5).** `kadhi ingest`, `kadhi prune-prompt`, `kadhi ab`, and `kadhi data active-sample` all accept `--slack-url` / `--discord-url` and POST a one-line summary on completion through the same SSRF-hardened validator as `kadhi drift-alarm` (scheme allowlist, loopback-only HTTP, RFC1918 / link-local / reserved / multicast rejected; the post never raises, so a flaky webhook can't fail the command). `kadhi ab` only fires when the sequential test actually decides (`reject_h0` / `accept_h0`), not while it's still `continue`-ing.
...
kadhi data generate --prompt "Create math word problems" --count 100 --format alpaca

# Use a different model
kadhi data generate --prompt "Medical Q&A pairs" --model gpt-4o --count 500

# Deduplicate against existing data
kadhi data generate --prompt "..." --count 200 --dedup-with existing.jsonl

# Use seed examples to guide style
kadhi data generate --prompt "..." --seed examples.jsonl --count 100

# Use a local OpenAI-compatible server (kadhi serve, Ollama, etc.)
kadhi data generate --prompt "..." --provider server --api-base http://localhost:11434/v1
```
...
kadhi data generate --prompt "..." --provider ollama --model llama3.1
kadhi data generate --prompt "..." --ollama-model llama3.1  # shorthand

# Generate via Anthropic Claude API (set ANTHROPIC_API_KEY env var)
kadhi data generate --prompt "..." --provider anthropic --model claude-3-haiku-20240307

# Generate via local vLLM server
kadhi data generate --prompt "..." --provider vllm --model meta-llama/Llama-3.1-8B-Instruct
```
...
kadhi data generate --prompt "..." --template code --language Python --task-type function

# Multi-turn conversations
kadhi data generate --prompt "..." --template conversation --turns 6 --topic "science"

# QA from context document
kadhi data generate --prompt "..." --template qa --context document.txt

# Preference data (DPO/KTO/ORPO)
kadhi data generate --prompt "..." --template preference --pref-task dpo

# Chain-of-thought reasoning (GRPO)
kadhi data generate --prompt "..." --template reasoning --domain math
```
...
kadhi data generate --prompt "..." --validate

# Auto-filter by quality (coherence scoring)
kadhi data generate --prompt "..." --filter

# Auto-dedup (MinHash, requires: pip install "kadhi-cli[data]")
kadhi data generate --prompt "..." --dedup

# Full quality pipeline: validate + filter + dedup
kadhi data generate --prompt "..." --quality-pipeline
```
...
kadhi data augment ./data/train.jsonl --strategy rephrase --count 3 \
  --output ./data/train_augmented.jsonl

# Translate into multiple languages
kadhi data augment ./data/train.jsonl --strategy translate --lang es,fr,de \
  --output ./data/train_multilingual.jsonl

# Style transfer (formal / casual / technical / etc.)
kadhi data augment ./data/train.jsonl --strategy style --styles formal,casual \
  --output ./data/train_styled.jsonl

# Local provider (Ollama / vLLM) — loopback-only, pick the model + base URL
kadhi data augment ./data/train.jsonl --strategy rephrase --count 2 \
  --provider ollama --model qwen2.5:0.5b --output ./data/train_local.jsonl
```

Works with any provider supported by `kadhi data generate` (OpenAI, Ollama, vLLM, local server). `--model` and `--base-url` select a specific local model/endpoint; the Ollama/vLLM paths are loopback-only (SSRF-hardened). `--count` is capped at 10; `--lang` and `--styles` each capped at 10 entries × 32 chars.
...
kadhi data from-traces --logs ./logs/langchain.jsonl \
  --format langchain --signal thumbs_up --output prefs.jsonl

# OpenAI API logs + regeneration signal (second response wins)
kadhi data from-traces --logs ./logs/openai.jsonl \
  --format openai --signal regeneration --output prefs.jsonl

# Kadhi-serve logs + user-edit signal (edited response wins over original)
kadhi data from-traces --logs ./logs/kadhi-serve.jsonl \
  --format kadhi_serve --signal user_edit --output prefs.jsonl

# Preview generated pairs before training
kadhi data review prefs.jsonl --sample 10
```

**Supported log formats:** `langchain`, `openai`, `kadhi_serve`
...
kadhi migrate --from llamafactory llama3_lora_sft.yaml

# Import from Axolotl
kadhi migrate --from axolotl axolotl_config.yml

# Import from Unsloth notebook
kadhi migrate --from unsloth finetune.ipynb

# Preview without writing
kadhi migrate --from llamafactory config.yaml --dry-run
```
...
Kadhi supports these formats (auto-detected). Files can be JSONL, JSON, CSV, Parquet, or TXT.
...
`task: asr` and `kadhi infer --task asr`. See [Training → ASR](training.md).
...
```
Use with `data.format: pre_tokenized` and `data.tokenized_path: ./.kadhi-tokenized/<key>` after running `kadhi data preprocess`.
...
Kadhi speaks the same dataset surface as Axolotl + LlamaFactory + Unsloth — remote URIs, streaming, sharding, multi-dataset interleaving, vocab expansion, and document ingestion all live in one schema.
...
that stream, then Kadhi materialises up to 1M rows — the same shape as remote (#689).
...
- **All entries local files and/or remote URIs:** with `data.streaming: false` (default),
  ...
  SSRF-hardened `validate_remote_uri` allowlist used everywhere else in Kadhi (bucket
...
kadhi data preprocess kadhi.yaml --output ./.kadhi-tokenized

# Then in kadhi.yaml:
#   data:
#     format: pre_tokenized
#     tokenized_path: ./.kadhi-tokenized/<16-char-cache-key>
```

**Document ingestion (PDF / DOCX / MD / TXT → JSONL):**

```bash
kadhi data ingest report.pdf --output report.jsonl
kadhi data ingest README.md
kadhi data ingest notes.docx
```
...
```bash
# Inspect a dataset
kadhi data inspect ./data/train.jsonl

# Validate format (auto-detects if --format not specified)
kadhi data validate ./data/train.jsonl
kadhi data validate ./data/train.jsonl --format alpaca

# Require at least 90% of rows to be usable
kadhi data validate ./data/train.jsonl --min-valid-fraction 0.9

# Convert between formats
kadhi data convert ./data/train.jsonl --to sharegpt --output converted.jsonl

# Merge multiple datasets
kadhi data merge data1.jsonl data2.jsonl --output merged.jsonl --shuffle

# Remove near-duplicates (requires: pip install "kadhi-cli[data]")
kadhi data dedup ./data/train.jsonl --threshold 0.8

# Extended statistics (length distribution, token counts, languages)
kadhi data stats ./data/train.jsonl

# Filter by quality (perplexity + coherence scoring)
kadhi data filter ./data/train.jsonl --coherence 0.3
kadhi data filter ./data/train.jsonl --perplexity 500 --coherence 0.3
kadhi data filter ./data/train.jsonl --score-only  # add scores without filtering
```

`kadhi data validate` exits with code `0` when at least one row is usable and the
...
## Demo Datasets (`kadhi data demo`)

Tiny JSONL fixtures bundled with Kadhi so you can warm up `kadhi train` without
hunting for data:
...
kadhi data demo alpaca_demo --output ./alpaca.jsonl
```
...
`kadhi data from-traces --judge` filters harvested preference pairs through an LLM judge:

```bash
kadhi data from-traces \
...
kadhi data score --input training.jsonl

# Standalone subcommands — JSONL-in, enriched JSONL-out
kadhi data pii          --input training.jsonl --output pii_flagged.jsonl
kadhi data toxicity     --input training.jsonl --output tox_flagged.jsonl --threshold 0.1
kadhi data langdetect   --input training.jsonl --output tagged.jsonl
kadhi data educational  --input training.jsonl --output scored.jsonl
kadhi data decontaminate --input training.jsonl --benchmarks mmlu,gsm8k,humaneval --output clean.jsonl
```

The scorecard reports PII matches, abuse-keyword matches, language distribution, mean heuristic educational value, and decontamination removals. PII detection uses a narrow ReDoS-hardened regex set (email / phone / SSN / credit-card) with a 50 KB pre-cap on every input. Language detection is a stopword heuristic across six languages. `kadhi data toxicity` is retained as a compatible command name, but its output is explicitly an abuse-keyword heuristic, not a toxicity classifier. Ambiguous technical and medical terms such as process `kill`, thread `die`, and heart `attack` are not treated as standalone safety signals. This trades one known failure mode for explicit limitations: in maintainer review, 9 of 10 held-out abusive examples scored zero and 10 of 12 benign technical or editorial examples were flagged at the default threshold. Use it only for keyword triage, never as a safety decision. The default Magpie quality filter therefore applies only non-empty and educational heuristics; provide an explicit model-backed policy outside Kadhi when safety classification is required. The `[data-pro]` extra currently adds `langdetect` and Presidio only; it does not install Llama Guard or FineWeb-Edu. Decontamination uses n-gram containment against benchmark corpora: use `--benchmarks mmlu,gsm8k` for built-in allowlist, or `--benchmark-file custom_benchmark.jsonl` for your own corpus.


## Remote Datasets (S3 / GCS / Azure / OCI)

Point `data.train` at any object in the v0.42.0 fsspec allowlist and `kadhi train` will stream it through `fsspec.open` after running the URI through the same SSRF-hardened validator used everywhere else in Kadhi (bucket regex, no userinfo / query / fragment):
...
```bash
pip install kadhi-cli[remote]   # fsspec + s3fs + gcsfs + adlfs
```
...
```bash
kadhi data recipe my_recipe.yaml
```
...
```bash
kadhi data mix --optimize --budget 1h \
    --datasets dolma.jsonl,wikipedia.jsonl,arxiv.jsonl \
    --num-probes 8 --output mix_recipe.yaml
```

Writes a YAML recipe you can splice into your `kadhi.yaml`: `data.train` renders as the full ranked dataset list (index-aligned with `data.interleave.probs`), and `data.interleave` carries the searched mixture weights — as of #443, `data.interleave` is fully wired into training-time dataset loading, so `kadhi train` consumes the real N-dataset mixture this search found rather than collapsing to one path. `--budget` accepts `60s` / `5m` / `1h` / `24h`. Per-candidate proxy failures are isolated (DEBUG-logged, sentinel high loss recorded) so a single OOM combo does not abort the whole search; `partial=True` is surfaced in the report when the budget cap trips mid-loop.

Re-apply a previously written recipe:

```bash
kadhi data mix --apply mix_recipe.yaml
```

Pass `--live --base-yaml kadhi.yaml` to score each candidate with a short `kadhi train` proxy run. Without `--live`, Kadhi uses a synthetic offline proxy (quadratic penalty around the uniform simplex) so the budget tracker, optimiser surface, and recipe writer can be exercised without GPUs. `scikit-optimize` is opt-in via `OptimizerProtocol`; the default fallback is a deterministic Dirichlet sampler.


## AOT Tokenization with `kadhi data preprocess`

Pre-tokenize your dataset once and cache Arrow shards keyed by
`(dataset, tokenizer, max_length, format)`:

```bash
kadhi data preprocess kadhi.yaml --output ./tokenized_cache
```
...
## Data Recipe DAG Runner (`kadhi data recipe --execute`)

Execute a Data Recipe DAG end-to-end:

```bash
kadhi data recipe path/to/recipe.yaml --execute --output ./out \
...
## Fine-tune Doctor (`kadhi data doctor`)

Chat-template compatibility report — catches the top *silent* fine-tuning failures
before a single training step:

```bash
kadhi data doctor ./data/train.jsonl --model meta-llama/Llama-3.1-8B-Instruct
...
kadhi data doctor ./data/train.jsonl --model meta-llama/Llama-3.1-8B-Instruct --show-mask 5
```

Eight checks, same OK/MINOR/MAJOR taxonomy as `kadhi diagnose` (exit 0 on OK/MINOR,
...
select the same masking strategy `kadhi train` would use, so the report and
`--show-mask` preview can never disagree about what's actually trained.


## Preference-Data Linter (`kadhi data lint`)

Catches the top silent degradations in DPO/ORPO/SimPO/IPO/BCO/KTO preference data:

```bash
kadhi data lint ./data/prefs.jsonl
kadhi data lint ./data/prefs.jsonl --model meta-llama/Llama-3.1-8B-Instruct  # exact token-length bias, not word count
```
...
(MinHash/LSH, reuses the `kadhi data dedup` kernel; requires
`pip install "kadhi-cli[data]"`, degrades to an advisory skip otherwise),
