...
```bash
for s in 1 2 3; do
  kadhi train --config kadhi.yaml --output "runs/seed-$s" \
...
`torch.use_deterministic_algorithms(True)`, which Kadhi does not set for you.
...
`kadhi`'s classifier trainer has read as "no adapter" since v0.71.12, and the one
`kadhi card` already resolves to a dense model rather than an adapter — so the
model card, the registry entry and the trainer all agree without extra flags. On
a rank of 0 the SFT trainer skips `get_peft_model` entirely: `kadhi train` prints
...
"LoRA off" modes, [Spectrum](#spectrum--targeted-training-on-layer-snr-kadhi-spectrum-scan-v07123)
...
| `unfrozen_parameters` ([Spectrum](#spectrum--targeted-training-on-layer-snr-kadhi-spectrum-scan-v07123)) | a hand-picked / SNR-ranked set | you want full-FT quality on the layers that matter |
...
```bash
kadhi train --config new_task.yaml --replay old_task.jsonl --replay-ratio 0.1
```
...
kadhi train --config grpo.yaml --reward-hack-mitigation kl_control   # raise KL, recover
kadhi train --config grpo.yaml --reward-hack-mitigation log_only     # observe only, no action
...
kadhi train --config kadhi.yaml --uld-strategy wasserstein
...
kadhi train --config kadhi.yaml --minillm-enabled \
...
kadhi train --config kadhi.yaml --minillm-enabled --minillm-on-policy

# Mid-epoch checkpoint for PPO/GRPO — TorchTune punts this; Kadhi ships it
kadhi train --config grpo.yaml \
...
kadhi train --config unlearn.yaml --yes

# Score the run on TOFU / MUSE / WMDP (OK / MINOR / MAJOR verdict).
kadhi eval unlearning <run-id> --benchmark tofu --evidence evidence.json --output report.json
```
...
FP32 intermediates cost time and memory. The maintainer's PR #736 measurement
...
Kadhi ships live math kernels for 6 GRPO objective variants in addition to the
...
training:
  prm_reward: ./my-prm       # a `kadhi train task=prm` checkpoint dir (or HF id)
...
default; `prod` assumes calibrated `[0,1]` step scores (Kadhi's PRM head is
...
training:
  ...
  rollout_func: kadhi_cli.envs.calculator:rollout   # or retrieval_qa / guess_number
...
The judge is Kadhi's own OpenAI-compatible `JudgeEvaluator` adapted to TRL's
...
Kadhi auto-detects MoE architectures. Works with all training tasks.

```bash
kadhi init --template moe
kadhi train
```
...
pip install "kadhi-cli[vision]"

# Create a vision config
kadhi init --template vision
...
`kadhi data inspect` automatically shows image statistics (count, formats, missing files) for vision datasets.
...
pip install "kadhi-cli[audio]"

# Create an audio config
kadhi init --template audio
...
```bash
kadhi infer --task asr --model ./out --input eval.jsonl --output preds.jsonl --audio-dir ./data/audio
...
to an `asr_generation.json` sidecar so `kadhi infer --task asr` restores them
...
Kadhi ships seven GRPO objective variants, between-rollouts vLLM standby, four agent-rollout backends, seven stability/efficiency knobs, plus Process Reward Models and Vision-RL.

```yaml
# kadhi.yaml — DAPO with replay buffer and TIS truncation masking
...
For GRPO, Kadhi preserves source dataset columns and TRL passes them to reward functions as
...
one finite numeric score per completion; Kadhi checks this contract and reports the reward name
...
### Synthesize a verifier from your data (`kadhi reward synth`)

Don't hand-write a verifier — generate one from reference (gold) outputs. Kadhi infers a
...
kadhi reward synth references.jsonl -o reward.py --output-report calib.json

# preview the induced spec without writing anything
kadhi reward synth references.jsonl --plan-only

# force a family instead of auto-detecting
kadhi reward synth answers.jsonl -o reward.py --kind numeric --tolerance 1e-6
```
...
### Stress-test a verifier for gameability (`kadhi reward stress`)

A verifier that passes calibration still might pay out for junk. `kadhi reward stress` feeds the
...
kadhi reward stress reward.py --references golds.jsonl --output-report stress.json

# probe a builtin verifier instead of a .py file
kadhi reward stress verifiable --verifiable-domain math --references golds.jsonl

# JSON-schema references may be stored as objects in a `schema` field
kadhi reward stress verifiable --verifiable-domain json_schema \
    --references schemas.jsonl --field schema

# tune the attack set / accept threshold / gameability tolerance
kadhi reward stress reward.py --references golds.jsonl \
...
> **Note:** `code` domain runs untrusted generations. Kadhi sandboxes aggressively but never trust it for production-grade isolation — run in a VM or container for public data.
...
Arguments are parsed as JSON only — never `eval()`. `kadhi eval custom` can score tool-call accuracy (function name + argument JSON equality).

```bash
kadhi init --template tool-calling
```
...
controls optimization passes within each PPO update. Kadhi forwards both values,
...
kadhi recipes show raft-llama3-8b
kadhi recipes use raft-llama3-8b
```

Citation scoring is exposed as a pure kernel for the eval gate:

```python
from kadhi_cli.utils.citation_faithful import score_citations
...
kadhi eval citation preds.jsonl --style bracket
# RAFT rows: pass the train-time shuffle seed so the golden id lines up
kadhi eval citation raft.jsonl --shuffle-seed 0 --output citation.json
```

`kadhi diagnose` also gains a `citation` failure mode that flags a model that stopped citing the supporting document.
...
```bash
# Stage 1 — train the retriever (uses Kadhi's v0.16 embedding trainer)
kadhi recipes use ra-dit-retriever
kadhi train

# Stage 2 — train the generator on RAFT data, pointing at the retriever
kadhi recipes use ra-dit-llama3-8b
kadhi train
```
...
Visualise the recorded bucket-weight evolution with `kadhi runs curriculum-curve <run_id>`.
...
`spark-tts`, `oute-tts` — copy with `kadhi recipes use <name>`. Cross-validators
...
## Spectrum — Targeted Training on Layer SNR (`kadhi spectrum scan`, v0.71.23)

Spectrum (arXiv:2406.06623) fine-tunes only the layers with the most signal. `kadhi spectrum scan`
...
```bash
kadhi spectrum scan --model HuggingFaceTB/SmolLM2-135M --top-percent 25 --modules mlp,attn -o patch.yaml
```

```yaml
# patch.yaml — paste into your kadhi.yaml
training:
  unfrozen_parameters:
...
a conflicting combo is rejected loudly at config load. Scans cache under `~/.kadhi/spectrum/`
(override with `KADHI_SPECTRUM_CACHE_DIR`); `--no-cache` skips it. `--modules mlp,attn` (vs the
