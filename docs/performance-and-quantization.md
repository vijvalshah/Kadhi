...
- **Post-training quantization** (default): Faster training, good enough for most use cases. Quantize after training with `kadhi export --quant q4_k_m`.

QAT works with all training tasks (SFT, DPO, GRPO, PPO, KTO, ORPO, SimPO, IPO, Pretrain) and vision modality. Not compatible with the unsloth backend. After QAT training, export to GGUF normally with `kadhi export`.
...
```bash
pip install "kadhi-cli[qat]"   # torchao >= 0.5.0 includes torchao.float8
```
...
```bash
pip install "kadhi-cli[cce]"    # or: pip install cut-cross-entropy
```
...
`training.packing_cross_doc_attn_mask` is rejected at config load. Kadhi used to set TRL `packing_strategy="attention_free"`, which has never been a valid strategy on any released trl (allowlist is `bfd` / `bfd-requeue` / `wrapped`), so the flag has always been a `TypeError` at setup rather than a working mask.
...
Pick the right quantization format for your base model and hardware. Kadhi
...
# FSDP + QLoRA — Kadhi resolves this to the compute dtype automatically;
...
**Compatibility matrix.** `kadhi train` runs `check_quant_distributed_compat()` at
...
would make the shared layer buffer as large as the whole table. Kadhi keeps the
...
copies the table into Kadhi's shard cache. For dense Transformers checkpoints,
...
oMLX/oQ affine Qwen4 bundles are supported by this narrow text-only path. Kadhi
...
Kadhi's optional NF4 transform, not the accepted oQ source encoding).
...
to the MPS allocator while `is_pinned()` is still false (#434). Kadhi refuses that state at
...
- **Apple APFS disk detection.** On macOS, an APFS volume may report `Apple Fabric`
  even when its physical store is Apple's internal NVMe. Kadhi resolves the target
...
- **The bf16 3B throughput above is a LOWER BOUND.** The reference box could not page-lock the 5.55 GB base (its measured page-locked ceiling is 7.65 GB, and a CUDA context plus the model skeleton did not leave room), so that run fell back to a pageable store. Pageable memory makes the host-to-device copy synchronous, which costs overlap — visible as the GPU-utilisation drop from 96.8% (1.5B, pinned) to 79.3% (3B, pageable). Kadhi does this fallback automatically **and prints the cost** rather than absorbing it silently. NF4 lifts this at 3B: the store drops under the ceiling and pins.
- Numbers are Windows/WDDM and therefore systematically pessimistic versus Linux. `expandable_segments:True` is silently ignored on Windows; Kadhi detects that and does not claim it is active.
...
Refusing on a non-CUDA target would make `stream_pin: true` uncommittable to a `kadhi.yaml`
...
there now — a `kadhi.yaml` carrying `stream_pin: true` that used to warn on the disk tier can
refuse instead.
...
So `kadhi train` predicts peak VRAM before building the model, and **refuses a run it
expects not to fit**:
...
clock. A per-card constant compiled into Kadhi would be a fabrication. Real streamed runs landed at
...
`batch_size` until the VRAM pre-flight refuses, then accumulate for the rest.** Kadhi
...
> **v0.72.0 adapters are unloadable — re-run them on v0.72.1.** In v0.72.0 a streamed run saved every adapter tensor under a key carrying an extra `.inner.` segment, so `kadhi merge`, `kadhi serve`, `kadhi chat` and `PeftModel.from_pretrained` loaded **zero** tensors and silently returned the untuned base (PEFT emitted only a `UserWarning`). The training itself was correct — only the saved file was affected. Check with:
>
> ```bash
...
- **"trainable LoRA parameters remain on the meta device"** — PEFT attached an
  adapter without real storage and Kadhi refused the run before installing the
  streaming runtime. This guard is deliberately based on the final parameter
  state rather than on how many tensors Kadhi materialised: some PEFT versions
...
- **"base exceeds the physical RAM safety ceiling"** — `stream_source: auto` fell back to the NVMe disk tier because the RAM tier would keep the store plus resident extras above Kadhi's physical-host ceiling. Set `stream_source: ram` only when you want that case to refuse instead of falling back.
...
second model instance that doubles memory and there is no point streaming at all. Kadhi
...
- **`kto` needs `batch_size: 2` or more.** TRL's KL term is degenerate at batch 1, so
  the run cannot work; Kadhi refuses it when your config is read rather than after
...
**Disk pre-flight and shard cache.** Before Kadhi materialises or shards a checkpoint, it
reports the complete projected footprint: the HF/local source, any regular-file copy Kadhi
...
roots with `KADHI_SPECTRUM_CACHE_DIR` and `KADHI_LAYER_STREAM_CACHE_DIR`; both retain Kadhi's
home/cwd/tmp containment policy.

Hugging Face snapshots normally expose symlinks into their blob cache, which the sharder
deliberately does not follow. Kadhi materialises those weights under its Spectrum cache. If the
HF cache already exposes real files, Kadhi now reads them in place instead of creating a second
copy. The layer shards remain under `~/.kadhi/layer-stream/`. Their index records each source
...
snapshot is complete. Kadhi pins the commit resolved by the initial cache lookup and copies
...
Four silent-failure modes Kadhi had → loud failures.

### Assistant-only loss masking

By default, Kadhi masks every non-assistant token with `-100` so the SFT loss reflects only what the model should *generate*. Toggle via `data.train_on_responses_only` (default `true`):
...
When the tokenizer ships a chat template with `{% generation %}` markers, the mask is exact. Without those markers, Kadhi falls back to an incremental tokenize-delta walk and documents the looseness.

**On the MLX backend the masking differs, and the two backends are not comparable**
(#683). MLX supervises every
...
causal-loss target. Kadhi rejects the row by split and row number when
...
Coverage:
- `kadhi train` (every task — SFT, DPO, GRPO, KTO, ORPO, SimPO, IPO, PPO, Reward Model, Pretrain, Embedding, BCO, and the unified Preference dispatcher)
- `kadhi chat`, `kadhi serve`, `kadhi data download`, `kadhi eval auto`
- `kadhi diff`, `kadhi export`, `kadhi merge`, `kadhi infer`, `kadhi data generate`

```bash
kadhi train --config kadhi.yaml --trust-remote-code
kadhi infer --model my-org/custom-arch-model --input prompts.jsonl --trust-remote-code
kadhi export --model ./adapter --format gguf --trust-remote-code
```

`kadhi serve` resolves this gate **once** and hands the result to whichever backend
...
Replaces the static memory formula with a real try-halve-then-double-to-ceiling loop. Picked size is cached at `~/.kadhi/batch_cache.json` keyed on `(model, max_length, quantization, lora_r, gpu_name, gpu_memory_gb)` so repeat runs short-circuit.
...
better than Kadhi can, and refusing here would reject files DeepSpeed accepts.
...
kadhi train --config kadhi.yaml --gpus auto

# Explicit GPU count
kadhi train --config kadhi.yaml --gpus 4
```

`kadhi` detects NVLink / PCIe interconnect and prints the correct
...
```yaml
# kadhi.yaml — performance options
training:
...
```bash
pip install "kadhi-cli[liger]"     # Liger Kernel fused operations
pip install flash-attn --no-build-isolation  # FlashAttention
pip install "kadhi-cli[ring-attn]" # Ring FlashAttention (sequence parallelism)
```


## Live CUDA Batch-Size Probe

Set `auto_batch_size_strategy: probe` in `training:` and Kadhi will run a real OOM-probe before training:
...
kadhi merge-sharded-fsdp-weights ./fsdp-checkpoint -o ./merged.safetensors --plan-only

# Consolidate for real
kadhi merge-sharded-fsdp-weights ./fsdp-checkpoint -o ./merged.safetensors
```
...
kadhi export --model ./output --format tq1_0     # same flavour, explicit name
```
...
```bash
kadhi recipes use falcon-e-bitnet-sft
kadhi train --config kadhi.yaml
```
...
```bash
kadhi serve --model ./output --kv-cache-type bf16     # cache stored in the model compute dtype
kadhi serve --model ./output --kv-cache-type q8_0     # 8-bit quantized KV cache (needs `hqq`)
```
...
- vLLM / SGLang serve wiring is still tracked under #140 (`infra-blocked`).
...
`kadhi merge --save-format 4bit` and `--save-format 4bit_forced` will write a single BNB-4bit-quantized merged checkpoint without the wasteful dequant → merge → requant cycle (unsloth `merged_4bit` recipe). v0.53.0 ships the closed allowlist + spec metadata; the live writer lands in v0.53.1.

`kadhi export --format torchao --quant-config <yaml>` is the planned PTQ export surface for `torchao.quantize_` + `save_pretrained`. Four schemes are allowlisted: `Int4WeightOnly`, `Int8DynActInt4`, `Float8DynActFloat8`, `NVFP4`. CASE-SENSITIVE — these are PyTorch class names and `torchao.quantize_` looks them up by exact name. Diverges from `--save-format` (lowercase-normalised) on purpose; documented at both validators.
...
kadhi merge -a ./adapter -o ./merged_4bit --save-format 4bit
...
kadhi deploy autopilot --target rtx-4090-24gb \
...
`kadhi deploy autopilot --measure` caches results at `~/.kadhi/deploy_autopilot_cache.json` keyed on `(base, profile, eval-tasks)`. Repeat invocations short-circuit; pass `KADHI_DEPLOY_AUTOPILOT_CACHE=<path>` to redirect (constrained to home / cwd / tempdir). The recommended candidate uses soft-fallback: first `OK` by insertion order, else the candidate with the smallest delta (least drop relative to its own baseline).
