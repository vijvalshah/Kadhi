...
# Install Python 3.10 (Ubuntu 22.04 native — Kadhi supports 3.9+).
...
# Install Kadhi from PyPI (always the latest published release, not local source).
# v0.71.0 split the training stack into the [train] extra — include it so the
# GPU image can still fine-tune.
RUN pip install --no-cache-dir "kadhi-cli[train,serve,data,eval]"
