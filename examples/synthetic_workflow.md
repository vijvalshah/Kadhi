...
Generate, filter, score, and train on synthetic data — all with `kadhi`. Pairs
...
`kadhi data generate --help` matrix.
...
```bash
kadhi data filter ./synth_raw.jsonl \
...
against your downstream evals. `kadhi data score` reports a scorecard to the
terminal and does not write a file, so the decontamination step reads the
filtered set:

```bash
kadhi data score --input ./synth_filtered.jsonl
kadhi data decontaminate \
...
Point `kadhi train` at `synthetic_workflow.yaml`:

```bash
kadhi train --config examples/synthetic_workflow.yaml --yes
```
