...
column is what `kadhi train` predicts for them, margin included.

Run any of them:

```bash
kadhi train --config examples/configs/sft_basic.yaml
```

Validate a config without training:

```bash
kadhi train --config examples/configs/sft_basic.yaml --dry-run
```
...
`kadhi train` estimates peak VRAM before it loads anything and refuses to start a run it
...
```bash
kadhi train --config examples/configs/rlhf_step1_sft.yaml
kadhi train --config examples/configs/rlhf_step2_reward.yaml
kadhi train --config examples/configs/rlhf_step3_ppo.yaml
```
...
```bash
kadhi train --config examples/configs/vision_llama.yaml
```

### Built-in templates

`kadhi init` writes a starter `kadhi.yaml` you fill in with your own data:

```bash
kadhi init --template kto     # unpaired preference (thumbs up/down labels)
kadhi init --template orpo    # reference-free alignment
kadhi init --template simpo   # length-normalized preference
kadhi init --template ipo     # regularized preference (squared hinge)
kadhi init --template pretrain    # continued pre-training on raw text
kadhi init --template moe         # Mixture-of-Experts (Qwen3, Mixtral, DeepSeek V3)
kadhi init --template longcontext # RoPE scaling for 128k+ context
kadhi init --template embedding   # sentence embeddings (BGE, E5, GTE)
kadhi init --template vision      # vision-language
kadhi init --template audio       # audio/speech — needs pip install "kadhi-cli[audio]"
kadhi train
```

Full list: `kadhi init --help`. Available templates are `audio`, `bco`, `chat`, `code`,
...
functions for the closed-loop mitigation feature (`kadhi train
...
Kadhi auto-detects and normalizes:
...
```bash
kadhi data inspect examples/data/alpaca_tiny.jsonl
```
...
```bash
kadhi data convert examples/data/alpaca_tiny.jsonl \
  --to chatml \
...
```bash
kadhi train --config your_config.yaml
```

## Config shape

Configs are nested: `base`, `task`, `data`, `training`, `output` at the top level.
[`config/schema.py`](../src/kadhi_cli/config/schema.py) is the single source of truth.
...
```bash
kadhi infer --model ./output_sft_basic/ --input prompts.jsonl --output results.jsonl
```

### Merge the LoRA adapter into a full model

```bash
kadhi merge --adapter ./output_sft_basic/ --output ./merged_model/
```

### Export to GGUF

```bash
kadhi export --model ./output_sft_basic/ --format gguf --quant q8_0 --output model.gguf
```

Requires a built llama.cpp — see [docs/serving-and-export.md](../docs/serving-and-export.md).
`kadhi export --model ... --deploy ollama` exports and registers with Ollama in one step.

### Monitor with Weights & Biases

```bash
pip install wandb
kadhi train --config your_config.yaml --wandb
```
...
- Paths in a config resolve from the directory you run `kadhi` in, not from the config's
  location. Run from the repo root, or use an absolute path.
- Check the file parses: `kadhi data validate your_data.jsonl`
...
`kadhi train` validates before doing any work. `base: Field required` or
