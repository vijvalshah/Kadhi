...
**Kadhi** is a CLI-first LLM fine-tuning tool. Python 3.10+, Apache-2.0.
...
ruff check src/kadhi_cli/ tests/  # Lint — must be clean before any commit
```
...
- **Config** is Pydantic v2 in `src/kadhi_cli/config/schema.py` — single source of truth.
...
- **Feature reference** lives in [`docs/`](docs/README.md) — per-topic guides plus the full `kadhi` command list ([`docs/commands.md`](docs/commands.md)). Read the relevant page before changing a feature.
...
- The Pydantic config schema in [`src/kadhi_cli/config/schema.py`](src/kadhi_cli/config/schema.py) is the single source of truth for every config field.
