...
project policy — ``python -m kadhi_cli.cli --help`` must not pull torch.
"""
...
def make_mole_trainer_class(base_cls: type) -> type:
    ...
    class _MoleTrainer(base_cls):  # type: ignore[misc, valid-type]
        ...
        def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
            ...
            adapter_names = model._kadhi_mole_adapter_names
...
class MoleRoutingTrainerWrapper:
    """High-level wrapper for MoLE per-token routing from a KadhiConfig.
    ...
    def setup(self, dataset: dict) -> None:
        ...
        model._kadhi_mole_adapter_names = adapter_names
