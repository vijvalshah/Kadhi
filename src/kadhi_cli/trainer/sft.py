    ...
    if os.path.isfile(metadata_path):
        ...
        if stored_key != current_key:
            raise ValueError(
                ...
                "re-run `kadhi data preprocess`"
            )
    else:
        console_obj.print(
            "[yellow]pre-tokenized cache has no metadata.json — assuming "
            "compatible; consider re-running `kadhi data preprocess`[/]"
        )
...
class SFTTrainerWrapper(StreamingSetupMixin):
    """High-level wrapper that sets up model + tokenizer + trainer from KadhiConfig."""
    ...
    def setup(self, dataset: dict):
        ...
        if batch_size == "auto":
            from kadhi_cli.utils.batch_probe import pick_batch_size
        ...
        if tcfg.curriculum:
            from kadhi_cli.utils.curriculum import sort_by_length
        ...
        if self._is_raft:
            ...
            from kadhi_cli.trainer.raft import (
                RaftDataCollator,
        ...
        from kadhi_cli.utils.peft_wiring import attach_compile_prefix_callback
    ...
    def _prepare_raft_dataset(self, dataset: dict, cfg, tcfg):
        ...
        (:func:`kadhi_cli.utils.raft.build_raft_prompt`) then tokenised with the
        prompt span masked. When ``citation_faithful`` is set, citation spans
        in the answer get a boosted ``loss_weights`` entry.
        """
        from datasets import Dataset

        from kadhi_cli.utils.raft import build_raft_prompt, tokenize_raft_example
    ...
    def _prepare_raft_raw_dataset(self, dataset: dict, cfg, tcfg):
        ...
        :class:`~kadhi_cli.trainer.raft.RaftEpochShuffleCollator` re-composes +
    ...
    def _resolve_mixed_precision(self, tcfg, base_model: str) -> tuple[bool, bool]:
        ...
        *"Your setup doesn't support bf16/gpu. You need Ampere+ GPU with
        ...
        from kadhi_cli.utils.mixed_precision import pick_mixed_precision
    ...
    def _setup_transformers(self, cfg, tcfg):
        ...
        if tcfg.use_cut_ce:
            from kadhi_cli.utils.cut_ce import NO_MATCHING_ARCHITECTURE_MESSAGE, apply_cut_ce
        ...
        if tcfg.freeze_layers is not None or tcfg.freeze_ratio is not None:
            from kadhi_cli.utils.freeze import freeze_model_layers
        ...
        if tcfg.unfrozen_parameters:
            from kadhi_cli.utils.freeze import apply_unfrozen_parameters
    ...
    def _apply_quantization_aware(self, tcfg) -> None:
        ...
        if tcfg.quantization_aware == "fp8":
            from kadhi_cli.utils.fp8 import apply_fp8_training
    ...
    def _prepare_audio_dataset(self, dataset: dict):
        ...
        try:
            ...
            raise ImportError(
                ...
                "Install with: pip install \"kadhi-cli[audio]\""
            )
    ...
    def train(
        ...
        from kadhi_cli.utils.ebft_gdpo import attach_ebft_compute_loss
        ...
        with self._training_context(
            ...
            if getattr(tcfg, "use_longlora", False) and self.config.backend == "transformers":
                from kadhi_cli.utils.longlora import apply_longlora_forward_override
        ...
        if getattr(self.config.training, "use_fsdp2_compile", False) and getattr(
            ...
            from kadhi_cli.utils.peft_wiring import strip_compile_prefix
