...
The dispatcher constructs a temporary :class:`KadhiConfig` view with the
...
def is_multi_objective_preference(cfg: KadhiConfig) -> bool:
...
def get_loss_weights(cfg: KadhiConfig) -> Optional[dict]:
...
def resolve_preference_loss(cfg: KadhiConfig) -> Optional[str]:
...
def _make_inner_cfg(cfg: KadhiConfig, loss: str) -> KadhiConfig:
    """Build a KadhiConfig view with task=<loss> so the inner wrapper can run.
...
class PreferenceTrainerWrapper:
    ...
    def _build_multi_objective(self):
        ...
        The math kernel ships in :mod:`kadhi_cli.utils.preference_combine`
        ...
        if primary == "dpo":
            from kadhi_cli.trainer.dpo import DPOTrainerWrapper as PrimaryWrapper
        elif primary == "simpo":
            from kadhi_cli.trainer.simpo import SimPOTrainerWrapper as PrimaryWrapper
        elif primary == "orpo":
            from kadhi_cli.trainer.orpo import ORPOTrainerWrapper as PrimaryWrapper
        elif primary == "ipo":
            from kadhi_cli.trainer.ipo import IPOTrainerWrapper as PrimaryWrapper
        elif primary == "bco":
            from kadhi_cli.trainer.bco import BCOTrainerWrapper as PrimaryWrapper
    ...
    def setup(self, dataset: dict) -> None:
        ...
        if is_multi_objective_preference(self.config):
            ...
            from kadhi_cli.utils.preference_combine import validate_weight_compat
