...
def make_judge_reward_func(evaluator: object, *, name: str = "kadhi_judge"):
    """Build a trl-1.x OnlineDPO POINTWISE reward function from a Kadhi evaluator.
...
def _base_pairwise_judge_cls() -> type:
    ...
    try:
        ...
        raise ImportError(
            "KadhiPairwiseJudge needs trl with BasePairwiseJudge support "
            '(pip install "kadhi-cli[train]")'
        ) from exc


def make_kadhi_pairwise_judge(evaluator: "PairwiseJudge") -> "BasePairwiseJudge":
    ...
    class _KadhiPairwiseJudge(base_cls):  # type: ignore[misc, valid-type]
    ...
    return _KadhiPairwiseJudge(evaluator)
