"""Unsloth notebook (.ipynb) → Kadhi config migration.
...
# Trainer class → Kadhi task mapping
_TRAINER_MAP = {
    "SFTTrainer": "sft",
    "DPOTrainer": "dpo",
    "GRPOTrainer": "grpo",
...
# Trainer config class → Kadhi task mapping
_CONFIG_MAP = {
    "DPOConfig": "dpo",
    "GRPOConfig": "grpo",
    "KTOConfig": "kto",
    ...
    for node in ast.walk(tree):
        ...
        if func_name == "from_pretrained":
            ...
            if kwargs.get("packing"):
                warnings.append(
                    "packing=True is not supported in Kadhi. "
            ...
            if "max_steps" in kwargs:
                warnings.append(
                    ...
                    "Kadhi uses epochs; set training.epochs instead."
                )
