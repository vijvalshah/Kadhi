"""Axolotl config → Kadhi config migration."""
...
# Axolotl dataset type → Kadhi format
_FORMAT_MAP = {
    "alpaca": "alpaca",
    "sharegpt": "sharegpt",
...
def migrate_axolotl(config_path: Path) -> Dict[str, Any]:
    ...
    if datasets:
        ...
        if len(datasets) > 1:
            warnings.append(
                ...
                "Kadhi uses a single data.train path — merge datasets with "
                "'kadhi data merge' first."
            )
    else:
    ...
    if raw.get("sample_packing"):
        warnings.append(
            "sample_packing is not supported in Kadhi. "
            "Sequences will be padded individually."
        )
    if raw.get("wandb_project"):
        warnings.append(
            f"wandb_project: {raw['wandb_project']}. Use --wandb flag with kadhi train."
        )
