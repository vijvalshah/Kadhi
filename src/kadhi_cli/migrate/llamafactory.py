"""LLaMA-Factory config → Kadhi config migration."""
...
from kadhi_cli.migrate.common import to_number

# LLaMA-Factory stage → Kadhi task mapping
_STAGE_MAP = {
    "sft": "sft",
    "pt": "pretrain",
    "rm": "reward_model",
...
# LLaMA-Factory pref_loss → Kadhi task override (when stage=dpo)
_PREF_LOSS_MAP = {
    "sigmoid": "dpo",
    "orpo": "orpo",
    "simpo": "simpo",
...
def migrate_llamafactory(config_path: Path) -> Dict[str, Any]:
    """Parse a LLaMA-Factory YAML config and return a Kadhi config dict.
    ...
    if finetuning_type == "freeze":
        warnings.append(
            "finetuning_type: freeze is not supported in Kadhi. "
            "Using LoRA instead."
        )
        include_lora = True
    ...
    if finetuning_type == "full":
        warnings.append(
            ...
            "Kadhi will train all parameters."
        )
    ...
    if raw.get("bf16") or raw.get("fp16"):
        warnings.append("bf16/fp16 is auto-detected in Kadhi (no manual setting needed)")
    if raw.get("deepspeed"):
        warnings.append(
            f"DeepSpeed config: {raw['deepspeed']}. Use --deepspeed flag with kadhi train."
        )
    ...
    if raw.get("neftune_noise_alpha") is not None:
        warnings.append(
            ...
            "Add training.neftune_alpha in kadhi.yaml if supported."
        )
    template_name = raw.get("template")
    if template_name:
        warnings.append(f"Original template: {template_name} (auto-detected in Kadhi)")
