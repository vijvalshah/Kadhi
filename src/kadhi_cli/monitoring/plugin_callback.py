...
Bridges :mod:`kadhi_cli.plugins` registered hooks into the HF Trainer
...
def _collect_active_hooks() -> list[tuple[str, dict[str, Any]]]:
    ...
    Lazy-imports :mod:`kadhi_cli.plugins` so the callback module stays
    cheap to import in CI / non-training contexts.
    """
    from kadhi_cli.plugins import discover_hooks, list_plugins, load_plugins
...
def _build_callback_class() -> type:
    """Construct the ``KadhiPluginCallback`` class with transformers as parent.
    ...
    class KadhiPluginCallback(TrainerCallback):
        """Fans HF Trainer events out to every enabled Kadhi plugin."""
    ...
    return KadhiPluginCallback


def build_plugin_callback() -> Any:
    """Return a new ``KadhiPluginCallback`` instance, or ``None`` if no
