...
Every Kadhi command appends one line to ``~/.kadhi/audit.jsonl`` (override
via ``KADHI_AUDIT_LOG_PATH``). Lines are JSON objects with a fixed set of
...
def _validate_log_path_override(override: str) -> Optional[str]:
    """Validate the ``KADHI_AUDIT_LOG_PATH`` env override.

    Per v0.36.0 ``KADHI_BATCH_CACHE_PATH`` / v0.54.0 ``KADHI_ADVISE_HISTORY_PATH``
    ...
    if _CTRL_RE.search(override):
        _LOG.warning(
            "KADHI_AUDIT_LOG_PATH contains null/control chars; falling back to default"
        )
        return None
    if len(override) > 4096:
        _LOG.warning("KADHI_AUDIT_LOG_PATH too long; falling back to default")
        return None
    try:
    ...
    for allowed in (home, cwd, tmpdir):
        ...
        if common == allowed:
            return override
    _LOG.warning(
        "KADHI_AUDIT_LOG_PATH %r outside $HOME / $CWD / $TMPDIR; "
...
def default_log_path() -> str:
    """Resolve the audit log path (env override first, else ``~/.kadhi/audit.jsonl``).
    ...
    """
    try:
        override = os.environ.get("KADHI_AUDIT_LOG_PATH")
    except ValueError:
    ...
    return os.path.join(home, ".kadhi", "audit.jsonl")
