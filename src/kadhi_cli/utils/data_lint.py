...
from kadhi_cli.utils.data_doctor import VERDICTS, sample_indices
...
def check_near_duplicates(
    ...
    try:
        ...
        return LintCheck(
            ...
            evidence="pip install \"kadhi-cli[data]\" to enable",
        )
    if len(rows) < 2:
        return LintCheck(
...
def run_lint(
    ...
    *,
    ...
    if resolved_fmt not in _SUPPORTED_FORMATS:
        raise ValueError(
            f"kadhi data lint only supports preference-shaped data (dpo/kto); "
