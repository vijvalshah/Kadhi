...
def attach_ebft_compute_loss(trainer: object, tcfg: object) -> bool:
    ...
    Idempotent: a sentinel attribute (``_kadhi_ebft_wrapped``) on the trainer
    ...
    if variant is None:
        return False
    if getattr(trainer, "_kadhi_ebft_wrapped", False):
        return False
    ...
    trainer._kadhi_ebft_wrapped = True  # type: ignore[attr-defined]
    return True
...
def attach_gdpo_compute_loss(trainer: object, tcfg: object) -> bool:
    ...
    if variant is None:
        return False
    if getattr(trainer, "_kadhi_gdpo_wrapped", False):
        return False
    ...
    trainer._kadhi_gdpo_wrapped = True  # type: ignore[attr-defined]
    return True
