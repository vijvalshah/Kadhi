...
def _energy_properties(entry: BomEntry) -> list[dict]:
    ...
    if entry.energy_kwh is not None:
        props.append({"name": "kadhi:energy_kwh", "value": str(entry.energy_kwh)})
    if entry.co2_kg is not None:
        props.append({"name": "kadhi:co2_kg", "value": str(entry.co2_kg)})
    if entry.pue is not None:
        props.append({"name": "kadhi:pue", "value": str(entry.pue)})
    if entry.grid_intensity_g_per_kwh is not None:
        props.append({
            "name": "kadhi:grid_intensity_g_per_kwh",
            "value": str(entry.grid_intensity_g_per_kwh),
        })
    if entry.energy_source is not None:
        props.append({"name": "kadhi:energy_source", "value": entry.energy_source})
    return props
...
def _energy_annotations(entry: BomEntry, annotation_date: str) -> list[dict]:
    ...
    Mirrors :func:`_energy_properties` (same ``kadhi:<field>=<value>`` naming) so
    energy data lands in the SPDX output as well as CycloneDX (the #244 contract
    is "both outputs").
    """
    return [
        {
            "annotator": "Tool: kadhi-cli",
...
def build_cyclonedx_bom(entry: BomEntry) -> dict:
    ...
    properties = [
        {"name": "kadhi:task", "value": entry.task},
        {"name": "kadhi:config_sha256", "value": entry.config_sha},
    ]
    if entry.data_sha:
        properties.append({"name": "kadhi:data_sha256", "value": entry.data_sha})
    ...
    doc = {
        ...
        "metadata": {
            ...
            "tools": [{"name": "kadhi-cli", "version": _read_kadhi_version()}],
            "component": {
                "type": "machine-learning-model",
                "name": entry.name,
                "version": entry.version,
                "bom-ref": f"kadhi:{entry.name}@{entry.version}",
                **({"licenses": licenses} if licenses else {}),
                "properties": [
                    {"name": "kadhi:base_model", "value": entry.base_model},
                    {"name": "kadhi:task", "value": entry.task},
                ],
            },
...
def build_spdx_bom(entry: BomEntry) -> dict:
    ...
    doc = {
        ...
        "name": entry.name,
        "documentNamespace": f"https://kadhi.local/bom/{entry.name}-{secrets.token_hex(8)}",
        "creationInfo": {
            "created": entry.created_at,
            "creators": [f"Tool: kadhi-cli-{_read_kadhi_version()}"],
        },
