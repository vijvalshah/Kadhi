...
(:func:`kadhi_cli.utils.embed.embed_texts`), so cosine is a plain dot
product.
...
def greedy_semdedup(
    ...
    if n_rows > _MAX_SEMDEDUP_ROWS:
        raise ValueError(
            ...
            f"is {_MAX_SEMDEDUP_ROWS}. Sample first (`kadhi data sample`) — "
            "Kadhi refuses rather than silently subsampling."
        )
