...
Workflow:
1. ``kadhi plan --config kadhi.yaml`` writes ``kadhi.tfstate`` with a
...
- ``DEFAULT_STATE_FILE = "kadhi.tfstate"``.
"""
...
@dataclass(frozen=True)
class TrainingState:
    """Persisted plan + apply metadata. Written as ``kadhi.tfstate``."""
...
def build_plan(config: Mapping[str, Any]) -> TrainingPlan:
    """Render a TrainingPlan from a kadhi-config-shaped dict."""
    if not isinstance(config, Mapping):
        raise TypeError(f"config must be a mapping, got {type(config).__name__}")
