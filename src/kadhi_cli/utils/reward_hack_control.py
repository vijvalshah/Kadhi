...
:mod:`kadhi_cli.utils.reward_hacking` detector) and SELF-CORRECTS instead of
only halting:
...
- ``RewardHackMitigationCallback`` lazily resolves ``transformers.TrainerCallback``
  (falls back to ``object``), mirroring :mod:`kadhi_cli.utils.reward_hacking`.

Security:
- ``MitigationLogWriter`` mirrors :class:`kadhi_cli.monitoring.trace_logger.TraceLogWriter`:
...
from kadhi_cli.monitoring.trace_logger import redact_value
...
def _completion_text(completion: Any) -> str:
    ...
    from kadhi_cli.utils.rl_signal_buffer import _completion_to_text
...
def _shaping_penalty(kind: str, text: str, *, sentinel: str) -> float:
    ...
    if kind == "repetition":
        ...
        if not tokens:
            return 0.0
        from kadhi_cli.utils.echo_trap import score_trajectory_repetition
...
def mean_repetition(completions: Sequence[Any]) -> float:
    ...
    Reuses :func:`kadhi_cli.utils.echo_trap.score_echo_signal` (lazy import —
    ...
    if not trajectories:
        return 0.0
    from kadhi_cli.utils.echo_trap import score_echo_signal
...
def _verdict_for(vote: float) -> str:
    from kadhi_cli.utils.reward_hacking import classify_hack_signal
...
class MitigationLogWriter:
    ...
    Mirrors :class:`kadhi_cli.monitoring.trace_logger.TraceLogWriter`.
    """
...
class _RewardHackMitigationCallback_body:  # type: ignore[misc, valid-type]  # noqa: N801
    ...
    Reads the shared :class:`~kadhi_cli.utils.rl_signal_buffer.RLSignalBuffer`
    snapshot each step, computes a multi-signal hacking score (reusing the
    v0.70.0 :class:`~kadhi_cli.utils.reward_hacking.RewardHackCallback` for the
