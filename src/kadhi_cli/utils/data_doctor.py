...
def resolve_tokenizer(tokenizer: Any, *, trust_remote_code: bool = False) -> Any:
    ...
    try:
        ...
        raise ValueError(
            "kadhi data doctor needs transformers to load a tokenizer — "
            "install with: pip install \"kadhi-cli[train]\""
        ) from exc
    try:
    ...
    if not template:
        return DoctorCheck(
            ...
            evidence=(
                ...
                "chat_template, or set data.chat_template in kadhi.yaml"
            ),
        )
    return DoctorCheck(
...
def check_truncation_risk(
    ...
    from kadhi_cli.data.loss_mask import coerce_token_ids
    from kadhi_cli.utils.tail_latency import percentile
...
def run_doctor(
    ...
    *,
    ...
    if total > 0 and not normalized:
        raise ValueError(
            f"no rows converted to chat messages for format={fmt!r} — kadhi data doctor "
            "targets chat/SFT data (chatml/alpaca/sharegpt/llava/audio/tool-calling/video/"
            "multimodal); preference data (dpo/kto) should use `kadhi data lint`, and RAFT "
