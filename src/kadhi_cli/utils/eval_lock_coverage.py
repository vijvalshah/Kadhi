...
from kadhi_cli.utils.advise import TASK_CATEGORIES
...
def lock_suite(design: EvalDesign, output_path: str) -> LockedSuite:
    ...
    fd, tmp = tempfile.mkstemp(prefix=".kadhi-locked-suite.", dir=parent)
    try:
...
def compute_coverage(
    ...
    *,
    ...
    if not design.dimensions:
        recommendations.append(
            "suite has no dimensions; run `kadhi eval design <data>` first"
        )
    elif not missing:
        recommendations.append(
