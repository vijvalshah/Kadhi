...
def _default_train_fn(
    *,
    ...
    yaml_text = yaml.safe_dump(
        {
            ...
            # KadhiConfig.output is a plain string — render it flat, not
    ...
    fd, tmp_yaml = tempfile.mkstemp(suffix=".yaml", prefix=".kadhi_idpo_", dir=os.getcwd())
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as fh:
            fh.write(yaml_text)
        subprocess.run(  # noqa: S603 — argv list, no shell
            [sys.executable, "-m", "kadhi_cli.cli", "train", "--config", tmp_yaml, "--yes"],
            check=True,
        )
    finally:
        try:
