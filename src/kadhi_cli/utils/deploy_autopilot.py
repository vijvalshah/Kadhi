...
def render_recipe_yaml(profile: DeployProfile, base: str, output_dir: str) -> str:
    ...
    lines = [
        f"# Kadhi deploy autopilot — profile: {profile.name}",
...
def render_deploy_script(profile: DeployProfile, model_path: str) -> str:
    ...
    if profile.runtime == "ollama":
        body = (
            'NAME="kadhi-${USER:-local}-$(basename \"$MODEL\")"\n'
            'echo "Planned: kadhi deploy ollama --model $MODEL --name $NAME"\n'
        )
        ...
        body = 'echo "Planned: kadhi serve --backend mlx --model $MODEL"\n'
    else:
        spec_flag = " --auto-spec" if profile.spec_decoding else ""
        body = (
            'echo "Planned: kadhi serve --backend '
