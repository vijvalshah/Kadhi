...
def list_kadhi_models() -> List[Dict[str, str]]:
    """List Ollama models deployed by Kadhi (prefixed with kadhi-).
    ...
    for line in result.stdout.strip().splitlines()[1:]:  # Skip header
        ...
        if not parts:
            continue
        model_name = parts[0]
        # Filter to kadhi- prefixed models
        if model_name.startswith(KADHI_MODEL_PREFIX) or (
            ":" in model_name and model_name.split(":")[0].startswith(KADHI_MODEL_PREFIX)
        ):
