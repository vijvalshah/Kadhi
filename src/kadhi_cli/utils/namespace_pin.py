...
``owner/name`` with malicious weights. Anyone with ``kadhi`` pinned to that
namespace silently pulls poison on next ``kadhi train`` / ``kadhi download``.

Defence: trust-on-first-use. The first time Kadhi sees a ``owner/name`` repo
...
``KADHI_NAMESPACE_PIN_DB`` (validated through the same containment policy
as ``KADHI_AUDIT_LOG_PATH``). Default: ``~/.kadhi/namespace_pin.db``.
...
class NamespacePinStore:
    ...
    def __init__(self, path: str) -> None:
        ...
        # (security-review HIGH fix; mirrors v0.36.0 KADHI_BATCH_CACHE_PATH
...
def _validate_db_path_override(override: str) -> Optional[str]:
    """Apply v0.36.0 ``KADHI_BATCH_CACHE_PATH`` / v0.54.0 / v0.59.0 policy."""
...
def default_db_path() -> str:
    """Resolve the pin DB path (env override first, else ``~/.kadhi/namespace_pin.db``)."""
    try:
        override = os.environ.get("KADHI_NAMESPACE_PIN_DB")
    except ValueError:
        override = None
    if override:
        ...
        if validated is not None:
            return validated
    return os.path.join(os.path.expanduser("~"), ".kadhi", "namespace_pin.db")
