...
from kadhi_cli.utils.net_guard import is_private_or_link_local as _is_private_or_link_local
...
def _validate_cache_dir(cache_dir: str, *, field: str = "cache_dir") -> str:
    """Containment check for a ``~/.kadhi``-style cache dir (home/cwd/tmp).

    Unlike :func:`_validate_local_path` (which is cwd-only), an artifact cache
    legitimately lives under ``$HOME/.kadhi`` so reusable downloads survive a
    ``cd``. Mirrors the v0.36.0 ``KADHI_BATCH_CACHE_PATH`` / v0.54.0 /
...
def upload_repo(
    ...
    *,
    folder_path: str,
    commit_message: str = "Upload via Kadhi",
    token: str | None = None,
    repo_type: str = "model",
) -> None:
    ...
    *,
    ...
    Args:
        ...
        cache_root: optional override for the cache root directory. Defaults
            to ``<cwd>/.kadhi_hub_cache``. The resolved cache subdir is
    ...
    if cache_root is None:
        root_path = os.path.realpath(os.path.join(os.getcwd(), ".kadhi_hub_cache"))
    else:
