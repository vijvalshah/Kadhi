...
a subprocess via ``subprocess.Popen([sys.executable, "-m", "kadhi_cli.cli",
...
from kadhi_cli.utils.canary_router import BucketStats, CanaryPolicy, rollback
...
from kadhi_cli.utils.loop_state import LoopState, read_state, write_state
...
def default_harvest(state: LoopState) -> Mapping[str, object]:
    ...
    Real implementation wires v0.26.0 ``kadhi_cli.data.traces.parsers`` +
...
def watch(config: WatchConfig) -> "tuple[LoopState, int]":
    ...
    try:
        while not stop.is_set():
            ...
            # cycle didn't actually run, so cluttering .kadhi-loops/ with
