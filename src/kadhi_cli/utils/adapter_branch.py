...
``~/.kadhi/branches/<name>.json`` (or ``KADHI_BRANCHES_DIR``-overridden) with
``0o600`` perms.
...
``$HOME`` / ``$CWD`` / ``$TMPDIR`` per v0.36.0 ``KADHI_BATCH_CACHE_PATH`` policy.
"""
...
from kadhi_cli.utils.paths import enforce_under_cwd_and_no_symlink, is_under
...
def _branches_dir() -> Path:
    ...
    override = os.environ.get("KADHI_BRANCHES_DIR")
    if override:
    ...
    if override:
        ...
        if any(is_under(str(candidate), str(b)) for b in bounds):
            ...
            return candidate
    default = Path.home() / ".kadhi" / "branches"
...
def load_branch(name: str) -> Branch:
    ...
    return Branch(
        ...
        kadhi_version=_validate_str_field(raw["kadhi_version"], "kadhi_version", max_len=64),
...
def attach_branch_to_registry(name: str, registry_ref: str) -> int:
    ...
    pointer lives in ``~/.kadhi/branches`` outside cwd — a library-produced
