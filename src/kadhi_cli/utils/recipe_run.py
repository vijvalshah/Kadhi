...
- ``llm_text`` + ``judge`` route through the v0.20.0 provider catalog via
  :func:`kadhi_cli.utils.data_forge.make_judge_provider_fn` (so the same
  SSRF / env-key surface applies).
- ``code`` reuses the v0.25.0 RLVR sandbox via
  :func:`kadhi_cli.trainer.rewards._run_code_sandbox`.
"""
...
_LOG = logging.getLogger("kadhi_cli.utils.recipe_run")
...
def _redact_exc_message(exc: BaseException, limit: int = 256) -> str:
    ...
    def _protect_url(m: "re.Match[str]") -> str:
        ...
        return f"__KADHI_SAFE_ORIGIN_{len(safe_origins) - 1}__"
    ...
    for index, origin in enumerate(safe_origins):
        msg = msg.replace(f"__KADHI_SAFE_ORIGIN_{index}__", origin)
    return msg[:limit]
...
def run_recipe(
    ...
    *,
    ...
    from kadhi_cli.utils.recipe_dag import RecipeDAG
