...
loop lives in a single ``.kadhi/loop.yaml`` next to the project, with
...
v0.26.0 Kadhi Cans under ``.kadhi-loops/``.
"""
...
class LoopState:
    ...
    detail lives in the ``.kadhi-loops/`` Kadhi Can artifacts.
    """
...
def write_state(state: LoopState, path: Optional[str] = None) -> str:
    """Atomically persist ``state`` to ``path`` (default: ``./.kadhi/loop.yaml``).
