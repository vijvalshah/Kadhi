...
``kadhi ci init`` writes ``.github/workflows/kadhi-gate.yml`` — a CI job that
gates every PR on three Kadhi checks:

    kadhi data validate <data>      # dataset format compliance
    kadhi expect <data> <suite>     # expectations suite (PII / length / refusal)
    kadhi ship --evidence <ev.json> # SHIP / DON'T-SHIP verdict (exit 2 = block)
...
_WORKFLOW_TEMPLATE = """\
# Kadhi fine-tuning gate — generated by `kadhi ci init` (v0.71.35).
# Gates every PR on: data validate -> expectations -> SHIP verdict.
# Edit the paths below to match your repo. The `kadhi ship` step needs a
# committed evidence JSON (see `kadhi ship --evidence`); remove it if unused.
name: Kadhi Fine-tuning Gate
...
jobs:
  kadhi-gate:
    ...
    steps:
      ...
      - name: Install Kadhi CLI
        # Core install is enough: `data validate`, `expect` and
        # `ship --evidence` are all no-torch paths.
        run: pip install kadhi-cli
      - name: Validate training data
        run: kadhi data validate {data}
      - name: Run expectations suite
        run: kadhi expect {data} {suite}
      - name: SHIP / DON'T-SHIP gate
        run: kadhi ship --evidence {evidence}{ship_config_arg}
"""
...
_CONFIG_NOTE = (
    "# The gate is bound to {config}: `kadhi ship` refuses evidence whose\n"
    "# config_sha != this config's (produce it in your train job with\n"
    "# `kadhi ship ... --config {config} --emit-evidence {evidence}`).\n"
)
...
def render_kadhi_gate_workflow(
    *,
    data_path: str,
    suite_path: str,
    evidence_path: str,
    ...
    When ``config_path`` is given, the ``kadhi ship`` step is bound to that
...
def write_kadhi_gate_workflow(
    *,
    ...
    python_version: str = "3.11",
    branch: str = "main",
    output_path: str = ".github/workflows/kadhi-gate.yml",
    ...
    fd, tmp = tempfile.mkstemp(prefix=".kadhi-gate.", dir=parent)
    try:
...
__all__ = ["render_kadhi_gate_workflow", "write_kadhi_gate_workflow"]
