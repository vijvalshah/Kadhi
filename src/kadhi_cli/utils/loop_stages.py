...
from kadhi_cli.utils.net_guard import LOOPBACK_HOSTS as _LOOPBACK_HOSTS
from kadhi_cli.utils.net_guard import parse_ip_literal
...
_PAIRS_DIR = ".kadhi-loops/pairs"
_ADAPTERS_DIR = ".kadhi-loops/adapters"
_DEFAULT_TRACE_DIR = ".kadhi-loops/traces"
...
def _resolve_trace_dir(state: LoopState) -> Optional[str]:
    ...
    Resolution order: ``KADHI_LOOP_TRACE_DIR`` env → ``served_model`` if it is
    an existing local dir → ``.kadhi-loops/traces``. Returns ``None`` when no
    ...
    outside the project dir (e.g. ``/var/log/kadhi-serve``). The directory is
    ...
    env = os.environ.get("KADHI_LOOP_TRACE_DIR")
    candidates = []
    if env:
...
def harvest_from_traces(state: LoopState) -> Mapping[str, object]:
    ...
    Writes the harvested pairs to ``.kadhi-loops/pairs/<uuid>.jsonl`` (cwd-
    ...
    """
    from kadhi_cli.data.traces.pair_builder import build_pairs
    from kadhi_cli.data.traces.parsers import parse_kadhi_serve

    trace_dir = _resolve_trace_dir(state)
    if not trace_dir:
        return {"pairs_harvested": 0, "pairs_path": None, "traces_collected": 0}

    traces = list(parse_kadhi_serve(trace_dir))
...
def train_dpo_from_pairs(
    ...
    if rc != 0:
        _LOG.warning("kadhi train exited rc=%s", rc)
        return {"run_id": None, "skipped": True, "adapter_path": None}
...
def gate_against_baseline(
    ...
    from kadhi_cli.eval.gate import load_suite, resolve_baseline, run_gate
...
def deploy_to_canary(
    ...
    via ``KADHI_LOOP_SERVE_ENDPOINT``. The endpoint is SSRF-validated
    ...
    if not adapter_path:
        return {"deployed": False, "canary_verdict": None}
    raw_endpoint = os.environ.get("KADHI_LOOP_SERVE_ENDPOINT")
    if not raw_endpoint:
        return {
            "deployed": False,
            "canary_verdict": None,
            "notes": "no KADHI_LOOP_SERVE_ENDPOINT configured",
        }
    from kadhi_cli.utils.drift_alarm import validate_webhook_url
    ...
    try:
        ...
        _LOG.warning("KADHI_LOOP_SERVE_ENDPOINT rejected by SSRF guard")
        return {
            "deployed": False,
            "canary_verdict": None,
            "notes": "KADHI_LOOP_SERVE_ENDPOINT rejected (SSRF guard)",
        }
    ...
    # private + link-local only, so a poisoned KADHI_LOOP_SERVE_ENDPOINT cannot
    ...
    if not _endpoint_is_local(endpoint):
        _LOG.warning("KADHI_LOOP_SERVE_ENDPOINT must be loopback/LAN (SSRF guard)")
        return {
            "deployed": False,
            "canary_verdict": None,
            "notes": "KADHI_LOOP_SERVE_ENDPOINT must be loopback/LAN (SSRF guard)",
        }
...
def estimate_cost(state: LoopState) -> float:
    ...
    try:
        ...
        from kadhi_cli.utils.run_cost import estimate_run_cost_usd
