...
'kadhi-cli[compile]'``) so the command works without them installed.
...
The CLI command lives in ``kadhi_cli/commands/compile_cmd.py`` (named
...
_INSTALL_HINT = (
    "Run: pip install \"kadhi-cli[compile]\"  (installs dspy-ai / textgrad / gepa)"
)
...
def _load_program_module(program_path: str) -> Any:
    ...
    spec = importlib.util.spec_from_file_location("kadhi_compile_program", program_path)
...
def run_compile(plan: CompilePlan) -> CompileResult:
    ...
    friendly ``ImportError`` (naming ``pip install kadhi-cli[compile]``) when it
