...
def apply_mod_patch(model: Any, *, capacity_factor: object = DEFAULT_CAPACITY_FACTOR) -> int:
    ...
    already carrying ``_kadhi_mod_patched`` is skipped). Best-effort: returns 0
    ...
    patched = 0
    for layer in layers:
        if getattr(layer, "_kadhi_mod_patched", False):
            continue
        ...
        router._kadhi_is_mod_router = True  # type: ignore[attr-defined]
        ...
        layer.add_module("_kadhi_mod_router", router)
        original_forward = layer.forward
        layer.forward = _make_mod_forward(original_forward, router, cf)
        layer._kadhi_mod_patched = True  # type: ignore[attr-defined]
        patched += 1
    return patched
