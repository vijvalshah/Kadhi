...
Wraps v0.26.0 :mod:`kadhi_cli.eval.quant_check` with disk-cache so that
...
def compute_cache_key(
    *,
    ...
    digest inputs. The orchestrator in :mod:`kadhi_cli.commands.deploy`
...
def _default_cache_path() -> str:
    """Return ``~/.kadhi/deploy_autopilot_cache.json`` (allowed override via env)."""
    override = os.environ.get("KADHI_DEPLOY_AUTOPILOT_CACHE")
    if override:
    ...
    return os.path.join(
        os.path.expanduser("~"), ".kadhi", "deploy_autopilot_cache.json"
    )
...
def _load_measure_model(
    ...
    *,
    ...
    :func:`kadhi_cli.utils.live_eval.make_generator`'s ``loaded=`` kwarg.
...
def build_before_generator(
    ...
    *,
    ...
    :func:`kadhi_cli.utils.live_eval.make_generator`) on the first prompt —
...
def build_after_generator_factory(
    ...
    *,
    ...
    factory._kadhi_first_party_measure_factory = True  # type: ignore[attr-defined]
    return factory
...
def run_measure(
    *,
    ...
    if getattr(after_gen_factory, "_kadhi_first_party_measure_factory", False):
