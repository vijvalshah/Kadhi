...
recipe, invokes ``python -m kadhi_cli.cli train --config <tmp> --yes`` via a
...
- Per-candidate temp dir lives under ``tempfile.gettempdir()``; cleaned up
  ...
  policy).
- Subprocess invocation uses ``sys.executable + ['-m', 'kadhi_cli.cli', ...]``
...
def _validate_path_under_cwd(name: str, raw: object) -> str:
    ...
    from kadhi_cli.utils.paths import is_under_cwd  # noqa: PLC0415
...
def _read_final_eval_loss(
    ...
    """
    from kadhi_cli.experiment.tracker import ExperimentTracker  # noqa: PLC0415

    if tracker_db_path is not None:
        os.environ["KADHI_DB_PATH"] = tracker_db_path
    ...
    *,
    ...
    Args:
        ...
        tracker_db_path: Optional override of the SQLite DB location (used
            in tests). Honoured via ``KADHI_DB_PATH``.
    ...
    tmp_dir = tempfile.mkdtemp(prefix=".kadhi_mix_proxy.")
    try:
        tmp_yaml = os.path.join(tmp_dir, "kadhi.yaml")
        ...
        env = os.environ.copy()
        if tracker_db_path is not None:
            env["KADHI_DB_PATH"] = tracker_db_path
