...
from kadhi_cli.utils.terminal import strip_control as for_terminal
...
def render_ship_panel(verdict: ShipVerdict) -> Panel:
    ...
    parts.append(footer)
    body = Group(*parts)
    return Panel(body, title="kadhi ship", border_style=style)
...
def render_ship_pr_markdown(verdict: ShipVerdict) -> str:
    ...
    return (
        f"## kadhi ship: {verdict.decision} {emoji}\n\n"
        f"{fence}\n{body}\n{fence}\n\n"
        f"<sub>Generated by <code>kadhi ship</code> v{verdict.kadhi_version}.</sub>\n"
    )
...
def verdict_from_evidence(
    ...
    *,
    ...
    try:
        stored_numerics = numerics_from_evidence(payload.get("numerics"))
    except (TypeError, ValueError, OverflowError) as exc:
        raise ValueError(f"invalid evidence.numerics: {exc}") from exc
    if stored_numerics is not None:
        return replace(verdict, numerics=stored_numerics)
    return verdict
...
def verdict_to_dict(verdict: ShipVerdict) -> Dict[str, object]:
    ...
    return {
        ...
        "kadhi_version": verdict.kadhi_version,
        "noise_floor": (
            None
