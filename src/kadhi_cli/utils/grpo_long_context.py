...
def apply_vllm_sleep_mode(engine_args: object) -> object:
    ...
    if version is None:
        raise RuntimeError(
            ...
            "installed (pip install \"kadhi-cli[serve-fast]\")."
        )
...
def maybe_enable_trl_sleep_mode(
    ...
    ``GRPOTrainer`` (server / colocate modes), so Kadhi cannot reach the
    ...
    if console is not None:
        console.print(
            ...
            "expose a sleep-mode option; Kadhi's vLLM engine factory honors "
