...
def default_governor_db_path() -> str:
    """Resolve the governor DB path (env override first, else ``~/.kadhi``)."""
    try:
        override = os.environ.get("KADHI_EDIT_GOVERNOR_DB")
    except ValueError:
        override = None
    if override:
        ...
        if validated is not None:
            return validated
    return os.path.join(os.path.expanduser("~"), ".kadhi", "edit_governor.db")
