...
``kadhi_cli/data/_fixtures/behavior/<name>.jsonl`` so the surface is usable
...
def load_battery_probes(name: str) -> tuple[dict, ...]:
    ...
    ``kadhi_cli/data/_fixtures/behavior/<name>.jsonl``.

    Uses ``importlib.resources.files("kadhi_cli")`` ``Traversable`` ``/``
