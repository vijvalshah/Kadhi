...
# OpenAI's finish_reason vocabulary that Kadhi emits. Anything the engine
...
def encode_chat_prompt(
    ...
    """Render ``messages`` with the model's template and tokenize them as Kadhi trains.
...
def build_engine_prompt(
    ...
    (``bos_token_id`` 128000): for the string Kadhi sends today the engine
...
def resolve_finish_reason(output: Any, max_tokens: Optional[int]) -> str:
    ...
    Trusts the engine's own ``finish_reason`` when it is one Kadhi emits, and
...
def create_vllm_app(
    ...
    app = FastAPI(title="Kadhi Inference Server (vLLM)", version="1.0.0")
