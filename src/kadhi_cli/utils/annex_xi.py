...
class AnnexXIData:
    ...
    def __post_init__(self) -> None:
        ...
        _validate_text(self.kadhi_version, "kadhi_version", max_len=32)
...
_Generated by kadhi-cli {_md_escape(data.kadhi_version)} at {_md_escape(data.created_at)}._
...
def render_annex_pdf(data: AnnexXIData, section: str) -> bytes:
    ...
    Lazy-imports ``reportlab`` (install via ``pip install kadhi-cli[pdf]``).
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "`pip install kadhi-cli[pdf]` (or `pip install reportlab`)."
        ) from exc
