...
class KadhiLock:
    ...
    def __post_init__(self) -> None:
        _check_str(self.kadhi_version, "kadhi_version", _MAX_VERSION_LEN)
...
def read_lock(path: str) -> KadhiLock:
    ...
    # KadhiLock __post_init__ does the rest of the validation
    return KadhiLock(
        kadhi_version=data.get("kadhi_version", ""),
...
def check_lock_drift(expected: KadhiLock, actual: KadhiLock) -> LockDrift:
    ...
    drift detection. ``kadhi_version`` / ``created_at`` differences are
    tracked separately as advisory-only (operators upgrading Kadhi will
    legitimately see them change).
    """
    if not isinstance(expected, KadhiLock):
        raise TypeError("expected must be KadhiLock")
    if not isinstance(actual, KadhiLock):
        raise TypeError("actual must be KadhiLock")
    changes: list[str] = []
