...
- ``default_sae_cache_dir()`` — ``~/.kadhi/sae-cache`` cache root (#216)
...
# auto-fetch any repo in this set into ``~/.kadhi/sae-cache`` via the
...
def _read_safetensors_dict(real_path: str) -> dict[str, Any]:
    ...
    ``~/.kadhi/sae-cache`` snapshot, whose internal HF symlinks are legitimate).
...
def default_sae_cache_dir() -> str:
    """``~/.kadhi/sae-cache`` — the reusable SAE download cache root (#216)."""
    return os.path.join(os.path.expanduser("~"), ".kadhi", "sae-cache")
...
def download_sae(
    ...
    *,
    ...
    :func:`kadhi_cli.utils.hubs.snapshot_download` (SSRF-hardened, with the #186
    namespace-pin TOFU gate). The download lands under
    ``~/.kadhi/sae-cache/<repo-slug>/`` (override via ``cache_dir``). The
