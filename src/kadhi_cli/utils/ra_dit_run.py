...
def discover_latest_retriever(*, store: Any = None) -> Optional[str]:
    ...
    """
    from kadhi_cli.utils.ra_dit import (  # noqa: PLC0415
    ...
    if store is None:
        try:
            from kadhi_cli.registry.store import RegistryStore  # noqa: PLC0415
...
def validate_ra_dit_config_path(name: str, raw: str) -> str:
    ...
    """
    from kadhi_cli.utils.paths import enforce_under_cwd_and_no_symlink  # noqa: PLC0415
...
def _run_train_subprocess(config_path: str, *, timeout_seconds: int) -> None:
    """Default stage runner: ``python -m kadhi_cli.cli train --config <p> --yes``.
...
def run_ra_dit(
    ...
    *,
    ...
    tmp_dir = tempfile.mkdtemp(prefix=".kadhi_ra_dit.")
    try:
        import yaml  # noqa: PLC0415
