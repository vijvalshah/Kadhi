    ...
    *,
    ...
    return Verdict(
        ...
        reverse_when=(
            ...
            "metric (run `kadhi advise --probe` to measure)."
        ),
        task_category=task_category,
        estimated_roi=roi,
    )
...
def synth_probe_baselines(
    ...
    *,
    ...
    component stays a label-variance heuristic (Kadhi ships no retriever to
    measure).
...
def next_command_for(verdict: Verdict) -> str:
    ...
    close-comments ("Try: ``kadhi <command> <args>``"). Architect-review
    ...
    if verdict.choice == "PROMPT_ENG":
        return "kadhi chat --model <base>   # iterate on prompts first"
    if verdict.choice == "RAG":
        return (
            "# RAG is outside Kadhi core — wire your data into a vector store "
            "(pgvector / weaviate / faiss) and prompt the base model."
        )
    if verdict.choice == "DPO":
        return "kadhi autopilot --data <data.jsonl> --task dpo"
    if verdict.choice == "GRPO":
        return "kadhi autopilot --data <data.jsonl> --task grpo"
    if verdict.choice == "SFT":
        return "kadhi autopilot --data <data.jsonl> --task sft"
