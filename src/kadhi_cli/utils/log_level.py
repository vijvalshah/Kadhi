...
def setup_logging(tier: LogLevel) -> logging.Logger:
    """Configure the top-level "kadhi" logger for the given tier.
    ...
    # Drop any prior kadhi handler whose tier no longer matches so we can
    # rebuild with the right RichHandler kwargs for the new tier.
    for existing in list(logger.handlers):
        if getattr(existing, "_kadhi_log_tier", None) is not None:
            if getattr(existing, "_kadhi_log_tier", None) != tier:
                logger.removeHandler(existing)

    if not any(getattr(handler, "_kadhi_log_tier", None) == tier for handler in logger.handlers):
        ...
        handler._kadhi_log_tier = tier  # type: ignore[attr-defined]
...
def apply_logging_level(tier: LogLevel) -> None:
    """Set the *root* logger level so non-kadhi libraries respect the tier (#G2/N1).

    ``setup_logging`` only configures the ``kadhi`` logger; library logs
