...
All imports are lazy so that `kadhi --help` stays fast.
"""
...
def create_mii_pipeline(
    ...
    if not is_mii_available():
        raise ImportError(
            ...
            "Install with: pip install \"kadhi-cli[mii]\" "
...
def build_mii_app(
    ...
    # here rather than at module scope to keep ``kadhi --help`` fast, per the
    ...
    from kadhi_cli.utils.vllm import build_chat_prompt, resolve_finish_reason

    # Models are module-level (not closure) so FastAPI's introspection can
    # resolve forward refs.
    _ensure_mii_request_models()

    app = FastAPI(title=f"kadhi-cli MII serve [{model_name}]")
    ...
    def _list_models() -> dict:
        return {
            ...
            "data": [{
                ...
                "created": 0, "owned_by": "kadhi-cli-mii",
            }],
        }
