...
``cryptography`` ships in the ``[sign]`` extra (``pip install kadhi-cli[sign]``)
...
_SIGNING_KEY_ENV = "KADHI_SIGNING_KEY"
...
def _require_cryptography() -> None:
    if not is_signing_available():
        raise ValueError(
            ...
            "Install with: pip install kadhi-cli[sign]"
        )
...
def resolve_signing_key(
    ...
    """Resolve the ed25519 private key from ``--key`` or ``KADHI_SIGNING_KEY``.

    Precedence: explicit ``key_path`` wins; else the ``KADHI_SIGNING_KEY`` env
    ...
    raise ValueError(
        ...
        "`kadhi adapters sign --backend ed25519 --generate-key`)"
    )
