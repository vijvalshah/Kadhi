...
(their own model card documents the equivalence), so Kadhi needs NO new
...
def resolve_pooling(model_id: str) -> str:
    ...
    if config is None:
        raise ValueError(
            ...
            "and not in the verified allowlist. Kadhi refuses rather than "
    ...
    for key, label in _NON_MEAN_POOLING_KEYS:
        if config.get(key):
            raise ValueError(
                f"{cleaned!r} uses {label!r} pooling; Kadhi's embedding kernel "
...
def _validate_texts(texts: object) -> list:
    ...
    if len(items) > _MAX_ROWS:
        raise ValueError(
            ...
            "dataset first (`kadhi data sample`) — Kadhi refuses rather than "
...
def embed_texts(
    ...
    *,
    ...
    importable on the light core (a ``pip install kadhi-cli`` without the
    ``[train]`` extra).
    """
