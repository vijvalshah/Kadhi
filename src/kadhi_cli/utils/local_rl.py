...
def _default_train_fn(
    *,
    ...
    ``python -m kadhi_cli.cli train --config <tmp> --yes`` with an argv list.
    """
    import subprocess
    import sys
    import tempfile
    ...
    fd, tmp_yaml = tempfile.mkstemp(
        suffix=".yaml", prefix=".kadhi_localrl_", dir=os.getcwd()
    )
    try:
...
def run_nightly_train(
    ...
    *,
    ...
    fd, tmp_jsonl = tempfile.mkstemp(
        suffix=".jsonl", prefix=".kadhi_localrl_pairs_", dir=os.getcwd()
    )
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as fh:
            for row in rows:
