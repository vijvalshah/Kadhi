...
- Part C (``kadhi_cli.utils.sleeper_probe``) — the live sleeper probe
- Part A (``kadhi_cli.utils.sae_diff``) — the SAE feature-diff surface
- Part D (``kadhi_cli.utils.interference``) — adapter pairwise eval
...
class ProbePack:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.kadhi_version, str):
            raise TypeError("kadhi_version must be str")
...
def _make_bundled_packs() -> Mapping[str, ProbePack]:
    ...
    from kadhi_cli.utils.harm_probe import BUNDLED_HARM_PROBES
    from kadhi_cli.utils.sleeper_probe import BUNDLED_PROBES
    from kadhi_cli.utils.truth_probe import BUNDLED_TRUTH_PROBES
...
def render_pack_json(pack: ProbePack) -> str:
    ...
    payload = {
        "base": pack.base,
        "kadhi_version": pack.kadhi_version,
...
def render_pack_markdown(pack: ProbePack) -> str:
    ...
    lines = [
        ...
        "",
        f"- Kadhi version: **{_md_escape(pack.kadhi_version)}**",
