"""Git-hook regression gate — `kadhi eval gate install` (v0.55.0 Part D).

Generates a portable pre-push hook that runs `kadhi eval against` against
...
from kadhi_cli.eval.aider_polyglot import TASK_NAME as AIDER_POLYGLOT_TASK_NAME
from kadhi_cli.eval.custom import TASK_NAME as CUSTOM_TASK_NAME
...
class GateMetricSpec:
    ...
    benchmark written by ``kadhi eval custom``.
    """
...
def resolve_gate_metric(metric: str) -> GateMetricSpec:
    ...
    Kadhi-owned benchmark names with stable score semantics are accepted
...
# Generated by `kadhi eval gate-install` (v0.55.0) — do not edit by hand.
# Pre-push regression gate: blocks the push when `kadhi eval against`
...
CANDIDATE_RUN_ID="${{KADHI_CANDIDATE_RUN_ID:-}}"
if [ -z "$CANDIDATE_RUN_ID" ]; then
    echo "[kadhi] KADHI_CANDIDATE_RUN_ID not set; skipping regression gate." >&2
    exit 0
fi

kadhi eval against "$BASELINE_RUN_ID" --candidate "$CANDIDATE_RUN_ID" \\
    --suite "$GATE_SUITE" --json-only \\
    || {{
        echo "[kadhi] pre-push gate blocked: comparison failed vs $BASELINE_RUN_ID" >&2
        exit 1
    }}
...
def write_pre_push_hook(
    *,
    ...
    fd, tmp = tempfile.mkstemp(prefix=".kadhi-pre-push.", dir=parent)
    try:
