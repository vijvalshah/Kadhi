...
# Local draft registry — mirrors the ~/.kadhi/spectrum cache (v0.71.23) and
# KADHI_REGISTRY_DB_PATH (v0.26.0) precedents.
_DRAFT_REGISTRY_ENV = "KADHI_DRAFT_REGISTRY_PATH"
...
def count_accepted_spans(
    ...
    from kadhi_cli.utils.uld import _MAX_ALIGN_CHARS, _char_spans
...
# ---------------------------------------------------------------------------
# Local draft registry (~/.kadhi/drafts.json)
# ---------------------------------------------------------------------------
def draft_registry_path() -> str:
    """Path to the local draft registry (``KADHI_DRAFT_REGISTRY_PATH`` wins)."""
    override = os.environ.get(_DRAFT_REGISTRY_ENV)
    if override:
        return override
    return str(Path.home() / ".kadhi" / "drafts.json")
...
def _read_registry() -> list[dict]:
    ...
    try:
        ...
        # planted at ~/.kadhi/drafts.json must not be transparently followed.
