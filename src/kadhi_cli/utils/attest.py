...
def build_slsa_provenance(s: AttestationStatement) -> dict[str, Any]:
    ...
    return {
        "buildDefinition": {
            "buildType": "https://kadhi.local/build/v1",
...
def sign_attestation(
    ...
    *,
    ...
    ``KADHI_SIGNING_KEY`` env var. ``sigstore`` keyless signing stays
...
def verify_attestation(
    ...
    malformed hex). Thin wrapper over :func:`kadhi_cli.utils.signing.verify_payload`.
    """
    if not isinstance(payload, (bytes, bytearray)):
        raise TypeError("payload must be bytes")
    from kadhi_cli.utils.signing import verify_payload
