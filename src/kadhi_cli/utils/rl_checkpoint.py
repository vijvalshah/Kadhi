...
epoch"); Kadhi ships a real mid-epoch save/load surface here.
...
class RLCheckpointState:
    ...
    - ``kadhi_version``: capped at 32 chars; null-byte rejected.
    """
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.kadhi_version, str) or not self.kadhi_version:
            raise ValueError("kadhi_version must be a non-empty string")
        if "\x00" in self.kadhi_version:
            raise ValueError("kadhi_version must not contain null bytes")
        if len(self.kadhi_version) > _MAX_KADHI_VERSION_LEN:
            raise ValueError(
                f"kadhi_version exceeds {_MAX_KADHI_VERSION_LEN} chars"
            )
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "kadhi_version": self.kadhi_version,
        }
...
class _RLCheckpointCallback_body:  # type: ignore[misc, valid-type]  # noqa: N801
    ...
    def __init__(
        ...
        *,
        output_dir: str,
        task: str = "grpo",
        kadhi_version: Optional[str] = None,
    ) -> None:
        ...
        self.task = task
        if kadhi_version is None:
            from kadhi_cli import __version__ as _v
