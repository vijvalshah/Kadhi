...
``com.kadhi.local-rl.plist`` (macOS launchd) that invokes
...
SYSTEMD_SERVICE_NAME = "kadhi-local-rl.service"
SYSTEMD_TIMER_NAME = "kadhi-local-rl.timer"
LAUNCHD_PLIST_NAME = "com.kadhi.local-rl.plist"
...
def _validate_kadhi_python(value: object) -> str:
    """The python executable that runs ``-m kadhi_cli.cli``."""
    if isinstance(value, bool):
        raise TypeError("kadhi_python must not be bool")
    if not isinstance(value, str):
        raise TypeError("kadhi_python must be str")
    if not value:
        raise ValueError("kadhi_python must be non-empty")
    if "\x00" in value or "\n" in value or "\r" in value:
        raise ValueError("kadhi_python must not contain NUL / newline")
    return value
...
def build_train_argv(
    *,
    ...
    kadhi_python = _validate_kadhi_python(kadhi_python)
...
def render_systemd_service(
    *,
    ...
    return (
        "[Unit]\n"
        "Description=Kadhi local-RL nightly DPO/KTO/ORPO train\n"
        "\n"
        "[Service]\n"
        "Type=oneshot\n"
...
def render_systemd_timer(*, hour: int = 3, minute: int = 0) -> str:
    ...
    return (
        "[Unit]\n"
        "Description=Kadhi local-RL nightly train timer\n"
        "\n"
        "[Timer]\n"
