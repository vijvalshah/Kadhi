...
``.kadhi-signature.json``. Two backends:
...
_SIGNATURE_FILENAME = ".kadhi-signature.json"
_MAX_FILES = 1024
...
def sign_adapter(
    ...
    *,
    ...
    """Compute the manifest, sign it, and write ``.kadhi-signature.json``.
    ...
    Args:
        ...
        key_path: ed25519 private-key PEM path (``ed25519`` backend). When None,
            falls back to ``KADHI_SIGNING_KEY`` env, then ``generate_key_path``.
    ...
    if chosen == SignBackend.UNSIGNED:
        ...
        if not key_path and generate_key_path and not os.environ.get("KADHI_SIGNING_KEY"):
...
def verify_adapter(
    ...
    *,
    ...
    if record.backend == SignBackend.ED25519.value:
        from kadhi_cli.utils import signing as _signing
