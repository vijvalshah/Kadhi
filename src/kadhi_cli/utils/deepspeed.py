...
def resolve_user_deepspeed_file(path: str, *, gpu_count: int | None = None) -> str:
    ...
    reports a malformed config better than a Kadhi traceback would, and refusing
...
def attach_empty_param_group_guard(trainer) -> bool:
    ...
    Scope: every wrapper under ``kadhi_cli/trainer/`` that builds its own trainer
    ...
    """
    if getattr(trainer, "_kadhi_empty_group_guard", False):
        return False
    ...
    trainer._kadhi_empty_group_guard = True
    return True
