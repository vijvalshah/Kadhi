...
- Scan results are cached at ``~/.kadhi/spectrum/<slug>.json`` (override via
  ``KADHI_SPECTRUM_CACHE_DIR``, containment-checked under ``$HOME`` / ``$CWD`` /
  ``$TMPDIR`` per the v0.36.0 cache-dir policy).

All heavy deps (numpy / torch / safetensors / huggingface_hub) are imported
lazily inside functions so the module — and the ``kadhi`` CLI — load without
...
class ModelWeightsPlan:
    """A weight source located before Kadhi creates a second on-disk copy."""
            ...
            for key in handle.keys():
                ...
                try:
                    ...
                    raise RuntimeError(
                        ...
                        f"'kadhi-cli[train]'): {type(exc).__name__}"
                    ) from exc
                yield key, array
...
def default_spectrum_cache_dir() -> str:
    """``~/.kadhi/spectrum`` — the scan cache root (no side effects)."""
    return os.path.join(os.path.expanduser("~"), ".kadhi", "spectrum")


def resolve_cache_dir(cache_dir: Optional[str] = None) -> str:
    """Resolve the cache dir (explicit arg > ``KADHI_SPECTRUM_CACHE_DIR`` > default).
    ...
    override = os.environ.get("KADHI_SPECTRUM_CACHE_DIR")
...
def _materialize_cached_snapshot(plan: ModelWeightsPlan) -> str:
    ...
    if target == weights_root or not is_under(target, weights_root):
        raise ValueError("materialized weights path must stay inside Kadhi's cache")
    ...
    try:
        ...
        if staged_sizes != expected_sizes:
            raise FileNotFoundError(
                "cached snapshot changed while Kadhi materialized its weight files"
            )
...
def plan_model_weights(model: str) -> ModelWeightsPlan:
    """Locate weights and quantify any copy Kadhi would need, without making it."""
    if not isinstance(model, str) or not model.strip():
        raise ValueError("model must be a non-empty string")
