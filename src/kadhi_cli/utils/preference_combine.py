...
def attach_weighted_preference_combine(trainer: object, weights: Mapping[str, float]) -> bool:
    ...
    Idempotent: re-attaching detects the ``_kadhi_weighted_combine`` marker.
    """
    validate_weight_compat(weights)
    if not hasattr(trainer, "compute_loss"):
        return False
    original = trainer.compute_loss
    if getattr(original, "_kadhi_weighted_combine", False):
        return True
    ...
    wrapped._kadhi_weighted_combine = True  # type: ignore[attr-defined]
