...
_BENCHMARK_METADATA: Mapping[str, Mapping[str, str]] = MappingProxyType({
    ...
    "wmdp": MappingProxyType({
        "description": (
            ...
            "ships REDACTED forget-set probes — Kadhi never bundles "
...
class UnlearnReport:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.kadhi_version, str) or not self.kadhi_version:
            raise ValueError("kadhi_version must be non-empty str")
...
def get_fixture_path(benchmark: str) -> Optional[Path]:
    ...
    ``kadhi_cli/data/_fixtures/unlearning/``). MUSE / WMDP loaders are
    deferred to v0.61.1.
