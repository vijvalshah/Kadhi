...
``kadhi_cli/registry`` (which tracks trained model artifacts) — verdicts
are decision records, not training artifacts.

Path: ``~/.kadhi/advise_history.jsonl`` (override via
``KADHI_ADVISE_HISTORY_PATH``). The override must resolve under one of
``{$HOME, $CWD, tempfile.gettempdir()}`` — same containment policy as
v0.36.0 ``KADHI_BATCH_CACHE_PATH``.
"""
...
from kadhi_cli.utils.advise import CHOICES, TASK_CATEGORIES, Verdict
...
def history_path() -> str:
    ...
    Mirrors v0.36.0 ``KADHI_BATCH_CACHE_PATH`` containment: the override
    must sit under ``$HOME`` / ``$CWD`` / ``tempfile.gettempdir()``;
    out-of-bounds values fall through to the default.
    """
    default = os.path.join(os.path.expanduser("~"), ".kadhi", "advise_history.jsonl")
    override = os.environ.get("KADHI_ADVISE_HISTORY_PATH")
    if not override:
        return default
    if "\x00" in override:
        return default
...
def _project_name() -> str:
    ...
    name = os.path.basename(os.path.realpath(os.getcwd())) or "kadhi-project"
    return name[:128]
...
def record_verdict(
    ...
    *,
    ...
    ``KADHI_ADVISE_HISTORY_PATH`` env var which IS containment-checked in
