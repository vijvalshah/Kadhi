...
class DataConfig(BaseModel):
    ...
    tokenized_path: Optional[str] = Field(
        default=None,
        description=(
            "Path to a pre-tokenized cache produced by `kadhi data preprocess`. "
    ...
    def _validate_buffer_size_v042(cls, value: Optional[int]) -> Optional[int]:
        from kadhi_cli.utils.data_pipeline import validate_buffer_size
    ...
    def _validate_shards_v042(cls, value: Optional[int]) -> Optional[int]:
        from kadhi_cli.utils.data_pipeline import validate_shards
    ...
    def _validate_image_pixels_v042(cls, value, info):
        from kadhi_cli.utils.data_pipeline import validate_image_pixels
    ...
    def _validate_video_fps_v042(cls, value: Optional[float]) -> Optional[float]:
        from kadhi_cli.utils.data_pipeline import validate_video_fps
    ...
    def _validate_video_maxlen_v042(cls, value: Optional[int]) -> Optional[int]:
        from kadhi_cli.utils.data_pipeline import validate_video_maxlen
    ...
    def _validate_new_tokens_v042(
        ...
        from kadhi_cli.utils.data_pipeline import validate_new_tokens
    ...
    def _validate_prompt_strategy_v042(cls, value: Optional[str]) -> Optional[str]:
        from kadhi_cli.utils.data_pipeline import validate_prompt_strategy
    ...
    def _validate_interleave_v042(cls, value):
        ...
        # in KadhiConfig._validate_interleave_compat below — num_datasets is
        # a parse-time constant since #443 widened data.train to accept a
        # list.
        if value is None:
            return None
        if isinstance(value, str):
            from kadhi_cli.utils.data_pipeline import INTERLEAVE_STRATEGIES
    ...
    def _validate_v042_pre_tokenized_path(self) -> "DataConfig":
        ...
        if self.format == "pre_tokenized" and not self.tokenized_path:
            raise ValueError(
                ...
                "at a cache directory produced by `kadhi data preprocess`."
            )
        return self
...
class AdviseConfig(BaseModel):
    ...
    ``kadhi_cli/commands/advise.py``; this field is informational on
    ``KadhiConfig`` only.
    """
    ...
    goal: Optional[str] = Field(
        ...
        max_length=4096,
        description=(
            "Default goal string for `kadhi advise`. Sharpens task "
    ...
    probe: bool = Field(
        default=False,
        description=(
            "Run the 10-minute ROI probe by default when `kadhi advise` is "
    ...
    record: bool = Field(
        ...
        description=(
            ...
            "~/.kadhi/advise_history.jsonl with accepted=True."
        ),
    )
...
class TrainingConfig(BaseModel):
    ...
    optimizer: str = Field(
        ...
        description=(
            ...
            "See kadhi_cli.utils.optimizer_zoo.SUPPORTED_OPTIMIZERS for the "
            "full list."
        ),
    )
    ...
    prm_reward: Optional[str] = Field(
        default=None,
        description=(
            "Path (or HF id) to a Kadhi-trained PRM (task='prm') used as the "
    ...
    prm_aggregate: Literal["min", "prod", "last"] = Field(
        ...
        description=(
            ...
            "probability of step-correctness). Kadhi's PRM head is trained with "
    ...
    def _validate_rollout_func_field(cls, value):
        ...
        from kadhi_cli.utils.agent_rollout import validate_rollout_func
    ...
    hub: Literal["hf", "modelscope", "modelers"] = Field(
        ...
        description=(
            ...
            "(Openmind hub). `kadhi push --hub` selects its upload destination "
    ...
    kv_cache_type: Optional[Literal["q8_0", "bf16", "f16", "fp8"]] = Field(
        ...
        description=(
            ...
            "Applied by kadhi serve on the transformers backend; backend and "
    ...
    def _validate_minillm_anchor_path(cls, v):
        ...
        if v is None:
            return None
        from kadhi_cli.utils.minillm import _check_path_shape
    ...
    def _validate_teacher_model(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v is None:
            return v
        from kadhi_cli.utils.distill import validate_teacher_model
    ...
    def _validate_distill_temperature(cls, v):
        ...
        if v is None:
            return None
        from kadhi_cli.utils.distill import validate_distill_temperature
    ...
    def _validate_distill_mode(cls, v):
        ...
        if v is None:
            return "token"
        from kadhi_cli.utils.distill import validate_distill_mode
    ...
    def _validate_mod_capacity_factor(cls, v):
        ...
        if v is None:
            return 0.125
        from kadhi_cli.utils.mod import validate_capacity_factor
    ...
    def _validate_ebft_temperature(cls, v):
        ...
        if v is None:
            return None
        from kadhi_cli.utils.ebft_gdpo import validate_ebft_temperature
    ...
    def _validate_label_names(cls, v):
        ...
        if v is None:
            return None
        from kadhi_cli.utils.classifier import validate_label_names
    ...
    def _validate_mole_task_adapters(cls, v):
        ...
        if v is None:
            return None
        from kadhi_cli.utils.mole_routing import validate_mole_task_adapters
    ...
    def _validate_num_labels(cls, v):
        ...
        if v is None:
            return None
        from kadhi_cli.utils.classifier import validate_num_labels
    ...
    def _validate_reasoning_effort(cls, v):
        ...
        if v is None:
            return None
        from kadhi_cli.utils.reasoning_effort import validate_reasoning_effort
    ...
    def _normalize_hub(cls, v):
        ...
        from kadhi_cli.utils.hubs import validate_hub_name
        if v is None:
            return v
    ...
    packing_cross_doc_attn_mask: bool = Field(
        default=False,
        description=(
            "Never worked: Kadhi mapped this to packing_strategy="
    ...
    unfrozen_parameters: Optional[List[str]] = Field(
        ...
        description=(
            ...
            "fine-tuning, LoRA off. Generate with `kadhi spectrum scan`. "
        ...
        for pat in value:
            ...
            if _UNFROZEN_REDOS_RE.search(pat):
                raise ValueError(
                    ...
                    f"'model.layers.0.mlp.down_proj' (run `kadhi spectrum scan`)."
                )
        return value
    ...
    # resident. See kadhi_cli.utils.layer_stream.
    ...
    def _validate_prequantized_no_qat(self) -> "TrainingConfig":
        ...
        """
        from kadhi_cli.utils.quant_menu import is_quant_menu_format
    ...
    def _validate_optimizer(cls, value: str) -> str:
        ...
        from kadhi_cli.utils.optimizer_zoo import validate_optimizer_name
    ...
    def _validate_lr_groups(cls, value):
        ...
        if value is None:
            return None
        from kadhi_cli.utils.lr_groups import parse_lr_groups
    ...
    def _validate_expand_layers_field(cls, value):
        ...
        if value is None:
            return None
        from kadhi_cli.utils.block_expansion import validate_expand_layers
    ...
    # Schema-only release: validators here are reused by the KadhiConfig
    ...
    def _validate_unlearn_method(cls, v):
        ...
        if v is None:
            return None
        from kadhi_cli.utils.unlearning import validate_unlearn_method
    ...
    def _validate_unlearn_alpha(cls, v):
        ...
        if v is None:
            return None
        from kadhi_cli.utils.unlearning import validate_unlearn_alpha
    ...
    def _validate_ra_dit_stage(cls, v):
        ...
        if v is None:
            return None
        from kadhi_cli.utils.ra_dit import validate_ra_dit_stage
    ...
    def _validate_ra_dit_retriever_model(cls, v):
        ...
        if v is None:
            return None
        from kadhi_cli.utils.ra_dit import validate_ra_dit_retriever_model
    ...
    def _validate_citation_style(cls, v):
        ...
        if v is None:
            return None
        from kadhi_cli.utils.citation_faithful import validate_citation_style
    ...
    def _validate_citation_recall_threshold(cls, v):
        ...
        if v is None:
            return None
        from kadhi_cli.utils.citation_faithful import validate_citation_threshold
    ...
    def _validate_grace_codebook_size(cls, v):
        ...
        if v is None:
            return None
        from kadhi_cli.utils.grace_codebook import validate_grace_codebook_size
    ...
    def _validate_grace_codebook_dim(cls, v):
        ...
        if v is None:
            return None
        from kadhi_cli.utils.grace_codebook import validate_grace_codebook_dim
...
class ShipConfig(BaseModel):
    ...
    # Keep these values in sync with kadhi_cli.utils.ship_verdict.TASK_MODES.
...
class EvalConfig(BaseModel):
    ...
    ship: Optional[ShipConfig] = Field(
        default=None,
        description="kadhi ship verdict defaults (task_eval / task_mode / "
...
def _validate_reward_hack_controller(tcfg: Any) -> None:
    ...
    from kadhi_cli.utils.reward_hack_control import SIGNAL_NAMES
...
#: Root-level keys that :class:`KadhiConfig` accepts and moves under ``training``
...
def remap_root_level_misplaced_keys(values):
    ...
    convention) while Kadhi nests it under ``training``. Without the remap,
...
class KadhiConfig(BaseModel):
    ...
    @model_validator(mode="after")
    def _validate_chat_template_supported_tasks(self) -> "KadhiConfig":
    ...
    def _remap_root_level_misplaced_keys(cls, values):
        ...
        ``lora:`` (LlamaFactory / Axolotl convention) but Kadhi nests it
    ...
    @model_validator(mode="after")
    def _validate_v028_speed_memory_supported_tasks(self) -> "KadhiConfig":
        ...
        """
        from kadhi_cli.utils.v028_features import supports_v028_features
        ...
        raise ValueError(
            ...
            "kadhi_cli.utils.v028_features.supports_v028_features."
        )

    @model_validator(mode="after")
    def _validate_multipack_supported_tasks(self) -> "KadhiConfig":
        ...
        if not self.training.multipack:
            return self
        from kadhi_cli.utils.multipack import supports_multipack
    ...
    @model_validator(mode="after")
    def _validate_curriculum_dynamic_supported(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_longlora_compat(self) -> "KadhiConfig":
        ...
        if not self.training.use_longlora:
            return self
        from kadhi_cli.utils.longlora import validate_longlora_compat
    ...
    @model_validator(mode="after")
    def _validate_grpo_variant_supported(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_long_context_grpo(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_prm_compat(self) -> "KadhiConfig":
        ...
        if self.task != "prm":
            return self
        from kadhi_cli.utils.prm import validate_prm_compat
    ...
    @model_validator(mode="after")
    def _validate_vision_grpo(self) -> "KadhiConfig":
        """v0.50.0 Part E — ``vision_grpo=True`` compat gate."""
        if not self.training.vision_grpo:
            return self
        from kadhi_cli.utils.prm import validate_vision_grpo_compat
    ...
    @model_validator(mode="after")
    def _validate_grpo_stability_task_gate(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_grpo_fp16_amp_exclusive(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_hub_supported(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_classifier_compat(self) -> "KadhiConfig":
        ...
        if self.task not in classifier_tasks and not classifier_fields_set:
            return self
        from kadhi_cli.utils.classifier import (
            is_classifier_task,
    ...
    @model_validator(mode="after")
    def _validate_distill_compat(self) -> "KadhiConfig":
        ...
        if self.task == "distill":
            from kadhi_cli.utils.distill import validate_distill_compat
    ...
    @model_validator(mode="after")
    def _validate_bitnet_compat(self) -> "KadhiConfig":
        """v0.52.0 Part D — ``quantization='bitnet_1.58'`` gate."""
        if self.training.quantization != "bitnet_1.58":
            return self
        from kadhi_cli.utils.bitnet import validate_bitnet_compat
    ...
    @model_validator(mode="after")
    def _validate_ebft_compat(self) -> "KadhiConfig":
        ...
        from kadhi_cli.utils.ebft_gdpo import validate_ebft_compat
    ...
    @model_validator(mode="after")
    def _validate_gdpo_compat(self) -> "KadhiConfig":
        """v0.52.0 Part E — ``gdpo_variant`` requires DPO/preference, non-MLX."""
        if self.training.gdpo_variant is None:
            return self
        from kadhi_cli.utils.ebft_gdpo import validate_gdpo_compat
    ...
    @model_validator(mode="after")
    def _validate_reasoning_effort_task_gate(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_moe_expert_quant_compat(self) -> "KadhiConfig":
        """v0.52.0 Part F — ``moe_expert_quant`` + ``train_router_only`` gates."""
        tcfg = self.training
        from kadhi_cli.utils.moe_quant import (
    ...
    @model_validator(mode="after")
    def _validate_unfrozen_parameters(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_lisa_compat(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_full_finetune(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_lora_target_parameters_scope(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_stream_layers_compat(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_rollout_backend(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_prm_reward(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_online_dpo_compat(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_reward_fn_multi_compat(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_asr_compat(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_replay_compat(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_interleave_compat(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_vllm_sleep_mode(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_fp8_attention_compat(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_nvfp4_compat(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_unsloth_bnb_4bit_compat(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_bnb_4bit_double_quant(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_llm_int8_alias(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_quantize_ref_reward(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_kv_cache_type_supported(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_relora_supported_tasks(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_quant_menu_supported_tasks(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_preference_dispatcher(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_dpo_variants_supported_tasks(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_preference_loss_weights(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_unlearn_compat(self) -> "KadhiConfig":
        ...
        from kadhi_cli.utils.unlearning import validate_unlearn_compat
    ...
    @model_validator(mode="after")
    def _validate_grace_codebook_compat(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_citation_faithful_compat(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_ra_dit_compat(self) -> "KadhiConfig":
        ...
        * ``generator`` -> ``task='sft'``
        ...
        from kadhi_cli.utils.ra_dit import validate_ra_dit_compat
    ...
    @model_validator(mode="after")
    def _validate_mole_routing_compat(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_mlx_task_support(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_uld_compat(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_echo_trap_compat(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_rl_checkpoint_compat(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_minillm_compat(self) -> "KadhiConfig":
    ...
    @model_validator(mode="after")
    def _validate_reward_hack_compat(self) -> "KadhiConfig":
...
# The canonical source is `kadhi_cli/templates/*.yaml` with `manifest.json`.
...
TEMPLATES: dict[str, str] = {
    "chat": """# Kadhi template: Chat Assistant
...
output: ./output
""",
    "code": """# Kadhi template: Code Model
...
output: ./output
""",
    "reasoning": """# Kadhi template: Reasoning / GRPO
...
output: ./output
""",
    "vision": """# Kadhi template: Vision / Multimodal
...
output: ./output
""",
    "medical": """# Kadhi template: Medical / Domain Expert
...
output: ./output
""",
    "kto": """# Kadhi template: KTO (Kahneman-Tversky Optimization)
...
output: ./output
""",
    "orpo": """# Kadhi template: ORPO (Odds Ratio Preference Optimization)
...
output: ./output
""",
    "bco": """# Kadhi template: BCO (Binary Classifier Optimization)
...
output: ./output
""",
    "simpo": """# Kadhi template: SimPO (Simple Preference Optimization)
...
output: ./output
""",
    "ipo": """# Kadhi template: IPO (Identity Preference Optimization)
...
output: ./output
""",
    "pretrain": """# Kadhi template: Continued Pre-training
...
""",
    "moe": """# Kadhi template: MoE (Mixture of Experts) Fine-tuning
...
output: ./output
""",
    "longcontext": """# Kadhi template: Long-Context Fine-tuning (128k+)
...
training:
  ...
  use_flash_attn: true
  # use_liger: true       # pip install "kadhi-cli[liger]" for fused ops
  # use_ring_attention: true  # Multi-GPU sequence parallelism

output: ./output_longctx
""",
    "embedding": """# Kadhi template: Embedding Model Fine-tuning
...
""",
    "audio": """# Kadhi template: Audio / Speech
...
output: ./output_audio
""",
    "tool-calling": """# Kadhi template: Tool-Calling / Agentic Fine-tuning
...
output: ./output
""",
    "rlhf": """# Kadhi template: Full RLHF Pipeline (SFT + Reward Model + PPO)
# Three-stage training: 1) SFT warmup, 2) Reward model, 3) PPO alignment
#
# Usage:
#   Step 1: kadhi train --config kadhi_sft.yaml       # SFT warmup
#   Step 2: kadhi train --config kadhi_rm.yaml         # Train reward model
#   Step 3: kadhi train --config kadhi_ppo.yaml        # PPO with reward model
#
