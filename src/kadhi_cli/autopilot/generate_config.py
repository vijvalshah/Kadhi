...
def build_kadhi_config(
    model: str,
    data_path: str,
    goal: str,
    vram_gb: float | None = None,
) -> KadhiConfig:
    """Run the full autopilot decision pipeline and return a ``KadhiConfig``."""
...
def write_yaml(config: KadhiConfig, path: Path) -> None:
    """Dump a ``KadhiConfig`` to YAML."""
    path = Path(path)
...
def generate_config(
    ...
    The ``decisions`` dict mirrors what ``build_kadhi_config`` produces but
