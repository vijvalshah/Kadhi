"""Kadhi template registry (v0.39.0 Part E).
...
``kadhi_cli.config.schema`` for back-compat.
"""
...
def list_templates() -> list[str]:
    ...
    try:
        from kadhi_cli.config.schema import TEMPLATES as _INLINE
...
def load_template(name: str) -> Optional[str]:
    ...
    Order:
      ...
      2. Inline ``TEMPLATES`` dict in ``kadhi_cli.config.schema``.
    """
    _validate_name(name)
    manifest = _load_manifest()
