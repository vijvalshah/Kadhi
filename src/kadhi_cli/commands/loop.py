...
State lives in ``.kadhi/loop.yaml``; per-iteration artifacts under
``.kadhi-loops/<iteration-id>/iteration.json``. Both paths are cwd-
...
from kadhi_cli.utils.canary_router import CanaryPolicy
from kadhi_cli.utils.loop_budget import parse_budget_string
...
def _safe_read() -> LoopState:
    try:
        ...
        console.print(
            f"[red]No loop state found.[/] Run [bold]kadhi loop init[/] first."
...
def init_cmd(
    ...
    """Create the .kadhi/loop.yaml control file (one-time setup)."""
...
def status_cmd() -> None:
    ...
    state = _safe_read()
    table = Table(title="kadhi loop status", show_header=False)
...
def watch_cmd(
    ...
    pack_cans: bool = typer.Option(
        False, "--pack-cans",
        help="Pack each iteration as a v0.26 Kadhi Can + Registry entry (v0.71.4).",
    ),
) -> None:
...
def replay_cmd(
    ...
    if extract is not None:
        from kadhi_cli.cans.unpack import extract_can
        from kadhi_cli.utils.paths import enforce_under_cwd_and_no_symlink

        can_path = os.path.join(".kadhi-loops", iteration_id, "iteration.can")
