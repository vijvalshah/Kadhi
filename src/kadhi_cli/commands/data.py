...
from kadhi_cli.utils.embed import DEFAULT_EMBED_MODEL, embed_texts
from kadhi_cli.utils.paths import is_under_cwd
from kadhi_cli.utils.semdedup import DedupReport, greedy_semdedup
    ...
    *,
    ...
    try:
        ...
        console.print(
            ...
            # markup tag and print `pip install "kadhi-cli"` -- a command that
            ...
            "Install with: [bold]pip install \"kadhi-cli\\[train]\"[/]"
        )
        raise typer.Exit(1)
...
def dedup(
    ...
    semantic: bool = typer.Option(
        ...
        help="Use embedding cosine (SemDeDup) instead of MinHash. Catches "
             "paraphrases MinHash misses. Requires kadhi-cli\\[train].",
    ),
    ...
    try:
        ...
        console.print(
            ...
            "Install with: [bold]pip install \"kadhi-cli\\[data]\"[/]"
        )
        raise typer.Exit(1)
...
def filter_data(
    ...
    from kadhi_cli.utils.quality import compute_coherence_score
    ...
    if perplexity is not None or score_only:
        try:
            from kadhi_cli.utils.quality import compute_perplexity_scores
...
def stats(
    ...
    from kadhi_cli.data.validator import extended_stats
    ...
    try:
        ...
        if lengths:
            ...
            try:
                from kadhi_cli.utils.plotext_compat import render_histogram
...
def download_dataset(
    ...
    fmt: str = typer.Option(
        None, "--format", "-f",
        help="Convert to Kadhi format after download: alpaca, sharegpt, chatml",
    ),
    ...
    if hub_canonical != "hf":
        from kadhi_cli.utils.hubs import download_repo as _download_repo
        ...
        try:
            if hub_canonical == "modelscope":
                ...
                console.print(
                    ...
                    "use kadhi_cli.utils.hubs.download_repo for raw "
                    "snapshot download.[/]"
                )
            else:  # modelers
                try:
                    out_dir = _download_repo(
                        hub_canonical,
                        dataset_id,
                        local_dir=str(Path.cwd() / ".kadhi_hub_cache"
                                      / "datasets"
...
def _get_registry_path() -> Path:
    """Get the default registry path (~/.kadhi/datasets.json)."""
    from kadhi_cli.utils.registry import _default_registry_path
...
def augment_data(
    ...
    from kadhi_cli.data.augment import STRATEGIES
...
def _load_augment_provider(
    ...
    *,
    ...
    from kadhi_cli.utils.data_forge import JUDGE_PROVIDERS, make_judge_provider_fn
...
def register_data(
    ...
    """
    from kadhi_cli.utils.registry import register_dataset
...
def unregister_data(
    ...
    """
    from kadhi_cli.utils.registry import unregister_dataset
...
def from_traces_cmd(
    logs: str = typer.Option(
        ..., "--logs", help="Path to JSONL trace log (or directory for kadhi-serve)",
    ),
    format: str = typer.Option(
        ..., "--format", help="Trace format: langchain | openai | kadhi-serve",
    ),
    ...
    from kadhi_cli.data.traces import (
        SUPPORTED_FORMATS,
        SUPPORTED_SIGNALS,
        build_pairs,
        parse_langchain,
    ...
    from kadhi_cli.utils.paths import is_under_cwd as _under_cwd
    ...
    events: list[dict] = []
    if format == "kadhi-serve":
        trace_iter = parse_kadhi_serve(str(logs_path))
    else:
    ...
    if judge:
        ...
        from kadhi_cli.data.traces.quality import judge_filter_pairs
        from kadhi_cli.eval.judge import VALID_PROVIDERS, JudgeEvaluator
...
def list_registry():
    ...
    from kadhi_cli.utils.registry import load_registry
    ...
    if not registry:
        ...
        console.print(
            "[dim]Register with: "
            "kadhi data register --name my-data --path data.jsonl --format alpaca[/]"
        )
        return
...
def push_dataset_cmd(
    ...
    from kadhi_cli.utils.hf import (
        get_hf_api,
        resolve_endpoint,
        resolve_token,
        validate_repo_id,
...
def _push_dataset_non_hf(
    ...
    from kadhi_cli.utils.hubs import upload_repo

    # Stage UNDER cwd — `upload_repo` enforces cwd-containment on folder_path
    # (the system tempdir would be rejected).
    staging = tempfile.mkdtemp(prefix=".kadhi_dataset_push.", dir=os.getcwd())
    try:
...
def preprocess_dataset(
    config_path: str = typer.Argument(
        ..., help="Path to kadhi.yaml — uses data.train + tokenizer + max_length"
    ),
    output_dir: str = typer.Option(
        "./.kadhi-tokenized", "--output", "-o",
    ...
    for idx, row in enumerate(raw_rows):
        ...
        if not is_pretrain:
            ...
            from kadhi_cli.data.loss_mask import strip_doubled_leading_bos
    ...
    console.print(
        ...
        "in your kadhi.yaml to short-circuit tokenization on subsequent runs.[/]"
    )
...
def ingest_document(
    ...
    from kadhi_cli.utils.data_pipeline import detect_ingest_format
    from kadhi_cli.utils.paths import is_under_cwd
    ...
    if kind == "txt":
        ...
        # known limitation.
        from kadhi_cli.utils.data_pipeline import split_markdown_by_headings
...
def demo_bundle(
    ...
    from kadhi_cli.utils.demo_bundles import (
        copy_bundle_to,
        get_bundle,
        list_bundles,
    )
    ...
    if name is None:
        ...
        console.print(table)
        console.print(
            "[dim]Run: kadhi data demo <name> --output <path>[/]"
        )
        return
...
def recipe(
    ...
    if execute:
        ...
        if provider is not None:
            from kadhi_cli.utils.data_forge import JUDGE_PROVIDERS
...
def gen_magpie(
    ...
    from kadhi_cli.utils.magpie import build_magpie_config, run_magpie
    ...
    console.print(
        Panel(
            ...
            title="kadhi data gen-magpie — plan",
        )
    )
    ...
    console.print(
        Panel(
            ...
            title="kadhi data gen-magpie — done",
        )
    )
...
def best_of_n(
    ...
    from kadhi_cli.utils import best_of_n as bon
    from kadhi_cli.utils import best_of_n_artifact as bon_artifact
    from kadhi_cli.utils import best_of_n_checkpoint as bon_checkpoint
    from kadhi_cli.utils.magpie import make_magpie_generate_fn
    from kadhi_cli.utils.paths import enforce_under_cwd_and_no_symlink
    ...
    if offline_mode and export_mode:
        ...
        raise typer.Exit(2)
    if offline_mode:
        from kadhi_cli.utils import best_of_n_stream as bon_stream
            ...
            with bon_stream.index_offline_artifacts(
                ...
                console.print(
                    Panel(
                        ...
                        title="kadhi data best-of-n — offline plan",
                    )
                )
                if plan_only:
        ...
        console.print(Panel(body, title="kadhi data best-of-n — offline done"))
        return
    ...
    console.print(
        Panel(
            ...
            title="kadhi data best-of-n — plan",
        )
    )
    if plan_only:
        return
    ...
    if export_mode:
        ...
        console.print(
            Panel(
                ...
                title="kadhi data best-of-n — candidates exported",
            )
        )
        return
    ...
    body += f"\nManifest:   [bold]{_escape(os.path.relpath(manifest_path))}[/]"
    console.print(Panel(body, title="kadhi data best-of-n — done"))
...
def evolve(
    ...
    from kadhi_cli.utils.evolve import STRATEGIES, run_evolve
    from kadhi_cli.utils.magpie import make_magpie_generate_fn
...
def persona_mix(
    ...
    from kadhi_cli.utils.paths import enforce_under_cwd_and_no_symlink
    from kadhi_cli.utils.persona_hub import (
    ...
    fd, tmp_path = tempfile.mkstemp(prefix=".kadhi.", suffix=".tmp", dir=parent)
    try:
...
def brain_rot_cmd(
    ...
    from kadhi_cli.utils.brain_rot import (
        refuse_if_rotten,
        score_dataset_brain_rot,
    )
    from kadhi_cli.utils.brain_rot_lang import validate_lang_code
    from kadhi_cli.utils.paths import enforce_under_cwd_and_no_symlink
