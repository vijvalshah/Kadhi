...
def status_cmd(
    ...
    table = Table(title=f"kadhi local-rl status — {db}")
...
def train_cmd(
    ...
    if not once:
        ...
        from kadhi_cli.utils.local_rl_scheduler import write_scheduler_files
        ...
        console.print(
            Panel(
                ...
                "[dim]Linux:[/] cp kadhi-local-rl.{service,timer} ~/.config/systemd/user/ "
                "&& systemctl --user enable --now kadhi-local-rl.timer\n"
                "[dim]macOS:[/] cp com.kadhi.local-rl.plist ~/Library/LaunchAgents/ "
                "&& launchctl load ~/Library/LaunchAgents/com.kadhi.local-rl.plist\n"
                "[dim]Now:[/] re-run with [bold]--once[/] for an ad-hoc train.",
                title="kadhi local-rl train — scheduler scaffold",
            )
        )
        return
    ...
    try:
        ...
        console.print(
            Panel(
                ...
                title="kadhi local-rl train — failed",
            )
        )
    ...
    if result.status.startswith("skipped"):
        console.print(
            Panel(
                ...
                title="kadhi local-rl train — skipped",
            )
        )
        return
    ...
    console.print(
        Panel(
            ...
            title="kadhi local-rl train — done",
        )
    )
