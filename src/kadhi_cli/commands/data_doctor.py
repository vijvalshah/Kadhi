...
from kadhi_cli.utils import data_doctor as engine
from kadhi_cli.utils import data_lint as lint_engine
from kadhi_cli.utils.paths import atomic_write_text
from kadhi_cli.utils.terminal import for_terminal, strip_control
from kadhi_cli.utils.trust_remote import model_requires_trust_remote_code, resolve_trust_remote_code
...
def _render_doctor_report(report: engine.DoctorReport) -> None:
    _render_checks_table("kadhi data doctor", report.checks)
    console.print(
        Panel.fit(
...
def _render_lint_report(report: lint_engine.LintReport) -> None:
    _render_checks_table(f"kadhi data lint ({escape(report.fmt)})", report.checks)
    console.print(
        Panel.fit(
            f"[bold {_verdict_style(report.overall)}]{report.overall}[/] — "
            f"{report.rows_scanned}/{report.total_rows} rows scanned",
...
def doctor(
    ...
    train_on_messages_with_train_field: bool = typer.Option(
        ...
        help="Per-message train:bool field masking (mirrors the same kadhi.yaml flag).",
    ),
    ...
    if resolved_fmt in _PREFERENCE_FORMATS:
        console.print(
            ...
            "use [bold]kadhi data lint[/] instead of [bold]kadhi data doctor[/]."
        )
...
def lint(
    ...
    if resolved_fmt not in _PREFERENCE_FORMATS:
        console.print(
            ...
            "kadhi data lint only supports dpo/orpo/simpo/ipo/bco (chosen/rejected) and kto. "
            "Use [bold]kadhi data doctor[/] instead for chat/SFT data."
        )
        raise typer.Exit(code=2)
