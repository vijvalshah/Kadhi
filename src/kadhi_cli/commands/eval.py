...
def benchmark(
    ...
    try:
        ...
        console.print(
            ...
            "Install with: [bold]pip install \"kadhi-cli\\[eval]\"[/]"
        )
        raise typer.Exit(1)
...
def aider(
    ...
    output: str = typer.Option(
        ..., "--output", "-o",
        help="Contained directory for upstream and Kadhi result files",
    ),
    ...
    run_id: Optional[str] = typer.Option(
        None, "--run-id",
        help="Save the aggregate score under an existing Kadhi run ID",
    ),
    ...
    from kadhi_cli.eval.aider_polyglot import (
        AiderEvalError,
    ...
    try:
        ...
        result_path = write_kadhi_result(output_dir, row)
    except AiderEvalError as exc:
        if tracker is not None:
            tracker.close()
    ...
    console.print(f"[green]Kadhi result written to:[/] {escape(str(result_path))}")
...
def custom(
    ...
    results_list = []
    from kadhi_cli.eval.custom import _create_default_generator, score_task
    ...
    from kadhi_cli.eval.custom import EvalResults
    ...
    if output or attach_to_registry:
        from kadhi_cli.registry.attach import attach_artifact, write_eval_json
...
def judge(
    ...
    from kadhi_cli.eval.judge import JudgeResults
...
def auto(
    ...
    kadhi_config = load_config(str(config_path))
    output_dir = Path(kadhi_config.output)

    if not output_dir.exists():
        console.print(f"[red]Output directory not found: {output_dir}[/]")
        console.print("[dim]Run kadhi train first to create a model.[/]")
        raise typer.Exit(1)

    eval_config = getattr(kadhi_config, "eval", None)
    ...
    if not bench_list and not tasks_file:
        console.print(
            ...
            "Use --benchmarks or --tasks, or add eval config to kadhi.yaml."
        )
        raise typer.Exit(1)
...
def compare(
    ...
    from kadhi_cli.eval.leaderboard import compare_runs
    from kadhi_cli.experiment.tracker import ExperimentTracker
    ...
    if not comparison["comparisons"]:
        ...
        console.print("[dim]Run kadhi eval benchmark --run-id ... first.[/]")
        raise typer.Exit(1)
...
def leaderboard(
    ...
    from kadhi_cli.eval.leaderboard import (
    ...
    if not lb.entries:
        ...
        console.print(
            "[dim]Run kadhi eval benchmark or kadhi eval custom first.[/]"
        )
        raise typer.Exit(1)
...
def human(
    ...
    from kadhi_cli.eval.human import (
        HumanEvalResults,
        HumanJudgment,
        load_prompts,
        save_results,
    ...
    from kadhi_cli.eval.custom import _create_default_generator
...
def _save_custom_results(
    ...
    from kadhi_cli.eval.custom import TASK_NAME
    from kadhi_cli.experiment.tracker import ExperimentTracker
...
def gate_cmd(
    ...
    from kadhi_cli.eval.gate import (
        load_suite,
        resolve_baseline,
        run_gate,
        write_baseline_file,
    ...
    if model is None:
        ...
        from kadhi_cli.eval.quant_check import make_model_generator
...
def quant_check_cmd(
    ...
    tasks: str = typer.Option(
        ..., "--tasks",
        help="JSONL eval tasks file (see 'kadhi eval custom')",
    ),
    fmt: str = typer.Option(
        "table", "--format",
...
from kadhi_cli.commands._eval_v0550 import register as _register_v0550  # noqa: E402

_register_v0550(app, console)

# Register v0.61.0 subcommands (eval unlearning)
from kadhi_cli.commands._eval_v0610 import register as _register_v0610  # noqa: E402

_register_v0610(app, console)

# Register v0.65.0 subcommands (eval behavior / capability / checklist / irt-subset)
from kadhi_cli.commands._eval_v0650 import register as _register_v0650  # noqa: E402

_register_v0650(app, console)

# Register v0.71.10 subcommand (eval citation — #202)
from kadhi_cli.commands._eval_v07110 import register as _register_v07110  # noqa: E402
