...
def pack_cmd(
    ...
    from kadhi_cli.cans.pack import pack_entry
    ...
    console.print(Panel(
        ...
        title="Kadhi Can", border_style="green",
    ))
...
def verify_cmd(
    ...
    from kadhi_cli.cans.verify import verify_can
...
def run_cmd(
    ...
    if not yes:
        try:
            ...
            _fail(str(exc))
        console.print(Panel(
            "[yellow]`kadhi can run` will:[/]\n"
            "  - Extract the can\n"
            "  - Auto-fetch any data referenced inside\n"
            "  - Run [bold]kadhi train[/] against the embedded config\n\n"
    ...
    if result is None:
        _fail("kadhi can run did not produce a result")
...
def fork_cmd(
    ...
    from kadhi_cli.cans.pack import fork_can
