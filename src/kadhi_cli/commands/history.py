...
from kadhi_cli.registry.store import RegistryStore, validate_name
        ...
        if not entries:
            ...
            if _name_exists_in_dataset_registry(name):
                console.print(
                    ...
                    f"`kadhi data registry` or `kadhi data inspect {escape(name)}`?[/]"
                )
            console.print(
                "[dim]`kadhi history` queries the *model* registry only.[/]"
            )
            raise typer.Exit(1)
...
def _name_exists_in_dataset_registry(name: str) -> bool:
    ...
    try:
        from kadhi_cli.utils.registry import DatasetRegistry  # type: ignore
