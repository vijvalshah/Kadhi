"""kadhi agent — Agent Forge: spec → tool-calling SFT dataset / train / eval.
...
def synth(
    ...
    from kadhi_cli.utils.agent_forge import (
        load_spec_file,
        parse_spec,
        synthesise_dataset,
        write_dataset,
...
def train(
    ...
    output_dir: str = typer.Option(
        ...
        help="Where the planned kadhi train run will store checkpoints.",
    ),
    examples_per_endpoint: int = typer.Option(
...
def eval(
    ...
    *executed* against a generated mock of the endpoint.
    ...
    from kadhi_cli.utils.agent_forge import load_spec_file, parse_spec
...
def _eval_sandbox(predictions: str, tool_to_endpoint: dict, max_pred_lines: int):
    ...
    from kadhi_cli.trainer.rewards import _get_isolation_strategy
