...
if TYPE_CHECKING:  # static types only — the import is numpy-lazy at runtime
    from kadhi_cli.utils.spectrum_scan import LayerSNR, ScanResult
...
def scan(
    ...
    no_cache: bool = typer.Option(
        False, "--no-cache", help="Skip the ~/.kadhi/spectrum scan cache."
    ),
) -> None:
    """Scan a model's layer SNR and emit a ``training.unfrozen_parameters`` patch."""
    import yaml

    from kadhi_cli.utils.spectrum_scan import scan_model, select_unfrozen_parameters
    ...
    console.print(
        ...
        f"into your kadhi.yaml (full fine-tuning, LoRA off).[/]"
    )
