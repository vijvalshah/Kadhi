...
if TYPE_CHECKING:  # pydantic models — import for typing only (no eager cost)
    from kadhi_cli.config.schema import ShipConfig, KadhiConfig
...
def _parse_ship_config(path: str) -> "Tuple[KadhiConfig, Optional[ShipConfig]]":
    """Load a kadhi.yaml and return ``(KadhiConfig, ShipConfig | None)``.
...
def _config_sha_of(cfg: "KadhiConfig") -> str:
    ...
    """
    from kadhi_cli.registry.hashing import hash_config
...
def _compute_provenance(cfg: "KadhiConfig") -> Dict[str, object]:
    ...
    *different* recipe than the one in the diff. This is staleness detection,
    ...
    if not isinstance(got, str) or not _SHA256_RE.match(got):
        _fail(
            ...
            "`kadhi ship ... --config <cfg> --emit-evidence <ev>`",
            _EXIT_USAGE,
        )
    if got != expected_sha:
...
def _live_eval_quantization_from_config(kadhi_config: Optional["KadhiConfig"]) -> Optional[str]:
    ...
    """
    if kadhi_config is None:
        return None
    quant = kadhi_config.training.quantization
...
def _expected_numerics_family(kadhi_config: "KadhiConfig") -> str:
    ...
    """
    quant = _live_eval_quantization_from_config(kadhi_config)
...
def _check_evidence_numerics(payload: dict, expected_family: str) -> None:
    ...
    if raw is None:
        console.print(
            ...
            "Re-emit with kadhi ship ... --emit-evidence."
        )
        return
    try:
...
def _resolve_generators(
    ...
    from kadhi_cli.eval.gate_suites import BEHAVIOURAL_MAX_NEW_TOKENS
    from kadhi_cli.utils import live_eval
...
def _leg1_metric(
    ...
    from kadhi_cli.eval.custom import load_eval_tasks, run_eval
...
def _build_judge_scorer(
    ...
    across every call.
    """
    from kadhi_cli.eval.custom import load_eval_tasks
...
def _build_pairwise_scorer(
    ...
    from kadhi_cli.eval.custom import load_eval_tasks
    from kadhi_cli.eval.gate import _parse_judge_url
    from kadhi_cli.eval.judge import JudgeEvaluator, pairwise_winrate
...
def _lm_eval_leg2(
    ...
    from kadhi_cli.commands import eval as eval_cmd
...
def _measure_noise_floor(
    ...
    *,
    ...
    """
    from kadhi_cli.eval.gate_suites import is_bundled_suite, score_bundled_suite
...
def _parse_suite(general_suite: Optional[str]) -> List[str]:
    from kadhi_cli.eval.gate_suites import DEFAULT_GENERAL_SUITE
...
def _push_pr_comment(verdict: ShipVerdict, target: str) -> None:
    ...
    """
    from kadhi_cli.utils import adapter_pr
    from kadhi_cli.utils.ship_verdict import render_ship_pr_markdown
...
def ship(
    ...
    config: Optional[str] = typer.Option(
        None,
        "--config",
        help="kadhi.yaml whose eval.ship block supplies defaults (CLI flags win). "
    ...
    if config is not None:
        kadhi_config, ship_cfg = _parse_ship_config(config)
    ...
    config_sha = _config_sha_of(kadhi_config) if kadhi_config is not None else None
    ...
    if evidence:
        ...
        if config_sha is not None and not emit_evidence:
            ...
            assert kadhi_config is not None  # config_sha is computed from it
            _check_evidence_numerics(
                payload, _expected_numerics_family(kadhi_config)
            )
        ...
        verdict = _verdict_live(
            ...
            quantization=_live_eval_quantization_from_config(kadhi_config),
        )
    else:
        _fail(
    ...
    if emit_evidence:
        ...
        if kadhi_config is not None:
            provenance.update(_compute_provenance(kadhi_config))
    _emit_and_exit(
