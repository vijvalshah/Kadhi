...
def merge(
    ...
    from kadhi_cli.utils.paths import is_under_cwd as _is_under_cwd
    ...
    try:
        ...
        if save_format_canonical == "fp16":
            ...
            with tempfile.TemporaryDirectory(
                prefix=".kadhi_4bit_merge_", dir=str(Path.cwd()),
            ) as staged:
    ...
    console.print(
        Panel(
            ...
            f"Next steps:\n"
            f"  [bold]kadhi chat --model {output_path}[/]\n"
            f"  [bold]kadhi push --model {output_path} --repo user/model[/]\n"
            f"  [bold]kadhi export --model {output_path} --format gguf[/]",
