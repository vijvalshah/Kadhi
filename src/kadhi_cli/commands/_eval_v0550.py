...
def register(app: typer.Typer, console: Console) -> None:
    ...
    def design_cmd(
        ...
        from kadhi_cli.utils.advise import load_advise_dataset
        from kadhi_cli.utils.eval_design import (
    ...
    def discover_cmd(
        ...
        base: Optional[str] = typer.Option(
            None, "--base",
            help="Base model id (recorded; consumed by `kadhi diagnose`).",
        ),
        ...
        from kadhi_cli.utils.advise import load_advise_dataset
        from kadhi_cli.utils.canary_discovery import (
            discover_canaries,
            write_canary_set,
        )
    ...
    def lock_cmd(
        design_path: str = typer.Argument(
            ..., help="Path to an EvalDesign JSON (from `kadhi eval design`)",
        ),
        output: str = typer.Option(
        ...
        from kadhi_cli.utils.eval_design import load_eval_design
    ...
    def coverage_cmd(
        ...
        from kadhi_cli.utils.eval_design import load_eval_design
        from kadhi_cli.utils.eval_lock_coverage import compute_coverage
    ...
    def against_cmd(
        baseline_run_id: str = typer.Argument(
            ..., help="Baseline run id (from `kadhi runs list`).",
        ),
        ...
        suite: str = typer.Option(
            None, "--suite",
            help=(
                "Locked eval suite (from `kadhi eval lock`) to validate as a gate "
        ...
        ``0`` when no regression is detected and ``1`` otherwise. Designed to
        be invoked from the pre-push hook generated by
        ``kadhi eval gate-install``.
        """
        import json as _json

        from kadhi_cli.utils.eval_gate_hook import (
            GateThresholds,
            decide_regression,
            resolve_gate_metric,
        )

        # Validate before constructing ExperimentTracker: an unknown metric is
        # a usage error, not an empty database query (#812).
        try:
            metric_spec = resolve_gate_metric(metric)
        except (TypeError, ValueError) as exc:
            console.print(f"[red]Invalid --metric:[/] {escape(str(exc))}")
            raise typer.Exit(1) from exc
        ...
        # passed the gate.
        if suite:
            from kadhi_cli.utils.eval_lock_coverage import load_locked_suite
        ...
        try:
            candidates = [
                (
                    source_metric,
                    tracker.get_metric_series(baseline_run_id, source_metric),
                    tracker.get_metric_series(candidate_run_id, source_metric),
                )
                for source_metric in metric_spec.source_metrics
            ]
        ...
        # Prefer the first source with a complete pair. If only one run has a
        # source, retain that partial pair so the diagnostic reports the true
        # side-specific counts rather than silently falling through.
        selected = next(
            (candidate for candidate in candidates if candidate[1] and candidate[2]),
            None,
        )
        if selected is None:
            selected = next(
                (candidate for candidate in candidates if candidate[1] or candidate[2]),
                candidates[0],
            )
        source_metric, baseline_series, candidate_series = selected

        if not baseline_series or not candidate_series:
            console.print(
                f"[red]Comparison unavailable — no eval results or tracked metrics "
                f"for {escape(source_metric)}: "
                f"baseline {escape(baseline_run_id)}={len(baseline_series)}, "
                f"candidate {escape(candidate_run_id)}={len(candidate_series)}.[/]"
            )
            raise typer.Exit(1)
    ...
    def gate_install_cmd(
        ...
        from kadhi_cli.utils.eval_gate_hook import write_pre_push_hook
