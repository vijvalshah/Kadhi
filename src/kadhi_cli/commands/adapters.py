"""kadhi adapters — LoRA adapter management (list, info, compare, diff, merge, blame, branch)."""
...
def diff(
    ...
    from kadhi_cli.utils.adapter_diff import (
...
def merge(
    ...
    from kadhi_cli.utils.adapter_merge import SUPPORTED_STRATEGIES, merge_adapters
    from kadhi_cli.utils.license_matrix import (
    ...
    if not allow_unscanned:
        from kadhi_cli.utils.adapter_scan import scan_adapter
...
def arithmetic(
    ...
    from kadhi_cli.utils.adapter_arithmetic import (
        ArithmeticReport,
        _detect_lora_rank,
    ...
    from kadhi_cli.utils.adapter_diff import load_adapter_weights
    from kadhi_cli.utils.adapter_merge import write_merged_adapter
    from kadhi_cli.utils.paths import enforce_under_cwd_and_no_symlink
...
def blame(
    ...
    """
    from kadhi_cli.utils.blame import (
        parse_budget,
        plan_blame,
        render_blame_json,
...
def branch(
    ...
    config: Optional[str] = typer.Option(
        None, "--config", "-c",
        help="Path to kadhi.yaml (required unless --from-registry)",
    ),
    ...
    console.print(
        Panel(
            ...
            f"Version: {snap.kadhi_version}{reg_line}",
...
def checkout(
    ...
    output: str = typer.Option("kadhi.yaml", "--output", "-o",
                               help="Where to write the restored config"),
):
    """Restore a snapshotted branch's config into cwd (v0.57.0)."""
    from kadhi_cli.utils.adapter_branch import load_branch, write_checkout
...
def list_branches_cmd():
    ...
    from kadhi_cli.utils.adapter_branch import list_branches, load_branch
...
def scan(
    ...
    """
    from kadhi_cli.utils.adapter_scan import render_report_text, scan_adapter
...
def sign(
    ...
    """Compute manifest + write ``.kadhi-signature.json`` (v0.60.0, ed25519 v0.71.2).
    ...
    root — pass ``--key <priv.pem>``, set ``KADHI_SIGNING_KEY``, or use
...
def verify(
    ...
    """Verify ``.kadhi-signature.json`` against current files (v0.60.0, ed25519 v0.71.2).
...
def check_safetensors(
    ...
    """
    from kadhi_cli.utils.strict_safetensors import check_strict_safetensors
...
def adapter_pr(
    ...
    push: Optional[str] = typer.Option(
        ...
        help=(
            ...
            "owner/repo#N (e.g. owner/repo#42). Auth via "
    ...
    """
    from kadhi_cli.utils.adapter_pr import (
        build_adapter_pr,
        post_pr_comment,
        render_pr_json,
        render_pr_markdown,
...
def adapter_bisect(
    history: list[str] = typer.Argument(
        ...
        help=(
            ...
            "Example: `kadhi adapters bisect ckpt-100 ckpt-200 ckpt-300 ckpt-400`"
        ),
    ),
    eval_command: str = typer.Option(
        ...
        help=(
            ...
            "Example: 'kadhi eval custom --model {ckpt} --tasks tasks.jsonl'"
        ),
    ),
    ...
    Once the boundary is found, ``kadhi adapters blame`` can attribute the
    regression to specific dataset rows.
    """
    from kadhi_cli.utils.adapter_bisect import build_bisect_plan, run_bisect
