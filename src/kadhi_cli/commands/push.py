...
def push(
    ...
    commit_message: str = typer.Option(
        "Upload model trained with Kadhi CLI",
        "--message",
    ...
    card: Optional[str] = typer.Option(
        ...
        help=(
            ...
            "model card (via `kadhi card`) and upload it as README.md, overriding "
    ...
    from kadhi_cli.utils.hf import resolve_endpoint, resolve_token, validate_repo_id
    ...
    try:
        ...
        if card_override is not None:
            api.upload_file(
                ...
                repo_id=repo,
                commit_message="Add model card (kadhi card)",
            )
            ...
            api.upload_file(
                ...
                path_in_repo="README.md",
                repo_id=repo,
                commit_message="Add model card (generated by Kadhi CLI)",
            )
...
def _load_training_config(model_path: Path) -> dict:
    """Read sidecar ``training_config.yaml`` written by Kadhi training runs."""
    for name in ("training_config.yaml", "kadhi.yaml"):
...
def _render_training_section(training_cfg: dict) -> str:
    ...
    validator in ``KadhiConfig``, so a crafted-but-schema-valid config could
...
def generate_model_card_v2(
    ...
    sections = [
        ...
        f"# {model_name}",
        "",
        "Fine-tuned model uploaded with Kadhi CLI.",
        "",
        "## Model Details",
        "",
        details_block,
