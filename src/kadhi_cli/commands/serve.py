...
def serve(
    ...
    steer: Optional[str] = typer.Option(
        ...
        help=(
            ...
            "`kadhi steer train`; the transformers backend installs the "
    ...
    if steer is not None:
        ...
        from kadhi_cli.utils.steering import (
    ...
    if hub and hub != "hf":
        from kadhi_cli.utils.hubs import apply_hub_to_cli_model
    ...
    try:
        ...
        console.print(
            ...
            "Install with: [bold]pip install \"kadhi-cli\\[serve]\"[/]"
        )
        raise typer.Exit(1)
    ...
    if backend == "mii":
        from kadhi_cli.utils.mii import (
            build_mii_app,
            create_mii_pipeline,
            is_mii_available,
        )
    ...
    if backend == "transformers":
        from kadhi_cli.utils.vllm import is_vllm_available
        ...
        if is_vllm_available():
            ...
            from kadhi_cli.utils.sglang import check_sglang_available
    ...
    if backend == "vllm":
        ...
        if not is_vllm_available():
            console.print(
                ...
                "Install with: [bold]pip install \"kadhi-cli\\[serve-fast]\"[/]"
            )
            raise typer.Exit(1)
    ...
    if backend == "sglang":
        ...
        if not check_sglang_available():
            console.print(
                ...
                "Install with: [bold]pip install \"kadhi-cli\\[sglang]\"[/]"
            )
            raise typer.Exit(1)
    ...
    if auto_spec and not speculative_model:
        ...
        # ESC/OSC sequences live (mirrors kadhi_cli.utils.terminal.for_terminal).
        _safe = for_terminal
    ...
    from kadhi_cli.utils.structured_output import validate_mode
    ...
    if auto_quant:
        from kadhi_cli.utils.auto_quant import (
    ...
    if backend == "vllm":
        ...
        if mole is not None:
            ...
            from kadhi_cli.utils.mole_routing import load_mole_for_serve
        ...
        if bank is not None:
            ...
            from kadhi_cli.utils.vector_bank import (
                apply_bank_to_serve,
                load_bank,
            )
        ...
        from kadhi_cli.utils.paths import is_under_cwd
        from kadhi_cli.utils.structured_output import build_constraint
        ...
        from kadhi_cli.utils.tracing import build_tracer
        ...
        if trace_log is not None:
            from kadhi_cli.monitoring.trace_logger import TraceLogWriter
        ...
        if reasoning_parser:
            from kadhi_cli.utils.reasoning_parser import validate_parser_name
        ...
        if record_thumbs is not None:
            ...
            from kadhi_cli.utils.local_rl import init_local_rl_db, validate_db_path
...
def _serve_vllm(
    ...
    from kadhi_cli.utils.vllm import create_vllm_app, create_vllm_engine
    ...
    # model dir first (kadhi writes the tokenizer beside the adapter), then the
...
def _serve_sglang(
    ...
    from kadhi_cli.utils.sglang import create_sglang_app, create_sglang_runtime
...
def _load_draft_model(speculative_model: str, device: str):
    ...
    # A locally-registered draft (kadhi draft distill, v0.71.33) can be selected
    # here via --auto-spec. Refuse pickle / PyTorch-classic weights before
    # from_pretrained torch.load's them — a poisoned ~/.kadhi/drafts.json entry
...
def _create_app(
    ...
    from kadhi_cli.utils.metrics import ServerMetrics

    app = FastAPI(title="Kadhi Inference Server", version="1.0.0")
        ...
        with _contextlib.ExitStack() as stack:
            ...
            try:
                try:
                    ...
                    from kadhi_cli.utils.structured_output import (
                ...
                if reasoning_parser is not None:
                    from kadhi_cli.utils.reasoning_parser import strip_reasoning
    ...
    def anthropic_messages(payload: dict) -> dict:
        from kadhi_cli.utils.anthropic_messages import (
            from_anthropic,
    ...
    def tool_python(
        ...
        if len(code) > tool_max_code_len:
            raise HTTPException(status_code=400, detail="Invalid request")
        try:
            from kadhi_cli.trainer.rewards import _run_code_sandbox
    ...
    def record_thumb_endpoint(
        ...
        if (
            ...
            raise HTTPException(status_code=400, detail="Invalid request")
        from kadhi_cli.utils.local_rl import record_thumb
