...
``kadhi_cli.utils.diagnose.live.run_live_diagnose``. Without ``--base-model``
...
from kadhi_cli.utils.diagnose import FailureReport, compose_report
...
def _render_report(report: FailureReport) -> None:
    table = Table(title=f"kadhi diagnose: {escape(report.adapter or report.run_id)}")
