...
def export(
    ...
    llama_cpp_path: Optional[str] = typer.Option(
        None,
        "--llama-cpp",
        help="Path to llama.cpp directory. Auto-detected or cloned to ~/.kadhi/llama.cpp",
    ),
    ...
    if is_adapter:
        ...
        merge_dir = model_path.parent / f".kadhi_merge_tmp_{model_path.name}"
        _merge_adapter(
    ...
    # already-present ~/.kadhi/llama.cpp died on `ModuleNotFoundError:
    ...
    try:
        ...
        if quant in ("f16", "f32"):
            ...
            tmp_dir = Path(
                tempfile.mkdtemp(prefix=".kadhi_gguf_", dir=str(output_path.parent))
            )
            try:
...
def _install_convert_deps() -> None:
    ...
    and an already-present `~/.kadhi/llama.cpp` both skipped it and every
...
def _find_llama_cpp(user_path: Optional[str] = None) -> Path:
    ...
    """
    from kadhi_cli.utils.constants import KADHI_DIR
    ...
    # 3. Check ~/.kadhi/llama.cpp
    # KADHI_DIR is a bare name (".kadhi"), so it MUST be anchored to the home
    ...
    # ~/.kadhi, and the auto-clone dropped a fresh ~200 MB checkout into whatever
    # directory the user happened to run from (v0.71.35 GGUF validation).
    kadhi_llama = Path.home() / KADHI_DIR / LLAMA_CPP_DIR_NAME
    if kadhi_llama.exists() and (kadhi_llama / "convert_hf_to_gguf.py").exists():
        return kadhi_llama

    # 4. Auto-clone
    console.print("[yellow]llama.cpp not found. Cloning to ~/.kadhi/llama.cpp...[/]")
    console.print("[dim]This is a one-time setup for GGUF export.[/]")

    try:
        subprocess.run(
            ["git", "clone", "--depth", "1", "--branch", LLAMA_CPP_TAG,
             "https://github.com/ggerganov/llama.cpp.git", str(kadhi_llama)],
            check=True,
            capture_output=True,
            text=True,
        )
        ...
        console.print(
            ...
            f"  [bold]git clone https://github.com/ggerganov/llama.cpp.git {kadhi_llama}[/]\n"
...
def _export_onnx(
    ...
    try:
        ...
        console.print(
            ...
            "Install with: [bold]pip install \"kadhi-cli\\[onnx]\"[/]\n"
...
def _export_tensorrt(
    ...
    if not trtllm_available:
        console.print(
            ...
            "Install with: [bold]pip install \"kadhi-cli\\[tensorrt]\"[/]\n"
...
def _export_awq(
    ...
    try:
        ...
        console.print(
            ...
            "Install with: [bold]pip install \"kadhi-cli\\[awq]\"[/]\n"
...
def _export_gptq(
    ...
    try:
        ...
        console.print(
            ...
            "Install with: [bold]pip install \"kadhi-cli\\[gptq]\"[/]\n"
...
def _auto_deploy_ollama(
    ...
    ollama_name = deploy_name or f"kadhi-{model_name}"
...
def _maybe_attach_export(
    *, artifact_path: str, kind: str,
    ...
    succeeded).
    """
    from kadhi_cli.registry.attach import attach_artifact, lookup_entry_by_output_dir
...
def _export_torchao_cli(
    ...
    if quant_config is None:
        console.print(
            ...
            "Example: [bold]kadhi export --format torchao "
...
def _export_bitnet_gguf(
    *,
    ...
    llama.cpp toolchain.
    """
    from kadhi_cli.utils.bitnet import export_bitnet_gguf, validate_bitnet_export
    ...
    *,
    ...
    if not is_advanced_gguf_format(gguf_flavour):
        console.print(
            ...
            "See kadhi_cli.utils.gguf_quant.ALL_ADVANCED_GGUF_FORMATS.[/]"
        )
        raise typer.Exit(2)
