...
def _last_verdict_path() -> str:
    ...
    Lives under ``~/.kadhi/`` so concurrent `kadhi advise` invocations from
    ...
    """
    return os.path.join(os.path.expanduser("~"), ".kadhi", "advise_last.json")
...
def _render_verdict_panel(verdict: Verdict, *, profile_row_count: int) -> Panel:
    ...
    return Panel(body, title="[bold green]kadhi advise — verdict[/]")
...
def advise_run(
    ...
    record: bool = typer.Option(
        ...
        help=(
            ...
            "Omit --record to skip. Stored under ~/.kadhi/."
        ),
    ),
    ...
    console.print(
        "[dim]Next: `kadhi advise explain` for the rubric, "
        "or `kadhi advise compare` for past verdicts.[/]"
    )
...
def explain() -> None:
    ...
    if verdict is None:
        console.print(
            "[yellow]No prior verdict cached.[/] Run `kadhi advise <data>` "
            "first."
        )
        raise typer.Exit(1)
    if verdict.choice not in CHOICES:
        console.print(
            "[red]Cached verdict is malformed[/] — re-run `kadhi advise`."
        )
        raise typer.Exit(1)
...
def compare(
    ...
    if not entries:
        console.print(
            "[yellow]No history yet.[/] Run `kadhi advise <data> --record` "
