...
run and writes ``kadhi.tfstate`` for ``kadhi apply`` to consult.
"""
...
def plan_cmd(
    ...
    state_file: str = typer.Option(
        DEFAULT_STATE_FILE,
        "--state",
        help="Path to write the state file (default: ./kadhi.tfstate).",
    ),
) -> None:
    """Render a pre-flight training plan + write ``kadhi.tfstate``."""
    try:
    ...
    console.print(
        Panel(
            ...
            "Review the numbers, then run `kadhi apply` to execute.",
            title="plan",
            border_style="green",
        )
    )
...
def apply_cmd(
    ...
    state_file: str = typer.Option(
        DEFAULT_STATE_FILE,
        "--state",
        help="Path to the state file written by `kadhi plan`.",
    ),
    ...
    from kadhi_cli.utils.terraform_plan import build_plan, detect_drift, read_state
    ...
    try:
        ...
        console.print(
            f"[red]No state file at {escape(state_file)}; run `kadhi plan` first.[/]"
        )
        raise typer.Exit(1) from None
    except (TypeError, ValueError) as exc:
        console.print(f"[red]{escape(str(exc))}[/]")
    ...
    if drift.has_drift:
        ...
        console.print(
            Panel(
                ...
                f"Run `kadhi plan` again to refresh, then re-apply.",
                title="apply",
                border_style="red",
            )
        )
    ...
    console.print(
        Panel(
            ...
            f"Now run: [bold]kadhi train --config {escape(config)}[/]\n"
