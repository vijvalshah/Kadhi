"""kadhi probe — post-train activation probes (v0.66.0).
...
def pack(
    ...
    if base is None:
        console.print(
            "[red]Pass <base> or --list (see `kadhi probe pack --help`).[/]"
        )
        raise typer.Exit(2)
...
# ---------------------------------------------------------------------------
# kadhi probe truth / harm <base>  (v0.71.8 #217)
# ---------------------------------------------------------------------------
...
def _run_kind_probe_cli(
    ...
    *,
    ...
    from kadhi_cli.utils.probe_kernel import ProbeResult, load_probe_weights
...
def truth(
    ...
    from kadhi_cli.utils.truth_probe import (
...
def harm(
    ...
    from kadhi_cli.utils.harm_probe import (
        BUNDLED_HARM_PROBES,
        render_harm_json,
...
# ---------------------------------------------------------------------------
# kadhi probe interference <losses.json>   (--measure: v0.71.8 #218)
# ---------------------------------------------------------------------------
...
def _emit_matrix(matrix: Any, output: Optional[str]) -> None:
    ...
    from kadhi_cli.utils.interference import render_matrix_json, render_matrix_markdown
...
def _run_interference_measure(
    ...
    from kadhi_cli.utils.interference import build_interference_matrix
...
# ---------------------------------------------------------------------------
# kadhi probe sae-diff <sae> <pre.json> <post.json>
# ---------------------------------------------------------------------------
...
def sae_diff(
    ...
    auto_download: bool = typer.Option(
        ...
        help=(
            ...
            "~/.kadhi/sae-cache/ first (must be in HF_HUB_ALLOWLIST; v0.71.8 #216)."
        ),
    ),
    output: Optional[str] = typer.Option(
    ...
    from kadhi_cli.utils.sae_diff import (
        compute_sae_diff,
        download_sae,
        load_sae_weights,
        render_report_json,
