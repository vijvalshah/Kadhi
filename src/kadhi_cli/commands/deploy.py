...
if TYPE_CHECKING:
    from kadhi_cli.utils.deploy_autopilot import DeployProfile
...
_GRADIO_APP_PY = '''"""Kadhi CLI-generated Gradio Chat Space."""
...
chat = gr.ChatInterface(respond, title="Kadhi CLI Fine-tuned Chat")
chat.launch()
'''
...
Generated with Kadhi CLI — model: `{MODEL_REPO}`
"""


_STREAMLIT_APP_PY = '''"""Kadhi CLI-generated Streamlit Chat Space."""
...
def ollama(
    ...
    name: Optional[str] = typer.Option(
        ...
        "--name",
        "-n",
        help="Ollama model name (e.g. kadhi-my-model)",
    ),
    ...
    list_models: bool = typer.Option(
        ...
        "--list",
        "-l",
        help="List Kadhi-deployed models in Ollama",
    ),
    ...
    if list_models:
        ...
        if not models:
            console.print("[yellow]No Kadhi-deployed models found in Ollama.[/]")
            console.print("[dim]Deploy a model with: kadhi deploy ollama --model <gguf>[/]")
            raise typer.Exit(0)

        table = Table(title="Kadhi Models in Ollama")
    ...
    if not model:
        ...
        console.print("[dim]Usage: kadhi deploy ollama --model <gguf> --name <name>[/]")
        raise typer.Exit(1)
...
def hf_space(
    ...
    try:
        if template_dir is not None:
            from kadhi_cli.utils.hf_space import (
                detect_space_sdk,
    ...
    try:
        ...
        for in_repo_name, content in files.items():
            api.upload_file(
                ...
                repo_id=space,
                repo_type="space",
                commit_message=f"Kadhi CLI: add {in_repo_name}",
            )
...
def autopilot(
    ...
    recipe_out: str = typer.Option(
        ...
        "--recipe-out",
        help="Where to write the rendered kadhi.yaml recipe (under cwd).",
    ),
    ...
    measure: bool = typer.Option(
        ...
        help=(
            ...
            "~/.kadhi/deploy_autopilot_cache.json (v0.53.1 #109)."
        ),
    ),
    ...
    """
    from kadhi_cli.utils.deploy_autopilot import (
        get_profile,
        list_profiles,
        write_deploy_script,
        write_recipe,
    ...
    try:
        ...
        console.print(
            "[dim]Run [bold]kadhi deploy autopilot --list[/] to see options.[/]"
        )
...
def _auto_detect_template() -> Optional[str]:
    ...
    from kadhi_cli.utils.ollama import infer_chat_template

    config_path = Path("kadhi.yaml")
...
def _run_deploy_autopilot_measure(
    *,
    ...
    if cache_hit:
        console.print(
            "[dim](cache hit — delete ~/.kadhi/deploy_autopilot_cache.json or "
