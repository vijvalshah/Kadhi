...
def quickstart(
    ...
    console.print(
        Panel(
            ...
            f"  2. Create [bold]{out_dir}/quickstart_kadhi.yaml[/] config\n"
            "  3. Train a tiny LoRA adapter (~1 min on GPU)\n\n"
            f"Model: [bold]{model_id}[/]",
            title="[bold]Kadhi Quickstart[/]",
        )
    )
    ...
    config_path = out_dir / "quickstart_kadhi.yaml"
    ...
    kadhi_yaml = out_dir / "kadhi.yaml"
    if not kadhi_yaml.exists() and output is not None:
        try:
            kadhi_yaml.write_text(
    ...
    if dry_run:
        ...
        console.print(f"To train: [bold]kadhi train --config {config_path}[/]")
        raise typer.Exit()
    ...
    result = subprocess.run(
        [sys.executable, "-m", "kadhi_cli.cli", "train", "--config", str(config_path), "--yes"],
        check=False,
    )
