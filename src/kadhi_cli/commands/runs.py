...
from kadhi_cli.utils.run_cost import format_cost_usd
...
def list_runs(
    ...
    cwd_only: bool = typer.Option(
        ...
        help=(
            ...
            "working directory (default: show all in ~/.kadhi/experiments.db)."
        ),
    ),
):
    """List all training runs.

    By default lists every run from the global ``~/.kadhi/experiments.db``.
    ...
    if not runs:
        console.print("[dim]No runs found. Train a model with:[/] [bold]kadhi train[/]")
        raise typer.Exit()
...
def show(
    ...
    if not run:
        ...
        console.print("[dim]Use [bold]kadhi runs[/] to see all runs.[/]")
        raise typer.Exit(1)
...
def clean(
    ...
    if all_runs:
        ...
        console.print("[dim]Example: kadhi runs clean run_2026...[/]")
        raise typer.Exit(1)
...
def replay(
    ...
    from kadhi_cli.experiment.tracker import ExperimentTracker
    from kadhi_cli.utils.replay import downsample, summarise
...
def _plot_loss_curve(metrics: list[dict]) -> None:
    ...
    from kadhi_cli.utils.plotext_compat import render_line
...
def curriculum_curve(
    ...
    from kadhi_cli.experiment.tracker import ExperimentTracker
    from kadhi_cli.utils.curriculum_dynamic import parse_history_jsonl, render_curve
    from kadhi_cli.utils.paths import is_under_cwd
