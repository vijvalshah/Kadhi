...
from kadhi_cli.cans.unpack import extract_can, inspect_can
...
def _deploy_target(target: DeployTarget, extract_dir: Path) -> int:
    ...
    if target.kind == "ollama":
        ...
        return _run_subprocess([
            sys.executable, "-m", "kadhi_cli.cli",
            "deploy", "ollama",
            "--gguf", str(gguf_path),
            "--name", target.name,
        ])
...
def run_can(
    ...
    if not yes:
        if confirm_callback is None:
            ...
            raise ValueError(
                "kadhi can run requires --yes or an explicit confirm_callback "
    ...
    if extract_dir is None:
        owned_dir = Path(tempfile.mkdtemp(prefix="kadhi-can-run-"))
    else:
    ...
    train_argv = [
        sys.executable, "-m", "kadhi_cli.cli", "train",
